...
# Embedded file name: advent_calendar/scripts/client/advent_calendar/gui/impl/lobby/feature/ny_components/ny_resources_balance_view.py
...
class NYResourceBalance(SubModelPresenter):
    __slots__ = ('__closeClb', '__updateClb', '__isConvertPopoverAvailable')
    __nyController = dependency.descriptor(INewYearController)
    __wallet = dependency.descriptor(IWalletController)

    def __init__(self, viewModel, parentView, closeClb, updateClb, isConvertPopoverAvailable=None):
        super(NYResourceBalance, self).__init__(viewModel, parentView)
        self.__closeClb = closeClb
    ...
    def initialize(self, *args, **kwargs):
        super(NYResourceBalance, self).initialize(*args, **kwargs)
    ...
    def createPopOverContent(self, event):
        return AdventCalendarNyResourcesConvertPopover(self.__closeClb, self.parentView.getParentWindow()) if event.contentID == R.views.lobby.new_year.popovers.NyResourcesConvertPopover() and self.__isConvertPopoverAvailable() else super(NYResourceBalance, self).createPopOverContent(event)
