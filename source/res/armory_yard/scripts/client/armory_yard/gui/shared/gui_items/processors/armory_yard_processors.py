...
# Embedded file name: armory_yard/scripts/client/armory_yard/gui/shared/gui_items/processors/armory_yard_processors.py
import logging
import BigWorld
...
class BuyStepTokenCountValidator(SyncValidator):
    ...
    def _validate(self):
        ...
        return makeError('result_token_count_more_than_max') if progressionTokenCount + self.__buyCount > self.__armoryYardCtrl.maxNumberOfSteps else makeSuccess()
...
class CollectRewardsProcessor(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        if msg is not None:
            SystemMessages.pushMessage(text=backport.text(R.strings.armory_yard.notifications.ReceivedRewards()), type=SystemMessages.SM_TYPE.ArmoryYardReceivingAwards, priority=NotificationPriorityLevel.MEDIUM, messageData={'rewards': msg})
