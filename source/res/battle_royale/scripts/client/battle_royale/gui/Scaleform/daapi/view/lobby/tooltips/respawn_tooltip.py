...
class RespawnTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        items.append(formatters.packBuildUpBlockData(self.__constructDescriptionBlock(leftPadding), linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, padding=formatters.packPadding(left=leftPadding, right=rightPadding, top=verticalPadding, bottom=verticalPadding)))
        items.append(formatters.packTextBlockData(text=text_styles.standard(backport.text(R.strings.battle_royale.tooltips.respawnIcon.definition())), padding=formatters.packPadding(left=innerBlockLeftPadding, top=verticalPadding - 4)))
        return items
