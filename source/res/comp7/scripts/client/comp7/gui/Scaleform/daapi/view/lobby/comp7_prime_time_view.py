...
from comp7.gui.Scaleform.daapi.view.meta.Comp7PrimeTimeMeta import Comp7PrimeTimeMeta
...
class Comp7ServerPresenter(ServerListItemPresenter):
    ...
    def _buildTooltip(self, peripheryID):
        if not self.getTimeLeft():
            tooltipStr = text_styles.expText(backport.text(R.strings.comp7_ext.primeTimeView.endOfCycle(), server=self.getName()))
        else:
            ...
            if self._getIsAvailable():
                tooltipStr = text_styles.expText(backport.text(R.strings.comp7_ext.primeTimeView.serverTooltip(), server=self.getName(), time=timeStr))
            else:
                tooltipStr = text_styles.expText(backport.text(R.strings.comp7_ext.primeTimeView.serverUnavailableTooltip(), time=timeStr, server=self.getName()))
...
class Comp7PrimeTimeView(Comp7PrimeTimeMeta):
    ...
    def _populate(self):
        ...
        self.as_setHeaderTextS(backport.text(R.strings.comp7_ext.primeTimeView.title()))
        self.as_setBackgroundSourceS(backport.image(R.images.comp7.gui.maps.icons.backgrounds.prime_time_back()))
    ...
    def __getStatusTitle(self):
        ...
        if not self._hasAvailableServers():
            return text_styles.grandTitle(backport.text(R.strings.comp7_ext.primeTimeView.status.allServersDisabled()))
        if status == PrimeTimeStatus.NOT_AVAILABLE:
            ...
            return text_styles.grandTitle(backport.text(R.strings.comp7_ext.primeTimeView.status.untill(), startTime=startTime, server=currServerName))
        return text_styles.grandTitle(backport.text(R.strings.comp7_ext.primeTimeView.status.disableFirst(), server=currServerName)) if status in (PrimeTimeStatus.FROZEN, PrimeTimeStatus.NOT_SET) else ''
    ...
    def __getTimeText(self, serverInfo):
        if serverInfo is None:
            ...
            if isAvailable:
                resId = R.strings.comp7_ext.primeTimeView.status.primeIsAvailable()
            else:
                resId = R.strings.comp7_ext.primeTimeView.status.primeWillBeAvailable()
