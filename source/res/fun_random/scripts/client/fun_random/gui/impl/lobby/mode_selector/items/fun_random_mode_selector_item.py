...
class FunRandomSelectorItem(ModeSelectorLegacyItem, FunSubModesWatcher, FunProgressionWatcher):
    ...
    def __getStatusText(self, status):
        return backport.text(R.strings.mode_selector.mode.dyn(self.modeName).notStarted()) if not self._bootcamp.isInBootcamp() and status.state in FunSubModesState.BEFORE_STATES else ''
