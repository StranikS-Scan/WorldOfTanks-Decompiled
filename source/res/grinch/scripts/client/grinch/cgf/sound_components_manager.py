...
# Embedded file name: grinch/scripts/client/grinch/cgf/sound_components_manager.py
import BigWorld
import CGF
import logging
...
from .sound_components import SoundNotification, ConditionalSound2D, ConditionalSound3D, VehicleSound
...
class SoundComponentManager(CGF.ComponentManager):
    ...
    def onEnterVehicleSound(self, sound, go):
        ...
        if vehicle:
            ...
            if not vehicle.isPlayerVehicle and sound.onEnterSoundNPC:
    ...
    def __checkAvatarCondition(self, condition, vehicle=None):
        if condition == 'only_for_player':
            return sound_helpers.isPlayerVehicle(vehicle)
        if 'dist_to' in condition:
            return self.__checkDistToCondition(condition)
        if condition == 'player_team_only':
            return sound_helpers.isPlayerTeam(vehicle)
    ...
    def __checkDistToCondition(self, condition):
        ...
        if self.__getListElement(condition, 0) and condition[0] == 'dist_to_boss':
            bossVehicle = sound_helpers.getBossVehicle()
            playerVehicle = sound_helpers.getPlayerVehicle()
