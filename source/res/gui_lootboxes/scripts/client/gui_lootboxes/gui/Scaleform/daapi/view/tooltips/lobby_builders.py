...
def getTooltipBuilders():
    return (DataBuilder(TOOLTIPS_CONSTANTS.LOOT_BOXES_VEHICLE_BLUEPRINT_FRAGMENT, TOOLTIPS_CONSTANTS.BLOCKS_DEFAULT_UI, LootBoxVehicleBlueprintFragmentTooltipData(contexts.ToolTipContext(None))),
     ...
     TooltipWindowBuilder(TOOLTIPS_CONSTANTS.LOOT_BOX_EXTENDED_TOOLTIP, None, ExtendedLootBoxTooltipData(contexts.ToolTipContext(None))),
...
class ExtendedLootBoxTooltipData(ToolTipBaseData):
    __itemsCache = dependency.descriptor(IItemsCache)

    def __init__(self, context):
        super(ExtendedLootBoxTooltipData, self).__init__(context, TOOLTIPS_CONSTANTS.LOOT_BOX_EXTENDED_TOOLTIP)

    def getDisplayableData(self, lootboxId):
        lootBox = self.__itemsCache.items.tokens.getLootBoxByTokenID(lootboxId)
        return None if lootBox is None else DecoratedTooltipWindow(ExtendedLootboxTooltip(lootBox), useDecorator=False)
