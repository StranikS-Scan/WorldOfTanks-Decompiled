...
# Embedded file name: halloween/scripts/client/HWVehicleEffect.py
import enum
import Math
import BigWorld
...
class SequenceEffect(object):
    __slots__ = ('__active', '__animator', '__bindNode', '__spaceID', '__model', '__loadingTaskID', '__owner', '__config', '__sequencePatch', '__unapplyCbkID', '__isPlaying', '__timeToFinish', '__soundGameObject', '__soundStart', '__weakref__')
...
class Effect(object):
    ...
    __slots__ = ('__playerEffect', '__applyAttempts', '__owner', '__config', '__duration', '__isPlaying', '__soundStart')

    def __init__(self, owner, config, soundStart):
...
class HWVehicleEffect(BigWorld.DynamicScriptComponent):
    ...
    def __init__(self):
        super(HWVehicleEffect, self).__init__()
    ...
    def _activate(self, startTime):
        if self.eqId <= 0 and self.specialId <= 0:
            return
        if self.ownerType < 0:
            return
        ...
        if self._waitAppearanceReady:
            return
        isSpecial = self.specialId > 0
        self.__initSequncesData(isSpecial)
        self.__initEffectsData(isSpecial)
        self.__initExhaustData(isSpecial)
    ...
    def _getEffectData(self, isSpecial=False):
        if isSpecial:
            ...
            buff = buffsRepo.getSpecialBuffById(self.specialId)
            if buff:
                return buff.effects
            return {}
    ...
    def __stopAllEffects(self):
        ...
        if self.__exhaust is not None:
            for _effects in self.__exhaust:
    ...
    def __startExhausts(self):
        if self.__exhaust[self.ownerType] is not None:
            self.__exhaust[self.ownerType].apply()
        return
    ...
    def __initSequncesData(self, isSpecial=False):
        if self.__sequences is None:
            self.__sequences = [self.__getSequence(self.OwnerType.SELF, isSpecial), self.__getSequence(self.OwnerType.TEAM_MATE, isSpecial), self.__getSequence(self.OwnerType.ENEMY, isSpecial)]
        return

    def __initEffectsData(self, isSpecial=False):
        if self.__effects is None:
            self.__effects = [self.__getEffect(self.OwnerType.SELF, isSpecial), self.__getEffect(self.OwnerType.TEAM_MATE, isSpecial), self.__getEffect(self.OwnerType.ENEMY, isSpecial)]
        return

    def __initExhaustData(self, isSpecial=False):
        if self.__exhaust is None:
            selfExhaust = self._getEffectData(isSpecial).get('exhaust')
            self.__exhaust = [Exhaust(self.entity, selfExhaust) if selfExhaust else None, None, None]
        return

    def __getSequence(self, ownerType, isSpecial=False):
        sequences = self._getEffectData(isSpecial).get('sequences')
    ...
    def __getEffect(self, ownerType, isSpecial=False):
        effects = self._getEffectData(isSpecial).get('effects')
        if effects is not None:
            key = self.__ownerTypeToStr(ownerType)
            effect = effects.get(key)
            if effect:
                soundStart = effects.get('{}_soundStart'.format(key), '')
                return Effect(self.entity, effect, soundStart)
        return
