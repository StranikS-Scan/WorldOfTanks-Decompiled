...
class SequenceBuffComponent(ClientBuffComponent):
    ...
    def __init__(self, name, config, owner):
        ...
        self._soundStart = config.soundStart
    ...
    def _onLoaded(self, resourceRefs):
        ...
        if self._active and self._owner.model and self._animator is not None:
            self._attachModel()
        return
    ...
    def _attachModel(self):
        ...
        if canStart:
            ...
            if self._soundStart:
                self.__play3DSoundOnGameObject()
        return
    ...
    def __play3DSoundOnGameObject(self):
        if self.__soundGameObject is None:
            self.__soundGameObject = CGF.GameObject(self._owner.appearance.spaceID)
        else:
        ...
        go = self._owner.entityGameObject
        ...
        self.__soundGameObject.createComponent(Sound3DComponent, self._soundStart, self._soundStart, True)
        return
