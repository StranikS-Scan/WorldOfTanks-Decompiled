...
from halloween.gui.game_control.halloween_controller import Phase
...
class BaseEventView(ViewImpl, IGlobalListener):
    ...
    def onPrbEntitySwitched(self):
        if not self._hwController.isEventPrbActive() or self._hwController.isPostPhase():
            self._onClose()
    ...
    def _fillViewModel(self):
        with self.viewModel.transaction() as model:
            phase = self._hwController.phases.getActivePhase()
            if phase is None:
                return
