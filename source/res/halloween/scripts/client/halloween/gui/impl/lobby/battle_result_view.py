...
class BattleResultView(BaseEventView, CallbackDelayer):
    ...
    def _addEventListeners(self):
        ...
        self._hwController.onPrimeTimeStatusUpdated += self._updateEventStatus
    ...
    def _sendPlatoonInvitation(self, args):
        if not (args and self._hwController.isEnabled()):
            return
        dbID = args.get('playerId')
        if not dbID:
            return
        yield self.prbDispatcher.doSelectAction(PrbAction(PREBATTLE_ACTION_NAME.HALLOWEEN_BATTLE_SQUAD, accountsToInvite=(int(dbID),)))
    ...
    def _onSentInviteListModified(self, added, changed, deleted):
        if not self._hwController.isEnabled():
            return
        ...
        for inviteID in allChangedInvites:
            ...
            if invite.type != PREBATTLE_TYPE.HALLOWEEN_BATTLES:
                continue
    ...
    def __fillQuests(self, model, questsVO):
        if not questsVO:
            ...
            phase = self._hwController.phases.getPhaseByIndex(currentPhaseIndex)
            if not phase:
                return
            ...
            for quest in questsVO:
                ...
                questObject = self.eventsCache.getQuestByID(questInfo['qID'])
                ability = phase.getQuestConditionAbilityInfo(questObject)
    ...
    def __isRequestSquadSent(self, dbID):
        for invite in self.prbInvites.getInvites(incoming=False):
            if invite.type != PREBATTLE_TYPE.HALLOWEEN_BATTLES or dbID != invite.receiverID:
                continue
    ...
    def __isPlayerInSquad(self, dbID):
        notHalloween = self.platoonCtrl.getPrbEntityType() != PREBATTLE_TYPE.HALLOWEEN_BATTLES
        return False if not self.platoonCtrl.isInPlatoon() or notHalloween else dbID in self.prbEntity.getPlayers()
    ...
    def __canCreateSquad(self):
        result = self.prbEntity.getPermissions().canCreateSquad() and self._hwController.isEnabled() and not self._hwController.isPostPhase()
        if self.platoonCtrl.isInPlatoon() and self.platoonCtrl.getPrbEntityType() == PREBATTLE_TYPE.HALLOWEEN_BATTLES:
    ...
    def __updateMembersStatus(self):
        if self._hwController.isEnabled() and not self._hwController.isPostPhase():
            return
        with self.viewModel.transaction() as tx:
            for baseTeamMember in (tx.getPlayerTeam(), tx.getEnemyTeam()):
                for index in xrange(len(baseTeamMember)):
