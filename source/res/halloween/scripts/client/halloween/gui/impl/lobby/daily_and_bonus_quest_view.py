...
class HWApplyDailyBonusProcessor(Processor):
    ...
    def _request(self, callback):
        ...
        accountComponent.applyDailyQuest(self._phaseIndex, lambda _, code, errStr: self._response(code, callback, errStr=errStr))
...
class DailyAndBonusQuestView(BaseEventView):
    ...
    def _fillViewModel(self):
        ...
        phase = self._hwController.phasesHalloween.getActivePhase(PhaseType.REGULAR)
        if not phase:
            return
    ...
    def __applyDailyBonus(self):
        phaseIndex = self._hwController.phasesHalloween.getActivePhaseIndex(PhaseType.REGULAR)
        if phaseIndex == 0:
            return
    ...
    def __fillDaily(self, phase, model):
        ...
        lastPhase = phase.phaseIndex == len(self._hwController.phasesHalloween.getPhasesByType(PhaseType.REGULAR))
