...
from gui.server_events.bonuses import DossierBonus, CustomizationsBonus
...
class GlobalProgressionView(BaseEventView):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if bonusType == HWBonusesType.STYLE:
                return self.__styleTooltip(bonusArg)
    ...
    def __getTankmenProgressInfo(self):
        ...
        return {'type': QuestTypeEnum.TANKMAN,
         ...
         'totalProgress': len(self._hwController.phases.getPhasesByType(PhaseType.REGULAR)),
    ...
    def __getMainHWXPProgressInfo(self):
        ...
        if not quest:
            return {}
        bonusesType = sorted(getMissionInfoData(quest).getSubstituteBonuses(), reverse=True)
        data = []
        for bonusType in bonusesType:
            if isinstance(bonusType, DossierBonus):
                for item in bonusType.getValue().itervalues():
                    data.extend([ key for key in item ])

            if isinstance(bonusType, CustomizationsBonus):
                for item in bonusType.getValue():
                    c11nItem = bonusType.getC11nItem(item)
                    data.append((c11nItem.itemTypeName, c11nItem.intCD))
    ...
    def __styleTooltip(self, itemCD):
        window = BackportTooltipWindow(backport.createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TECH_CUSTOMIZATION_ITEM_AWARD, specialArgs=CustomizationTooltipContext(itemCD=int(itemCD))), self.getParentWindow())
        window.load()
        return window
