...
class MetaView(BaseEventView):
    ...
    def createToolTip(self, event):
        ...
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            if self.selectedPhase:
                ...
                name = tooltipId.startswith(HWBonusesType.BATTLE_BOOSTER) and event.getArgument('name', '')
    ...
    def __equipmentTooltip(self, equipmentName=None, isBattleBooster=False):
        ...
        if eq is not None:
            window = BackportTooltipWindow(createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.AWARD_BATTLE_BOOSTER if isBattleBooster else TOOLTIPS_CONSTANTS.AWARD_MODULE, specialArgs=[eq.compactDescr]), self.getParentWindow())
            window.load()
            return window
        else:
            return
