...
from gui.Scaleform.daapi.view.lobby.store.browser.shop_helpers import getHalloween2022ShopUrl
...
def showMetaView(selectedPhase=INVALID_PHASE_INDEX):
    ...
    if isViewLoaded(layoutID):
        return
    loadGuiImplViewWithAnimation(layoutID, MetaView, scope=ScopeTemplates.LOBBY_TOP_SUB_SCOPE, selectedPhase=selectedPhase)
...
def showHalloweenShop():
    ...
    if dispatcher is None:
        ...
        if not result:
            ...
            return
        showShop(getHalloween2022ShopUrl())
        return
