...
class EventShopAccountComponentBase(BigWorld.StaticScriptComponent):
    ...
    def _onAccountBecomePlayer(self):
        diff = self.lobbyContext.getServerSettings().getSettings()
        self.__updateShop({'serverSettings': diff})
    ...
    def __updateShop(self, diff):
        serverSettingsDiff = diff.get('serverSettings', {})
        if not self._isShopDataUpdated(serverSettingsDiff):
            return
        self._updateShopData(serverSettingsDiff)
        self._cacheShopBundles()
        self.onShopUpdated()
