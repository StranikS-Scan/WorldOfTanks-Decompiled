...
class SequenceEffect(object):
    ...
    def unapply(self):
        if self.__loadingTaskID:
            _logger.error(msg='unapply triggered, before _onLoaded() started', exc_info=traceback.print_stack())
    ...
    def _attachModel(self):
        ...
        if canStart:
            ...
            if self.__timeToFinish > 0:
                ...
                _logger.warning('timeToFinish %s less than or equals to 0. Unapply callback was not delayed.', self.__timeToFinish, exc_info=traceback.print_stack())
        return
