...
class SequenceEffect(object):
    ...
    def apply(self, startTime):
        if self.__config['duration'] > 0:
            ...
            timeToFinish = finishTime - BigWorld.serverTime()
            if timeToFinish <= 0:
                return
            self.__timeToFinish = timeToFinish
        else:
            _logger.warning('Trying to apply HBVehicleEffect, but effect duration less than or equals to 0')
            return
        if self.__isPlaying:
            _logger.warning('[HBVehicleEffect] Sequence is already playing')
            return
        self.__active = True
    ...
    def _attachModel(self):
        ...
        if canStart:
            ...
            if self.__timeToFinish > 0:
                ...
                _logger.warning('timeToFinish %s less than or equals to 0. Unapply callback was not delayed.', self.__timeToFinish)
        return
...
class HBVehicleEffect(DynamicScriptComponent):
    ...
    def __init__(self):
        ...
        self._waitAppearanceReady = True
    ...
    def _onAvatarReady(self):
        ...
        self._waitAppearanceReady = appearance is None or not appearance.isConstructed
