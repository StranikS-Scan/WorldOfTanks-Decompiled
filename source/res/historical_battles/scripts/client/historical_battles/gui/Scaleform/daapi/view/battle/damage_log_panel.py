...
import BigWorld
from gui.Scaleform.daapi.view.battle.shared.damage_log_panel import DamageLogPanel, LogViewComponent, EMPTY_SHELL_VO_BUILDER, _LogRecordVOBuilder, _DamageActionImgVOBuilder, _DAMAGE_VALUE_VO_BUILDER, _DamageShellVOBuilder, VehicleVOBuilder, _ShellVOBuilder, _ActionImgVOBuilder, _AssistActionImgVOBuilder, _CritsShellVOBuilder, _CriticalHitValueVOBuilder
...
class _HistoricalBattlesLogViewComponent(LogViewComponent):

    def _buildLogMessageVO(self, info):
        builder = _ETYPE_TO_RECORD_VO_BUILDER.get(info.getType(), None)
        return builder.buildVO(info, self._arenaDP) if builder is not None else super(_HistoricalBattlesLogViewComponent, self)._buildLogMessageVO(info)


class _HistoricalBattlesVehicleVOBuilder(VehicleVOBuilder):
...
class _HistoricalBattlesReceivedHitVehicleVOBuilder(_HistoricalBattlesVehicleVOBuilder):
    ...
    def _populateVO(self, vehicleVO, info, arenaDP):
        ...
        if info.isDamagingSmoke():
            vehicleVO.vehicleName = ''
            vehicleVO.vehicleTypeImg = ''
        if info.isArtilleryEqDamage() or info.isArtilleryEqDamage(primary=False):
            vehicleVO.vehicleTypeImg = _HB_IMAGES.DAMAGELOG_BY_ARTILLERY_FIELD_16X16
            if info.isProtectionZoneDamage() or info.isProtectionZoneDamage(primary=False):
                vehicleVO.vehicleName = ''
        if info.isBombersDamage() or info.isBombersDamage(primary=False) or info.isBomberEqDamage() or info.isBomberEqDamage(primary=False):
            vehicleVO.vehicleName = ''
...
_ETYPE_TO_RECORD_VO_BUILDER = {_ETYPE.DAMAGE: _LogRecordVOBuilder(_DEFAULT_HB_VEHICLE_VO_BUILDER, EMPTY_SHELL_VO_BUILDER, _DAMAGE_VALUE_VO_BUILDER, _HistoricalBattlesDamageActionImgVOBuilder(shotIcon=_IMAGES.DAMAGELOG_DAMAGE_16X16, fireIcon=_IMAGES.DAMAGELOG_FIRE_16X16, ramIcon=_IMAGES.DAMAGELOG_RAM_16X16, wcIcon=_IMAGES.DAMAGELOG_ICON_WORLD_COLLISION, mineFieldIcon=_IMAGES.DAMAGELOG_MINE_FIELD_16X16, spawnBotDmgIcon=_IMAGES.DAMAGELOG_YOUR_SPAWNED_BOT_DMG_16X16, airstrikeIcon=_IMAGES.DAMAGELOG_AIRSTRIKE_ENEMY_16X16, artilleryIcon=_IMAGES.DAMAGELOG_ARTILLERY_ENEMY_16X16)),
 ...
 _ETYPE.ASSIST_DAMAGE: _LogRecordVOBuilder(_DEFAULT_HB_VEHICLE_VO_BUILDER, EMPTY_SHELL_VO_BUILDER, _DAMAGE_VALUE_VO_BUILDER, _AssistActionImgVOBuilder()),
 ...
 _ETYPE.STUN: _LogRecordVOBuilder(_DEFAULT_HB_VEHICLE_VO_BUILDER, EMPTY_SHELL_VO_BUILDER, _DAMAGE_VALUE_VO_BUILDER, _ActionImgVOBuilder(image=_IMAGES.DAMAGELOG_STUN_16X16))}
