...
class HistoricalBattlePage(ClassicPage):
    ...
    def _startBattleSession(self):
        ...
        HBAvatarComponent.onSpawn += self.__onSpawn
        HBAvatarComponent.onRespawn += self.__onRespawn

    def __onSpawn(self, avatarID):
        if avatarID == BigWorld.player().id:
            self.__onRespawnVisibility(True)

    def __onRespawn(self, avatarID):
        if avatarID == BigWorld.player().id:
    ...
    def _onPostMortemSwitched(self, noRespawnPossible, respawnAvailable):
        ...
        self.__callbackDelayer.delayCallback(self._RESPAWN_WINDOW_DELAY, self.__onRespawnVisibility, BigWorld.player().HBAvatarComponent.respawnPrepared)
