...
def _healVehicleFormatter(ribbon, arenaDP, updater, vehicleDataGetter):
...
def _totalVehiclesHealFormatter(ribbon, arenaDP, updater, vehicleDataGetter):
    leftFieldStr = backport.getIntegralFormat(ribbon.actionValue)
...
class HistoricalBattlesRibbonsPanel(BattleRibbonsPanel):
    ...
    def _createRibbonAggregator(self):
        return ribbons_aggregator.createHBRibbonsAggregator()
    ...
    def _getViewData(self):
        return super(HistoricalBattlesRibbonsPanel, self)._getViewData() + [[_BET.BOMBERS, backport.text(R.strings.hb_battle.efficiencyRibbons.bombers())],
    ...
    def _getRibbonFormatter(self, ribbonType):
        return _RIBBONS_FMTS.get(ribbonType) or super(HistoricalBattlesRibbonsPanel, self)._getRibbonFormatter(ribbonType)
