...
from gui.Scaleform.daapi.view.battle.shared.status_notifications.sn_items import _DestroyTimerSN, TimerSN
...
class HBArtilleryOnYourselfSN(TimerSN):

    def getItemID(self):
        return VEHICLE_VIEW_STATE.HB_ARTILLERY_ON_YOURSELF

    def getViewTypeID(self):
        return BATTLE_NOTIFICATIONS_TIMER_TYPES.HB_ARTILLERY_ON_YOURSELF

    def _getDescription(self, value):
        return backport.text(R.strings.hb_battle.timersPanel.artilleryOnYourself())

    def _update(self, value):
        self._isVisible = value['isActive']
        self._updateTimeParams(value['duration'], value['endTime'])
        self._sendUpdate()


class HBLastStandSN(TimerSN):

    def getItemID(self):
        return VEHICLE_VIEW_STATE.HB_LAST_STAND

    def getViewTypeID(self):
        return BATTLE_NOTIFICATIONS_TIMER_TYPES.HB_LAST_STAND

    def _getDescription(self, value):
        pass

    def _update(self, value):
        self._updateTimeParams(value['destructionDelay'], value['destructionTime'])
...
class HBStatusNotificationTimerPanel(StatusNotificationTimerPanel):
    ...
    def _generateNotificationTimerSettings(self):
        ...
        self._addNotificationTimerSetting(data, BATTLE_NOTIFICATIONS_TIMER_TYPES.HB_ARTILLERY_ON_YOURSELF, HBBATTLE_NOTIFICATIONS_TIMER_LINKAGES.AIR_STRIKE_ICON, link, BATTLE_NOTIFICATIONS_TIMER_COLORS.ORANGE, False, False, iconOffsetY=-4, descriptionFontSize=24, descriptionOffsetY=10)
        self._addNotificationTimerSetting(data, BATTLE_NOTIFICATIONS_TIMER_TYPES.HB_LAST_STAND, HBBATTLE_NOTIFICATIONS_TIMER_LINKAGES.HB_LAST_STAND_ICON, link, BATTLE_NOTIFICATIONS_TIMER_COLORS.ORANGE, False, False, iconOffsetY=-4, descriptionFontSize=24, descriptionOffsetY=10)
        return data
