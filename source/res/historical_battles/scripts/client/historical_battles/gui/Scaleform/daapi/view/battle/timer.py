...
from historical_battles.gui.Scaleform.daapi.view.meta.HBTimerMeta import HBTimerMeta
...
class SimpleGoalFormatter(object):
    ...
    def _formatFinishReason(cls, goalsInfo):
        ...
        if state == GoalState.WIN:
            ...
            message = R.strings.hb_battle.goals.lose.dyn(goalID).dyn(subState)
            key = 'lose'
...
class HistoricalBattlesTimer(HBTimerMeta, CallbackDelayer):
    ...
    _SPG_DEF_GOAL_ID = 'SPG_def'
    _DEF_COUNTER_ATTACK = 'def_counter_attack'
    ...
    def __phaseUpdate(self, arenaPhases):
        if self.__goalID == self._SPG_DEF_GOAL_ID and arenaPhases.currentWave > self.__currentWave:
            self.__currentWave = arenaPhases.currentWave
            fakeGoal = {'id': self.__goalID,
             'subState': 'new_wave',
             'state': GoalState.ACTIVE}
            self.as_updateTitleS(SimpleGoalFormatter.formatTitle(fakeGoal))
            self.__highlightMessage(True)
    ...
    def __onGoalsUpdated(self, goalsInfo):
        ...
        showProgress = goalID != self._SPG_DEF_GOAL_ID and goalID != self._DEF_COUNTER_ATTACK and totalProgress != 0 and not goalFinished and goalID != GoalBossId.ONE.value and goalID != GoalBossId.FEW.value
        ...
        self.as_updateTitleS(SimpleGoalFormatter.formatTitle(lastGoal))
        self.as_updateObjectiveBigS(SimpleGoalFormatter.formatDescription(lastGoal))
        self.as_updateObjectiveS(SimpleGoalFormatter.formatProgress(lastGoal))
