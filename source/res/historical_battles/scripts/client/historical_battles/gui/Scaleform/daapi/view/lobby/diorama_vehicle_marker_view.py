...
class DioramaVehicleMarkerView(LobbyVehicleMarkerView):
    ...
    def __createHBVehicleMarker(self, vehicle, ctx):
        vClass, _, vMatrix = self.__getHBVehicleInfo(vehicle)
        playerName = ctx['name']
        clanName = ctx['clan']
        ...
        if isInBattle:
            vClass = None
        vNameFull = backport.text(R.strings.hb_lobby.dyn('division_capitalAll_{}'.format(divisionID)).name()) + ', ' + vehicle.typeDescriptor.type.i18nInfo.shortString
        ...
        markersCache[vehicle.id] = GUI.HangarVehicleMarker()
