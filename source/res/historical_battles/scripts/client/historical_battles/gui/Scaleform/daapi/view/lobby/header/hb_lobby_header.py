...
class HBLobbyHeader(LobbyHeader):
    ...
    def _addListeners(self):
        ...
        self._gameEventController.onSubdivisionLockChanged += self.__onSelectedFrontChanged
        self._gameEventController.onSelectedFrontChanged += self.__onSelectedFrontChanged
        self._gameEventController.onFrontTimeStatusUpdated += self.__onSelectedFrontChanged
    ...
    def _updatePrebattleControls(self, *_):
        ...
        if self._gameEventController.isEnabled() and self._gameEventController.isHistoricalBattlesMode():
            self.as_doDisableNavigationS()
            division = self._gameEventController.frontController.getSelectedSubdivision()
            ...
            if not self._gameEventController.isBattlesEnabled():
                ...
                return
            if division.isInBattle():
                self._disableFightBtn(r.notReady.header(), r.divisionNotReady.body())
            ...
            if self._isHeaderButtonPresent(HB_SQUAD_BUTTON):
                ...
                if selected.isInSquad(state):
                    ...
                    if restrict == UNIT_RESTRICTION.COMMANDER_VEHICLE_NOT_SELECTED and not self._gameEventController.isBanned:
    ...
    def __onSelectedFrontChanged(self, *_):
        self._updatePrebattleControls()
