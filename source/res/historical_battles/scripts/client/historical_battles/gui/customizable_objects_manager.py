...
from skeletons.gui.game_control import IHangarSpaceSwitchController, IHangarGuiController
...
class CustomizableObjectsManager(ICustomizableObjectsManager, IGlobalListener):
    ...
    __hangarGuiController = dependency.descriptor(IHangarGuiController)
    ...
    def __init__(self):
        ...
        self.__hangarsNameByQueueType = {}
        self.__currentHangarName = ''
        return
    ...
    def __onCheckSceneChange(self):
        ...
        if prbDispatcher:
            ...
            if queueType in QUEUE_TYPE.HB_RANGE:
                if queueType not in self.__hangarsNameByQueueType:
                    presetGetter = self.__hangarGuiController.getPresetGetter()
                    self.__hangarsNameByQueueType[queueType] = presetGetter.getHangarName()
                hangarName = self.__hangarsNameByQueueType[queueType]
                self.__currentHangarName = hangarName
                self.spaceSwitchController.hangarSpaceUpdate(hangarName)

    def __onAccountBecomePlayer(self):
        self.__currentHangarName = ''

    def __onSpaceUpdated(self):
        if self.spaceSwitchController.currentSceneName != self.__currentHangarName:
            return
        self.switchByAnchorName()

    @property
    def currentHangarName(self):
        return self.__currentHangarName
    ...
    def fini(self):
        ...
        self.__hangarsNameByQueueType.clear()
        self.hangarSpace.onSpaceDestroy -= self.__onSpaceDestroy
        self.eventsCache.onSyncCompleted -= self.__onEventsCacheSyncCompleted
        self.spaceSwitchController.onCheckSceneChange -= self.__onCheckSceneChange
        self.spaceSwitchController.onSpaceUpdated -= self.__onSpaceUpdated
    ...
    def __changeEnvironmentAndVisibilityMask(self, frontIndex):
        if self.spaceSwitchController.currentSceneName != self.__currentHangarName or not self.hangarSpace.inited or self.hangarSpace.spaceLoading():
            return
        ...
        BigWorld.setSpaceItemsVisibilityMask(self.hangarSpace.space.getSpaceID(), visibilityMask)
