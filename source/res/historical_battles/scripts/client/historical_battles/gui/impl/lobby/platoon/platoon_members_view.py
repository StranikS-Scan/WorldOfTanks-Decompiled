...
from historical_battles_common.hb_constants_extension import INVALID_DIVISION_ID
...
class HistoricalBattlesMembersView(SquadMembersView):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self._gameEventController.disableFrontsWidget(not self.prbEntity.isCommander())
        self._updateButtons()
        self._fillModel()

    def _addListeners(self):
        super(HistoricalBattlesMembersView, self)._addListeners()
        self._gameEventController.onSubdivisionLockChanged += self._updateReadyButton
        self._gameEventController.onGameParamsChanged += self._onGameParamsChanged
        self._gameEventController.onSelectedFrontChanged += self._onFrontChanged
    ...
    def _finalize(self):
        self._gameEventController.disableFrontsWidget(False)
        self._gameEventController.setDivisionsWidgetDisabled(False)
    ...
    def _onGameParamsChanged(self, *args):
        if not self._gameEventController.isEnabled():
            return
        self._updateReadyButton(args)
    ...
    def _updateMembers(self):
        ...
        self._fillModel()
        self.__updateIsReadyState()

    def _fillModel(self):
        currentFront = self._gameEventController.frontController.getSelectedFront()
        frontName = currentFront.getName()
        with self.viewModel.transaction() as model:
            model.setFrontName(frontName)
            model.header.setShowInfoIcon(False)

    def _updateUnitCommanderData(self):
        isUnitCommander = self._isCommander()
        self._gameEventController.disableFrontsWidget(not isUnitCommander)
    ...
    def _updateSlotsData(self):
        ...
        with self.viewModel.transaction() as model:
            ...
            for it in slots:
                ...
                slot.setDivisionID(it.get('divisionID', INVALID_DIVISION_ID))
                slot.setDivisionLevel(it.get('divisionLevel', 0))
    ...
    def __updateIsReadyState(self):
        isReady = self.prbEntity.getPlayerInfo().isReady
        self._gameEventController.setDivisionsWidgetDisabled(isReady)
