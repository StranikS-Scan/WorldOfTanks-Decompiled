...
from historical_battles.gui.impl.gen.view_models.views.lobby.hb_meta_view.bundle_model import BundleLayout
...
class BoosterBuyDialogView(DialogTemplateView):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self.setBackgroundImagePath(R.images.historical_battles.gui.maps.icons.backgrounds.buyBundle.dyn(frontName)())
        ...
        if layout == BundleLayout.NEWBIE:
            self.setSubView(Placeholder.ICON, IconSet(rIcons.order.c_220x220.dyn(order.type.value)(), backgroundResIDList=[rIcons.bundlesWindow.lightRed_huge()], isBottomPushingDown=False, tooltip=DialogTemplateTooltip(self._orderTooltipFactory)))
        else:
            self.setSubView(Placeholder.CONTENT, OrderWithBonuses(order, self._data['bonuses'], bundlesCount=self._data['count']))
    ...
    def _getTitle(self):
        ...
        if layout == BundleLayout.NEWBIE:
            ...
            order = self._data['order']
            return backport.text(rTitle.newbie(), orderType=order.type.value, number=count)
        if layout == BundleLayout.SPECIALIST:
            count = int(self._data['count'])
            return backport.text(rTitle.specialist(), number=count)
        return backport.text(rTitle.master())

    def _orderTooltipFactory(self):
        return OrderTooltip(self._data['order'].type.value)
