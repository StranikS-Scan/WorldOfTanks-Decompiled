...
class OptionalDevicesView(BaseShopGroupView):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.historical_battles.lobby.tooltips.HbCoinExchangeTooltip():
            return HbCoinExchangeTooltip()
        return HbCoinTooltip() if contentID == R.views.historical_battles.lobby.tooltips.HbCoinTooltip() else super(OptionalDevicesView, self).createToolTipContent(event, contentID)
    ...
    def __onItemClicked(self, args):
        ...
        if bundleID is None:
            ...
            data = {'titleText': backport.text(R.strings.hb_shop.opt_device_buy_dialog.title(), name=device.userName),
             ...
             'bundle': bundle}
            yield th_await(asyncDoAction(HBShopBuyBundleAction(bundle, ShopBuyDialogView, data, callback=showShopView)))
            return
