...
class OrderTooltip(ViewImpl):
    __slots__ = ('__orderType', '__showStatus')
    _gameEventController = dependency.descriptor(IGameEventController)

    def __init__(self, orderType, showStatus=None):
        ...
        self.__showStatus = showStatus
        model = self.viewModel
        self.__fillOrderTooltip(model, orderType)

    @property
    def viewModel(self):
        return super(OrderTooltip, self).getViewModel()

    def __fillOrderTooltip(self, model, orderType):
        ...
        model.setShowStatus(self.__showStatus)
...
class OrderTooltipWindowData(ToolTipBaseData):
    ...
    def getDisplayableData(self, orderType, showStatus):
        return DecoratedTooltipWindow(OrderTooltip(orderType, showStatus), useDecorator=False)
