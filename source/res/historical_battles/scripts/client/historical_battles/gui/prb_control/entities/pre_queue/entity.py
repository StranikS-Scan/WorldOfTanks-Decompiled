...
class HistoricalBattlesEntity(PreQueueEntity):
    _QUEUE_TIMEOUT_MSG_KEY = '#historical_battles.hb_lobby:arena_start_errors/prb/kick/timeout'
    gameEventController = dependency.descriptor(IGameEventController)
    ...
    def doSelectAction(self, action):
        return SelectResult(True) if action.actionName == PREBATTLE_ACTION_NAME.HISTORICAL_BATTLES else super(HistoricalBattlesEntity, self).doSelectAction(action)
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return HistoricalBattlesQueueCtx(subdivision.getID())

    def _doQueue(self, ctx):
        self._accountComponent.enqueueBattle(self._queueType, ctx.subdivionID)
