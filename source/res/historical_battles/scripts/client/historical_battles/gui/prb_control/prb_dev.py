...
def handleKeyDown(event):
    if event.key is Keys.KEY_O and event.isCtrlDown():
        createDevPrbEntry(bonusType=ARENA_BONUS_TYPE.HB_OFFENCE, arenaGuiType=ARENA_GUI_TYPE.HB_OFFENCE)
    elif event.key is Keys.KEY_D and event.isCtrlDown():
        createDevPrbEntry(bonusType=ARENA_BONUS_TYPE.HB_DEFENCE, arenaGuiType=ARENA_GUI_TYPE.HB_DEFENCE)
