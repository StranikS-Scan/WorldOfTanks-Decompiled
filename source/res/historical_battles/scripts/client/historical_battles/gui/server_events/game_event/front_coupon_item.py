...
from historical_battles_common.hb_constants import FRONT_COUPON_RECHARGE_QUEST_GROUP_ID
...
class GroupedFrontCouponsItem(BaseFrontCoupon):
    ...
    def __init__(self, frontCoupons):
        self._em = EventManager()
        self.onItemsUpdated = Event(self._em)
        self._frontCouponID = first(frontCoupons).getFrontCouponID()
        self._modifier = getFrontCouponModifier(self._frontCouponID)
