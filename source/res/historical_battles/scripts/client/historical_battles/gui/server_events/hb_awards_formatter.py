...
class HBQuestsTokenBonusFormatter(TokenBonusFormatter):
    ...
    def __getTokenImages(bonusName):
        ...
        for size in AWARDS_SIZES.ALL():
            res = R.images.gui.maps.icons.quests.bonuses.dyn(size).dyn(bonusName)
            images[size] = backport.image(res()) if res else None

        return images


class HBQuestUIDataPacker(DailyQuestUIDataPacker):

    def __init__(self, quest, initialTooltipIndex=None):
        super(HBQuestUIDataPacker, self).__init__(quest)
    ...
    def _packBonuses(self, model):
        ...
        self._tooltipData = {}
        packBonusModelAndTooltipData(bonuses=bonuses, bonusModelsList=questsBonusList, tooltipData=self._tooltipData, packer=packer, initialTooltipIndex=self.__tooltipIndex)
