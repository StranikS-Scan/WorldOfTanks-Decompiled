...
def showCoinsExchangeWindow():
    ...
    yield th_await(dialogs.show(FullScreenDialogWindowWrapper(ExchangeCoinsView(), layer=WindowLayer.FULLSCREEN_WINDOW)))
...
def showHBMetaView(tabID, *args):
    ...
    g_eventBus.handleEvent(events.LoadGuiImplViewEvent(GuiImplViewLoadParams(viewRes, HBMetaView, scope=ScopeTemplates.LOBBY_SUB_SCOPE), tabID, *args), scope=EVENT_BUS_SCOPE.LOBBY)


def showHBProgressionView(frontId=None):
    showHBMetaView(TabId.PROGRESS, frontId)


def showHBOrderView():
    showHBMetaView(TabId.ORDER)


def showHBDivisionsView():
    showHBMetaView(TabId.DIVISION)


def showHBQuestsView():
    showHBMetaView(TabId.QUESTS)
...
def showHBFairplayDialog(data=None, callback=None):
    ...
    result = yield th_await(dialogs.showSingleDialogWithResultData(data=data or {}, layoutID=FairPlayWindow.LAYOUT_ID, wrappedViewClass=FairPlayWindow, layer=WindowLayer.WINDOW))
    if result.busy:
...
def showHBFairplayWarningDialog(reason='', callback=None):
    ...
    result = yield th_await(dialogs.showSingleDialogWithResultData(reason=reason, layoutID=FairPlayWarningWindow.LAYOUT_ID, wrappedViewClass=FairPlayWarningWindow, layer=WindowLayer.WINDOW))
    if result.busy:
        if callback is not None:
            callback(False)
    else:
