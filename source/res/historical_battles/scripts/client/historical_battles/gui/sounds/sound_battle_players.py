...
class EquipmentSoundPlayer(_SoundsPlayer):
    ...
    def _initialize(self):
        ...
        HBAvatarComponent.onDeath += self.__onVehicleDestroyed
        return
    ...
    def __onVehicleDestroyed(self, avatarID, *_):
        if BigWorld.player().id == avatarID and self.__isNitroSoundPlaying:
            SoundGroups.g_instance.playSound2D(HBUISound.NITRO_DEACTIVATION)
            self.__isNitroSoundPlaying = False
