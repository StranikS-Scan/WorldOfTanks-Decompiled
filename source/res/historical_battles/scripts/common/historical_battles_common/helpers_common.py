...
from hb_constants import EventShop, HBTokens
...
def isCompensationToken(token):
    return token.startswith(HBTokens.MAIN_VEHICLE_DISCOUNT_COMPENSATION)


def findCompensationToken(tokens):
    return next(ifilter(isCompensationToken, tokens), None)


def buildCompensationToken(compensation):
    currency = next(ifilter(lambda c: c in compensation, ['gold', 'credits']), None)
    amount = compensation.get(currency)
    if not amount:
        _logger.error('Can not build compensation token %r', compensation)
        return ''
    else:
        compensationToken = '{}:{}:{}'.format(HBTokens.MAIN_VEHICLE_DISCOUNT_COMPENSATION, currency, amount)
        return compensationToken


def parseCompensationToken(token):
    parts = token.split(':')
    if len(parts) == 3 and parts[0] == HBTokens.MAIN_VEHICLE_DISCOUNT_COMPENSATION:
        return (parts[1], int(parts[2]))
    _logger.error('Can not parse compensation token %s', token)
