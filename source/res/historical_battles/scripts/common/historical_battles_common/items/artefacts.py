...
from items.artefacts import AreaOfEffectEquipment, Equipment, VehicleFactorsXmlReader, AttackBomberEquipment, _CommonMinefieldEquipment, ReconConfigReader
...
class HBAreaOfEffectEquipment(AreaOfEffectEquipment):
    __slots__ = ('wwsoundFiring',)

    def __init__(self):
        super(HBAreaOfEffectEquipment, self).__init__()
        self.wwsoundFiring = None
        return

    def _readConfig(self, xmlCtx, section):
        super(HBAreaOfEffectEquipment, self)._readConfig(xmlCtx, section)
        self.wwsoundFiring = section.readString('wwsoundFiring')


class DeathZoneEvent(HBAreaOfEffectEquipment):
    __slots__ = ()
...
class HBVehicleNitro(Equipment):
    __slots__ = ('shortDescription', 'longDescription', 'shortFilterAlert', 'longFilterAlert', 'tooltipIdentifiers', 'soundNotification', 'soundNotificationActive', 'effects', 'factors', 'cooldownSeconds', 'durationSeconds', 'activeSeconds', 'reuseCount')
...
class HBHealPoint(Equipment):
    __slots__ = ('shortDescription', 'longDescription', 'shortFilterAlert', 'longFilterAlert', 'tooltipIdentifiers', 'soundNotificationActive', 'cooldownSeconds', 'durationSeconds', 'reuseCount', 'radius', 'healPercent', 'effects')
...
class HBAvatarArtilleryStrike(HBAreaOfEffectEquipment):
...
class HBAvatarArtilleryMortar(HBAreaOfEffectEquipment):
    __slots__ = ('cooldownSeconds',)
...
class HBAvatarBomber(HBAreaOfEffectEquipment):
    __slots__ = ('cooldownSeconds',)
...
class HBAvatarArtilleryRocket(HBAreaOfEffectEquipment):
    __slots__ = ('cooldownSeconds',)
...
class HBReconPlane(HBAreaOfEffectEquipment, ReconConfigReader):
