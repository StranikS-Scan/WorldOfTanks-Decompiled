...
BONUS_ORDER = [BoostersSettings.BONUS_NAME,
 'lootBox',
 'dossier',
 'vehicles',
 'slots',
...
class LSArtefactsController(ILSArtefactsController, IGlobalListener):
    ...
    def isArtefactOpened(self, artefactID):
        ...
        return self.remainNotOpened(artefactID) == 0 if arft else False

    def remainNotOpened(self, artefactID):
        openedTokenID = artefactID + self._getConfig().get('openedSuffix', '')
        arft = self.getArtefact(artefactID)
        return max(0, arft.limit - self.eventsCache.questsProgress.getTokenCount(openedTokenID)) if arft else 0
    ...
    def getLackOfPointsForArtefact(self, artefactID):
        return 0 if self.isArtefactOpened(artefactID) else max(0, self.getArtefactProgressPointsCost(artefactID) - self.getProgressPointsQuantity())
