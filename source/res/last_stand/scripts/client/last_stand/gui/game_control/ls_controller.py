...
class _LSConfig(namedtuple('_LSConfig', ('isEnabled', 'isBattlesEnabled', 'startDate', 'endDate', 'rewardsSettings', 'artefactsSettings', 'dailyBonusSettings', 'vehicles', 'shop', 'isPromoScreenEnabled', 'prominentBonus', 'modeSelectorShowRewards'))):
    __slots__ = ()

    def __new__(cls, **kwargs):
        defaults = dict(isEnabled=False, isBattlesEnabled={}, startDate=0, endDate=0, rewardsSettings={}, artefactsSettings={}, dailyBonusSettings={}, vehicles={}, shop={}, isPromoScreenEnabled=False, prominentBonus={}, modeSelectorShowRewards={})
...
class LSController(ILSController, Notifiable, SeasonProvider, IGlobalListener, PerformanceAnalyzerMixin):
    ...
    def onLobbyInited(self, event):
        super(LSController, self).onLobbyInited(event)
    ...
    def __onHangarLoadedAfterLogin(self):
        if self.isPromoScreenEnabled() and self.__limitedUIController.isRuleCompleted(LUI_RULES.LastStandEntryPoint):
            showPromoWindowView()
        if self.isEventHangar():
