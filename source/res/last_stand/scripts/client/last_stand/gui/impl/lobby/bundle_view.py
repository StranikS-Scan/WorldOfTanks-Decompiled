...
_BUNDLE_WITH_ONLY_KEYS = 'ls25bundle1'
...
class BundleView(BaseView):
    ...
    def __fillBalance(self, *args, **kwargs):
        balanceLayoutID = R.views.last_stand.lobby.virtual_res.MoneyBalanceView()
    ...
    def _loadBundles(self, model, artefact):
        ...
        for bundle in sorted(bundles, key=lambda bundle: (bundle.groupID, bundle.orderInGroup)):
            if bundle.isWebShopBundle:
                if bundle.limit is not None and self.lsShopCtrl.getPurchaseCount(bundle.bundleID) >= bundle.limit or lackKeys == 0 and bundle.bundleID == _BUNDLE_WITH_ONLY_KEYS:
                    continue
            elif lackKeys == 0:
                continue
            ...
            if bundle.descrGroupKey is not None:
