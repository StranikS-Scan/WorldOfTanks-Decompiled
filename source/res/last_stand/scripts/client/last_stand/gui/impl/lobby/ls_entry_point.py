...
class LSEventBanner(BaseEventBanner):
    ...
    def inProgressDescription(self):
        res = R.strings.hangar_event_banners.event.LSEntryPoint
        return backport.text(res.completed.description()) if self._lsArtefactsCtrl.isProgressCompleted() and self._lsDifficultyMissionsCtrl.isProgressCompleted() else backport.text(res.inProgress.description())
