...
# Embedded file name: last_stand/scripts/client/last_stand/gui/impl/lobby/widgets/quests_view.py
import typing
from gui.impl.backport import BackportTooltipWindow, createTooltipData
from gui.server_events.event_items import ServerEventAbstract
from helpers import dependency
from last_stand.gui.impl.lobby import fillProminentBonus, PROMINENT_REWARD_TOOLTIP_ID, getQuestFinishTimeLeft
from last_stand.gui.impl.lobby.tooltips.daily_quests_tooltip import DailyQuestsTooltip
from last_stand.gui.sounds.sound_constants import BADGE_MISSION_APPEAR, DAILY_WIDGET_APPEAR
from shared_utils import first, findFirst
from frameworks.wulf import ViewSettings, ViewFlags, View, ViewEvent
...
class QuestsView(ViewImpl):
    lsArtifactsCtrl = dependency.descriptor(ILSArtefactsController)
    eventsCache = dependency.descriptor(IEventsCache)

    def __init__(self, layoutID=R.views.last_stand.lobby.virtual_res.QuestsView(), isBadgeWidget=False):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return DailyQuestsTooltip(self.__quest, self.__isBadgeWidget) if contentID == R.views.last_stand.mono.lobby.tooltips.daily_quests_tooltip() else super(QuestsView, self).createToolTipContent(event, contentID)
    ...
    def _onLoading(self, *args, **kwargs):
        super(QuestsView, self)._onLoading()
        self._init()
        self.__statusChangeNotifier.startNotification()
        if self.__quest is not None:
            event = BADGE_MISSION_APPEAR if self.__isBadgeWidget else DAILY_WIDGET_APPEAR
            playSound(event)
        return
    ...
    def _subscribe(self):
        super(QuestsView, self)._subscribe()
    ...
    def _unsubscribe(self):
        self.eventsCache.onSyncCompleted -= self.__onSyncQuestCompleted
        self.lsArtifactsCtrl.onArtefactStatusUpdated -= self.__onArtefactStatusUpdated
        super(QuestsView, self)._unsubscribe()
