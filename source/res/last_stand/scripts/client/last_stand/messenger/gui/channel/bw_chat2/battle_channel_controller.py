    ...
    def _formatCommand(self, command):
        ...
        if command.getCommandType() == MESSENGER_COMMAND_TYPE.BATTLE:
            ...
            if battleChatCommand.name in self._BASE_RELATED_COMMANDS_NAME:
                ...
                if entity and LSMarkerComponentNames.CAMP in entity.dynamicComponents and entity.lsCampMarker.style == LSMarkersType.CAMP:
                    if battleChatCommand.name == BATTLE_CHAT_COMMAND_NAMES.MOVING_TO_TARGET_POINT:
                        commandText = backport.text(R.strings.last_stand_battle.arena.marker.move_to_camp())
                        ...
                        if mapsCtrl and mapsCtrl.hasMinimapGrid():
                            ...
                            commandText = backport.text(R.strings.last_stand_battle.arena.marker.attention_to_camp(), gridId=mapsCtrl.getMinimapCellNameById(cellId))
                else:
                    return (isCurrent, u'')
