...
# Embedded file name: new_year/scripts/client/new_year/cgf/lobby_customization_components.py
import logging
import typing
import Math
import CGF
from collections import namedtuple
from GenericComponents import AnimatorComponent, HierarchyComponent, TransformComponent, ParticleComponent
...
def _isParticleHoverEffect(hoverEffect):
    return hoverEffect.endswith('.eff')


def _isSequenceHoverEffect(hoverEffect):
    return hoverEffect.endswith('.seq')
...
class LobbyCustomizableObjectsManager(CGF.ComponentManager):
    ...
    def updateSlotHighlight(self, slotId, isEnabled):
        ...
        if slotInfo is None:
            ...
            hoverEffect = slotInfo.slotComponent.hoverEffect
            if not hoverEffect:
                return
            if isEnabled:
                ...
                for node in nodes:
                    ...
                    if _isParticleHoverEffect(hoverEffect):
                        gameObject.createComponent(ParticleComponent, hoverEffect, True, 1.0)
                    elif _isSequenceHoverEffect(hoverEffect):
                        gameObject.createComponent(AnimatorComponent, hoverEffect, 0, 1, -1, True, '')
                    gameObject.activate()
