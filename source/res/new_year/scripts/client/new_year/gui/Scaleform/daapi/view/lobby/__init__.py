...
def getViewSettings():
    ...
    from new_year.gui.Scaleform.daapi.view.lobby.ny_quest_entry_point import NYQuestEntryPoint
    return (ViewSettings(VIEW_ALIAS.NY_BROWSER_VIEW, NyBrowserView, 'browserScreen.swf', WindowLayer.FULLSCREEN_WINDOW, VIEW_ALIAS.NY_BROWSER_VIEW, ScopeTemplates.LOBBY_SUB_SCOPE),
     ...
     ComponentSettings(HANGAR_ALIASES.NY_QUEST_SECONDARY_ENTRY_POINT, NYQuestEntryPoint, ScopeTemplates.DEFAULT_SCOPE))
...
class _NewYearBusinessHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        listeners = ((VIEW_ALIAS.NY_SELECT_VEHICLE_FOR_DISCOUNT_POPOVER, self.loadViewByCtxEvent), (VIEW_ALIAS.NY_BROWSER_VIEW, self.loadViewByCtxEvent))
