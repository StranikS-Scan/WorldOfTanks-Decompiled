...
from gui.server_events.bonuses import SimpleBonus, getNonQuestBonuses, splitBonuses, mergeBonuses
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, BACKPORT_TOOLTIP_CONTENT_ID, BonusUIPacker, getDefaultBonusPacker
...
from new_year_common.items.components.ny_constants import CurrentNYConstants, MIN_TOY_RANK, YEARS_INFO, TOKEN_MANDARIN
...
class NYBoxWithToysBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getSplitToys(cls, bonus, withMandarins=False):
        ...
        if CurrentNYConstants.MANDARINS in values and withMandarins:
            toys.append(MandarinsTokenBonus(CurrentNYConstants.MANDARINS, {TOKEN_MANDARIN: {'count': values[CurrentNYConstants.MANDARINS]}}, isCompensation=True))
        return toys
