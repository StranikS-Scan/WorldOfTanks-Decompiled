...
from new_year.gui.shared.ny_level_helper import LevelInfo, getLevelIndexes
...
class NewYearController(INewYearController, IGlobalListener):
    ...
    def onLobbyStarted(self, ctx):
        for name, boxCardUrl in GUI_SETTINGS.lootBoxShopURL.iteritems():
            shopBoxCardUrl = getShopURL() + boxCardUrl
            self._GUILootboxes.addShopWindowHandler(name, partial(showShop, shopBoxCardUrl))
    ...
    def isWidgetVisible(self, prbState, alias=None):
        correctAlias = alias == VIEW_ALIAS.LOBBY_HANGAR
        correctPrb = self.__isCorrectPrb(prbState)
        return (self.isEnabled() or self.isSuspended()) and correctPrb and correctAlias

    def isQuestEntryPointVisible(self, prbState, alias=None):
        correctAlias = alias == VIEW_ALIAS.LOBBY_HANGAR
        correctPrb = self.__isCorrectPrbForQuest(prbState)
        return (self.isEnabled() or self.isSuspended()) and correctPrb and correctAlias
    ...
    def __initLootboxes(self):
        ...
        addBonusesToGroup(LootBoxBonusGroup.FEATUREITEMS, (CurrentNYConstants.IP_TYPE_CUSTOM_TOYS, CurrentNYConstants.IP_TYPE_CUSTOM_ANYOF_TOYS, CurrentNYConstants.IP_TYPE_CUSTOM_MANDATINS))
        from gui_lootboxes_common import constants_utils
        from new_year.gui.bonuses.bonuses_packers import NYMysteryBoxWithToysBonusUIPacker, NYBoxWithToysBonusUIPacker, NYToyBonusUIPackerLarge
        ...
        registerHandler(NewYearUniqueRewardHandler)
    ...
    def __finiLootboxes(self):
        ...
        from new_year.gui.impl.lobby.new_year.gui_lootbox_integration.unique_reward_handler import NewYearUniqueRewardHandler
        unregisterHandler(NewYearUniqueRewardHandler)
    ...
    def __sendRacoonNotification(self, diff):
        ...
        if isRacoonEnabled is not None and not isRacoonEnabled:
