...
# Embedded file name: new_year/scripts/client/new_year/gui/impl/lobby/new_year/albums/ny_album_view.py
import typing
from account_helpers.AccountSettings import NY_OLD_COLLECTIONS_BY_YEAR_VISITED
from gui.impl.gen import R
from new_year.gui.impl.gen.view_models.views.lobby.new_year.views.albums.ny_album_view_model import NyAlbumViewModel
from new_year.gui.impl.lobby.new_year.albums.current_album_sub_model_presenter import CurrentAlbumSubModelPresenter
from new_year.gui.impl.lobby.new_year.sub_model_presenter import HistorySubModelPresenter
from new_year.gui.impl.new_year.navigation import NewYearTabCache as TabState
...
if typing.TYPE_CHECKING:
    ...
    from new_year.gui.impl.lobby.new_year.albums.album_sub_model_presenter import AlbumSubModelPresenter
...
class NyAlbumView(HistorySubModelPresenter):
    ...
    def createToolTipContent(self, event, ctID):
        if ctID == R.views.new_year.lobby.new_year.tooltips.NyAlbumDecorationTooltip():
            return self._currentAlbumPresenter.createToyTooltip(event)
        elif ctID == R.views.new_year.lobby.new_year.tooltips.NyCollectionBonusTooltip():
            return self._currentAlbumPresenter.createCollectionBonusTooltip()
        else:
            return NyTotalBonusTooltip() if ctID == R.views.new_year.lobby.new_year.tooltips.NyTotalBonusTooltip() else None
    ...
    def __onOldCollectionsPreviewAttempt(self):
        self.__yearName = CURRENT_YEAR
        self.__goToCoverState()
        g_eventBus.handleEvent(NewYearEvent(NewYearEvent.SELECT_SIDEBAR_TAB_OUTSIDE, ctx={'menuName': NyWidgetTopMenu.COLLECTIONS,
         'tabName': Collections.CURRENT}), scope=EVENT_BUS_SCOPE.LOBBY)
