...
from new_year.gui.impl.new_year.sounds import NewYearCelebVoiceOvers, OVERLAY_HANGAR_SOUND_SPACE
...
class NyAtmosphereLevelUpView(ViewImpl):
    __slots__ = ('__tooltips', '__rewards', '__currentRewardId', '__currentRewardType', '__completedLevels', '__completedCollections', '__backCallback', '__voiceOverEnabled')
    ...
    def _onLoading(self, *_, **kwargs):
        ...
        self.viewModel.setIsDeferredGraphics(isRendererPipelineDeferred())
    ...
    def _finalize(self):
        ...
        if self.__voiceOverEnabled:
            SoundGroups.g_instance.setState(NewYearCelebVoiceOvers.LEVEL_UP_GROUP, NewYearCelebVoiceOvers.LEVEL_UP_CLOSE)
        return
    ...
    def __setVoiceOverViewState(self):
        if self.__nyController.isCelebVoiceoverEnabled() and self.__currentRewardId in NewYearCelebVoiceOvers.ENABLE_SOUND_RANGE:
            SoundGroups.g_instance.setState(NewYearCelebVoiceOvers.LEVEL_UP_GROUP, NewYearCelebVoiceOvers.LEVEL_UP_OPEN)
            self.__voiceOverEnabled = True
