...
# Embedded file name: new_year/scripts/client/new_year/gui/impl/lobby/new_year/break_decorations/ny_break_decorations_view.py
...
from new_year.gui.impl.gen.view_models.views.lobby.new_year.views.break_decorations.ny_break_decorations_view_model import NyBreakDecorationsViewModel, ShowState
from gui.impl.lobby.missions.daily_quests_view import DailyTabs
from new_year.gui.impl.lobby.new_year.break_decorations.ny_break_shards_tip import CraftTip
from new_year.gui.impl.lobby.new_year.popovers.ny_break_filter_popover import NyBreakFilterPopoverView
...
from new_year.ny_constants import SyncDataKeys, NewYearCategories
from new_year.gui.shared.gui_items.processors.ny_processor import NewYearBreakToysProcessor
from shared_utils import findFirst
from skeletons.gui.game_control import IGuiLootBoxesController
if typing.TYPE_CHECKING:
    from new_year.gui.impl.lobby.new_year.break_decorations.ny_break_shards_tip import BaseShardsTip
...
class NyBreakDecorationsView(HistorySubModelPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.new_year.lobby.new_year.tooltips.NyShardsTooltip():
            return NyShardsTooltip()
        return NyDecorationStateTooltip(event.getArgument('atmosphereBonus')) if contentID == R.views.new_year.lobby.new_year.tooltips.NyDecorationStateTooltip() else super(NyBreakDecorationsView, self).createToolTipContent(event, contentID)

    def createPopOverContent(self, event):
        return NyBreakFilterPopoverView(self.__selectedToyFilters) if event.contentID == R.views.new_year.lobby.new_year.popovers.NyBreakFilterPopover() else super(NyBreakDecorationsView, self).createPopOverContent(event)
    ...
    def _getListeners(self):
        return ((NewYearEvent.ON_BREAK_TOYS_FILTER_APPLIED, self.__onFilterAppliedEvent, EVENT_BUS_SCOPE.LOBBY),)
    ...
    def __onBreakAnimationComplete(self, _=None):
        ...
        with self.viewModel.transaction() as model:
            ...
            g_eventBus.handleEvent(NewYearEvent(NewYearEvent.ON_BREAK_TOYS_ANIMATION_COMPLETED, ctx={'totalToyCount': filteredToysInfo.totalToyCount}), scope=EVENT_BUS_SCOPE.LOBBY)
