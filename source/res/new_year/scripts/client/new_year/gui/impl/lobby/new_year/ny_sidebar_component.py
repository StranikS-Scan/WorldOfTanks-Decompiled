...
# Embedded file name: new_year/scripts/client/new_year/gui/impl/lobby/new_year/ny_sidebar_component.py
import typing
...
from new_year.gui.impl.new_year.navigation import NewYearTabCache
from new_year.gui.impl.new_year.sounds import NewYearSoundConfigKeys, NewYearSoundEvents, NewYearSoundsManager
from new_year.gui.impl.new_year.views.tabs_controller import RewardsTabsController
...
class NYSidebar(HistorySubModelPresenter):
    ...
    def initialize(self, ctx=None, *args, **kwargs):
        ...
        self.__controllers = {NyWidgetTopMenu.REWARDS: RewardsTabsController()}
        g_eventBus.addListener(NewYearEvent.ON_SWITCH_VIEW, self.__onSwitchViewEvent, scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(NewYearEvent.SELECT_SIDEBAR_TAB_OUTSIDE, self.__onSelectTabOutside, scope=EVENT_BUS_SCOPE.LOBBY)
        if ctx is not None:
    ...
    def finalize(self):
        ...
        AccountSettings.onSettingsChanging -= self.__onAccountDataUpdated
        self.__settingsCore.onSettingsChanged -= self.__onSettingsChanged
        g_eventBus.removeListener(NewYearEvent.ON_SWITCH_VIEW, self.__onSwitchViewEvent, scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(NewYearEvent.SELECT_SIDEBAR_TAB_OUTSIDE, self.__onSelectTabOutside, scope=EVENT_BUS_SCOPE.LOBBY)
        self.__tabsController = None
    ...
    def __onSettingsChanged(self, diff):
        return None if self.__tabsController is None else None
    ...
    def __selectTab(self, tabName):
        needToInvokeSound = self.__newYearController.isMaxAtmosphereLevel()
        ...
        g_eventBus.handleEvent(NewYearEvent(NewYearEvent.ON_SIDEBAR_SELECTED, ctx=NYTabCtx(tabName=tabName, menuName=self.viewModel.getViewName())), scope=EVENT_BUS_SCOPE.LOBBY)
        if needToInvokeSound:
            self.__soundsManager.onEnterView()
