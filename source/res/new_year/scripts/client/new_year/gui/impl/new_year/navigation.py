...
class _NavigationState(object):
    ...
    def __init__(self):
        ...
        self.__isInCustomizationObject = False
        return
    ...
    @property
    def isInCustomizationObject(self):
        return self.__isInCustomizationObject
    ...
    def setCurrentObject(self, objectName):
        ...
        self.__isInCustomizationObject = objectName in CustomizationObjects.ALL
...
VIEW_ALIAS_TO_MENU_NAME = {ViewAliases.CITY_VIEW: NyWidgetTopMenu.CITY,
 ViewAliases.QUESTS_VIEW: NyWidgetTopMenu.LEADERS,
...
ROOT_OBJECT_BY_VIEW = {ViewAliases.CITY_VIEW: NewYearObjects.CITY_VIEW,
 ViewAliases.QUESTS_VIEW: ViewAliases.QUESTS_VIEW,
 ViewAliases.SURPRISE_MACHINE_VIEW: ViewAliases.SURPRISE_MACHINE_VIEW,
 ViewAliases.PET_VIEW: ViewAliases.PET_VIEW,
 ViewAliases.INFO_VIEW: ViewAliases.INFO_VIEW}
...
class NewYearNavigation(INewYearNavigation):
    ...
    def closeMainView(cls):
        if cls._navigationState.getCurrentObject() is None:
            _logger.warning('[NY] failed to close main view.')
            return
        else:
            showHangar()
            return
    ...
    @classmethod
    def showNavigationView(cls, viewAlias=ViewAliases.CITY_VIEW, **kwargs):
        if cls.getCurrentObject() is None:
            cls.switchTo(NewYearObjects.CITY_VIEW, True, viewAlias=viewAlias, **kwargs)
        else:
    ...
    def onHeroTankReady(cls):
        params = cls.__delayedSwitchViewCtx
        if params is None:
            return
        else:
            cls.showViewAfterPrbSwitch(params.viewAlias, *params.args, **params.kwargs)
            cls.__delayedSwitchViewCtx = None
            return
    ...
    @classmethod
    def isInCustomizationObject(cls):
        return cls._navigationState.isInCustomizationObject
    ...
    def switchToView(cls, aliasName, tabName=None, instantly=False, *args, **kwargs):
        cls._navigationState.setCurrentObject(ROOT_OBJECT_BY_VIEW[aliasName])
    ...
    def switchTo(cls, objectName, instantly=False, viewAlias=None, withFade=False, *args, **kwargs):
        if cls._navigationState.isSceneInFade:
            ...
            return
        elif cls.__spaceSwitchController.currentSceneName != DEFAULT_HANGAR_SCENE:
    ...
    def __playTransitionSound(cls, objectName):
        if cls.__nyController.isOnboardingOpen():
            return
        else:
            ...
            if objectName in CustomizationObjects.ALL:
                ...
                NewYearSoundsManager.playGladeEvent(objectName, '_ENTER')
            return
    ...
    @classmethod
    def setIsSceneInFade(cls, isFade):
        cls._navigationState.setIsSceneInFade(isFade)
