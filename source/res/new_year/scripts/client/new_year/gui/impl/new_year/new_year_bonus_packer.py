...
from gui.shared.missions.packers.bonus import TokenBonusUIPacker, BonusUIPacker, getDefaultBonusPackersMap, SimpleBonusUIPacker, BACKPORT_TOOLTIP_CONTENT_ID, DossierBonusUIPacker, BaseBonusUIPacker, VehiclesBonusUIPacker, CustomizationBonusUIPacker, getLocalizedBonusName
from gui.shared.money import Currency
from gui.shared.utils.functions import makeTooltip
from helpers import dependency
from new_year_common.items.components.ny_constants import CurrentNYConstants, TOKEN_VARIADIC_DISCOUNT_PREFIX, NY_PET_FOOD_CURRENCY, PET_TYPE_BY_NAME
...
def getNewYearBonusPacker():
    ...
    mapping.update({'entitlements': NewYearEntitlementBonusUIPacker(),
     ...
     'currencies': NewYearCurrenciesBonusUIPacker(),
     CurrentNYConstants.NY_STATIC_DOGTAG: NewYearStaticDogTagUIPacker()})
...
class NewYearTokenBonusUIPacker(TokenBonusUIPacker):
    ...
    def _getTokenBonusType(cls, tokenID, complexToken):
        ...
        return CurrentNYConstants.HIDDEN_VEHICLE if tokenID.startswith(CurrentNYConstants.HIDDEN_VEHICLE) else super(NewYearTokenBonusUIPacker, cls)._getTokenBonusType(tokenID, complexToken)
    ...
    def _getTokenBonusPackers(cls):
        ...
        mapping.update({TOKEN_VARIADIC_DISCOUNT_PREFIX: cls.__packVariadicDiscounts,
         ...
         CurrentNYConstants.HIDDEN_VEHICLE: cls.__packHiddenVehicle})
        return mapping
    ...
    def _getTooltipsPackers(cls):
        ...
        mapping.update({TOKEN_VARIADIC_DISCOUNT_PREFIX: cls.__getVariadicDiscountsTooltipData,
         ...
         CurrentNYConstants.HIDDEN_VEHICLE: cls.__packHiddenVehicleTooltipData})
        return mapping
    ...
    @classmethod
    def __packHiddenVehicle(cls, model, bonus, complexToken, token):
        model.setName(CurrentNYConstants.HIDDEN_VEHICLE)
        model.setValue(str(token.count))
        model.setLabel(CurrentNYConstants.HIDDEN_VEHICLE)
    ...
    @classmethod
    def __packHiddenVehicleTooltipData(cls, complexToken, token):
        return createTooltipData(makeTooltip(header=backport.text(R.strings.ny.weeklyRewards.secretVehicle.tooltip.header()), body=backport.text(R.strings.ny.weeklyRewards.secretVehicle.tooltip.body())))
...
class NewYearVehiclesBonusUIPacker(VehiclesBonusUIPacker):
    ...
    def _packVehicleBonusModel(cls, bonus, vehInfo, isRent, vehicle):
        ...
        model.setIsInInventory(vehicle.isInInventory)
        return model
...
class NewYearStaticDogTagUIPacker(SimpleBonusUIPacker):
    ...
    @classmethod
    def _getToolTip(cls, bonus):
        return [ cls._getDogTagTooltip(record, bonus.getContext()) for record in bonus.getValue() ]
    ...
    @classmethod
    def _getDogTagTooltip(cls, dogTagRecord, ctx):
        return TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.NY_STATIC_DOG_TAG_INFO, specialArgs=[dogTagRecord['id'], ctx.get('topEndPos')])
...
class NYMandarinsBonusPacker(SimpleBonusUIPacker):
    ...
    def _pack(cls, bonus):
        if bonus.isCompensation():
            ...
            label = backport.text(R.strings.ny.currencyPanel.tooltip.title.ny_mandarin())
...
class NewYearCurrenciesBonusUIPacker(SimpleBonusUIPacker):

    @classmethod
    def _pack(cls, bonus):
        label = getLocalizedBonusName(bonus.getCode())
        return [cls._packSingleBonus(bonus, label or '')]

    @classmethod
    def _packCommon(cls, bonus, model):
        model.setName(NY_PET_FOOD_CURRENCY)
        model.setIsCompensation(bonus.isCompensation())
        return model
    ...
    def _packSingleBonus(cls, bonus, label):
        ...
        model.setIcon(PET_TYPE_BY_NAME.get(bonus.getCode(), ''))
        model.setBigIcon(bonus.getCode())
        return model
