...
# Embedded file name: new_year/scripts/client/new_year/gui/impl/new_year/new_year_helper.py
import random
...
from new_year_common.items.components.ny_constants import CurrentNYConstants, TOKEN_VARIADIC_DISCOUNT_PREFIX, ToySettings, YEARS
...
_WEEKLY_LEADERBOARD_ORDER_GF = ('vehicles',
 'hidden_vehicle',
 'nyStaticDogTag',
 'dossier',
 SupportedTokenTypes.LOOTBOX_TOKEN)
...
def __getAdditionalNameGoodies(bonus):
    ...
    return booster.boosterGuiType if booster.boosterType in [GOODIE_RESOURCE_TYPE.CREDITS, GOODIE_RESOURCE_TYPE.XP, GOODIE_RESOURCE_TYPE.FREE_XP_CREW_XP] else bonus.getName()
...
def nyWeeklyLeaderboardGFSortOrder(bonus):
    bonusName = bonus.getName()
    return _WEEKLY_LEADERBOARD_ORDER_GF.index(bonusName) if bonusName in _WEEKLY_LEADERBOARD_ORDER_GF else len(_BONUSES_ORDER_GF)
