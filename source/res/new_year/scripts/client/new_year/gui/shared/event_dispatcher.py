...
from new_year.gui.constants import VIEW_ALIAS as NY_VIEW_ALIAS
...
@dependency.replace_none_kwargs(notificationMgr=INotificationWindowController)
def showNYLeaderboardRewardWindow(data, seasonID, notificationMgr=None):
    from new_year.gui.impl.lobby.new_year.ny_leaderboard_reward_view import NyLeaderboardRewardsViewWindow
    window = NyLeaderboardRewardsViewWindow(data, seasonID)
    notificationMgr.append(WindowNotificationCommand(window))
...
def showNYBuyToyDialog(toyID, callback):
    ...
    result = yield th_await(dialogs.showSingleDialogWithResultData(wrappedViewClass=BuyToyView, layoutID=BuyToyView.LAYOUT_ID, toyID=toyID))
    if result.busy:
        callback((False, {}))
    else:
...
def showNYProgressView(parent=None):
    from gui.impl.pub.lobby_window import LobbyOverlay
    from new_year.gui.impl.lobby.new_year.ny_progress_view import NyProgressView
    window = LobbyOverlay(content=NyProgressView(R.views.new_year.lobby.new_year.NyProgressView()), parent=parent)
    window.load()


def showNYLeaderboardInfoWindow(parent=None):
    from new_year.gui.impl.lobby.new_year.ny_leaderboard_info_view import NyLeaderboardInfoViewWindow
    window = NyLeaderboardInfoViewWindow(parent=parent)
    window.load()
