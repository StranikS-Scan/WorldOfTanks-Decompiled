...
def _getMaxCollectionLevels():
    ...
    maxBonusLevel = len(getNewYearBonusConfig().getCollectionLevelsRating()) - 1
    return {collectionName:maxBonusLevel for collectionName in toyCollections}


def _getMaxUniqueToysLevel():
    return len(getNewYearBonusConfig().getUniqueToysLevelsRating()) - 1
...
class BonusHelper(object):
    ...
    def getMaxBonus(cls, bonusType):
        return SettingBonus.calculateBonusForType(bonusType=bonusType, atmosphereLevel=MAX_ATMOSPHERE_LVL, collectionLevels=_getMaxCollectionLevels(), toysLevel=_getMaxUniqueToysLevel(), config=getNewYearBonusConfig())
