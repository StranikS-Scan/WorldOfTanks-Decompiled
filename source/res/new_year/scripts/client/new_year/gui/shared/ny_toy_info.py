...
# Embedded file name: new_year/scripts/client/new_year/gui/shared/ny_toy_info.py
import logging
from collections import namedtuple
from helpers import dependency
from new_year.helpers.server_settings import getNewYearToyDecayCostConfig, getNewYearGeneralConfig, getNewYearCraftCostConfig
from new_year_common.items import collectibles
from gui.impl.gen import R
from new_year_common.items import new_year
from new_year_common.items.components.ny_constants import YEARS_INFO, MAX_TOY_RANK, ToySettings, ToyTypes, UNDEFINED_TOY_RANK, YEARS
from new_year.ny_constants import TOY_PREFIX, Collections
from skeletons.gui.shared import IItemsCache
from skeletons.gui.lobby_context import ILobbyContext
from new_year.skeletons.new_year import INewYearController
from gui.shared.money import MONEY_UNDEFINED, Money
...
_SLOT_TYPE_ORDER = {v:i for i, v in enumerate((ToyTypes.MEGA_FIR,
 ...
 ToyTypes.SNOW_ITEM,
 ToyTypes.PYRO,
 ToyTypes.LIGHTS_HOUSES,
 ToyTypes.LIGHTS_FIR,
 ToyTypes.SKATING,
 ToyTypes.PET_TOY,
 ToyTypes.PET_FOOD,
 ToyTypes.PET_BED,
 ToyTypes.PET_INTERACTIVE,
 ToyTypes.FIREWORKS,
 ToyTypes.SNOW_SLIDE,
 ToyTypes.TEREM,
 ToyTypes.FENCE))}
...
class _NewYearCommonToyInfo(object):
    ...
    def getPrice(self):
        return Money(**self._descriptor.price) if self._descriptor.hasPrice() else MONEY_UNDEFINED
    ...
    def getShards(self):
        if not self._shards:
            craftCostConfig = getNewYearCraftCostConfig(lobbyContext=self._lobbyCtx)
    ...
    def getIcon(self, size='big'):
        ...
        res = R.images.new_year.gui.maps.icons.newYear.toys.num(year).dyn(sizeStr).dyn(self.getIconName())
        if not res.exists():
    ...
    def isPremium(self):
        return self._descriptor.isPremiumToy()
...
@registerInDict(TOYS_INFO_REGISTRY, Collections.NewYear25)
class NewYear25ToyInfo(_NewYearBackportToyInfo):
    PREFIX = 'toys25'
    COLLECTION_NAME = Collections.NewYear25
    __slots__ = ('__sortPriority',)

    def __init__(self, toyID):
        descriptor = collectibles.g_cache.ny25.toys[toyID]
        super(NewYear25ToyInfo, self).__init__(descriptor)
        self.__sortPriority = _packSortPriority(descriptor)
...
@registerInDict(TOYS_INFO_REGISTRY, Collections.NewYear26)
class NewYear26ToyInfo(_NewYearBackportToyInfo):
    PREFIX = 'toys26'
    COLLECTION_NAME = Collections.NewYear26
    __slots__ = ('__sortPriority',)

    def __init__(self, toyID):
        descriptor = collectibles.g_cache.ny26.toys[toyID]
        super(NewYear26ToyInfo, self).__init__(descriptor)
        self.__sortPriority = _packSortPriority(descriptor)
...
class NewYearCurrentToyInfo(NewYear26ToyInfo):
    __nyController = dependency.descriptor(INewYearController)

    def getName(self):
        return R.strings.ny_toys.decorations.dyn(self.getLocalKey()).name()

    def getDesc(self):
        return R.strings.ny_toys.decorations.dyn(self.getLocalKey()).description()

    def getShards(self):
        if not self._shards:
            toyDecayCostConfig = getNewYearToyDecayCostConfig(lobbyContext=self._lobbyCtx)
    ...
    def getAtmosphere(self):
        if self.__nyController.isMaxAtmosphereLevel():
            return 0
        generalConfig = getNewYearGeneralConfig(lobbyContext=self._lobbyCtx)
