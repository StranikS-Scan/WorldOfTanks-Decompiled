...
# Embedded file name: new_year/scripts/client/new_year/gui/shared/utils/ny_requester.py
import typing
import BigWorld
...
class NewYearToy(NewYearCurrentToyInfo):
    ...
    def __init__(self, toyId, totalCount, unseenFlag, slotsCount):
        super(NewYearToy, self).__init__(toyId)
    ...
    def __cmp__(self, other):
        ...
        if not self.__unseenCount and other.getUnseenCount():
            return 1
        if self.__unseenInCollection and not other.isNewInCollection():
            return -1
        return -1 if not self.__unseenInCollection and other.isNewInCollection() else self.getID() - other.getID()


class NewYearRequester(AbstractSyncDataRequester):
    dataKey = CURRENT_PDATA_KEY
    ...
    def getMaxLevel(self):
        return self.getCacheValue(SyncDataKeys.MAX_LEVEL, 0)
    ...
    def _preprocessValidData(self, data):
        ...
        if SyncDataKeys.INVENTORY_TOYS in nyData:
            inventoryToys = {}
            for toyId, (totalCount, unseenFlag, slotsCount) in nyData[SyncDataKeys.INVENTORY_TOYS].iteritems():
                inventoryToys[toyId] = NewYearToy(toyId, totalCount, unseenFlag, slotsCount)
