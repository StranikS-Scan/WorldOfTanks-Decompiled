...
# Embedded file name: new_year/scripts/common/new_year_common/items/collectibles.py
from extension_utils import ResMgr
from ExtensionsManager import makeExtensionPath
from constants import IS_CLIENT
from debug_utils import LOG_CURRENT_EXCEPTION
from wotdecorators import singleton
from new_year_common.items.readers.collectibles_readers import _readCollections
from new_year_common.items.components.ny_constants import ToySettings, YEARS, YEARS_INFO
from new_year_common.items.components.ny_constants import PREMIUM_TOY
_CONFIG_FILE = makeExtensionPath('new_year', 'scripts/item_defs/collectibles/collectibles.xml')
...
class ToyDescriptor(object):
    ...
    def __init__(self, **kwargs):
        self.__cfg = {k:v for k, v in kwargs.iteritems() if k in ('id', 'rank', 'type', 'setting', 'collection', 'icon', 'price') or IS_CLIENT}
    ...
    def __getitem__(self, item):
        ...
        if item in cfg:
            return cfg[item]
        raise KeyError("'{}' object has no attribute '{}'".format(self.__class__, item))
    ...
    @property
    def price(self):
        return self.__cfg['price'] if self.hasPrice() else None

    def isMegaToy(self):
        return self.__cfg['setting'] in ToySettings.MEGA

    def isPremiumToy(self):
        return self.__cfg['rank'] in PREMIUM_TOY
