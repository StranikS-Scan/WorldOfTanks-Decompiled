...
# Embedded file name: new_year/scripts/common/new_year_common/items/new_year.py
import itertools
...
from new_year_common.items.components.ny_constants import YEARS_INFO, YEARS, MAX_MEGA_TOY_RANK, MIN_ATMOSPHERE_LVL, MAX_ATMOSPHERE_LVL, LEVEL_REWARD_ID_TEMPLATE, COLLECTION_REWARD_ID_TEMPLATE, ToySettings, TOY_USUAL_TYPES, MEGA_TOY_TYPES, COLLECTION_SLOTS_XML_PATH, VARIADIC_DISCOUNTS_XML_PATH, TOYS_TRANSFORMATIONS_XML_PATH, TOY_TYPE_IDS_BY_NAME, TOY_TYPES_BY_OBJECT, TOY_OBJECTS_IDS_BY_NAME, MAX_PET_TOY_RANK, PET_TOY_TYPES, TOY_PARAMETER
from new_year_common.items.readers.ny_readers import readSlots, readToysTransformations, buildVariadicDiscountsCache
from new_year_common.ny_exception import NYSoftException
from wotdecorators import singleton
if TYPE_CHECKING:
    from new_year_common.items.collectibles import ToyDescriptor
...
def _buildToyGroups(toysDescrs):
    ...
    toysInfos = [(currYearMaxToyRank, ToySettings.CURRENT_USUAL, TOY_USUAL_TYPES), (MAX_MEGA_TOY_RANK, ToySettings.CURRENT_MEGA, MEGA_TOY_TYPES), (MAX_PET_TOY_RANK, ToySettings.CURRENT_PET, PET_TOY_TYPES)]
...
class g_cache(object):
    ...
    def init(self, nofail=True):
        ...
        try:
            ...
            cfg['levelRewardsByID'] = {level:LEVEL_REWARD_ID_TEMPLATE.format(YEARS_INFO.CURRENT_YEAR_STR, level) for level in xrange(MIN_ATMOSPHERE_LVL + 1, MAX_ATMOSPHERE_LVL + 1)}
...
def getToyGroupsIterator(typeID, settingID, rank):
    flag = (typeID, settingID, rank) == (-1, -1, -1)
    if flag or isValidForRange(typeID, settingID, rank, *TOY_PARAMETER.PET_PARAMS):
        for cd in _generateCD(typeID, settingID, rank, *TOY_PARAMETER.PET_PARAMS):
            yield cd

    if flag or isValidForRange(typeID, settingID, rank, *TOY_PARAMETER.USUAL_PARAMS):
        for cd in _generateCD(typeID, settingID, rank, *TOY_PARAMETER.USUAL_PARAMS):
            yield cd


def _generateCD(typeID, settingID, rank, typeRange, settingRange, rankRange):
    for t, s, r in itertools.product(typeRange if typeID == -1 else (typeID,), settingRange if settingID == -1 else (settingID,), rankRange if rank == -1 else (rank,)):
        yield t << 16 | s << 8 | r


def isValidForRange(typeID, settingID, rank, typeRange, settingRange, rankRange):
    return _isValidParam(typeID, typeRange) and _isValidParam(settingID, settingRange) and _isValidParam(rank, rankRange)


def _isValidParam(paramID, paramRange):
    return paramID in paramRange or paramID == -1
