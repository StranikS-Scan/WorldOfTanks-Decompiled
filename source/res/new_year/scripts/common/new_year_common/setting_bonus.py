...
# Embedded file name: new_year/scripts/common/new_year_common/setting_bonus.py
from itertools import imap
from operator import mul
from functools import partial
from typing import List, Dict, Tuple
from new_year_common.settings import SettingBonusConsts
...
class SettingBonusConfig(object):
    ...
    def getMaxUniqueToysLevel(self):
        return len(self.getUniqueToysLevelsRating()) - 1

    def getMaxCollectionLevels(self):
        toyCollections = YEARS_INFO.getCollectionTypesByYear(YEARS_INFO.CURRENT_YEAR, useMega=False, usePet=False)
        maxBonusLevel = len(self.getCollectionLevelsRating()) - 1
...
class SettingBonus(object):
    ...
    def calculateBonusForType(cls, bonusType, atmosphereLevel, collectionLevels, toysLevel, config):
        collectionsFactor = 0.0
        multiplier = cls.getAtmosphereMultiplier(atmosphereLevel, config)
        toyBonus = cls.calculateUniqueToysBonus(bonusType, toysLevel, config)
        for collectionName, collectionLevel in collectionLevels.iteritems():
            collectionsFactor += cls.getCollectionFactor(bonusType, collectionName, collectionLevel, config)

        return multiplier * toyBonus + multiplier * collectionsFactor

    @classmethod
    def getCollectionsBonus(cls, bonusType, collectionLevels, config):
        bonus = 0
        for collectionName, collectionLevel in collectionLevels.iteritems():
            bonus += cls.getCollectionFactor(bonusType, collectionName, collectionLevel, config)
