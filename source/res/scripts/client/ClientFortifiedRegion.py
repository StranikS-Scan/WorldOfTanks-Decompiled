        ...
        for direction in self.getOpenedDirections():
            ...
            if availableTime is None or availableTime <= enemyDefHourTimestamp:
                ...
                def filterAttacks(item, requiredDirection=direction):
                    return True if enemyDefHourTimestamp <= item.getStartTime() <= enemyDefHourTimestamp + time_utils.ONE_HOUR and requiredDirection == item.getDirection() and not item.isEnded() else False
