...
class StressTimer(object):
    ...
    def timeToDelete(self):
        return self.leftTime if self.isPaused else max(0.0, self.endTime - BigWorld.serverTime())
...
class EntityStressTimer(DynamicScriptComponent):
    ...
    @property
    def timeToDelete(self):
        return self.timerInfo.left if self.isPaused else max(0.0, self.timerInfo.endTime - BigWorld.serverTime())
