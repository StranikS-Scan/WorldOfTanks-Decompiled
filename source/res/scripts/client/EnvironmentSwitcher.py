...
class EnvironmentSwitcher(BigWorld.Entity, CustomCallbackDelayer):
    ...
    def onEnterWorld(self, prereqs):
        ...
        if envData:
            ...
            currentEnvID = envData.getCurrentEnvironmentID()
            self.startSwitch(envData.getVisualLayerEnvironmentID(currentEnvID))
    ...
    def _onEnvironmentUpdated(self, envID):
        envData = self._getEnvironmentData()
        if envData:
            visualLayerEnvID = envData.getVisualLayerEnvironmentID(envID)
            self.startSwitch(visualLayerEnvID)
