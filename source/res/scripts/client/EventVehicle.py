...
class _EventBossAppearance(_EventVehicleAppearance):
    ...
    def __updateTickets(self):
        ...
        if descriptor is not None:
            ...
            isSpecial = VEHICLE_TAGS.WT_SPECIAL_BOSS in tags
        if isSpecial:
            hasTickets = bool(self.__gameEventCtrl.getSpecialBossBattlesRemaining())
        else:
            getBossTicketCount = self.__gameEventCtrl.getBossTicketCount
            bossType = getBossType(tags)
            hasTickets = bool(getBossTicketCount(bossType)) or bool(getBossTicketCount(bossType))
...
class EventVehicle(ClientSelectableCameraVehicle):
    ...
    def select(self, descriptor):
        ...
        if self.eventType == VEHICLE_TAGS.WT_HUNTER:
            ClientSelectableCameraObject.switchCamera(self, 'EventTankHunter')
        elif self.eventType == VEHICLE_TAGS.WT_BOSS:
            ClientSelectableCameraObject.switchCamera(self, 'EventTankBoss')
        elif self.eventType == VEHICLE_TAGS.WT_BOSS_2025:
            ClientSelectableCameraObject.switchCamera(self, 'EventTankBoss_2025')
    ...
    def _chooseVehicle(self):
        vehicle = None
        isBoss = self.eventType in (VEHICLE_TAGS.WT_BOSS, VEHICLE_TAGS.WT_BOSS_2025)
        if isBoss:
            vehicles = self._itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS([VEHICLE_TAGS.WT_SPECIAL_BOSS]))
            if vehicles:
        ...
        if vehicles:
            if isBoss:
                vehicle = findFirst(lambda veh: VEHICLE_TAGS.WT_SPECIAL_BOSS in veh.tags, vehicles.values(), vehicles.values()[0])
            elif self.eventType == VEHICLE_TAGS.WT_HUNTER:
    ...
    def _createAppearance(self):
        ...
        return _EventBossAppearance(self.spaceID, self, turretYaw=vehicleTurretYaw, gunPitch=vehicleGunPitch) if self.eventType in (VEHICLE_TAGS.WT_BOSS, VEHICLE_TAGS.WT_BOSS_2025) else _EventVehicleAppearance(self.spaceID, self, turretYaw=vehicleTurretYaw, gunPitch=vehicleGunPitch)
