...
class _EventBossAppearance(_EventVehicleAppearance):
    ...
    def __updateTickets(self):
        ...
        if descriptor is not None:
            isSpecial = VEHICLE_TAGS.WT_SPECIAL_BOSS in descriptor.type.tags
        if isSpecial:
            hasTickets = bool(self.__gameEventCtrl.getSpecialBossBattlesRemaining())
        else:
            hasTickets = bool(self.__gameEventCtrl.getTicketCount())
...
class EventVehicle(ClientSelectableCameraVehicle):
    ...
    def _chooseVehicle(self):
        vehicle = None
        if self.eventType == VEHICLE_TAGS.WT_BOSS:
    ...
    def _createAppearance(self):
        return _EventBossAppearance(self.spaceID, self) if self.eventType == VEHICLE_TAGS.WT_BOSS else _EventVehicleAppearance(self.spaceID, self)
