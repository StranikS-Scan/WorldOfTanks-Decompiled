...
class _EventBossAppearance(_EventVehicleAppearance):
    ...
    def __onLoad(self):
        if self.findComponentByType(InstantStatuses.GunReloadedComponent) is None:
            self.createComponent(InstantStatuses.GunReloadedComponent)
...
class EventVehicle(ClientSelectableCameraVehicle):
    ...
    def _getPortalEntity(self):
        ...
        return first(query.values()) if not query.empty() else None
    ...
    def _chooseVehicle(self):
        ...
        if vehicles:
            if self.eventType == VEHICLE_TAGS.EVENT_BOSS:
                vehicle = findFirst(lambda veh: VEHICLE_TAGS.EVENT_SPECIAL_BOSS in veh.tags, vehicles.values(), first(vehicles.values()))
            elif self.eventType == VEHICLE_TAGS.EVENT_HUNTER:
                lastInvID = self._prebattleVehicle.lastInvID
                vehicle = findFirst(lambda veh: veh.invID == lastInvID, vehicles.values(), first(vehicles.values()))
