...
from constants import ARENA_PERIOD, ARENA_BONUS_TYPE, EVENT
...
class HW21BattleResultsStories(object):
    __settingsCore = dependency.descriptor(ISettingsCore)
    __battleResults = dependency.descriptor(IBattleResultsService)
    __eventsCache = dependency.descriptor(IEventsCache)
    _NarrativeSoundInfo = namedtuple('_NarrativeSoundInfo', ('wasPlayed', 'eventId', 'storeKey', 'index'))
    _ArenaNarrativeInfo = namedtuple('_ArenaNarrativeInfo', ['environmentID', 'difficulty'])
    _ArenaNarrativeInfo.__new__.__defaults__ = (EVENT.INVALID_ENVIRONMENT_ID, EVENT.INVALID_DIFFICULTY_LEVEL)
    isInited = property(lambda self: self.__isInited)

    def __init__(self):
        self.__isInited = False
        self.__arenasNarrativeInfoCache = {}
    ...
    def getSoundEvents(self, event, wwSetup, arenaUniqueID):
        soundEvents = []
        narrativeInfo = self.__getBattleNarrativeInfo(arenaUniqueID)
        if narrativeInfo is not None and narrativeInfo.difficulty != EVENT.INVALID_DIFFICULTY_LEVEL and narrativeInfo.environmentID != EVENT.INVALID_ENVIRONMENT_ID:
            from gui.sounds.sound_constants import HW21SoundConsts
            if event == MUSIC_EVENT_COMBAT_VICTORY:
                soundEvents.append(HW21SoundConsts.HANGAR_BR_WIN_VO_TEMPLATE.format(narrativeInfo.difficulty))
            elif event == MUSIC_EVENT_COMBAT_LOSE:
                confg = self.__eventsCache.getGameEventData()['difficulty']['levelsEnvironment']
                difficultyConfg = confg[narrativeInfo.difficulty]
                isBossFight = narrativeInfo.environmentID == difficultyConfg['bossFightEnvironmentID']
                soundEvents.append(HW21SoundConsts.HANGAR_TEAMFIGHT_LOSE if not isBossFight else HW21SoundConsts.HANGAR_BOSSFIGHT_LOSE)
        return soundEvents

    def __getBattleNarrativeInfo(self, arenaUniqueID):
        return self.__arenasNarrativeInfoCache.get(arenaUniqueID)

    def __onBattleResultPosted(self, reusable, composer, *args):
        arenaUniqueID = reusable.arenaUniqueID
        if reusable.common.arenaBonusType == ARENA_BONUS_TYPE.EVENT_BATTLES:
            info = composer.getVO()['hw21NarrativeInfo']
            self.__arenasNarrativeInfoCache[arenaUniqueID] = self._ArenaNarrativeInfo(*info)
...
class MusicController(object):
    ...
    def __init__(self):
        ...
        self.__helpers = {'hw21BattleResultsStories': HW21BattleResultsStories()}
        self.init()
        return
    ...
    def __getArenaSoundEvents(self, eventId):
        if not self.__lastBattleArenaWwSetup and eventId in (MUSIC_EVENT_COMBAT_VICTORY, MUSIC_EVENT_COMBAT_LOSE, MUSIC_EVENT_COMBAT_DRAW):
            return []
        soundEvents = []
