...
class NewYearSelectableObject(NewYearVisualObject, ScriptGameObject, ISelectableObject):
    ...
    def onEnterWorld(self, prereqs):
        ...
        if self.outlineModelName:
            self.__outlineEntityId = self.createEntity(self.spaceID, self.matrix, {'modelName': self.outlineModelName,
             ...
             'ignoreDistanceOnPicking': self.ignoreDistanceOnPicking}, entityType=NewYearSelectableObject.__name__)
        elif self.ignoreDistanceOnPicking > 0:
            self.ignoreDistOnPicking = True
    ...
    def setHighlight(self, show):
        if show:
            if not self.__highlighted and self.__enabled:
                BigWorld.wgAddEdgeDetectEntity(self, 3, self.edgeMode, False, True, True)
