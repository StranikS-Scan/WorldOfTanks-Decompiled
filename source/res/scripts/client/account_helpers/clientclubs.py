...
InviteNotice = namedtuple('InviteNotice', ('actionType', 'clubDBID', 'accountDBID', 'inviterDBID'))
ApplicationNotice = namedtuple('ApplicationNotice', ('actionType', 'clubDBID', 'accountDBID'))
...
class ClientClubs(object):
    ...
    def synchronize(self, isFullSync, diff):
        ...
        if cacheDiff is not None:
            for key in ('relatedToClubs', 'cybersportSeasonInProgress'):
                if key in cacheDiff:
    ...
    def onClubNotification(self, pickledNotification):
        ...
        elif notificationType in ACCOUNT_NOTIFICATION_TYPE.WEB:
            notice = None
        else:
