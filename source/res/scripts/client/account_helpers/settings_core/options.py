    ...
    def fetch(self, type, excludedNames=None):
        ...
        for name, param in self.settings:
            ...
            if isinstance(param, SettingsContainer):
                ...
                if values:
                    result[name] = values
            if isinstance(param, SettingAbstract):
...
class MouseSetting(ControlSetting):
    CAMERAS = {CTRL_MODE_NAME.POSTMORTEM: (ArcadeCamera.getCameraAsSettingsHolder, 'postMortemMode/camera'),
     ...
     CTRL_MODE_NAME.ASSAULT_SPG: (AssaultCamera.getCameraAsSettingsHolder, 'assaultMode/camera')}
...
class KeyboardSettings(SettingsContainer):
    ...
    def __init__(self):
        ...
        settings = [(CONTROLS.KEYS_LAYOUT, GlobalReadOnlySetting(self._getLayout)), (CONTROLS.KEYS_TOOLTIPS, GlobalReadOnlySetting(lambda : self.KEYS_TOOLTIPS))]
...
class InterfaceScaleSetting(UserPrefsFloatSetting):
    ...
    def get(self):
        ...
        return scale if BattleReplay.isPlaying() else self.__checkAndCorrectScaleValue(scale)
