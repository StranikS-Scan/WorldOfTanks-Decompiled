...
class BRDeathZoneComponent(ClientArenaComponent):
    ...
    ZONE_WAITING = 1
    ZONE_ACTIVE = 2
    COLOR_WAITING = (1.0, 0.8, 0.0, 0.0)
    COLOR_ACTIVE = (0.8, 0.0, 0.0, 0.0)
    __slots__ = ('__cornerPosition', '__zoneSizeX', '__zoneSizeY', '__zonePositionOffset', '__zoneScale__visualizersId', 'onDeathZoneUpdate', '__updatedZones__zoneStates', '__callbackID', '__currentZoneState')
    ...
    def _drawZones(self):
        ...
        for zone in self.__updatedZones:
            ...
            if state == BRDeathZoneComponent.ZONE_ACTIVE:
                color = BRDeathZoneComponent.COLOR_ACTIVE
            else:
                color = BRDeathZoneComponent.COLOR_WAITING
