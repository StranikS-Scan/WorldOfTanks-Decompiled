...
from battleground.loot_object import ModelComponent, SequenceComponent
...
class ParachuteCargo(ScriptGameObject, CallbackDelayer):
    ...
    def activate(self):
        ...
        self.landingAnimation.bindToModel(self.model.compoundModel)
...
class Airdrop(ScriptGameObject, CallbackDelayer, ISelfAssembler):
    __dynamicObjectsCache = dependency.descriptor(IBattleDynamicObjectsCache)
    __sessionProvider = dependency.descriptor(IBattleSessionProvider)
...
class Airdrops(object):
    drops = property(lambda self: self.__drops)

    def __init__(self):
        self.__drops = {}

    def destroy(self):
        for drop in self.__drops.values():
            drop.destroy()
    ...
    def scheduleLoot(self, dropID, position, serverTime):
        if dropID in self.drops:
            return
        clientTime = serverTime + BigWorld.time() - BigWorld.serverTime()
        airdrop = Airdrop(dropID, position, clientTime)
        airdrop.activate()
