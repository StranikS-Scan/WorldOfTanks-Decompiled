...
from cgf_script.managers_registrator import tickGroup
...
class HangarCameraManager(CGF.ComponentManager):
    ...
    def activate(self):
        if not self._hangarSpace.inited:
            return
        else:
    ...
    def deactivate(self):
        ...
        FovExtended.instance().onSetFovSettingEvent -= self.__onSetFovSetting
        if self.onCameraSwitched is not None:
            self.onCameraSwitched.clear()
            self.onCameraSwitched = None
        if self.onCameraSwitchCancel is not None:
            self.onCameraSwitchCancel.clear()
            self.onCameraSwitchCancel = None
        self.__destroyCameraComponents()
        return
    ...
    def enablePlatoonMode(self, enable=True):
        self.__cameraMode = CameraMode.PLATOON if enable else CameraMode.DEFAULT
    ...
    def __setupCamera(self, gameObject, resetTransform=True):
        ...
        if idleComponent:
            ...
            self.__cameraIdle = HangarCameraIdle(self.__cam, idleComponent.easingInTime, pitchParams, distParams, idleComponent.yawPeriod)
        parallaxComponent = gameObject.findComponentByType(ParallaxComponent)
        if parallaxComponent:
            self.__cameraParallax = HangarCameraParallax(self.__cam, parallaxComponent.distanceDelta, parallaxComponent.angelsDelta, parallaxComponent.smoothing)
    ...
    def __destroyCameraComponents(self):
