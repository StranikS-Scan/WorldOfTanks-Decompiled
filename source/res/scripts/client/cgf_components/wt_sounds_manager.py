...
class EndBattleSoundManager(CGF.ComponentManager):
    ...
    _PLAYER_WT_HUNTERS_WIN_ALTERNATIVE = ('wt23_hunters_vo_hunters_win', 'wt23_hunters_vo_hunters_win_fb')
    _PLAYER_HUNTER_WT_WIN = 'wt23_dialogue_vo_wt_win'
    _PLAYER_HUNTER_HUNTERS_WIN = ('wt23_hunters_vo_hunters_win', 'wt23_hunters_vo_hunters_win_fb')
    _GAMEPLAY_EXIT = 'ev_white_tiger_gameplay_exit'

    def __init__(self):
        super(EndBattleSoundManager, self).__init__()
        self.__chanceAltLine = int(sound_helpers.getEventInfo('wt23_hunters_vo_hunters_win', 'altLineChance'))
        ...
        if period == ARENA_PERIOD.AFTERBATTLE:
            ...
            if isWinner:
                if wt_helpers.isBoss():
                    ...
                    for notification in self._PLAYER_HUNTER_HUNTERS_WIN:
                        sound_helpers.playNotification(notification)

            elif wt_helpers.isBoss():
                if self.__chanceAltLine and random.randint(1, 100) <= self.__chanceAltLine:
                    for notification in self._PLAYER_WT_HUNTERS_WIN_ALTERNATIVE:
                        sound_helpers.playNotification(notification)
...
class GeneratorCaptureSoundManager(CGF.ComponentManager):
    ...
    _PLAYER_HUNTER_GENERATOR_DESTROYED = ('wt23_hunters_vo_shield_destroyed', 'wt_hunters_vo_generator_destroyed')
    _PLAYER_HUNTER_LAST_GENERATOR_DESTROYED = ('wt23_hunters_vo_last_generator_destroyed', 'wt_hunters_vo_destruction_last_generator')
    _PLAYER_BOSS_GENERATOR_DESTROYED = ('wt23_w_vo_shield_destroyed', 'wt_w_vo_destruction_generator')
    _PLAYER_BOSS_LAST_GENERATOR_DESTROYED = ('ev_wt23_w_vo_last_generator_destroyed', 'wt_w_vo_destruction_last_generator')
    ...
    def __init__(self):
        ...
        self.__chanceAltLine = {True: int(sound_helpers.getEventInfo('wt_w_vo_destruction_generator', 'altLineChance')),
         False: int(sound_helpers.getEventInfo('wt_hunters_vo_generator_destroyed', 'altLineChance'))}
    ...
    def onBattleStageAdded(self, component, go):
        if component.current < component.maximum:
            isBoss = wt_helpers.isBoss()
            eventSelection = int(self.__useAltLine(isBoss))
            self.__capturedIDs = None
            if component.current == 0:
                if isBoss:
                    sound_helpers.playNotification(self._PLAYER_BOSS_LAST_GENERATOR_DESTROYED[eventSelection])
                else:
                    sound_helpers.playNotification(self._PLAYER_HUNTER_LAST_GENERATOR_DESTROYED[eventSelection])
            elif isBoss:
                sound_helpers.playNotification(self._PLAYER_BOSS_GENERATOR_DESTROYED[eventSelection])
            else:
                sound_helpers.playNotification(self._PLAYER_HUNTER_GENERATOR_DESTROYED[eventSelection])
        return

    def __useAltLine(self, isBoss):
        if not WTBattleCountManager.isExpert(isBoss) or not self.__chanceAltLine[isBoss]:
            return False
        if isBoss:
            if random.randint(1, 100) <= self.__chanceAltLine[isBoss]:
                return True
        else:
            playerIsCapturer = False
            if self.__capturedIDs:
                playerIsCapturer = BigWorld.player().vehicle.id in self.__capturedIDs
            if playerIsCapturer:
                if random.randint(1, 100) <= self.__chanceAltLine[isBoss]:
                    return True
        return False
...
class BossAbilitySoundManager(CGF.ComponentManager):
    ...
    def onStunComponentAdded(self, _, go):
        ...
        if self.__isBossImpulse():
            if wt_helpers.isBoss() and not wt_helpers.isBossTeam(vehicle):
