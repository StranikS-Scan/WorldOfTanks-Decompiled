...
import Projectiles
import logging
from constants import ARENA_PERIOD, ATTACK_REASON, ATTACK_REASONS, EQUIPMENT_STAGES
...
class ShieldSoundManager(CGF.ComponentManager):
    ...
    _PLAYER_W_HUNTER_SHOOTING_AT_WT_SHIELD = 'wt23_hunters_vo_shooting_at_wt_shield'
    _PLAYER_W_BOSS_SHOOTING_AT_WT_SHIELD = 'wt23_boss_vo_shooting_at_wt_shield'
    ...
    def onShotDamageReceived(self, _, go):
        ...
        if vehicle is not None and wt_helpers.isBossVehicle(vehicle):
            if not wt_helpers.getHasDebuff():
                ...
                if wt_helpers.getBossVehicleHealthPercent() > 3.0:
                    if wt_helpers.isBoss():
                        sound_helpers.playNotification(self._PLAYER_W_BOSS_SHOOTING_AT_WT_SHIELD)
                    else:
