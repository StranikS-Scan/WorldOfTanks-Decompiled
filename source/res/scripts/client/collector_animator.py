...
class CollectorSoundsContainer(GameEventGetterMixin):
    _FILL_NOTIFICATIONS_BY_ENV_ID = {0: {100: 'hw19_vo_notification_collector_fill_100'},
     1: {50: 'hw19_vo_notification_collector_fill_50',
         100: 'hw19_vo_notification_collector_fill_100'},
     2: {50: 'hw19_vo_notification_collector_fill_50',
         100: 'hw19_vo_notification_collector_fill_100'},
     3: {50: 'hw19_vo_notification_collector_fill_50',
         75: 'hw19_vo_notification_collector_fill_85'}}
    ...
    def __init__(self):
        ...
        self._fillPercentage = None
        self._currentEventEnvID = 0
        return
    ...
    def _soulsAndRingUpdate(self, ringLevel, currentSouls, maxSouls):
        if not currentSouls or not maxSouls:
            ...
            fillPercentage = min(100, max(1, 100 * currentSouls // maxSouls))
            if self._fillPercentage is None:
                self._fillPercentage = fillPercentage
            prevFill, self._fillPercentage = self._fillPercentage, fillPercentage
            envFillNotifications = self._FILL_NOTIFICATIONS_BY_ENV_ID[self._getEventEnvID()]
            for progress in xrange(prevFill + 1, self._fillPercentage + 1):
                notification = envFillNotifications.get(progress)
                if notification is not None:
                    getSoundNotifications().play(notification)

            return

    def _getEventEnvID(self):
        if self.environmentData is not None:
            self._currentEventEnvID = self.environmentData.getCurrentEnvironmentID() % 10
        return self._currentEventEnvID
