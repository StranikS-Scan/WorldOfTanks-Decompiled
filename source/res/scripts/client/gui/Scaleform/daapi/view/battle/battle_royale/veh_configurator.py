...
from gui.Scaleform.daapi.view.common.battle_royale_helpers import getHotKeyListByIndex
...
from gui.doc_loaders.battle_royale_settings_loader import getTreeModuleHeader
...
class BattleVehicleConfiguratorCmp(VehicleModulesConfiguratorCmp, IProgressionListener):
    ...
    def onClick(self, intCD, columnIdx, moduleIdx):
        ...
        success = super(BattleVehicleConfiguratorCmp, self).onClick(intCD, columnIdx, moduleIdx)
        if success:
...
class BattleVehicleConfigurator(BattleVehicleConfiguratorMeta, IProgressionListener, BattleGUIKeyHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__cmpAliases = {BATTLE_VIEW_ALIASES.BATTLE_VEH_MODULES_CONFIGURATOR_CMP}
    ...
    def onModuleMouseOver(self, intCD, icon):
        vo = self.__getModuleInfoPanel(intCD, icon)
        if vo is not None:
    ...
    def _onRegisterFlashComponent(self, viewPy, alias):
        ...
        if isinstance(viewPy, IProgressionListener):
            viewPy.setLevel(self.__xpLevelData.level, self.__xpLevelData.minXp, self.__xpLevelData.maxXp)
            viewPy.setCurrentXP(self.__xpLevelData.xp, self.__xpLevelData.xpPercent)
            if self.__xpLevelData.isMaxLevel:
                viewPy.onMaxLvlAchieved()
    ...
    def __onComponentModulesChanged(self, modules):
        if len(modules) == 2:
            ...
            _logger.warning('Unexpected modules count. Modules list: %s', str(modules))

    def __getModuleInfo(self, intCD, icon, index):
        module = self.__getModuleItem(intCD)
        return {'header': getTreeModuleHeader(module.descriptor.id),
         'parameters': getShortListParameters(module, self.__getVehicle(), self.__getCurrentModuleItem(intCD)),
         'hotKeys': getHotKeyListByIndex(index),
         'module': {'icon': icon,
                    'intCD': intCD}}

    def __getModuleInfoPanel(self, intCD, icon):
    ...
    def __getCurrentModuleItem(self, intCD):
        return self.__getProgressionCtrl().getInstalledAnalog(intCD)
