...
# Embedded file name: scripts/client/gui/Scaleform/daapi/view/battle/epic/timers_panel.py
...
class EpicDestroyTimersPanel(EpicDestroyTimersPanelMeta):
    ...
    def __init__(self):
        ...
        deathZonesCodes[DEATH_ZONES.SECTOR_AIRSTRIKE] = BATTLE_NOTIFICATIONS_TIMER_TYPES.SECTOR_AIRSTRIKE
        deathZonesSoundIDs = {}
    ...
    def _generateMainTimersData(self):
        ...
        data = super(EpicDestroyTimersPanel, self)._generateMainTimersData()
        data.append(self._getNotificationTimerData(BATTLE_NOTIFICATIONS_TIMER_TYPES.UNDER_FIRE, BATTLE_NOTIFICATIONS_TIMER_LINKAGES.UNDER_FIRE_ICON, link))
        data.append(self._getNotificationTimerData(BATTLE_NOTIFICATIONS_TIMER_TYPES.SECTOR_AIRSTRIKE, BATTLE_NOTIFICATIONS_TIMER_LINKAGES.AIRSTRIKE_ICON, link))
        data.append(self._getNotificationTimerData(BATTLE_NOTIFICATIONS_TIMER_TYPES.RECOVERY, BATTLE_NOTIFICATIONS_TIMER_LINKAGES.RECOVERY_ICON, link))
        return data
    ...
    def _onVehicleStateUpdated(self, state, value):
        ...
        if state == VEHICLE_VIEW_STATE.SWITCHING:
            if self.__underFireCount > 0:
                self.__underFireCount = 0
                self._hideTimer(BATTLE_NOTIFICATIONS_TIMER_TYPES.UNDER_FIRE)
    ...
    def __setUnderFire(self, isUnderFire):
        ...
        if self.__underFireCount == 1:
            self._showTimer(BATTLE_NOTIFICATIONS_TIMER_TYPES.UNDER_FIRE, 0, 'warning', BigWorld.serverTime())
    ...
    def __setRecoveryActive(self, values):
        ...
        if isActivated and avatar_getter.getInputHandler().ctrlModeName not in (CTRL_MODE_NAME.POSTMORTEM, CTRL_MODE_NAME.DEATH_FREE_CAM):
            self._showTimer(BATTLE_NOTIFICATIONS_TIMER_TYPES.RECOVERY, duration, 'critical', endOfTimer)
        else:
