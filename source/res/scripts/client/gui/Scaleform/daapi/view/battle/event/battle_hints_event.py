...
# Embedded file name: scripts/client/gui/Scaleform/daapi/view/battle/event/battle_hints_event.py
import logging
...
class BattleHintData(namedtuple('_HintData', ('name', 'componentAlias', 'iconPath', 'duration', 'maxWaitTime', 'priority', 'soundFx', 'soundNotification', 'htmlTemplate', 'rawMessage', 'backgroundColor', 'useCountdownTimer', 'applyCountdownTimerImmediately'))):
    ...
    def makeVO(self, data=None):
        ...
        if 'param1' in data and 'duration=' in data['param1']:
            duration = int(float(data['param1'].split('=')[1]))
            data.pop('param1')
        if 'param2' in data and 'useCountdownTimer=' in data['param2']:
            useCountdownTimer = bool(str(data['param2']).split('=')[1])
            data.pop('param2')
        ...
        return {'message': message,
         ...
         'applyCountdownTimerImmediately': self.applyCountdownTimerImmediately}
