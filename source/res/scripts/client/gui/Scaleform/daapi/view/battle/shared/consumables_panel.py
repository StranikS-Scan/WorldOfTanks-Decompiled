...
class ConsumablesPanel(ConsumablesPanelMeta, BattleGUIKeyHandler):
    ...
    def _addShellSlot(self, idx, intCD, descriptor, quantity, gunSettings):
        ...
        self.__keys[keyCode] = partial(self.__handleAmmoPressed, intCD)
    ...
    def _addEquipmentSlot(self, idx, intCD, item=None):
        ...
        if item is None:
            ...
            self.as_addEquipmentSlotS(idx, bwKey, sfKey, None, 0, 0, 0, None, EMPTY_EQUIPMENT_TOOLTIP, ANIMATION_TYPES.NONE)
            ...
            self.as_addEquipmentSlotS(idx, bwKey, sfKey, tag, quantity, timeRemaining, reloadingTime, iconPath, toolTip, animationType)
        return
    ...
    def __handleEquipmentExpanded(self, intCD):
        ...
        if ctrl is None:
            ...
            if intCD not in self._cds:
                LOG_ERROR('Equipment is not found in panel', intCD, self._cds)
                return
            if not item:
    ...
    def __onShellsUpdated(self, intCD, quantity, *args):
        if intCD in self._cds:
            ...
            LOG_ERROR('Ammo is not found in panel', intCD, self._cds)
    ...
    def __onGunReloadTimeSet(self, currShellCD, state):
        if currShellCD in self._cds:
            index = self._cds.index(currShellCD)
            ...
            LOG_ERROR('Ammo is not found in panel', currShellCD, self._cds)
    ...
    def __onOptionalDeviceUpdated(self, intCD, isOn):
        if intCD in self._cds:
            ...
            LOG_ERROR('Optional device is not found in panel', intCD, self._cds)
            ...
            if state == VEHICLE_VIEW_STATE.DEVICES:
                ...
                if idx > 0 and idx < len(self._cds):
                    ...
                    if item and item.isEntityRequired():
                        bwKey, _ = self.__genKey(idx)
                        self.__keys[bwKey] = partial(self.__handleEquipmentPressed, self._cds[idx], deviceName)
    ...
    def __canApplyingGlowEquipment(self, equipment):
        if equipment.getTag() in ('extinguisher', 'regenerationKit'):
            correction = True
            entityName = None
