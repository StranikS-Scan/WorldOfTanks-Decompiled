...
from aih_constants import CHARGE_MARKER_STATE, CTRL_MODE_NAME, GUN_MARKER_TYPE
...
class ArtyCameraDistancePlugin(_DistancePlugin):
    ...
    def __updateCameraDistance(self):
        ...
        if inputHandler is None or inputHandler.ctrlModeName not in (CTRL_MODE_NAME.STRATEGIC, CTRL_MODE_NAME.ARTY) or not inputHandler.ctrl.isEnabled:
            ...
            if inputHandler.ctrlModeName == CTRL_MODE_NAME.STRATEGIC:
                leftValue, rightValue = self._MIN_VALUE, self._MID_VALUE
            else:
                leftValue, rightValue = self._MID_VALUE, self._MAX_VALUE
            value = (1.0 - inputHandler.ctrl.getCamDistRatio()) * (rightValue - leftValue + 1) + leftValue
            self._parentObj.as_updateScaleWidgetS(self._MIN_VALUE, self._MID_VALUE, self._MAX_VALUE, int(value))
            return
