...
class _ExchangeCreditsForSackSubmitter(_ExchangeCreditsSubmitter):

    def _getResourceToExchange(self):
        return self._exchangeItem.price - self.itemsCache.items.stats.credits


class _SackInfoItem(InfoItemBase):
...
class _SackExchangeItem(_ExchangeItem):

    def __init__(self, price, infoItem):
        super(_SackExchangeItem, self).__init__(None, 1)
        self.__infoItem = infoItem
        self.__price = price
        return
...
class ExchangeCreditsForSackMeta(_ExchangeDialogMeta, _ExchangeCreditsSubscriber):

    def __init__(self, name, price, key='confirmExchangeDialog/exchangeCredits'):
        infoItem = _SackInfoItem(name)
        super(ExchangeCreditsForSackMeta, self).__init__(_SackExchangeItem(price, infoItem), key)

    def getEventType(self):
        return events.ShowDialogEvent.SHOW_EXCHANGE_DIALOG_TOP_WINDOW

    def _getSubmitterType(self):
        return _ExchangeCreditsForSackSubmitter
