...
from gui.Scaleform.daapi.view.common.battle_royale_helpers import getHotKeyString
...
def _getTabData():
    return ({'id': BATTLEROYALE_ALIASES.VEH_MODULES_CONFIGURATOR_CMP,
      'label': backport.text(R.strings.battle_royale.hangarVehicleInfo.moduleTreeTab()),
      'linkage': 'VehModuleConfiguratorCmpUI',
      'selected': True,
      'enabled': True,
      'tipText': text_styles.highlightText(backport.text(R.strings.battle_royale.hangarVehicleInfo.moduleTreeTip(), key=text_styles.neutral(getHotKeyString(CommandMapping.CMD_UPGRADE_PANEL_SHOW)))),
      'tooltipComplexStr': None,
      'tooltipSpecialId': TOOLTIPS_CONSTANTS.BATTLE_ROYALE_BATTLE_PROGRESSION}, {'id': BATTLEROYALE_ALIASES.VEHICLE_WEAK_ZONES_CMP,
...
def _getVehicleWeakZonesImage(vehicle):
    ...
    return backport.image(R.images.gui.maps.icons.battleRoyale.weakZones.dyn(vehicleName)())
...
class HangarVehicleInfo(BattleRoyaleVehicleInfoMeta):
    __itemsCache = dependency.descriptor(IItemsCache)
    ...
    def _populate(self):
        ...
        if self.__isFirstEnter:
            ...
            self.__blur.enable(APP_CONTAINERS_NAMES.SUBVIEW, [])
    ...
    def __showIntroPage(self):
        blurIntroLayers = [APP_CONTAINERS_NAMES.VIEWS,
        ...
        self.__blur.enable(APP_CONTAINERS_NAMES.OVERLAY, blurIntroLayers)
