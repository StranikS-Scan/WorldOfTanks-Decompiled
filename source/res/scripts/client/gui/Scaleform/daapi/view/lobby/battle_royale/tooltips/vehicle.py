...
class VehicleTooltipData(BlocksTooltipData):
    ...
    def __constructHeaderBlock(cls, vehicle):
        ...
        leftTime = getDueDateOrTimeStr(cls.__battleRoyaleController.getEndTime())
        headerBlock.append(formatters.packTextParameterWithIconBlockData(name=text_styles.neutral(backport.text(R.strings.tooltips.battle_royale.hangar.vehicle.leftTime(), leftTime=leftTime)), value='', icon=ICON_TEXT_FRAMES.RENTALS, iconYOffset=2, padding=formatters.packPadding(left=-60, bottom=-20)))
        return headerBlock
