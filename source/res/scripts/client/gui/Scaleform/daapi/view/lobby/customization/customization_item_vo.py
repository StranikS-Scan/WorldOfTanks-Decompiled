...
def buildCustomizationItemDataVO(item, count=None, isApplied=False, isDarked=False, isUsedUp=False, autoRentEnabled=False, vehicle=None, progressionLevel=None, icon=None, showDetailItems=True, plainView=False, showEditableHint=False, showEditBtnHint=False, addExtraName=True):
    if plainView:
        hasBonus = False
        locked = False
...
def __getEditableBlockData(item, vehicle=None):
    ...
    if isEditableStyle and vehicle is not None:
        ...
        editBtnEnabled = item.canBeEditedForVehicle(vehicleIntCD)
        if not editBtnEnabled:
        ...
        editBtnEnabled, editableIcon = False, ''
    return (editBtnEnabled, editableIcon)
