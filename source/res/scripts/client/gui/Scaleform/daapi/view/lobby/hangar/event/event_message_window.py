...
class EventMessageWindow(EventMessageWindowMeta):
    ...
    def __updateDynamicData(self):
        ...
        if 'currency' not in data:
            ...
            return
        if data['currency'] == 'hw_rewardBoxKey':
            currency = data['currency']
            style = getattr(text_styles, ''.join(('credits', 'TextBig')))
            icon = getattr(icons, currency.replace('hw_', ''))
            data['costString'] = text_styles.concatStylesWithSpace(text_styles.vehicleName(backport.text(R.strings.bootcamp.cost.messageViewBuy())), text_styles.concatStylesWithSpace(style(integralFormat(data['costValue'])), icon()))
            return
        currency = data['currency']
