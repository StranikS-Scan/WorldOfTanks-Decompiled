...
class _SelectorItem(object):
    ...
    def getFightButtonLabel(self, state, playerInfo, isEvent):
        if state.isInUnit():
            showRaceLabel = state.isInUnit(constants.PREBATTLE_TYPE.EVENT)
        else:
            showRaceLabel = isEvent
        label = MENU.HEADERBUTTONS_RACE if showRaceLabel else MENU.HEADERBUTTONS_BATTLE
...
class _SimpleSquadItem(_SelectorItem):
    ...
    def __init__(self, label, data, order, selectorType=None, isVisible=True, subscription=''):
    ...
    def getVO(self):
        vo = super(_SimpleSquadItem, self).getVO()
        vo['label'] = self.getFormattedLabel() + '\n' + text_styles.main(i18n.makeString(self._subscription))
        return vo
...
def _addSimpleSquadType(items):
    ...
    items.append(_SimpleSquadItem(label, PREBATTLE_ACTION_NAME.SQUAD, 0, subscription=MENU.HEADERBUTTONS_BATTLE_TYPES_SIMPLESQUADSUBSCRIPTION))


def _addEventSquadType(items):
    label = text_styles.middleTitle(MENU.HEADERBUTTONS_BATTLE_TYPES_EVENTSQUAD)
    items.append(_EventSquadItem(label, PREBATTLE_ACTION_NAME.EVENT_SQUAD, 1, subscription=MENU.HEADERBUTTONS_BATTLE_TYPES_EVENTSQUADSUBSCRIPTION))
