...
class _OpenTabWebApi(OpenTabWebApi):
    ...
    def _getVehiclePreviewReturnCallback(self, cmd):
        return functools.partial(self.__getShopSalesMainViewReturnCallback, cmd.back_url)

    def _getVehicleStylePreviewCallback(self, cmd):
        return functools.partial(self.__getShopSalesMainViewReturnCallback, cmd.back_url)

    def __getShopSalesMainViewReturnCallback(self, backUrl):
        return self.__shopSales.openMainView(url=backUrl, origin=Origin.VEHICLE_PREVIEW)
