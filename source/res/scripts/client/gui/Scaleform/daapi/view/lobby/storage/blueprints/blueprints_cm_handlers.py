...
class BlueprintsCMHandler(ContextMenu):
    ...
    def convertBlueprintFragment(self):
        isResearchClicked, (usedFragmentsData, _) = yield th_await(dialogs.blueprintsConversion(self._id))
    ...
    def convertMaxBlueprintFragments(self):
        isResearchClicked, (usedFragmentsData, fragmentCount) = yield th_await(dialogs.blueprintsConversion(self._id, fragmentCount=self.__getMaxFragmentCount()))
        if isResearchClicked:
