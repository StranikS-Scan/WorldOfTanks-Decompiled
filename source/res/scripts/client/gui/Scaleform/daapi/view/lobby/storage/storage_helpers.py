...
_HANDLERS_MAP = {GUI_ITEM_TYPE.OPTIONALDEVICE: (CONTEXT_MENU_HANDLER_TYPE.STORAGE_OPTIONAL_DEVICE_ITEM, CONTEXT_MENU_HANDLER_TYPE.STORAGE_OPTIONAL_MODERNIZED_DEVICE_ITEM),
 ...
 GUI_ITEM_TYPE.MENTORING_LICENSE: (CONTEXT_MENU_HANDLER_TYPE.STORAGE_MENTORING_LICENSE_ITEM,)}
...
class OptDeviceBonusesDescriptionBuilder(object):
    ...
    def getDescriptionByKpiStrings(self, item):
        if any((kpi.type == KPI.Type.AGGREGATE_MUL for kpi in item.getKpi())):
            return [item.shortDescriptionSpecial.format(colorTagOpen=self.KPI_HTML_TEMPLATE.source['colorTagOpen'], colorTagClose=self.KPI_HTML_TEMPLATE.source['colorTagClose'])]
