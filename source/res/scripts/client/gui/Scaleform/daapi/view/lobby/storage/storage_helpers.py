...
_CUSTOMIZATION_VEHICLE_CRITERIA = ~REQ_CRITERIA.VEHICLE.IS_PREMIUM_IGR | ~REQ_CRITERIA.VEHICLE.EVENT_BATTLE | ~REQ_CRITERIA.VEHICLE.IS_BOT | ~REQ_CRITERIA.VEHICLE.HIDDEN_IN_HANGAR
...
def isCustomizationAvailableForSell(item, vehicleCD=None):
    return False if item.getSellPrice() == ITEM_PRICE_EMPTY or item.isRentable or item.isHidden or item.isForbiddenToSell else getAvailableForSellCustomizationCount(item, vehicleCD) > 0
...
class OptDeviceBonusesDescriptionBuilder(object):
    ...
    def __createEffectString(self, item):
        ...
        return self._effectStringFormat(effectsList[0]) if effectsList else ''
