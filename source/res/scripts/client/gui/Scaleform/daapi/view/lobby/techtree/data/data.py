...
# Embedded file name: scripts/client/gui/Scaleform/daapi/view/lobby/techtree/data/data.py
import logging
import typing
from gui.Scaleform.daapi.view.lobby.techtree.dumpers import _BaseDumper
from gui.Scaleform.daapi.view.lobby.techtree.settings import NODE_STATE, SelectedNation
    ...
    def _checkTechTreeEvents(self, state, guiItem, unlockProps):
        if g_techTreeDP.techTreeEventsListener.hasActiveAction(guiItem.intCD) and guiItem.itemTypeID == GUI_ITEM_TYPE.VEHICLE:
            ...
            if noUnlockDiscount and guiItem.buyPrices.itemPrice.isActionPrice() and not guiItem.isRestorePossible():
                state = NODE_STATE.add(state, NODE_STATE_FLAGS.ACTION)
        return state

    def _checkEarlyAccessState(self, state, vehicleItem):
        state = NODE_STATE.removeIfHas(state, NODE_STATE_FLAGS.EARLY_ACCESS)
        if vehicleItem.isEarlyAccess:
            state = NODE_STATE.addIfNot(state, NODE_STATE_FLAGS.EARLY_ACCESS)
        return state
