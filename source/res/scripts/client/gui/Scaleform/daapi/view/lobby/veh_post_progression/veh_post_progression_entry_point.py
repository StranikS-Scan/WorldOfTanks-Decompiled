...
class VehPostProgressionEntryPoint(EventSystemEntity):
    ...
    def tryUnlock(self):
        if not self._vehicle:
            return
        isVehicleUnlocked = self._vehicle.postProgression.isUnlocked(self._vehicle)
        if (isVehicleUnlocked.result or isVehicleUnlocked.reason == PostProgressionAvailability.VEH_IS_RENT_OVER) and not self.isUnlockShowed:
    ...
    def __getUnlockState(self):
        isVehicleUnlocked = self._vehicle.postProgression.isUnlocked(self._vehicle)
        return POSTPROGRESSION_CONSTS.RESEARCH_STATE_UNLOCKED if (isVehicleUnlocked.result or isVehicleUnlocked.reason == PostProgressionAvailability.VEH_IS_RENT_OVER) and self.isUnlockShowed else POSTPROGRESSION_CONSTS.RESEARCH_STATE_LOCKED
