...
class PreviewTankman(object):
    ...
    @property
    def tooltip(self):
        return TOOLTIPS_CONSTANTS.VEHICLE_PREVIEW_20_CREW_MEMBER
        ...
        if self.__customCrew:
            ...
            for idx, tankman in currentVehicle.crew:
                ...
                regularCrewList.append({'crewId': roleIdx,
                 ...
                 'tooltip': TOOLTIPS_CONSTANTS.VEHICLE_PREVIEW_20_CREW_MEMBER,
                 'role': role})
...
def getCrewComment(skill, crewLevel, role, forOne):
    ...
    return _ms(key=tKey, level=crewLevel, role=_ms(TOOLTIPS.crewRole(role)), skillType=_ms(TOOLTIPS.VEHICLEPREVIEW_VEHICLEPANEL_INFO_HEADER_CREW_ISPERK if skill.isPerk else TOOLTIPS.VEHICLEPREVIEW_VEHICLEPANEL_INFO_HEADER_CREW_ISABILLITY), skillName=text_styles.statusAlert(_ms(MENU.QUOTE, string=skill.userName)))
