# Embedded file name: scripts/client/gui/arena_info/listeners.py
import weakref
import BigWorld
...
class ArenaListener(_Listener):
    ...
    def addController(self, controller):
        if super(ArenaListener, self).addController(controller) and self.__callbackID is None:
...
class ListenersCollection(_Listener):
    ...
    def addController(self, controller):
        self.__arenaListener.addController(controller)
        self.__chatRostersListener.addController(controller)
        self.__arenaLoadListener.addController(controller)

    def removeController(self, controller):
        self.__arenaListener.removeController(controller)
        self.__chatRostersListener.removeController(controller)
        self.__arenaLoadListener.removeController(controller)
