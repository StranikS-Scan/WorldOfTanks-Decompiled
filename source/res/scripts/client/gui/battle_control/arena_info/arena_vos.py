...
def isEventMiniBossVeh(tags):
    return VEHICLE_TAGS.EVENT_MINI_BOSS in tags


class PlayerInfoVO(object):
    __slots__ = ('accountDBID', 'avatarSessionID', 'name', 'fakeName', 'clanAbbrev', 'igrType', 'personaMissionIDs', 'personalMissionInfo', 'isPrebattleCreator', 'forbidInBattleInvitations', 'isTeamKiller', 'isEventBot', 'isEventMiniBoss')
    eventsCache = dependency.descriptor(IEventsCache)

    def __init__(self, accountDBID=0, avatarSessionID='', name=None, clanAbbrev='', igrType=IGR_TYPE.NONE, personalMissionIDs=None, personalMissionInfo=None, isPrebattleCreator=False, forbidInBattleInvitations=False, fakeName='', isEventBot=False, isEventMiniBoss=False, **kwargs):
        ...
        self.isEventMiniBoss = isEventMiniBoss
    ...
    def getPlayerLabel(self):
        if self.isEventMiniBoss:
            return i18n.makeString(EVENT.BOTNAMES_ERMELINDA)
        if self.isEventBot:
...
class VehicleTypeInfoVO(object):
    __slots__ = ('compactDescr', 'shortName', 'name', 'level', 'iconName', 'iconPath', 'isObserver', 'isPremiumIGR', 'isDualGunVehicle', 'hasDualAccuracy', 'isAutoShootGunVehicle', 'guiName', 'shortNameWithPrefix', 'classTag', 'nationID', 'turretYawLimits', 'maxHealth', 'strCompactDescr', 'isOnlyForBattleRoyaleBattles', 'tags', 'chassisType', 'role', 'isEventBot', 'isEventMiniBoss')
    ...
    def __setVehicleData(self, vehicleDescr=None, maxHealth=None):
        if vehicleDescr is not None:
            ...
            self.isEventMiniBoss = isEventMiniBossVeh(tags)
...
class VehicleArenaInfoVO(object):
    ...
    def __init__(self, vehicleID, team=0, isAlive=None, isAvatarReady=None, isTeamKiller=None, prebattleID=None, events=None, forbidInBattleInvitations=False, ranked=None, badges=None, overriddenBadge=None, prestigeLevel=None, prestigeGradeMarkID=None, teamPanelMode=TEAM_PANEL_MODE.SHOW.value, botDisplayStatus=BOT_DISPLAY_STATUS.REGULAR, **kwargs):
        ...
        if self.vehicleType.isEventMiniBoss:
            self.player.isEventMiniBoss = True
            self.gameModeSpecific.update({EventKeys.CAMP.value: self.player.name})
        return
