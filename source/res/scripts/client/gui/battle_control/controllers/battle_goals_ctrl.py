...
class BattleGoalsController(IArenaVehiclesController, GameEventGetterMixin):
    ...
    def _getRelevantGoal(self, soulsCollected, collectorCapacity):
        bossEnvID = self.environmentData.getBossFightEnvironmentID()
        currentEnvID = self.environmentData.getCurrentEnvironmentID()
        if bossEnvID and currentEnvID == bossEnvID:
            return EventBattleGoal.UNKNOWN
        elif collectorCapacity <= 0 or self._currentCollectorID is None:
            return EventBattleGoal.UNKNOWN
        else:
            aliveVehiclesSouls = self._getAliveAllyVehiclesSouls()
            if not aliveVehiclesSouls:
                return EventBattleGoal.UNKNOWN
            elif soulsCollected >= collectorCapacity:
