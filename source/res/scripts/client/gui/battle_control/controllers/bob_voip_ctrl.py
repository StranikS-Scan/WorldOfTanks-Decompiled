...
# Embedded file name: scripts/client/gui/battle_control/controllers/bob_voip_ctrl.py
import logging
import typing
import BigWorld
...
from account_helpers.AccountSettings import BOB_IS_VOIP_IN_BATTLE_ACTIVATED
...
class BobVOIPController(IBobVOIPController):
    ...
    def isTeamChannelAvailable(self):
        return self.__bobController.getConfig().createVivoxTeamChannels
    ...
    def startControl(self, *_, **__):
        _logger.debug('[BOB] BobVOIPController started.')
        self.__subscribe()

    def stopControl(self):
        _logger.debug('[BOB] BobVOIPController stopped.')
        self.__unsubscribe()
        self.__messageShown = False
        self.__clearCooldown()

    def getControllerID(self):
        return BATTLE_CTRL_ID.BOB_VOIP_CTRL
    ...
    def toggleChannelConnection(self, automatic=False):
        if self.__sessionProvider.isReplayPlaying:
            ...
            _logger.info('Failed to toggle Bob team VOIP channel. Cooldown is in progress.')
            return
        elif not self.isTeamVoipEnabled:
            _logger.error('Failed to toggle Bob team VOIP channel. Joining is not allowed.')
            return
        else:
            _logger.info('toggleChannelConnection')
            if not automatic and self.__isSoloPlayer():
                AccountSettings.setSettings(BOB_IS_VOIP_IN_BATTLE_ACTIVATED, not self.__VOIPManager.isCurrentChannelEnabled())
            event_dispatcher.toggleVoipChannelEnabled(ARENA_BONUS_TYPE.BOB)
    ...
    def __restoreChannelState(self):
        wasActivePreviously = AccountSettings.getSettings(BOB_IS_VOIP_IN_BATTLE_ACTIVATED)
    ...
    def __getMessage(self):
        ...
        resStr = R.strings.bob.battleMessages
        if not self.isVoipEnabled:
            ...
            text = backport.text(resStr.disableVOIPButton(), keyName=getReadableKey(command))
        return ClientActionMessage(msg=text)
