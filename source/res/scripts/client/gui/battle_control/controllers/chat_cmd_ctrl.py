...
TARGET_TYPE_TRANSLATION_MAPPING = {CONTEXTCOMMAND: {MarkerType.VEHICLE_MARKER_TYPE: {DefaultMarkerSubType.ALLY_MARKER_SUBTYPE: BATTLE_CHAT_COMMAND_NAMES.HELPME,
 ...
 BATTLE_CHAT_COMMAND_NAMES.HB_ARTILLERY_ON_YOURSELF: {MarkerType.VEHICLE_MARKER_TYPE: {DefaultMarkerSubType.ALLY_MARKER_SUBTYPE: BATTLE_CHAT_COMMAND_NAMES.HB_ARTILLERY_ON_YOURSELF}},
 BATTLE_CHAT_COMMAND_NAMES.HB_LAST_STAND: {MarkerType.VEHICLE_MARKER_TYPE: {DefaultMarkerSubType.ALLY_MARKER_SUBTYPE: BATTLE_CHAT_COMMAND_NAMES.HB_LAST_STAND}}}
...
class ChatCommandsController(IBattleController):
    ...
    def sendTargetedCommand(self, cmdName, targetID, isInRadialMenu=False):
        if self.__isProhibitedToSendIfDeadOrObserver(cmdName) or not self.__isEnabled or not self.__arenaDP.getVehicleInfo(targetID).isAlive():
            return
