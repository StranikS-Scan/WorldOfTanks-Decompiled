...
class _EquipmentItem(object):
    ...
    def requiresSoundNotification(self):
        return self._tag not in ('extinguisher', 'medkit', 'repairkit', 'festival_ability')
...
class _FestivalAbilityItem(_TriggerItem):

    def __init__(self, descriptor, quantity, stage, timeRemaining, totalTime, tag=None):
        super(_FestivalAbilityItem, self).__init__(descriptor, quantity, stage, timeRemaining, totalTime, tag)
        self._totalTime = totalTime
    ...
    def update(self, quantity, stage, timeRemaining, totalTime):
        super(_FestivalAbilityItem, self).update(quantity, stage, timeRemaining, totalTime)
        self._totalTime = totalTime
...
_EQUIPMENT_TAG_TO_ITEM = {'fuel': _AutoItem,
 ...
 'festival_ability': _FestivalAbilityItem}
...
class _ReplayFestivalItem(_ReplayItem):

    def update(self, quantity, stage, timeRemaining, totalTime):
        super(_ReplayFestivalItem, self).update(quantity, stage, timeRemaining, totalTime)
        self.__step = timeRemaining
        self._timeRemaining = totalTime
        self.__cooldownTime = 0
        if self.__step != 0:
            self.__cooldownTime = BigWorld.serverTime() + (100 - self._timeRemaining) / self.__step

    def getReplayTimeRemaining(self):
        return max(0, self.__cooldownTime - BigWorld.serverTime()) if self.__step != 0 else 0

    def getCooldownPercents(self):
        if self.__step == 0:
            return self._timeRemaining
        totalTime = 100 / self.__step
        timeRemaining = self.getReplayTimeRemaining()
        return round(float(totalTime - timeRemaining) / totalTime * 100.0) if totalTime > 0 else 0.0
...
_REPLAY_EQUIPMENT_TAG_TO_ITEM = {'fuel': _ReplayItem,
 ...
 'festival_ability': _ReplayFestivalItem}
