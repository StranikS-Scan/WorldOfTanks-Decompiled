...
# Embedded file name: scripts/client/gui/battle_control/controllers/korea_msgs_ctrl.py
import sys
import time
...
class KoreaMessagesController(Notifiable, IBattleController):
    ...
    def __getTimeLeftPriority(self):
        return min((self.__getCurfew(), self.CURFEW), (self.__getDailyPlayTimeLeft(), self.DAILY), (self.__getWeeklyPlayTimeLeft(), self.WEEKLY))
    ...
    def __getNotificationStr(self):
        ...
        return backport.text(R.strings.messenger.chat.koreaMessage.dailyLimit(), timeLeft=self.__minutesCount) if priority == self.DAILY else None
