    ...
    def start(self):
        ...
        if stepRepairPointComponent is not None:
            ...
            for idx, (action, time, hphealed) in viewitems(actions):
                ...
                if action not in (REPAIR_POINT_ACTION.CANCEL_REPAIR, REPAIR_POINT_ACTION.LEAVE_WHILE_CD):
    ...
    def start(self):
        ...
        if sectorBaseComp is not None:
            ...
            for idx, (action, time) in viewitems(actions):
                if action not in (SECTOR_BASE_ACTION.LEAVE, SECTOR_BASE_ACTION.LEAVE_WHILE_CD, SECTOR_BASE_ACTION.CAPTURED):
