...
from gui.battle_control.battle_cache.cache_records import TmpBRProgressionCacheRecord
from gui.shared.gui_items.Vehicle import Vehicle
from items import getTypeOfCompactDescr
from helpers import dependency
from gui.shared.gui_items.processors.module import getPreviewInstallerProcessor
from items.vehicle_items import checkModuleValidity
...
class _UpgradesAvailability(object):
    ...
    def onCameraChanged(self, eMode):
        self.__isStrategicMode = eMode == 'mapcase'
...
class _ProgressionWindowCtrl(object):
    ...
    def __onViewAddedToContainer(self, _, pyEntity):
        if pyEntity.alias == BATTLE_VIEW_ALIASES.BATTLE_VEHICLE_CONFIGURATOR:
            self.__disposeProgressionWindow()
...
class _VehicleHolder(object):
    __slots__ = ('__vehicle', '__maxLevel', '__currentVehicleLevel', '__vehicleChangeCallback', '__modulesHolder')
    ...
    def __initFromCD(self, strCD):
        self.__vehicle = _createGuiVehicle(strCD)
...
class _VehicleModulesHolder(object):
    __itemsFactory = dependency.descriptor(IGuiItemsFactory)
    ...
    def getModule(self, intCD):
        if intCD not in self.__modules:
            self.__modules[intCD] = newM = self.__registerItem(getTypeOfCompactDescr(intCD), intCD)
            return newM
        return self.__modules[intCD]

    def getInstalledAnalog(self, intCD, vehicle):
        targetModule = self.getModule(intCD)
        vDescr = vehicle.descriptor
        currModuleDescr, _ = vDescr.getComponentsByType(targetModule.itemTypeName)
        installedModuleIntCD = currModuleDescr.compactDescr
        return self.__registerItem(currModuleDescr.typeID, installedModuleIntCD) if installedModuleIntCD not in self.__modules else None
...
class _ModuleChangeRequester(object):
    ...
    def mayInstallModule(self, moduleItem, vehicle):
        ...
        if success:
            success, reason = checkModuleValidity(moduleItem.intCD, vehicle.descriptor)
    ...
    def __doRequest(self, intCD):
        try:
            BigWorld.player().getVehicleAttached().cell.upgradeVehicle(intCD)
...
class ProgressionController(IProgressionController, ViewComponentsController):
    __slots__ = ('__currentXP', '__currentXpPercents', 'onPageTriggered', '__progressionWindowCtrl', '__currentLevel', '_viewComponents', '__minXP', '__maxXP', '__modulesStorage', '__averageLevel', '__enemiesAmount', '__vehicleModulesHolder', '__enemyTeamsAmount', '__isStarted', '__upgradesAvailability', '__tmpProgressionRecord', 'onVehicleUpgradeStarted', 'onVehicleUpgradeFinished', '__vehicleHolder', '__moduleChangeReq')
    __itemsFactory = dependency.descriptor(IGuiItemsFactory)
    ...
    def __init__(self):
        ...
        self.__vehicleModulesHolder = None
    ...
    @property
    def modulesHolder(self):
        return self.__vehicleModulesHolder
    ...
    def startControl(self, battleCtx, arenaVisitor):
        self.__vehicleModulesHolder = _VehicleModulesHolder()
        self.__vehicleHolder = _VehicleHolder(self.__notifyVehicleChanged, self.__vehicleModulesHolder)
        pVehId = avatar_getter.getPlayerVehicleID()
        if pVehId != 0:
            vehicle = BigWorld.entities.get(pVehId)
            if vehicle:
                self.updateLevel(*vehicle.battleXpLvlData)
                self.updateXP(vehicle.battleXP)
    ...
    def stopControl(self):
        ...
        if self.__vehicleHolder:
            ...
            self.__vehicleHolder = None
        if self.__vehicleModulesHolder:
            self.__vehicleModulesHolder.destroy()
            self.__vehicleModulesHolder = None
    ...
    def getModule(self, intCD):
        return self.__vehicleModulesHolder.getModule(intCD)

    def getInstalledAnalog(self, intCD):
        return self.__vehicleModulesHolder.getInstalledAnalog(intCD, self.getCurrentVehicle())
    ...
    def __registerSelectedModule(self, intCD):
        ...
        if self.__tmpProgressionRecord.addModule(intCD):
            self.__tmpProgressionRecord.save()

    def __getCachedModules(self):
