...
# Embedded file name: scripts/client/gui/battle_control/controllers/ranked_voip_ctrl.py
import logging
import typing
import BigWorld
...
class RankedVOIPController(IRankedVOIPController):
    ...
    def startControl(self, *_, **__):
        _logger.debug('[Ranked] RankedVOIPController started.')
        self.__subscribe()

    def stopControl(self):
        _logger.debug('[Ranked] RankedVOIPController stopped.')
        self.__unsubscribe()
        self.__messageShown = False
    ...
    def toggleChannelConnection(self, automatic=False):
        if self.__sessionProvider.isReplayPlaying:
            ...
            _logger.info('Failed to toggle Ranked team VOIP channel. Cooldown is in progress.')
            return
        elif not self.isTeamVoipEnabled:
            _logger.error('Failed to toggle Ranked team VOIP channel. Joining is not allowed.')
            return
        else:
