...
class VehicleCountController(IVehicleCountController):
    ...
    def __addToVehicles(self, vInfoVO):
        classType, _, nationName = vInfoVO.getTypeInfo()
        vehicleInfo = [False,
         classType,
         True,
         vInfoVO.team]
        if self.__vehicles.get(nationName, None) is None:
            self.__vehicles[nationName] = {}
        self.__vehicles[nationName][vInfoVO.vehicleID] = vehicleInfo
        return

    def __setDead(self, vInfoVO):
        _, _, nationName = vInfoVO.getTypeInfo()
        vehByNations = self.__vehicles.get(nationName)
        if vehByNations and vInfoVO.vehicleID in vehByNations:
            vehByNations[vInfoVO.vehicleID][0] = True
