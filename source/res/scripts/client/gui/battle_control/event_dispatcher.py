...
def showEventHelp(source=''):
    ctx = {'source': source}
    g_eventBus.handleEvent(GameEvent(GameEvent.EVENT_HELP_SHOW, ctx=ctx), scope=_SCOPE)


def hideEventHelp():
    g_eventBus.handleEvent(GameEvent(GameEvent.EVENT_HELP_HIDE), scope=_SCOPE)
...
def dispatchEscPressed():
    g_eventBus.handleEvent(GameEvent(GameEvent.ESC_PRESSED), scope=_SCOPE)
