...
class RankInfoHelper(object):
    ...
    __TITLE_LABEL_MAP = {(RANK_CHANGE_STATES.RANK_EARNED, True): RANKED_BATTLES.BATTLERESULT_RANKEARNED,
     ...
     (RANKEDBATTLES_ALIASES.SHIELD_LOSE, True): RANKED_BATTLES.BATTLERESULT_SHIELDLOSE,
     (RANKEDBATTLES_ALIASES.SHIELD_LOSE, False): RANKED_BATTLES.BATTLERESULT_SHIELDLOSE,
    ...
    def makeSubTaskState(self):
        ...
        if rankState in (RANK_CHANGE_STATES.RANK_EARNED, RANK_CHANGE_STATES.RANK_LOST):
            _, isMaster = self.__getRankAfterBattle()
            if isMaster:
