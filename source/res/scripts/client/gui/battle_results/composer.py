...
class RacingStatsComposer(StatsComposer):
    __slots__ = ()

    def __init__(self, reusable):
        super(RacingStatsComposer, self).__init__(reusable, templates.RACING_COMMON_STATS_BLOCK.clone(), templates.RACING_PERSONAL_STATS_BLOCK.clone(), templates.RACING_TEAMS_STATS_BLOCK.clone(), templates.REGULAR_TEXT_STATS_BLOCK.clone())

    def _registerTabs(self, reusable):
        self._block.addNextComponent(templates.RACING_TABS_BLOCK.clone())
...
def createComposer(reusable):
    ...
    elif bonusType in (ARENA_BONUS_TYPE.EVENT_BATTLES, ARENA_BONUS_TYPE.TOURNAMENT_EVENT):
        composer = RacingStatsComposer(reusable)
    else:
