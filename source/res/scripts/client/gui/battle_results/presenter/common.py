...
from gui.battle_results.br_helper import getKillerID, isPlayerLeftBattle, getUserNames, isOwnSquad
...
def setBaseUserInfo(model, reusable, vehicleID):
    ...
    if not playerInfo.dbID:
        model.setUserName(backport.text(R.strings.wt_event.botName()))
        model.setHiddenUserName(backport.text(R.strings.wt_event.botName()))
        model.setIsFakeNameVisible(False)
    else:
