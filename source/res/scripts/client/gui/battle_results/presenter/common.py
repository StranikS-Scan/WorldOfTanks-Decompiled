...
def setAccountType(model, reusable, result):
    model.setAccountType(reusable.getPremiumType())
...
def getCreditsRecords(reusable):
    baseCredits, premiumCredits, _, additionalRecords, _, _ = first(reusable.personal.getMoneyRecords())
...
def getCrystalsRecords(reusable):
    _, _, _, additionalRecords, _, _ = first(reusable.personal.getMoneyRecords())
    crystalsRecords = reusable.personal.getUnpackedCrystalRecords()
    return None if not crystalsRecords or not reusable.personal.haveCrystalsChanged() else _FinancialRecords(main=crystalsRecords, additional=additionalRecords, alternative=None)


def getXpRecords(reusable):
    baseXP, premiumXP, baseFreeXP, premiumFreeXP, _, _, _, _ = first(reusable.personal.getXPRecords())
    if reusable.hasAnyPremiumInPostBattle:
        xpRecord = premiumXP
...
def getTotalXPToShow(reusable, records, hasPremium):
    return reusable.personal.getXPToShow()[0] if hasPremium else records.getRecord('xpToShow')
...
def getTotalCreditsToShow(reusable, records, hasPremium):
    return reusable.personal.getCreditsToShow()[0] if hasPremium else records.getRecord('credits', 'originalCreditsToDraw')
...
def _createPremiumBonusModel(reusable):
    return None if reusable.personal.isPremiumPlus or reusable.personal.isPremiumPlus else getCurrencies((reusable.personal.getCreditsDiff(), reusable.personal.getXPDiff()), (CurrencyModel.CREDITS, CurrencyModel.XP))
