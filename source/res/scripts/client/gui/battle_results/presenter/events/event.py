...
def getEvents(tooltipData, reusable, result):
    events = []
    arenaUniqueID = result.get('arenaUniqueID')
    questsHistory = _getBattleQuestsHistory()
    if questsHistory.get(arenaUniqueID, None) is None:
        ...
        questsHistory[arenaUniqueID] = quests
    else:
        quests = questsHistory[arenaUniqueID]
...
def _getBattleQuestsHistory():
    ctrl = dependency.instance(IQuestsController)
    return ctrl.eventsCache.getQuestsHistory()
