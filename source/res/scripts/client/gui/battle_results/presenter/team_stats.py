...
from gui.battle_results.reusable.shared import getPlayerAbsolutePlaceInTeam
...
def _fillModel(items, data, reusable, result, getter):
    ...
    for idx, info in enumerate(data):
        ...
        player.setEarnedXp(_getEarnedWtXp(reusable, result, info.damageDealt, playerTeam))
...
def _getEarnedWtXp(reusable, result, damageDealt, team):
    gameEventCtrl = dependency.instance(IGameEventController)
    winnerTeam = reusable.getWinnerTeam()
    xpConfig = gameEventCtrl.getXpConfig()
    xpTeamConfig = xpConfig['boss'] if team == WtTeams.BOSS else xpConfig['hunter']
    xps = xpTeamConfig['win'] if team == winnerTeam else xpTeamConfig['lose']
    rank = getPlayerAbsolutePlaceInTeam(reusable, result, 'damageDealt', damageDealt, team)
    return xps[rank] if rank < len(xps) else 0
