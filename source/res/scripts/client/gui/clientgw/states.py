...
from gui.clientgw.base.contexts import LogInCtx, LogOutCtx, PingCtx
from gui.clientgw.factory import g_webFactory
from gui.clientgw.requests import ClientgwRequestResponse
...
class _State(object):
    ...
    def _makeErrorResponse(self):
        return ClientgwRequestResponse(ResponseCodes.UNKNOWN_ERROR, 'Request cannot be sent from the current state', None)
...
class _WebState(_State):
    ...
    def init(self):
        ...
        self.__requestsCtrl = g_webFactory.createClientgwRequestsController(self._webCtrl, g_webFactory.createClientgwRequester(self.__webRequester))
...
class UnavailableState(_WebState):
    ...
    def _sendRequest(self, ctx, callback, allowDelay=True):
        if ctx.getRequestType() == WebRequestDataType.PING:
            ...
            result = ClientgwRequestResponse(ResponseCodes.CLIENTGW_ERROR, 'CLIENTGW is not available.', None)
        callback(result)
        return
    ...
    def _ping(self):
        ...
        self.__isPingRunning = False
        if result.isSuccess() and self._webCtrl.simClientgwEnabled():
...
class AvailableState(_WebState):
    ...
    def _sendRequest(self, ctx, callback, allowDelay=True):
        if ctx.isAuthorizationRequired() and not self.isLoggedOn():
            self.__waitingRequests.append((ctx,
             callback,
             ClientgwRequestResponse(ResponseCodes.AUTHENTIFICATION_ERROR, 'The user is not authorized.', None),
             allowDelay))
            self.login()
        else:
            self.clanSync(ctx)
            if self._webCtrl and self._webCtrl.simClientgwEnabled():
                result = yield super(AvailableState, self)._sendRequest(ctx, allowDelay=allowDelay)
            else:
                result = ClientgwRequestResponse(ResponseCodes.CLIENTGW_ERROR, 'Simulated CLIENTGW error!', None)
            resultCode = result.code
            if resultCode == ResponseCodes.CLIENTGW_ERROR:
                LOG_DEBUG('CLIENTGW error has occurred! The state will be changed to NA.')
                ...
                LOG_WARNING('Failed to do CLIENTGW request. Need to relogin.', ctx)
                self.__waitingRequests.append((ctx,
                 callback,
                 result,
                 allowDelay))
    ...
    def __doLogin(self, callback):
        if not LOGIN_STATE.canDoLogin(self.__loginState) or self.bootcamp.isInBootcamp():
            ...
            return
        LOG_DEBUG('Clientgw gate login processing...')
        ...
        if response and response.isValid():
            ...
            if response.getDatabaseID() == pDbID:
                LOG_DEBUG('Trying to login to the clientgw lib...')
    ...
    def __doLogOut(self, callback):
        LOG_DEBUG('Clientgw gate logout processing...')
