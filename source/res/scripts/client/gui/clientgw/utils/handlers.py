...
# Embedded file name: scripts/client/gui/clientgw/utils/handlers.py
...
class UtilsRequestHandlers(RequestHandlers):
    ...
    def get(self):
        handlers = {WebRequestDataType.SPA_GET_ACCOUNT_ATTRIBUTE: self.__getAccountAttributeByPrefix,
         ...
         WebRequestDataType.PLATFORM_FETCH_PRODUCT_LIST_PERSONAL: self.__fetchProductListPesonal}
        return handlers
    ...
    def __getUserSubscriptions(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, ('agate', 'agate_v6_get_user_subscriptions'), ctx.getParams())

    def __fetchProductListPesonal(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, ('agate', 'agate_v6_fetch_product_list_personal'), ctx.getParams())
