...
# Embedded file name: scripts/client/gui/clientgw/web_controller.py
import logging
...
from gui.clientgw.clan.contexts import ClanInfoCtx, ClanGlobalMapStatsCtx, GetClanInvitesCount, GetClanAppsCount, ClanMembersCtx, GetProvincesCtx, ClanFavouriteAttributesCtx, ClanRatingsCtx
from gui.clientgw.hof.contexts import HofUserInfoCtx, HofUserExcludeCtx, HofUserRestoreCtx
...
class WebController(WebListeners, IWebController):
    ...
    def __init__(self):
        ...
        self.__simClientgwEnabled = True
        return
    ...
    def getRequesterConfig(self):
        return self.lobbyContext.getServerSettings().clientgw

    def simEnableClientgw(self, enable):
        self.__simClientgwEnabled = enable
        if self.__profile:
            self.__profile.resync(force=True)

    def simEnableClan(self, enable):
        settings = self.lobbyContext.getServerSettings()
        clanSettings = {'clientgwProfile': {'isEnabled': enable,
                             'gateUrl': settings.clientgw.getGateUrl()}}
        settings.update(clanSettings)
        g_clientUpdateManager.update({'serverSettings': clanSettings})

    def simClientgwEnabled(self):
        return self.__simClientgwEnabled
    ...
    def start(self, force=True):
        if self.__started and not force:
            _logger.debug('Clientgw already started.')
            return
    ...
    def isEnabled(self):
        settings = self.lobbyContext.getServerSettings()
        return settings.clientgw.isEnabled() if settings is not None else True
