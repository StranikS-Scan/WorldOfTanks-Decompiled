...
class BobController(IBobController, Notifiable, SeasonProvider):
    ...
    def battleModifiers(self):
        return self.getConfig().battleModifiersDescr

    def isBattleModifiersAvailable(self):
        return self.isValidBattleType() and len(self.battleModifiers) > 0
    ...
    def isModeActive(self):
        return self.isEnabled() and not self.isPaused() and bool(self.getCurrentSeason())

    def isPaused(self):
        return self.getConfig().isPaused
    ...
    def isAvailable(self):
        return self.isEnabled() and not self.isPaused() and not self.isFrozen() and self.getCurrentSeason() is not None
    ...
    @decorators.adisp_process('bob/claimReward')
