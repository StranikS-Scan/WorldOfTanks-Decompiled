...
class BREvents(object):
    ...
    EQUIPMENT_ACTIVATED = {SteelHunterEquipmentNames.LARGE_REPAIRKIT: 'BR_perk_repair_activation',
     SteelHunterEquipmentNames.REGENERATION_KIT: 'BR_perk_hp_restore_activation',
     SteelHunterEquipmentNames.SELF_BUFF: 'BR_perk_selfbuff_activation',
     SteelHunterEquipmentNames.TRAP_POINT: 'BR_perk_trap_slowdown_activation',
     SteelHunterEquipmentNames.REPAIR_POINT: 'BR_perk_repairpoint_activation',
     SteelHunterEquipmentNames.HEAL_POINT: 'BR_perk_healzone_activation',
     SteelHunterEquipmentNames.SMOKE: 'BR_perk_smoke_activation',
     SteelHunterEquipmentNames.BOMBER: 'BR_perk_airstrike_activation',
     SteelHunterEquipmentNames.KAMIKAZE: 'BR_perk_kamikaze_activation',
     SteelHunterEquipmentNames.MINE_FIELD: 'BR_perk_minefield_zone'}
    EQUIPMENT_DEACTIVATED = {SteelHunterEquipmentNames.REGENERATION_KIT: 'BR_perk_hp_restore_deactivation',
     SteelHunterEquipmentNames.SELF_BUFF: 'BR_perk_selfbuff_deactivation'}
...
class _VehicleStateSoundPlayer(object):
...
class LootSoundPlayer(_VehicleStateSoundPlayer):
...
class DeathzoneSoundPlayer(_VehicleStateSoundPlayer):
...
class EnemiesAmountSoundPlayer(IVehicleCountListener):
    ...
    def setVehicles(self, count, vehicles, teams):
        if self.__enemiesCount is not None and count != self.__enemiesCount:
            eventName = BREvents.ENEMIES_AMOUNT.get(count)
            if eventName is not None:
                BREvents.playSound(eventName)
...
class StatusSoundPlayer(_VehicleStateSoundPlayer, CallbackDelayer):
    ...
    def destroy(self):
        ...
        _VehicleStateSoundPlayer.destroy(self)
    ...
    def _onVehicleStateUpdated(self, state, vehicleID):
        if state == VEHICLE_VIEW_STATE.OBSERVED_BY_ENEMY:
            self.__isObservedByEnemy = bool(vehicleID)
            self.__updateStatus()
...
class KamikazeSoundPlayer(BaseEfficiencySoundPlayer):
    ...
    def _onEfficiencyReceived(self, events):
        for e in events:
            if e.getBattleEventType() == BATTLE_EVENT_TYPE.DAMAGE and e.isSpawnedBotExplosion():
...
class _HealingRepairSoundPlayer(_VehicleStateSoundPlayer):
...
class _DamagingSmokeAreaSoundPlayer(_VehicleStateSoundPlayer):
