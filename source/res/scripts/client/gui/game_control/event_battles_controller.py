...
import CGF
import Event
from gui.impl.gen.view_models.views.lobby.wt_event.wt_event_entry_point_model import PerformanceRiskEnum
...
class WtPerfProblems(object):
    ...
    MEDIUM_RISK = 2
    LOW_RISK = 3
    PERFORMANCE_MAP = {HIGH_RISK: PerformanceRiskEnum.HIGHRISK,
     MEDIUM_RISK: PerformanceRiskEnum.MEDIUMRISK,
     LOW_RISK: PerformanceRiskEnum.LOWRISK}

    @staticmethod
    def getPerformanceRiskMap(risk):
        return WtPerfProblems.PERFORMANCE_MAP.get(risk, WtPerfProblems.LOW_RISK)
...
class EventBattlesController(IEventBattlesController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def __init__(self):
        ...
        self.__battleQueueSoundMgr = wt_event_sound.WTEventBattleQueueSoundPlayer()
    ...
    def fini(self):
        ...
        self.__battleQueueSoundMgr = None
    ...
    def getStartTime(self):
        ...
        if not season:
            return -1
        cycle = season.getCycleInfo() or season.getNextCycleInfo(time_utils.getCurrentLocalServerTimestamp())
        return cycle.startDate if cycle else -1

    def getEndTime(self):
        season = self.getCurrentSeason() or self.getNextSeason()
        if not season:
            return -1
        cycle = season.getCycleInfo() or season.getLastCycleInfo()
        return cycle.endDate if cycle else -1

    def getSeasonEndTime(self):
        season = self.getCurrentSeason() or self.getNextSeason()
        return season.getEndDate() if season else -1
    ...
    def getEventShowcaseVehicle(self):
        config = self.getConfig()
        return config.eventShowcaseVehicle
    ...
    def getBattleQueueSoundMgr(self):
        return self.__battleQueueSoundMgr
