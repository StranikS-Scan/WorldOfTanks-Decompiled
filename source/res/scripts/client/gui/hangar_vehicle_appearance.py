...
class HangarVehicleAppearance(ScriptGameObject):
    ...
    def _getTurretYaw(self):
        return self.__staticTurretYaw if self.__isSPG() else self.turretAndGunAngles.getTurretYaw()

    def _getGunPitch(self):
        return self.__staticGunPitch if self.__isSPG() else self.turretAndGunAngles.getGunPitch()
    ...
    def _getStyleIdForUniqueVeh(self, vehDesc):
        ...
        styleId, _ = findFirst(lambda (styleId, style): style.isLockedOnVehicle and style.matchVehicleType(vehDesc.type), cache.styles.iteritems(), (0, 0))
        return styleId
