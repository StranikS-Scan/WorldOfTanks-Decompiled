    ...
    def __assembleModel(self):
        ...
        if not ('AT-SPG' in self.__vDesc.type.tags or 'SPG' in self.__vDesc.type.tags):
            if turretYaw is None:
                turretYaw = cfg['vehicle_turret_yaw'] if self.hangarSpace.space and self.__vEntity.id == self.hangarSpace.space.vehicleEntityId else self.__vEntity.vehicleTurretYaw
            ...
            if gunPitch is None:
                gunPitch = cfg['vehicle_gun_pitch'] if self.hangarSpace.space and self.__vEntity.id == self.hangarSpace.space.vehicleEntityId else self.__vEntity.vehicleGunPitch
