...
from gui.impl.gen.view_models.views.loot_box_view.loot_attachment_special_renderer_model import LootAttachmentSpecialRendererModel
...
from gui.server_events.awards_formatters import getPackRentVehiclesAwardPacker, getRoyaleAwardsPacker, AWARDS_SIZES
...
class LootAttachmentRewardPresenter(LootRewardDefModelPresenter):
    __slots__ = ('_attachment', '__resourceName', '__sourceID', '_itemID')
    __itemsCache = dependency.descriptor(IItemsCache)

    def __init__(self):
        super(LootAttachmentRewardPresenter, self).__init__()
        self._attachment = None
        self._itemID = None
        return

    def _setReward(self, reward):
        super(LootAttachmentRewardPresenter, self)._setReward(reward=reward)
        specialArgs = self._reward.get('specialArgs')
        if specialArgs:
            attachment = self.__itemsCache.items.getItemByCD(specialArgs[0])
            if attachment.itemTypeID == GUI_ITEM_TYPE.ATTACHMENT:
                self._attachment = attachment
                self._itemID = attachment.id
            else:
                self._attachment = None
        else:
            _logger.error('SpecialArgs for style is not specified!')
    ...
    def _createModel(self):
        return LootAttachmentRendererModel()

    def _getRendererType(self):
        return LootRendererTypes.ATTACHMENT

    def _formatModel(self, model, ttId, showCongrats):
        super(LootAttachmentRewardPresenter, self)._formatModel(model, ttId, showCongrats)
        if self._attachment is not None:
            with model.transaction() as m:
                m.setAttachName(self._attachment.name)
                m.setRarity(self._attachment.rarity)
                m.setGroupName(self._attachment.groupUserName)
        return


class LootAttachmentSpecialCongratsRewardPresenter(LootAttachmentRewardPresenter):

    def _createModel(self):
        return LootAttachmentSpecialRendererModel()

    def _getRendererType(self):
        return LootRendererTypes.ATTACHMENT_SPECIAL

    def _formatModel(self, model, ttId, showCongrats):
        super(LootAttachmentSpecialCongratsRewardPresenter, self)._formatModel(model, ttId, showCongrats)
        if self._attachment is not None:
            with model.transaction() as m:
                m.congratsViewModel.setShowCongrats(True)
                m.congratsViewModel.setCongratsType(LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT)
                m.congratsViewModel.setIntCD(self._itemID)
                m.congratsViewModel.setAttachName(self._attachment.name)
                m.congratsViewModel.setAttachUserName(self._attachment.userName)
                m.congratsViewModel.setRarity(self._attachment.rarity)
                m.congratsViewModel.setGroupAttachName(self._attachment.groupUserName)
        return
