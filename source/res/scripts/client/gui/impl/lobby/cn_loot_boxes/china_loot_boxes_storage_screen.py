...
from gui.impl.lobby.cn_loot_boxes.china_loot_boxes_sound import playStorageOpened, playStorageClosed, enterLootBoxState, exitLootBoxState
from gui.impl.pub import ViewImpl, WindowImpl
from gui.impl.wrappers.function_helpers import replaceNoneKwargsModel
from gui.shared.event_dispatcher import showCNLootBoxOpenWindow, showShop, showCNLootBoxOpenErrorWindow
...
class ChinaLootBoxesStorageScreen(ViewImpl):
    ...
    def __addListeners(self):
        ...
        self.__lootBoxController.onStatusChange += self.__onEventUpdated
        self.__lootBoxController.onAvailabilityChange += self.__onEventUpdated
    ...
    def __updateLootBoxesAvailability(self, model=None):
        model.setIsLootBoxesEnabled(self.__lootBoxController.isLootBoxesAvailable())

    @replaceNoneKwargsModel
    def __updateBoxesCount(self, model=None):
        self.__hasPremiumBoxes = False
        storeInfo = self.__lootBoxController.getStoreInfo()
    ...
    def __updateBuyAvailability(self, model=None):
        ...
        boxesLeft = max(self.__lootBoxController.getDayLimit() - self.__lootBoxController.getDayInfoStatistics(), 0)
        ...
        if not boxesLeft and isCacheInited:
            ...
            dayTimeLeft = self.__lootBoxController.getTimeLeftToResetPurchase()
    ...
    def __onEventUpdated(self, *_):
        if self.__lootBoxController.isActive():
    ...
    def __onOpenBox(self, args):
        ...
        if boxType is not None:
            box = self.__lootBoxController.getStoreInfo().get(boxType)
