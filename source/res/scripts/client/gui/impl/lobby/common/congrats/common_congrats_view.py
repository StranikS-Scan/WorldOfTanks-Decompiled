...
import logging
from frameworks.wulf import ViewSettings, Window, WindowFlags, WindowSettings
...
class CongratsWindow(Window):
    ...
    def __init__(self, context):
        ...
        settings.content = _CommonCongratsView(context)
        settings.parent = view.getParentWindow() if view is not None else None
        super(CongratsWindow, self).__init__(settings)
        return
