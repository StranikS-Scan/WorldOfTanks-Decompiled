...
# Embedded file name: scripts/client/gui/impl/lobby/customization/context/context.py
import logging
    ...
    def changeMode(self, modeId, tabId=None, source=None):
        if modeId not in CustomizationModes.ALL:
            ...
            if tabId is not None and tabId not in newMode.tabs:
                tabId = None
                _logger.warning('Wrong tabId: %s for current customization mode: %s', tabId, self.modeId)
