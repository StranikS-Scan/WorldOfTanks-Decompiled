...
# Embedded file name: scripts/client/gui/impl/lobby/customization/context/custom_mode.py
import logging
...
from gui.impl.lobby.customization.shared import CustomizationTabs, EMPTY_PERSONAL_NUMBER, fitPersonalNumber, formatPersonalNumber, getCustomPurchaseItems, isItemsQuantityLimitReached, isSlotFilled
from gui.impl.lobby.customization.shared import getOutfitWithoutItems
...
from items.components.c11n_constants import DEFAULT_PALETTE, Options, SLOT_DEFAULT_ALLOWED_MODEL, SeasonType
from items.customizations import CamouflageComponent, PersonalNumberComponent, ProjectionDecalComponent
...
class CustomMode(CustomizationMode):
    __SELFINSTALL_ITEM_TYPES = {GUI_ITEM_TYPE.STYLE: C11nId(areaId=Area.MISC, slotType=GUI_ITEM_TYPE.STYLE, regionIdx=0)}
    ...
    def removeItemFromAllSeasons(self, slotId, silent=False):
    ...
    def enableEditMode(self, enabled, slotId=C11nId()):
        if self.__editModeEnabled == enabled:
            return
        self.__editModeEnabled = enabled
        self._events.onEditModeEnabled(self.__editModeEnabled, slotId)
    ...
    def _selectItem(self, intCD, progressionLevel=0):
        ...
        if item.itemTypeID in self.__SELFINSTALL_ITEM_TYPES:
            self.selectSlot(self.__SELFINSTALL_ITEM_TYPES[item.itemTypeID])
            self.installItem(intCD, self.selectedSlot)
            return False
        elif self.selectedSlot is None:
    ...
    def _selectSlot(self, slotId):
        ...
        if self.selectedItem is None:
            ...
            self.installItem(self.selectedItem.intCD, slotId, season=self._ctx.season)
            return False
            ...
            for areaId in Area.ALL:
                ...
                for regionIdx, anchor in g_currentVehicle.item.getAnchors(self.slotType, areaId):
                    ...
                    if model not in anchor.compatibleModels:
                        continue
                    anchors.append(C11nId(areaId, self.slotType, regionIdx))
