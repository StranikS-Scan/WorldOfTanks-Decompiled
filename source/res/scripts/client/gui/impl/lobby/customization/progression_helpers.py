...
# Embedded file name: scripts/client/gui/impl/lobby/customization/progression_helpers.py
import binascii
import logging
import struct
...
def getProgressionPostBattleInfo(itemIntCD, vehicleIntCD, progressionData, itemsCache=None):
    ...
    if level is None:
        ...
        if inProgress:
            ...
            info['awards'] = __makeAwards(item, level, vehicleIntCD)
        return info
...
def __makeProgressList(item, level, progressionData):
    ...
    for path, (diff, progress) in progressionData['progress'].iteritems():
        ...
        progressList.append(__makeProgress(conditionId=idx, description=condition['description'], maxProgress=condition['value'], currentProgress=progress, progressDiff=diff))
