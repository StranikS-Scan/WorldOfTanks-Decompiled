...
from gui.customization.shared import AdditionalPurchaseGroups, C11nId, EDITABLE_STYLE_APPLY_TO_ALL_AREAS_TYPES, EDITABLE_STYLE_IRREMOVABLE_TYPES, PurchaseItem, QUANTITY_LIMITED_CUSTOMIZATION_TYPES, appliedToFromSlotsIds, getAvailableRegions, isVehicleCanBeCustomized
from gui.hangar_cameras.hangar_camera_common import CameraMovementStates
from gui.impl import backport
from gui.impl.gen import R
from gui.impl.lobby.customization.settings_constants import CustomizationFilter
...
from items.components.c11n_components import StyleItem, getItemSlotType
from items.components.c11n_constants import ProjectionDecalFormTags, SeasonType
...
class CustomizationTabs(object):
    ...
    CUSTOM_ALL = (PAINTS,
     ...
     INSCRIPTIONS,
     MODIFICATIONS)
    STYLES_ALL = (STYLED_3D, STYLED_2D)
    STYLES_2D = (STYLED_2D,)
    STYLES_3D = (STYLED_3D,)
    REGIONS = (PAINTS,
     CAMOUFLAGES,
     ...
     STYLED_2D,
     STYLED_3D)
    GROUPS = ((STYLED_3D,), (STYLED_2D,), (CAMOUFLAGES,
      PAINTS,
      PROJECTION_DECALS,
      EMBLEMS,
      INSCRIPTIONS,
    ...
    SLOT_TYPES = {PAINTS: GUI_ITEM_TYPE.PAINT,
     ...
     STYLED_2D: GUI_ITEM_TYPE.STYLE,
     STYLED_3D: GUI_ITEM_TYPE.STYLE}
    ITEM_TYPES = {PAINTS: (GUI_ITEM_TYPE.PAINT,),
     ...
     STYLED_2D: (GUI_ITEM_TYPE.STYLE,),
     STYLED_3D: (GUI_ITEM_TYPE.STYLE,)}
    STYLE_PREFIX = {STYLED_2D: STYLE_2D_PREFIX,
     STYLED_3D: STYLE_3D_PREFIX}
    TAB_TO_GROUP = {PAINTS: CustomizationFilter.PAINTS_GROUP,
     CAMOUFLAGES: CustomizationFilter.CAMOUFLAGE_GROUP,
     ...
     STYLED_2D: CustomizationFilter.STYLES_2D_GROUP,
     STYLES_3D: CustomizationFilter.STYLES_3D_GROUP}
...
_CUSTOM_ITEM_TYPE_TO_TAB = {value:tab for tab in CustomizationTabs.CUSTOM_ALL for value in CustomizationTabs.ITEM_TYPES[tab]}
...
def getTabByItem(item):
    ...
    return _CUSTOM_ITEM_TYPE_TO_TAB[item.itemTypeID]
...
SCALE_SIZE = (backport.text(R.strings.vehicle_customization.customization.Popover.scale.small()), backport.text(R.strings.vehicle_customization.customization.Popover.scale.normal()), backport.text(R.strings.vehicle_customization.customization.Popover.scale.large()))
TYPES_ORDER = (GUI_ITEM_TYPE.PAINT,
 ...
 GUI_ITEM_TYPE.STYLE)
SEASON_TYPE_TO_INFOTYPE_MAP = {SeasonType.SUMMER: backport.text(R.strings.vehicle_customization.customization.infotype.mapType.summer()),
 SeasonType.DESERT: backport.text(R.strings.vehicle_customization.customization.infotype.mapType.desert()),
 SeasonType.WINTER: backport.text(R.strings.vehicle_customization.customization.infotype.mapType.winter())}
...
def isItemUsedUp(item, service=None):
    ...
    if item.itemTypeID == GUI_ITEM_TYPE.STYLE:
        isApplied = ctx.modeId in CustomizationModes.STYLED and ctx.mode.modifiedStyle == item
    else:
