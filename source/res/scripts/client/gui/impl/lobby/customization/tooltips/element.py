...
# Embedded file name: scripts/client/gui/impl/lobby/customization/tooltips/element.py
import logging
from CurrentVehicle import g_currentVehicle
from gui.impl.lobby.customization.shared import getItemInventoryCount, makeVehiclesShortNamesString, getSuitableText, ITEM_TYPE_TO_SLOT_TYPE
...
class ElementTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args):
        ...
        if config.vehicleIntCD == 0:
            ...
            self.__vehicle = self._getSuitableVehicleForCustomization(self._item)
        else:
        ...
        self._showStatusBlock = config.showStatusBlock
    ...
    def _packItemBlocks(self, statsConfig):
        ...
        if block:
            items.append(block)
        if not self._item.isVehicleBound and not isItemInStyle:
        ...
        if block and self._showStatusBlock:
            items.append(block)
        return items
    ...
    def _getSuitableVehicleForCustomization(self, customization):
        itemFilter = customization.descriptor.filter
        if itemFilter is not None and itemFilter.include:
            for node in itemFilter.include:
                if node.vehicles:
                    return self.__itemsCache.items.getItemByCD(first(node.vehicles))
    ...
    def _packAppliedBlock(self):
        if self.__ctx is not None and self.__ctx.modeId in CustomizationModes.STYLED:
            if self._item.itemTypeID != GUI_ITEM_TYPE.STYLE:
                return
    ...
    def __packAncestorBlock(self, dependentItems):
        ...
        for intCD in dependentItems:
            ...
            itemSlotType = ITEM_TYPE_TO_SLOT_TYPE[item.itemTypeID]
