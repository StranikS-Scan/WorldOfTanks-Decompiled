...
# Embedded file name: scripts/client/gui/impl/lobby/customization/widget/customization_cm_handlers.py
import logging
...
from gui.impl.lobby.customization.shared import isItemUsedUp
...
class CustomizationItemCMHandler(AbstractContextMenuHandler):
    ...
    def markFavorites(self):
        self.__ctx.updateCustomizationFavorites(self._item)
    ...
    def __getClearStyleBtn(self, item):
        from gui.impl.lobby.customization.shared import isStyleEditedForCurrentVehicle
    ...
    def __getSellBtn(self, item):
        ...
        sellPriceData = getItemPricesVO(sellPrice)
        showAlert = len(sellPriceData[0]) > 1
        if showAlert:
            ...
            price = sellPriceData[0]['price']
            sellPriceData[0] = {'price': price}
        else:
            tooltipVO = None
