...
from gui.impl.lobby.new_year.action_helper import getButtonAction, ACTION_TO_OBJECT
...
class ReceivingAwards(AwardNotificationBase):
    ...
    def _getEvents(self):
        return super(ReceivingAwards, self)._getEvents() + ((self.viewModel.onClick, self._onClick),)
    ...
    def __getRewardsAwards(self, level, bonuses):
        ...
        return action if action != ButtonActionType.UNDEFINED else ButtonActionType.TOREWARDS

    def _onClick(self):
        anchor, viewAlias = ACTION_TO_OBJECT.get(self.__buttonAction)
        if anchor or viewAlias:
            self._navigateToNy(anchor, viewAlias)
