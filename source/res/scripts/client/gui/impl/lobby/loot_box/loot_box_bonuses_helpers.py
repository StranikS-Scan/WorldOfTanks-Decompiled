...
from gui.impl.gen.view_models.views.lobby.new_year.views.lootboxes.reward_attachment_icon_bonus_model import RewardAttachmentIconBonusModel
...
from items.components.ny_constants import CurrentNYConstants, NyCurrency, RewartKitSettings
...
AUTO_OPEN_OTHER_ORDER = (Currency.GOLD,
 ...
 'handExtinguishers',
 'slots')
_GROUPED_SORT_C11N_INDEX = ('style_3d', 'attachment_legendary', 'attachment_epic', 'attachment_rare', 'style', 'projectionDecal', 'decal', 'emblem', 'inscription', 'insignia', 'personalNumber', 'paint')
...
def compareCustomization(zipped):
    ...
    if iconName == 'attachment':
        rarity = bonusModel.getRarity()
        iconName = 'attachment_{}'.format(rarity)
...
class NYCustomization3Dand2DbonusUIPacker(Customization3Dand2DbonusUIPacker):
    ...
    def _getBonusModel(cls):
        return RewardAttachmentIconBonusModel()
...
def packBonusGroups(bonuses, groupModelsList, groupsLayout, packer, tooltipsData=None):
    ...
    for group in groupsLayout:
        ...
        bonusModelsList = groupModel.rewards.getItems()
...
def getAttachmentsFromRewards(rewards, service=None):
    if not rewards or 'customizations' not in rewards:
        return []
    return [ service.getItemByID(GUI_ITEM_TYPE.ATTACHMENT, customization.get('id')) for customization in rewards['customizations'] if customization.get('custType', '') == 'attachment' ]
...
@dependency.replace_none_kwargs(service=ICustomizationService)
def __getLootboxBonusesData(rewards, service=None):
    ...
    for rewardType, rewardValue in rewards.iteritems():
        ...
        if rewardType == 'customizations':
            ...
            typeCust = rewardValue[0].get('custType', '')
            if typeCust == 'attachment':
                attachment = service.getItemByID(GUI_ITEM_TYPE.ATTACHMENT, rewardValue[0].get('id', 0))
                if attachment.rarity in Rarity.UI_EFFECT:
                    specialRewardType = LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT
...
class _NYSackCustomizationBonusUIPacker(CustomizationBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, label):
        ...
        model.setBigIcon('_'.join([customization.itemFullTypeName, str(customization.id)]))
...
class _NYSackRandomBonusUIPacker(SimpleBonusUIPacker):
    ...
    @classmethod
    def _getToolTip(cls, bonus):
        return [backport.createTooltipData()]
...
def getNYSackBonusPacker():
    ...
    mapping.update({'customizations': _NYSackCustomizationBonusUIPacker(),
     ...
     'randomNyToy': _NYSackRandomBonusUIPacker()})
    return BonusUIPacker(mapping)


def getNYRanndomsBonusPacker():
    mapping = getDefaultBonusPackersMap()
    mapping.update({'randomNyBooklet': _NYSackRandomBonusUIPacker(),
     'randomNyGuide': _NYSackRandomBonusUIPacker(),
     'randomNyInstruction': _NYSackRandomBonusUIPacker(),
     'randomNy25Toy': _NYSackRandomBonusUIPacker(),
     'randomNyCrewInstruction': _NYSackRandomBonusUIPacker()})
    return BonusUIPacker(mapping)
