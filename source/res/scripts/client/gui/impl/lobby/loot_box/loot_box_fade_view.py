...
class LootBoxFadeView(ViewImpl):
    ...
    def isCriticalError(self):
        return self.viewModel.getShowError()
    ...
    def __onFadeInCompleted(self, _=None):
        ...
        if not self.__withPause:
            self.__removeHideView()

    def __removeHideView(self, _=None):
        if not self.isCriticalError:
...
class LootBoxFadeWindow(LobbyWindow):
    ...
    def __init__(self, withPause=True, parent=None):
        super(LootBoxFadeWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN, content=LootBoxFadeView(withPause), parent=parent, layer=WindowLayer.OVERLAY)
