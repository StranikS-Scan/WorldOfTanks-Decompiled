...
from gui.impl.auxiliary.rewards_helper import getRewardTooltipContent, LootVehicleVideoRewardPresenter, LootNewYearToyPresenter, getRewardRendererModelPresenter, DEF_COMPENSATION_PRESENTERS, LootTankmanCongratsRewardPresenter, LootStyleCongratsRewardPresenter, LootAttachmentSpecialCongratsRewardPresenter, LootAttachmentRewardPresenter
...
SpecialAttachmentRewardData = namedtuple('SpecialAttachmentRewardData', ('congratsType', 'congratsSourceId', 'backToSingleOpening'))
...
def getLootboxRendererModelPresenter(reward, itemsCache=None):
    specialArgs = reward.get('specialArgs')
    bonusName = reward.get('bonusName', '')
    if bonusName == 'customizations':
        attachment = itemsCache.items.getItemByCD(specialArgs[0])
        if attachment.itemTypeID == GUI_ITEM_TYPE.ATTACHMENT:
            if attachment.rarity in Rarity.UI_EFFECT:
                return LootAttachmentSpecialCongratsRewardPresenter()
            return LootAttachmentRewardPresenter()
...
def fireCloseToExterior(c11nService=None):
    playSound(LootBoxViewEvents.ENTRY_VIEW_EXIT)
    _closeLootBoxWindows()
    nextTick(c11nService.showCustomization)()
