...
from gui.impl.auxiliary.rewards_helper import getRewardTooltipContent, LootVehicleVideoRewardPresenter, LootNewYearToyPresenter, getRewardRendererModelPresenter, DEF_COMPENSATION_PRESENTERS, LootTankmanCongratsRewardPresenter, LootStyleCongratsRewardPresenter
from gui.impl.auxiliary.rewards_helper import getVehByStyleCD
from gui.impl.gen import R
from gui.impl.gen.view_models.views.lobby.new_year.views.lootboxes.reward_kit_guaranteed_reward_model import RewardState
...
SpecialGuestRewardData = namedtuple('SpecialGuestRewardData', ('congratsType', 'backToSingleOpening'))
...
def showStyledVehicleByStyleCD(styleIntCD, isBackToMultiOpen=False, itemsCache=None, c11nService=None):
    ...
    if vehicle.isInInventory and vehicle.isCustomizationEnabled():
        ...
        c11nService.showCustomization(vehicle.invID, callback=_callback)
    else:
        ...
        showStylePreview(vehCD=vehicle.intCD, style=style, descr=style.getDescription(), backCallback=_backCallback, destroyCallback=_exitCallback, backBtnDescrLabel=backport.text(R.strings.vehicle_preview.header.backBtn.descrLabel.personalAwards()))
...
class LootBoxShowHideCloseHandler(object):
    ...
    def __onShowLBWindow(self, _=None):
        from gui.impl.lobby.loot_box.reward_kit_reward_view import RewardKitRewardWrapperWindow
        from gui.impl.lobby.new_year.ny_reward_kit_main_view import NyRewardKitViewWindow
        if self.__target:
            self.__isHidden = False
            if isinstance(self.__target, (RewardKitRewardWrapperWindow, NyRewardKitViewWindow)):
                nextTick(self.__target.show)()
            else:
                self.__target.show()
...
class LootBoxHideableView(ViewImpl):
    __slots__ = ('_showHideCloseHandler', '_hideCompleteCallback', '_isMemoryRiskySystem', '_isCanClose', '_holdByViews', '__holdIDs', '__videoFiles', '__invokeFinalize')
    _VIDEOS = []

    def __init__(self, settings):
        self._isMemoryRiskySystem = False
        self._isCanClose = not self._isMemoryRiskySystem
        lobbyViews = R.views.lobby
        self._holdByViews = (lobbyViews.new_year.views.NyRewardKitSpecialReward(),
         lobbyViews.new_year.views.NyRewardKitGuestReward(),
         lobbyViews.loot_box.views.loot_box_entry_video_view.LootBoxEntryVideoView(),
         lobbyViews.new_year.RewardKitRewardView(),
         lobbyViews.new_year.RewardKitPremiumMultiOpenView(),
...
def showLootBoxPremiumMultiOpen(lootBoxItem, rewards, boxesToOpen, parent=None, guaranteedReward=False):
    from gui.impl.lobby.loot_box.reward_kit_premium_multi_open_view import RewardKitPremiumMultiOpenWindow
    window = RewardKitPremiumMultiOpenWindow(lootBoxItem, rewards, boxesToOpen, parent, guaranteedReward)
    window.load()


def showLootBoxReward(lootBoxItem, rewards, parent=None, isBackward=False, lastStatisticsResetFailed=False, guaranteedReward=False):
    from gui.impl.lobby.loot_box.loot_box_opening_view import LootBoxOpeningWindow
    from gui.impl.lobby.loot_box.reward_kit_reward_view import RewardKitRewardWrapperWindow
    rewardWindow = RewardKitRewardWrapperWindow(lootBoxItem, rewards, parent, isBackward, lastStatisticsResetFailed, guaranteedReward)
...
def showRewardKitGuestRewad(responseDict, parent=None):
    from gui.impl.lobby.loot_box.reward_kit_guest_reward_view import RewardKitGuestRewardWindow
    specialGuestRewardData = SpecialGuestRewardData(congratsType=responseDict.get('congratsType'), backToSingleOpening=responseDict.get('backToSingleOpening'))
    window = RewardKitGuestRewardWindow(specialGuestRewardData, parent)
    window.load()
...
def showLootBoxSpecialReward2(specialRewardData, parent=None):
    from gui.impl.lobby.loot_box.loot_box_special_reward_view import LootBoxSpecialRewardWindow
    from gui.impl.lobby.loot_box.reward_kit_guest_reward_view import RewardKitGuestRewardWindow
    if specialRewardData.congratsType == LootCongratsTypes.CONGRAT_TYPE_GUEST_C:
        window = RewardKitGuestRewardWindow(specialRewardData, parent)
    else:
        window = LootBoxSpecialRewardWindow(specialRewardData, parent)
    window.load()
...
def getLootBoxByTypeAndCategory(boxType, boxCategory=None, isInInventory=False, itemsCache=None):
    ...
    for box in boxes:
        if box.getType() == boxType and (not isInInventory or box.getInventoryCount() > 0):
            if boxType == NewYearLootBoxes.PREMIUM and boxCategory == box.getCategory():
                return box
...
def getTooltipContent(event, storedTooltipData):
    ...
    elif event.contentID == tooltipContentRes.NyResourceTooltip():
        return NyResourceTooltip(event.getArgument('type'))
    elif event.contentID == tooltipContentRes.NyRewardKitGuestCTooltip():
...
class _ExitCallback(object):
    ...
    def apply(self):
        if not self.__wasCalled:
            ctx = self._c11nService.getCtx()
            if ctx:
                yield ctx.applyItems(ctx.mode.getPurchaseItems())
