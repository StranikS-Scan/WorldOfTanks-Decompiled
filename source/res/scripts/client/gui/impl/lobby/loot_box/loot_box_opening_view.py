...
from gui.impl.lobby.loot_box.loot_box_sounds import LootBoxVideoStartStopHandler, LootBoxVideosSpecialRewardType, PausedSoundManager
...
class LootBoxOpeningView(LootBoxHideableView):
    ...
    __slots__ = ('__boxItem', '__specialRewardType', '__videoStartStopHandler', '__idleGuestCPauseSoundManager')

    def __init__(self):
        settings = ViewSettings(layoutID=R.views.lobby.loot_box.views.loot_box_opening_view.LootBoxOpeningView(), flags=ViewFlags.VIEW, model=LootBoxOpeningModel())
        ...
        self.__idleGuestCPauseSoundManager = PausedSoundManager()
        return
    ...
    def _initialize(self, *args, **kwargs):
        ...
        g_eventBus.addListener(events.LootboxesEvent.ON_SHOW_GUEST_C_IDLE, self.__showIdleGuestC, scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def _finalize(self):
        ...
        self.__clearIdleSound()
        g_eventBus.removeListener(events.LootboxesEvent.ON_OPEN_LOOTBOX, self.__onOpenNextBox, scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.LootboxesEvent.ON_SHOW_GUEST_C_IDLE, self.__showIdleGuestC, scope=EVENT_BUS_SCOPE.LOBBY)
        super(LootBoxOpeningView, self)._finalize()
        return

    def __showIdleGuestC(self, _):
        self.viewModel.setIsIdleGuestCVisible(True)
        playSound(LootBoxViewEvents.SPECIAL_GUEST_IDLE_ENTER)
        self.__idleGuestCPauseSoundManager.start()
    ...
    def __onReloadEnd(self):
        ...
        self.viewModel.setIsIdleGuestCVisible(False)
    ...
    def __updateVideoOff(self, diff):
        if NewYearStorageKeys.LOOT_BOX_VIDEO_OFF in diff:
            ...
            self.viewModel.setIsVideoOff(isVideoOff)
            if self.__specialRewardType == LootCongratsTypes.CONGRAT_TYPE_GUEST_C:
                if isVideoOff:
                    playSound(LootBoxViewEvents.SPECIAL_GUEST_IDLE_EXIT)
                    self.__idleGuestCPauseSoundManager.stop()
                else:
                    playSound(LootBoxViewEvents.SPECIAL_GUEST_IDLE_ENTER)
                    self.__idleGuestCPauseSoundManager.start()
    ...
    def __onWindowAccessibilityChanged(self, isWindowAccessible):
        ...
        if self.__specialRewardType == LootCongratsTypes.CONGRAT_TYPE_GUEST_C:
            if isWindowAccessible:
                self.__idleGuestCPauseSoundManager.unpause()
            else:
                self.__idleGuestCPauseSoundManager.pause()
        self.viewModel.setIsWindowAccessible(isWindowAccessible)

    def canBeClosed(self):
        return False

    def __clearIdleSound(self):
        if self.__specialRewardType == LootCongratsTypes.CONGRAT_TYPE_GUEST_C:
            playSound(LootBoxViewEvents.SPECIAL_GUEST_IDLE_EXIT)
            self.__idleGuestCPauseSoundManager.stop()
