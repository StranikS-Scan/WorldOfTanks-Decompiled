...
from gui.impl.lobby.loot_box.loot_box_helper import getTooltipContent, fireHideMultiOpenView, isLootboxValid, LootBoxHideableView, SpecialRewardData, showLootBoxSpecialReward2, SpecialGuestRewardData, SpecialAttachmentRewardData, fireSpecialRewardsClosed
...
from gui.shared.event_dispatcher import showLootBoxEntry, showCustomizationRarityAwardScreen
...
class RewardKitPremiumMultiOpenView(LootBoxHideableView):
    ...
    def _getListeners(self):
        ...
        return listeners + ((CustomizationEvent.ON_RARITY_REWARD_SCREEN_CLOSED, self._onRewardScreenClosed, EVENT_BUS_SCOPE.LOBBY), (LootboxesEvent.ON_SHOW_SPECIAL_REWARDS_CLOSED, self.__onSpecialRewardsClosed, EVENT_BUS_SCOPE.LOBBY), (LootboxesEvent.ON_STATISTICS_RESET, self.__onStatisticsReset, EVENT_BUS_SCOPE.LOBBY))
    ...
    def __showSpecialReward(self, args):
        ...
        if specialId is None or specialId not in self.__specialRewardData:
            ...
            if self.__specialRewardData[specialId].congratsType == LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT:
                intCD = self.__specialRewardData[specialId].congratsSourceId
                if intCD != 0:
    ...
    def __openNextBox(self, forceClearPage=False):
        result = yield NyLootBoxOpenProcessor(self.__boxItem, 1).request()
        if result:
            if result.userMsg:
    ...
    def __customizationBonusCatcher(self, bonus, packedBonusList):
        for bonusData in bonus.getCustomizations():
            ...
            if item.itemTypeName == 'attachment':
                model = None
                for packedModel in packedBonusList:
                    if packedModel.getRarity() in Rarity.UI_EFFECT:
                        model = packedModel
                        break

                if model is not None:
                    self.__specialRewardData[int(model.getIndex())] = self.__createAttachmentSpecialRewardData(item.id, LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT)

        return

    @staticmethod
    def __createSpecialRewardGuestData():
    ...
    def __createSpecialRewardData(self, sourceID, congratsType, vehicle):
        return SpecialRewardData(sourceName=Vehicle.getIconResourceName(Vehicle.getNationLessName(vehicle.name)), congratsType=congratsType, congratsSourceId=sourceID, vehicleName=vehicle.userName, vehicleIsElite=vehicle.isElite, vehicleLvl=formatRomanNumber(vehicle.level), vehicleType=formatEliteVehicle(vehicle.isElite, vehicle.type), backToSingleOpening=False, isGuaranteedReward=self.__isGuaranteedReward)

    def __createAttachmentSpecialRewardData(self, sourceID, congratsType):
        return SpecialAttachmentRewardData(congratsType=congratsType, congratsSourceId=sourceID, backToSingleOpening=False)
