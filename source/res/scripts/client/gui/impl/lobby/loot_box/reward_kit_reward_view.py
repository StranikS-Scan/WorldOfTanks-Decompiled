...
from account_helpers.settings_core.settings_constants import NewYearStorageKeys, OnceOnlyHints
...
from gui.impl.auxiliary.rewards_helper import getBackportTooltipData, isVideoVehicle
...
from gui.impl.lobby.loot_box.loot_box_bonuses_helpers import getEpicFormattedLootboxBonuses, AWARDS_MAX_COUNT, ORDER_BY_BOX_TYPE, getAttachmentsFromRewards
from gui.impl.lobby.loot_box.loot_box_helper import fireCloseToHangar, fireHideRewardView, getLootboxRendererModelPresenter, getTooltipContent, isLootboxValid, LootBoxHideableView, showLootBoxSpecialReward, setGaranteedRewardData, fireSpecialRewardsClosed, fireCloseToExterior, showStyledVehicleByStyleCD
...
from gui.shared.events import LootboxesEvent, CustomizationEvent
...
class RewardKitRewardView(LootBoxHideableView, UsersInfoHelper):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId is None:
                return
            window = BackportTooltipWindow(self.__tooltips[tooltipId], self.getParentWindow(), event)
            window.load()
            return window
        else:
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.new_year.tooltips.NyAttachmentsStatisticsTooltip():
            return NyAttachmentsStatisticsTooltip(self.__boxItem.getID())
    ...
    def _getListeners(self):
        ...
        return listeners + ((CustomizationEvent.ON_RARITY_REWARD_SCREEN_CLOSED, self._onRewardScreenClosed, EVENT_BUS_SCOPE.LOBBY),
         (LootboxesEvent.ON_OPENING_START, self.__onOpeningStart, EVENT_BUS_SCOPE.LOBBY),
         (LootboxesEvent.NEED_SHOW_REWARDS, self.__needShowRewards, EVENT_BUS_SCOPE.LOBBY),
    ...
    def __showSpecialReward(self, responseDict):
        ...
        if congratsType == LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT:
            intCD = int(responseDict.get('intCD'))
            if intCD != 0:
                attachment = self.__customizationService.getItemByID(GUI_ITEM_TYPE.ATTACHMENT, intCD)
                if attachment.rarity in Rarity.UI_EFFECT:
                    self._showAward(intCD)
                else:
    ...
    def _showAward(self, ctx):
        attachment = self.__customizationService.getItemByID(GUI_ITEM_TYPE.ATTACHMENT, ctx)
        if attachment.rarity in Rarity.UI_EFFECT:
            self._showReward(attachment)

    def _showReward(self, attachment):
        isFirstEntry = not self.__settingsCore.serverSettings.getOnceOnlyHintsSetting(OnceOnlyHints.NEW_C11N_SECTION_HINT)
        self.viewModel.setIsFirstAttach(isFirstEntry or not isC11nEnabled())
    ...
    @adisp_process
    def __openBox(self):
        result = yield NyLootBoxOpenProcessor(self.__boxItem).request()
        if not result:
    ...
    def __bonusOrder(self, bonus):
        ...
        if name == 'customizations':
            ...
            typeCust = bonusValue[0].get('custType', '')
            if typeCust == 'attachment':
                attachment = self.__customizationService.getItemByID(GUI_ITEM_TYPE.ATTACHMENT, bonusValue[0].get('id', 0))
                name = 'attachmentWithEffect' if attachment.rarity in Rarity.UI_EFFECT else 'attachment'
        if name == 'vehicles':
            bonusValue = bonus.getValue()
            compactDescr = next(iter(bonusValue))
            vehicle = self.__itemsCache.items.getItemByCD(compactDescr)
            if vehicle is not None:
                name = 'vehicleWithVideo' if isVideoVehicle(vehicle) else 'vehicle'
        if name == 'currencies':
            name = bonus.getCode()
    ...
    def __updateRewards(self):
        with self.getViewModel().transaction() as tx:
            isFirstAttachment = not self.__settingsCore.serverSettings.getOnceOnlyHintsSetting(OnceOnlyHints.NEW_C11N_SECTION_HINT) and len(getAttachmentsFromRewards(self.__rewards)) != 0
            ...
            tx.setIsFirstAttach(isFirstAttachment or not isC11nEnabled())
    ...
    @staticmethod
    def __onBuyBoxBtnClick(_=None):
        event_dispatcher.showLootBoxBuyWindow()

    def __onSpecialAction(self, responseDict):
        congratsType = responseDict.get('congratsType')
        congratsSourceId = responseDict.get('congratsSourceId', 0)
        if congratsType == LootCongratsTypes.CONGRAT_TYPE_GUEST_C:
            ...
            fireCloseToHangar()
        elif congratsType == LootCongratsTypes.CONGRAT_TYPE_ATTACHMENT:
            isFirstAttach = self.viewModel.getIsFirstAttach()
            if isFirstAttach:
                fireCloseToHangar()
            else:
                fireCloseToExterior()
