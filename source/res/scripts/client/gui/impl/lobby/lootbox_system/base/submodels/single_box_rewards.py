...
class SingleBoxRewards(SubViewImpl):
    __slots__ = ('__isReopen', '__category', '__openCount', '__bonuses', '__extraBonuses', '__tooltipItems', '__isVideoPlaying', '__eventName', '__backCallback')
    ...
    def __reopen(self):
        hideItemPreview()
        Views.load(ViewID.MAIN, subViewID=SubViewID.SINGLE_BOX_REWARDS, isReopen=True, count=self.__openCount, category=self.__category, bonuses=[self.__bonuses + self.__extraBonuses], eventName=self.__eventName, backCallback=self.__backCallback)
