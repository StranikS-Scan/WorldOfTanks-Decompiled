...
_BONUSES_ORDER = []
_BONUS_NAME_TO_BUTTON_ACTION = {}
_ACTION_TO_OBJECT = {ButtonActionType.TOEVENT: NYObjects.TOWN,
 ButtonActionType.TOGUESTD: NYObjects.CELEBRITY_D,
 ButtonActionType.TOMARKERTPLACE: NYObjects.MARKETPLACE,
 ButtonActionType.TOGIFTMACHINE: NYObjects.GIFT_MACHINE}
...
class NyAtmosphereLevelUpView(ViewImpl):
    ...
    def __onAction(self, args):
        ...
        if action is None:
            ...
            anchor = _ACTION_TO_OBJECT.get(actionType)
            if anchor:
    ...
    def __getButtonAction(self, level, bonuses):
        if not _BONUS_NAME_TO_BUTTON_ACTION:
            _BONUS_NAME_TO_BUTTON_ACTION.update({NyATMReward.DOG: ButtonActionType.TOGUESTD,
             NyATMReward.MARKETPLACE: ButtonActionType.TOMARKERTPLACE,
             getCoinType(): ButtonActionType.TOGIFTMACHINE})
        if level == _FIRST_LVL:
