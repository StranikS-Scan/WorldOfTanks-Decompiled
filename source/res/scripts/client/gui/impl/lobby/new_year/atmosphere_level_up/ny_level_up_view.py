...
from gui.impl.gen.view_models.views.lobby.new_year.tooltips.ny_guest_tooltip_model import GuestType
...
from gui.impl.lobby.new_year.action_helper import getButtonAction, ACTION_TO_OBJECT, getBonusesNames
...
from gui.impl.new_year.new_year_bonus_packer import packBonusModelAndTooltipData, getNewYearLevelUpBonusPacker
...
from items.components.ny_constants import NyATMReward, CurrentNYConstants
...
_HIDDEN_BONUSES = (CurrentNYConstants.TOYS,)
_BONUSES_ORDER = []
...
def _filterBonuses(bonuses):
    return [ bonus for bonus in bonuses if bonus.getName() not in _HIDDEN_BONUSES ]


def _bonusesSortOrder(bonus):
    if not _BONUSES_ORDER:
        _BONUSES_ORDER.extend((NyATMReward.DOG,
...
class NyAtmosphereLevelUpView(ViewImpl):
    ...
    def createToolTipContent(self, event, ctID):
        if ctID == R.views.lobby.tooltips.AdditionalRewardsTooltip() and self.viewStatus == ViewStatus.LOADED:
            ...
            bonusPackers = getNewYearLevelUpBonusPacker()
            packedBonuses = []
            for bonus in bonuses:
        ...
        if ctID == R.views.lobby.new_year.tooltips.NyGuestTooltip():
            return NyGuestTooltip(GuestType.NY_DOG)
    ...
    def _onLoading(self, *_, **kwargs):
        ...
        self.__completedLevels = kwargs.get('completedLevels', [])
    ...
    def __onAction(self, args):
        ...
        if action is None:
            ...
            anchor, _ = ACTION_TO_OBJECT.get(actionType)
            if anchor:
    ...
    def __onStylePreview(self, args):
        ...
        if styleItem is None:
            ...
            event_dispatcher.showStylePreview(getVehiclePreviewID(styleItem), styleItem, styleItem.getDescription(), showBackBtn=False, showCloseBtn=True, backCallback=event_dispatcher.showHangar)
            self.__onClose()
            return
    ...
    def __setRewards(self):
        ...
        bonuses = _filterBonuses(bonuses)
        ...
        with self.getViewModel().transaction() as model:
            ...
            self.__fillRewardsList(rewardsList=model.hugeRewards.getItems(), bonuses=hugeBonuses, sortMethod=_bonusesSortOrder, packer=getNewYearLevelUpBonusPacker())
            self.__fillRewardsList(rewardsList=model.rewards.getItems(), bonuses=otherBonuses, sortMethod=_bonusesSortOrder, packer=getNewYearLevelUpBonusPacker())
    ...
    def __getButtonAction(self, level, bonuses):
        ...
        bonusName, action = getCoinType(), ButtonActionType.TOGIFTMACHINE
        return action if bonusName in bonusesNames else ButtonActionType.UNDEFINED
