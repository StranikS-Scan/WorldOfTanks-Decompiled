...
from gui.impl.gen.view_models.views.lobby.new_year.views.challenge.bundle_purchase_dialog_view_model import BundlePurchaseDialogViewModel
...
from gui.shared import events, event_dispatcher, EVENT_BUS_SCOPE, g_eventBus
...
from new_year.ny_constants import NYObjects, NyTabBarChallengeView, CHALLENGE_TAB_TO_CAMERA_OBJ, NyGFNotificationTemplates
from new_year.ny_helper import getNYGeneralConfig, getNYDogConfig
...
def fillBonuses(rewardsModel, bonusesList, bonusPacker, tooltips):
    rewardsModel.clear()
    packBonusModelAndTooltipData(bonusesList, rewardsModel, bonusPacker, tooltips)
...
class BundlePurchaseDialogView(ViewImpl):
    ...
    def __updateCurrentBundle(self, model=None):
        ...
        model.setIsApplied(self.__currentBundleLevel == self.__getCurrentLevel())
    ...
    def __updatePrice(self, model=None):
        ...
        model.setIsEnough(money.get(currency, 0) >= price)
    ...
    def __makeAndFillReward(self, model=None):
        ...
        instant, bonuses = self.__helper.getBoxBonusesInfo(NY_SACK_LEVEL_TO_CATEGORY.get(self.__currentBundleLevel, 1))
        bonuses.sort(cmp=_bonusesOrderCmp)
        fillBonuses(model.getRewards(), bonuses, bonusPacker, self._tooltips)
        if instant:
            fillBonuses(model.getInstantRewards(), instant, bonusPacker, self._tooltips)
    ...
    def __onBuy(self):
        g_eventBus.handleEvent(events.NyDogEvent(events.NyDogEvent.START_PURCHASE), scope=EVENT_BUS_SCOPE.LOBBY)
        ...
        if result.success:
            ...
            bonusesData = getNYDogConfig().getLevelBonuses(self.__currentBundleLevel - 1).get('customizations', [])
            if bonusesData:
                serviceChannel.pushClientMessage({'data': bonusesData[0],
        ...
        g_eventBus.handleEvent(events.NyDogEvent(events.NyDogEvent.END_PURCHASE), scope=EVENT_BUS_SCOPE.LOBBY)
        self.__isRequestToBuyProcessing = False
        self.destroyWindow()
    ...
    def __getSackState(cls, sackLevel):
        ...
        return SackState.LOCKED if sackLevel > currentLevel + 1 else SackState.AVAILABLE


class BundlePurchaseDialogWindow(LobbyWindow):
    __slots__ = ('_blur', '_doBlur')

    def __init__(self, parent=None, doBlur=True, layer=WindowLayer.FULLSCREEN_WINDOW, *args, **kwargs):
        super(BundlePurchaseDialogWindow, self).__init__(DialogFlags.TOP_FULLSCREEN_WINDOW, content=BundlePurchaseDialogView(*args, **kwargs), layer=layer, parent=parent)
        self._blur = None
        self._doBlur = doBlur
        return
