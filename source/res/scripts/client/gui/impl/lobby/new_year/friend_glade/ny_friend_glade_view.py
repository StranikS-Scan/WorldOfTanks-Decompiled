...
class NyFriendGladeView(SceneRotatableView, SelectableLogicPresenter):
    __slots__ = ('__currentObject', '__blur', '__resourceBoxPresenter', '__showGreetings', '__delayer')
    ...
    def __onSideBarSelected(self, ctx):
        ...
        NewYearNavigation.switchTo(camObj, False, withFade=False)
        self.__updateAll()
    ...
    def __updateAll(self):
        with self.viewModel.transaction() as model:
            ...
            self.__updateSlots(fullUpdate=True, model=model)
            self.__updateResourcesViewModel(model=model.resourcesViewModel)
        resourcesViewState = self.viewModel.resourcesViewModel.getState()
        isFavoriteFriend = resourcesViewState == State.AVAILABLE or resourcesViewState == State.TIMER
        if self.__currentObject == FriendGladeViewTabs.RESOURCES.value and not isFavoriteFriend:
            self.__blur.enable()
        else:
            self.__blur.disable()
    ...
    def __updateResourcesViewModel(self, model):
        ...
        if self._friendsService.friendHangarSpaId not in bestFriends:
            if len(bestFriends) >= self._friendsService.maxBestFriendsCount:
                if findFirst(lambda data: data[BestFriendsDataKeys.IS_REMOVED] is True, bestFriends.itervalues()):
    ...
    def __updateWelcomeText(self, model):
        ...
        titleIdx, descriptionIdx = parseHangarNameMask(friendHangarState.get(FriendsDataKeys.HANGAR_NAME, 0))
