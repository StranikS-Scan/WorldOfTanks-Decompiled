...
from new_year.ny_constants import NyTabBarFriendGladeView, NYObjects
...
class NyFriendsView(NyHistoryPresenter):
    __slots__ = ('__resourceBoxPresenter', '__delayer', '__notifier')
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.__notifier = PeriodicNotifier(lambda : time_utils.ONE_SECOND, self.__updateFriendlyTimers, periods=(time_utils.ONE_SECOND,))
    ...
    def __goToCollect(self, spaId):
        ...
        if spaId is None:
            ...
            if self._friendsService.isInFriendHangar:
                if not self.isLoaded:
                    self._friendsService.leaveFriendHangar()
                    return
                if getPossibleResourcesByCollectingFromFriend() > 0:
                    NewYearNavigation.switchToView(ViewAliases.FRIEND_GLADE_VIEW, tabName=NyTabBarFriendGladeView.RESOURCES)
                else:
                    NewYearNavigation.switchToView(ViewAliases.FRIEND_GLADE_VIEW, tabName=NyTabBarFriendGladeView.TOWN)
            return
        ...
        if spaId is None:
            ...
            if self._friendsService.isInFriendHangar:
                if not self.isLoaded:
                    self._friendsService.leaveFriendHangar()
                    return
                if len(self.viewModel.getBestFriends()) < self.viewModel.getMaxBestFriendsCount() and spaId not in self._friendsService.bestFriendList.keys():
                    NewYearNavigation.switchToView(ViewAliases.FRIEND_GLADE_VIEW, tabName=NyTabBarFriendGladeView.RESOURCES)
                else:
    ...
    def __updateAll(self):
        with self.viewModel.transaction() as model:
            ...
            model.setMaxBestFriendsCount(self._friendsService.maxBestFriendsCount)
    ...
    def __updateFriendList(self, model):
        friendList = self._friendsService.friendList
    ...
    def __cooldown(timeTill):
        ...
        return 0 if cd <= 0 else cd + time_utils.ONE_MINUTE
    ...
    def __updateFriendlyTimers(self):
        ...
        with self.viewModel.transaction() as model:
            ...
            for friendModel in friends:
                ...
                bestFriendInfo = self._friendsService.bestFriendList.get(fuid, None)
