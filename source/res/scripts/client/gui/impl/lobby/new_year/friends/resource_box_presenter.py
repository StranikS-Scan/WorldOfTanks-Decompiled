...
class ResourceBoxPresenter(SubModelPresenter):
    __slots__ = ('__piggyBankProgressToken', '__tooltips', '__isInFriendHangar', '__tokenCount')
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            window = BackportTooltipWindow(self.__tooltips[tooltipId], self.getParentWindow(), event) if tooltipId is not None else None
    ...
    def _getEvents(self):
        ...
        return events + ((viewModel.onGetReward, self.__onGetReward),
         ...
         (viewModel.onOpenAnimationTrigger, self.__onOpenAnimationTrigger),
    ...
    def __updateResourceBoxModel(self):
        with self.getViewModel().transaction() as model:
            ...
            savedTokensCount = self.__nyController.getPiggyTokensCount()
            rewards = []
            pointsForAwards = model.getPointsForAwards()
            pointsForAwards.clear()
            tokenCount = self.__getTokenCount()
            ...
            model.setSavedProgressValue(savedTokensCount)
            model.setIsOpenAnimationTriggered(self.__nyController.getIsPiggyOpenAnimationTriggered())
            if tokenCount != savedTokensCount:
                self.__nyController.setIsPiggyOpenAnimationTriggered(False)
                model.setIsOpenAnimationTriggered(False)
    ...
    def __onGetReward(self):
        result = yield PiggyBankRewardsProcessor().request()
        if result.userMsg:
            SystemMessages.pushMessage(result.userMsg, type=SystemMessages.SM_TYPE.ErrorSimple)
    ...
    def __getTokenCount(self):
        ...
        return self.__itemsCache.items.tokens.getTokenCount(self.__piggyBankProgressToken) if self.__piggyBankProgressToken is not None else 0

    def __syncPiggyToken(self):
        tokenCount = self.__getTokenCount()
        savedTokensCount = self.__nyController.getPiggyTokensCount()
        if tokenCount != savedTokensCount:
            self.__nyController.setPiggyTokensCount(tokenCount)

    def __onOpenAnimationTrigger(self):
        self.__nyController.setIsPiggyOpenAnimationTriggered(True)
        self.getViewModel().setIsOpenAnimationTriggered(True)
        self.__syncPiggyToken()
