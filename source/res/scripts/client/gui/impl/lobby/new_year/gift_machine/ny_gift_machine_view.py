...
from account_helpers.AccountSettings import NY_GIFT_MACHINE_BUY_TOKEN_VISITED, NY_GIFT_MACHINE_HINT_VISIBLE
...
class GiftMachineCameraHelper(object):
    ...
    @staticmethod
    def __selectPreviewVehicle(vehIntCD=None):
        g_currentPreviewVehicle.selectVehicle(vehIntCD, None, None, showWaitingBg=False)
        return

    @staticmethod
    def __isVehicleSwitched():
...
class NyGiftMachineView(SceneRotatableView, SelectableLogicPresenter):
    __slots__ = ('__cameraHelper', '__displayView', '__storedReward', '__isCameraHelperActive', '__isMessageBarVisible', '__redButtonPressCallbackID', '__returnToIdleCallbackID', '__onCoinsUpdateCallbackID', '__buttonPressCount', '__isButtonBlocked')
    ...
    def __init__(self, viewModel, parentView, soundConfig=None):
        ...
        self.__buttonPressCount = 0
        self.__isButtonBlocked = False
        return
    ...
    def _getEvents(self):
        ...
        return events + ((self.viewModel.onGoToBuyTokens, self.__onGoToBuyTokens),
         ...
         (self.__nyGiftMachineCtrl.onGoToBuyTokens, self.__onGoToBuyTokens),
         ...
         (self.__nyGiftMachineCtrl.onLootListInfoWindowStateChanged, self.__onBlockButtonStateChanged))
    ...
    def __updateState(self, state, model=None):
        if self.__displayView is None:
            ...
            model.setMachineState(state)
            if AccountSettings.getUIFlag(NY_GIFT_MACHINE_HINT_VISIBLE) and self.__buttonPressCount > 1:
                self.viewModel.setIsNeededShowHint(True)
    ...
    def __onRedButtonPress(self):
        if self.__isButtonBlocked:
            return
        elif self.__redButtonPressCallbackID is not None:
            return
        else:
    ...
    def __onBlockButtonStateChanged(self, state):
        self.__isButtonBlocked = state
    ...
    def __redButtonPress(self):
        ...
        if self.__nyGiftMachineCtrl.canApplyCoin:
            self.__tryApplyCoin()
            if AccountSettings.getUIFlag(NY_GIFT_MACHINE_HINT_VISIBLE):
                self.__buttonPressCount += 1
    ...
    def __onHintClose(self):
        AccountSettings.setUIFlag(NY_GIFT_MACHINE_HINT_VISIBLE, False)
        self.viewModel.setIsNeededShowHint(False)
