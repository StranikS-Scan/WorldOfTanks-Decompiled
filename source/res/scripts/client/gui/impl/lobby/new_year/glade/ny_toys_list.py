...
_HANG_SOUNDS_MAP = {ToyTypes.TOP: NewYearSoundEvents.ADD_TOY_TREE,
 ...
 ToyTypes.DOG_BREED: NewYearSoundEvents.ADD_TOY_LIGHT}
_UNHANG_SOUNDS_MAP = {ToyTypes.TOP: NewYearSoundEvents.REMOVE_TOY_EFFECT_SMALL,
 ...
 ToyTypes.DOG_BOWL: NewYearSoundEvents.REMOVE_DOG_EFFECT,
 ToyTypes.DOG_TOY: NewYearSoundEvents.REMOVE_DOG_EFFECT,
 ToyTypes.DOG_COLLAR: NewYearSoundEvents.REMOVE_DOG_EFFECT,
 ToyTypes.DOG_HOUSE: NewYearSoundEvents.REMOVE_DOG_EFFECT}
EXLUDE_SLOT_TYPE_FROM_REMOVAL = [ToyTypes.DOG_BREED]

class NyToysList(object):
    __slots__ = ('__slotType', '__slotID', '__installedToyID', '__es')
    ...
    def open(self, slotID, viewModel):
        if slotID > -1:
            ...
            self.__es.subscribeToEvent(viewModel.toysList.onApplySelection, self.__onApplySelection)
            self.__es.subscribeToEvent(viewModel.toysList.onListClose, self.__onListClose)
            self.__es.subscribeToEvent(viewModel.toysList.onAllToysSeen, self.__onAllToysSeen)
            self.__es.addCallbackOnUnsubscribe(lambda : self.__clearToys(viewModel))

    def finalize(self):
        self.clear()

    @staticmethod
    def __clearToys(viewModel):
    ...
    def __onApplySelection(self, args):
        ...
        canRemoveToy = self.__slotType not in EXLUDE_SLOT_TYPE_FROM_REMOVAL
        isRemoval = self.__installedToyID == toyID
        if toyID != INVALID_TOY_ID and self.__slotID in inventoryToys and toyID in inventoryToys[self.__slotID] and (inventoryToys[self.__slotID][toyID].getCount() or toyID == self.__installedToyID) and (canRemoveToy or not isRemoval):
            toy = inventoryToys[self.__slotID][toyID] if not isRemoval else None
            self.__hangToy(toy)
        return
    ...
    def __fillSlots(self, viewModel):
        with viewModel.toysList.transaction() as model:
            allToysDescr = self.__nyController.getAllToysDescrsByType(self.__slotType)
            inventoryToys = self.__nyController.getToysBySlot(self.__slotID)
            toys = model.getToys()
            toys.clear()
            for toyDescriptor in allToysDescr:
                toyID = toyDescriptor.id
                toyModel = ToyModel()
                toyModel.setToyID(toyID)
                toyModel.setIcon(toyDescriptor.icon)
                isAvailable = toyID in inventoryToys and (inventoryToys[toyID].getCount() or toyID == self.__installedToyID)
                if isAvailable:
