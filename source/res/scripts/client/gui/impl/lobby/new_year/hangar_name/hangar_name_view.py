...
class HangarNameView(ViewImpl):
    ...
    def __setName(self, nameID, descID, hasChanges=False):
        self.viewModel.setIsHidden(True)
        dialogResult = yield showHangarNameChangeConfirmDialog(parent=self.getParentWindow(), nameID=nameID, descID=descID, hasChanges=hasChanges)
    ...
    def __onSubmitName(self):
        self.__setName(self.__titleId, self.__descriptionId, self.viewModel.getHasChanges())
