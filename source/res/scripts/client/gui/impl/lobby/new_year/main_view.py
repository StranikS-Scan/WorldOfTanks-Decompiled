...
class MainView(ViewImpl):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self.viewModel.setTutorialState(self.__settingsCore.serverSettings.getNewYearStorage().get(NewYearStorageKeys.TUTORIAL_STATE, TutorialStates.INTRO))
    ...
    def _finalize(self):
        ...
        if self.__ctx and self.__ctx.menuName in NyWidgetTopMenu.ALL_FRIEND_HANGAR:
    ...
    def __updateWaitingStatus(self):
        if self.viewModel is None:
            return
        else:
            self.viewModel.setIsWaitingShown(Waiting.isVisible())
            return

    def __showWaiting(self, _):
        self.__updateWaitingStatus()
    ...
    def __onPreSwitchViewEvent(self, ctx):
        ...
        elif ctx.kwargs.get('isObjectSwitched', False):
    ...
    def __onChangeFriendService(self):
        if self.__friendsService.isServiceEnabled is False and self.__friendsService.friendHangarSpaId:
            self.__friendsService.leaveFriendHangar()
            NewYearNavigation.switchToView(ViewAliases.GLADE_VIEW, tabName=AdditionalCameraObject.UNDER_SPACE)
