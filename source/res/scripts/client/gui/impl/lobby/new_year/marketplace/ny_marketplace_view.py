...
from new_year.ny_constants import NyWidgetTopMenu, Collections, NyTabBarMarketplaceView, SyncDataKeys, RESOURCES_ORDER
from new_year.ny_marketplace_helper import getNYMarketplaceConfig, isCollectionItemReceived, getCollectionCompleteInfo
...
class NyMarketplaceView(SceneRotatableView, NyHistoryPresenter):
    __slots__ = ('_tooltips', '__settings', '__needToResetAppearance', '__needResetBloor', '__cameraCallbackId', '__blur', '__tabName', '__currentPreviewVehicle', '__currentPreviewStyle')
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.__settings.setResource(max([ (r, self._nyController.currencies.getResouceBalance(r.value)) for r in RESOURCES_ORDER ], key=itemgetter(1))[0])
    ...
    def __updateModel(self, yearName, kitIdx=0):
        ...
        with self.viewModel.transaction() as model:
            ...
            cards.clear()
            prevNYLevel = self._itemsCache.items.festivity.getPrevNYLevel(yearName)
    ...
    def __updatePrice(self):
        collectionDistributions = self._itemsCache.items.festivity.getCollectionDistributions()
        prevNYLevel = self._itemsCache.items.festivity.getPrevNYLevel(self.__tabName)
        config = getNYMarketplaceConfig()
    ...
    def __getKitState(self, item):
        if isCollectionItemReceived(item, self.__tabName):
