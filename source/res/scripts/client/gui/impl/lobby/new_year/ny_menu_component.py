...
from gui.shared.event_dispatcher import NYViewCtx, showyGiftMachineLootListView
...
from skeletons.new_year import ICelebritySceneController, IFriendServiceController, IGiftMachineController, INewYearTriggerHintsController
from uilogging.ny.loggers import NyTopMenuFlowLogger, NyWidgetTopMenuLogger, NyResourcesLogger
from gui.impl.gen.view_models.views.lobby.new_year.views.main_view_model import MainViews
...
class NYMainMenu(NyHistoryPresenter):
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.__notifier = PeriodicNotifier(lambda : time_utils.ONE_SECOND, self.__updateCollecting, periods=(time_utils.ONE_SECOND,))
    ...
    def _getEvents(self):
        return ((self.viewModel.onSwitchContent, self.__onMenuItemSelected),
         (self.viewModel.onShowLootList, self.__onShowLootList),
         ...
         (self.__nyGiftMachineCtrl.onLootListInfoUpdated, self.__onGiftMachineLootListUpdated),
    ...
    def __onCollectResources(self):
        ...
        if NewYearNavigation.getNavigationState().isCurrentObject(NYObjects.RESOURCES):
            NewYearSoundsManager.playEvent(NewYearSoundEvents.TRANSITION_RESOURCES)
        NewYearNavigation.switchTo(NYObjects.RESOURCES, False)
    ...
    def __onMenuItemSelected(self, args):
        ...
        if self.__currentView != menuName:
    ...
    def __onShowLootList(self):
        showyGiftMachineLootListView()
    ...
    def __onSwitchView(self, ctx):
        ...
        if menuName != self.__currentView:
            ...
            if wasInFriendHangar and not toFriendHangar or not wasInFriendHangar and not toFriendHangar and self._friendsService.isInFriendHangar:
    ...
    def __onObjectStateChanged(self):
        if self.viewModel is None:
            ...
            self.viewModel.balance.setIsResourcesTabOpen(NewYearNavigation.getCurrentObject() == NYObjects.RESOURCES)
            return
    ...
    def __updateMenu(self):
        ...
        if not self.__currentView:
            ...
            with self.viewModel.transaction() as model:
                ...
                if tabIdx == MainViews.GIFT_MACHINE.value:
                    model.setIsGiftMachineLootListAvailable(bool(self.__nyGiftMachineCtrl.getLootListInfo()))
    ...
    def __getCollectState(self):
        ...
        return CollectState.FINISHED if getCollectingCooldownTime() > eventEndTimeTill else CollectState.COLLECTED
    ...
    def __updateResources(self):
        if self.__lockForCustomAnimation:
            return
        with self.viewModel.transaction() as model:
            model.balance.setIsHintVisible(self.__triggerHintsController.triggerHintsState == TriggerHintsStates.RESOURCES and self.__currentView != NyWidgetTopMenu.GLADE)
            model.balance.setIsWalletAvailable(self.__wallet.isAvailable)
    ...
    def __onGiftMachineLootListUpdated(self):
        with self.viewModel.transaction() as model:
            model.setIsGiftMachineLootListAvailable(bool(self.__nyGiftMachineCtrl.getLootListInfo()))
    ...
    def __getSoundStateValue(self):
        return None if self.__friendsService.isInFriendHangar else _SOUNDS_MAP.get(NewYearSoundConfigKeys.STATE_VALUE, {}).get(self.__currentView)
