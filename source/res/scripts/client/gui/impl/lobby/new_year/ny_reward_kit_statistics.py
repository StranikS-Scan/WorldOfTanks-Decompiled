...
class NyRewardKitStatistics(object):
    ...
    def updateStatistics(self, model, lootboxID, lastStatisticsResetFailed):
        rewardsData, boxesCount, _ = self.getStatistics(lootboxID)
    ...
    def getStatistics(self, lootboxID):
        ...
        return (rewardsData, boxesCount, None) if statsKey is None else self.__itemsCache.items.tokens.getLootBoxesStats().get(statsKey, (rewardsData, boxesCount, None))

    def updateRewards(self, model, rewardsData):
        rewards = model.getRewards()
        rewards.clear()
        for rewardType in _REWARD_ORDER:
            if rewardType == Type.MODERNIZEDEQUIPMENT:
                ...
                rewardData = self.getValueOfSpecificCustomization(rewardsData, GUI_ITEM_TYPE_NAMES[GUI_ITEM_TYPE.STYLE])
            elif rewardType == Type.ATTACHMENT:
                rewardData = self.getValueOfSpecificCustomization(rewardsData, GUI_ITEM_TYPE_NAMES[GUI_ITEM_TYPE.ATTACHMENT])
            else:
    ...
    @staticmethod
    def getValueOfSpecificCustomization(rewardsData, custType):
        value = 0
        for item in rewardsData.get('customizations', []):
            if item.get('custType') == custType:
