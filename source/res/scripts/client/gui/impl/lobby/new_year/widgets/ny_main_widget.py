...
class NyMainWidget(ViewImpl):
    ...
    def _onLoading(self, *args, **kwargs):
        super(NyMainWidget, self)._onLoading(*args, **kwargs)
        with self.viewModel.transaction() as model:
    ...
    def __onGoToGladeView(self, *_):
        ...
        if not self.__tryToShowTutorial():
            NewYearNavigation.switchTo(CustomizationObjects.FIR, True)

    def __onGoToChallenge(self):
        self.__uiLogger.logClick(self.isLobbyMode)
        if not self.__tryToShowTutorial():
            NewYearNavigation.switchTo(NYObjects.CELEBRITY_A, viewAlias=ViewAliases.CELEBRITY_VIEW)

    def __tryToShowTutorial(self):
