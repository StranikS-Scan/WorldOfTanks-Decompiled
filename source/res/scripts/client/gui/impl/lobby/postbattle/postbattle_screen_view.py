...
from gui.wt_event.wt_event_helpers import isWtEventBattleQuest, BATTLE_QUEST_PREFIX
...
class PostbattleScreenView(ViewImpl):
    ...
    def _initialize(self, *args, **kwargs):
        ...
        self.__addListeners()
        g_eventBus.handleEvent(events.LobbyHeaderMenuEvent(LobbyHeaderMenuEvent.TOGGLE_VISIBILITY, ctx={'state': LobbyHeaderState(self.uniqueID, HeaderMenuVisibilityState.NOTHING, LobbyHeaderVisibilityAction.ENTER)}), EVENT_BUS_SCOPE.LOBBY)
        self.viewModel.setEventTooltipCategory(BATTLE_QUEST_PREFIX)

    def _finalize(self):
        super(PostbattleScreenView, self)._finalize()
        g_eventBus.handleEvent(events.LobbyHeaderMenuEvent(LobbyHeaderMenuEvent.TOGGLE_VISIBILITY, ctx={'state': LobbyHeaderState(self.uniqueID, HeaderMenuVisibilityState.ALL, LobbyHeaderVisibilityAction.EXIT)}), EVENT_BUS_SCOPE.LOBBY)
