...
class HangarAmmunitionGroupsController(AmmunitionGroupsController):
    ...
    def _getGroups(self):
        if self._vehicle is None:
            ...
            return HALLOWEEN_GROUPS if self.prbDispatcher is not None and (self.prbDispatcher.getFunctionalState().isInPreQueue(QUEUE_TYPE.EVENT_BATTLES) or self.prbDispatcher.getFunctionalState().isInUnit(PREBATTLE_TYPE.EVENT)) and self._vehicle.level in self.__epicMetaGameCtrl.getValidVehicleLevels() else RANDOM_GROUPS
