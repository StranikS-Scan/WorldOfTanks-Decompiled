...
def getContextMenuData(event, uniqueID, selectedSetup):
    ...
    if cmType == TankSetupFields.TANK_SETUP_CARD:
        contextMenuArgs = {paramName:event.getArgument(paramName) for paramName in ('intCD', 'slotType', 'isMounted', 'isMountedMoreThanOne', 'installedSlotId', 'itemInstalledSetupIdx', 'itemInstalledSetupSlotIdx', 'isDisabled', 'shellsCounts')}
...
def getHangarContextMenuData(event, uniqueID):
    ...
    if cmType == TankSetupFields.AMMO_PANEL_SLOT:
        contextMenuArgs = {paramName:event.getArgument(paramName) for paramName in ('intCD', 'slotType', 'isMounted', 'isMountedMoreThanOne', 'installedSlotId', 'itemInstalledSetupIdx', 'itemInstalledSetupSlotIdx', 'shellsCounts')}
        contextMenuArgs['emitterUID'] = uniqueID
        if contextMenuArgs['intCD'] > 0:
