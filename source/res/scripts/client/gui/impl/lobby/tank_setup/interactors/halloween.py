...
class HalloweenInteractor(BaseInteractor):
    ...
    def showExitConfirmDialog(self):
        eventBattlesController = self._eventBattlesController
        progressCtrl = eventBattlesController.getHWProgressCtrl()
        if not eventBattlesController.isEnabled() or not progressCtrl or progressCtrl.isPostPhase():
