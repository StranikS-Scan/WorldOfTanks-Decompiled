...
from gui.server_events.bonuses import getOfferBonuses
...
class WTEventSingleAwardView(WTEventAwardView):

    def _onLoading(self, reward, *args, **kwargs):
        rAwardsView = R.strings.event.WTEventAwardsView
        rewardKey = reward.keys()[0]
        bonuses = getOfferBonuses(rewardKey, reward[rewardKey])
        statusText = backport.text(rAwardsView.status.part())
        description = backport.text(rAwardsView.desciption())
        with self.getViewModel().transaction() as model:
            model.setIsFinalReward(False)
            model.setIsPostBattle(False)
            model.setTitle(backport.text(rAwardsView.title()))
            model.setStatus(statusText)
            model.setDescription(description)
            packBonusModelAndTooltipData(bonuses, model.mainRewards, self._tooltipItems, getWtEventBonusPacker())
...
class WTOfferAwardWindow(LobbyNotificationWindow):
    __slots__ = ()

    def __init__(self, reward, parent=None):
        super(WTOfferAwardWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN, content=WTEventSingleAwardView(reward=reward), parent=parent)
