...
class WTEventCarouselView(ViewImpl, IGlobalListener):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(layoutID=R.views.lobby.wt_event.WTEventCarousel(), flags=ViewFlags.VIEW, model=WtEventCarouselViewModel())
        settings.args = args
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.wt_event.tooltips.WtEventStampTooltipView():
...
@dependency.replace_none_kwargs(itemsCache=IItemsCache, gameEventCtrl=IEventBattlesController)
def _fillVehicles(array, item, itemsCache=None, gameEventCtrl=None):
    vehicles = sorted(itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_ANY_TAG(_TAGS.EVENT_VEHS)).values(), key=lambda veh: _VEHICLES_ORDER.index(veh.nationID))
    array.clearItems()
    ...
    for vehicle in vehicles:
        ...
        if _TAGS.EVENT_BOSS in vehicle.tags and _TAGS.EVENT_SPECIAL_BOSS not in vehicle.tags:
            if not gameEventCtrl.hasEnoughTickets():
                iconName += '_alt'
