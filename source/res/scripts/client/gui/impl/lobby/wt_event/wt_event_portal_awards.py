...
from gui.impl.lobby.wt_event.wt_event_sound import WTEventAwardsScreenVideoSound
from gui.impl.lobby.wt_event.wt_event_vehicle_portal import ReRollButton
from gui.impl.pub.dialog_window import DialogButtons
from gui.impl.pub.lobby_window import LobbyWindow
from gui.game_control.wt_lootboxes_controller import LootBoxAwardsManager
...
class WtEventPortalAwards(WtEventBasePortalAwards, CallbackDelayer):
    ...
    def _updateModel(self):
        ...
        with self.viewModel.transaction() as model:
            ...
            if hasUnclaimedLoot(self.__lootBoxType):
                model.portalAvailability.setLootBoxesCount(self._boxesCtrl.getLootBoxesCountByTypeForUI(self.__lootBoxType) - 1)
            if isBossLootBox:
    ...
    @wg_async
    def _onClose(self):
        if self.__dialogueOpen:
            return
        self.__dialogueOpen = True
    ...
    def __openMore(self, reRollButtonUsed):
        with self.viewModel.transaction() as model:
            if reRollButtonUsed == ReRollButton.CLAIM_AND_RELAUNCH:
                ...
                model.setOpeningType(LootBoxOpeningType.REROLL)
        parent = self.getParentWindow()
