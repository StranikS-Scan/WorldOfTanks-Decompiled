...
from gui.shared.event_dispatcher import showBrowserOverlayView, showEventTankmenVoiceover, showEventRewardsSelectionWindow
from gui.wt_event.wt_event_helpers import backportTooltipDecorator, getInfoPageURL
from helpers import dependency
from skeletons.gui.app_loader import IAppLoader
from skeletons.gui.game_control import IEventBattlesController
from gui.shared.event_dispatcher import showWtEventOfferAwardWindow
_logger = logging.getLogger(__name__)
if typing.TYPE_CHECKING:
    from typing import Dict, List, Union
    from frameworks.wulf import View

class WTEventProgressionView(ViewImpl, LobbyHeaderVisibility):
    ...
    @staticmethod
    def __onCommanderPreviewClicked():
        showEventTankmenVoiceover()

    @args2params(int)
    def __onTakeReward(self, step):
        rewards = []

        def _onRewardReceived(rs):
            rewards.extend(rs)
            showWtEventOfferAwardWindow(rewards[0])

        showEventRewardsSelectionWindow(level=step + 1, onRewardsReceivedCallback=_onRewardReceived)

    def __addListeners(self):
        self.viewModel.onClose += self.__onCloseHandler
        self.viewModel.onAboutClicked += self.__onAboutClicked
        self.viewModel.onCommanderPreview += self.__onCommanderPreviewClicked
