...
import WWISE
import SoundGroups
from cgf_components import sound_helpers, wt_sounds_manager
...
class LootBoxAreaSound(object):
    ...
    def popover(self):
        sound_helpers.play2d(_WTEventLootboxPortalStates.LOOTBOX_POPOVER_OPEN)
    ...
    def updatePortal(self, portalType):
        changePortalState(portalType)


class _SoundComponent(Component):

    def __init__(self, soundTargetNode, eventID, soundObjectName):
        matrix = Math.Matrix(soundTargetNode) if soundTargetNode is not None else Math.Matrix()
        self.__soundObject = SoundGroups.g_instance.WWgetSoundObject(soundObjectName, matrix)
...
class _TicketSoundComponent(_SoundComponent):
    __evtCtrl = dependency.descriptor(IEventBattlesController)
...
class WTEventBattleQueueSoundPlayer(object):
    _SOUND_MAPPING = {'ussr:R97_Object_140_hound_TLXXL': 'ev_wt24_hunters_vo_enter_queue',
     'france:F18_Bat_Chatillon25t_hound_TLXXL': 'ev_wt24_hunters_vo_enter_queue',
     'usa:A120_M48A5_hound_TLXXL': 'ev_wt24_hunters_vo_enter_queue',
     'czech:Cz04_T50_51_Waf_Hound_3DSt': 'ev_wt24_hunters_vo_enter_queue',
     'germany:G98_Waffentrager_E100_TLXXL': 'ev_wt24_w_vo_enter_queue',
     'germany:G98_Waffentrager_E100_TLXXL_S': 'ev_wt24_w_vo_enter_queue'}
    __itemsCache = dependency.descriptor(IItemsCache)

    def __init__(self):
        languageSwitch = wt_sounds_manager.LanguageSwitchManager.getLanguageSwitch()
        for name, value in languageSwitch.iteritems():
            SoundGroups.g_instance.setSwitch(name, value)

    def playSound(self, vehicleName):
        eventName = self._SOUND_MAPPING.get(vehicleName, None)
        if eventName:
