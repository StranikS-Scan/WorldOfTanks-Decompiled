...
# Embedded file name: scripts/client/gui/impl/new_year/components/new_year_pop_over_window.py
...
class NewYearPopOverWindow(Window):
    __slots__ = ()

    def __init__(self, event, content, parent):
        super(NewYearPopOverWindow, self).__init__(wndFlags=WindowFlags.POP_OVER, decorator=WindowView(layoutID=R.views.newYearPopOverWindow, viewModelClazz=PopOverWindowModel), content=content, parent=parent)
    ...
    def popOverModel(self):
        return super(NewYearPopOverWindow, self)._getDecoratorViewModel()
