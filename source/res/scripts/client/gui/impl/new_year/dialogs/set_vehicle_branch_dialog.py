...
class SetVehicleBranchDialogWindow(LobbyDialogWindow):
    ...
    def __init__(self, invID, slotID):
        ...
        super(SetVehicleBranchDialogWindow, self).__init__(bottomContent=DialogPricesContent(), balanceContent=CommonBalanceContent(), enableBlur=True, enableBlur3dScene=False)
    ...
    def __setBottomContent(self):
        ...
        with self.bottomContentViewModel.transaction() as model:
            model.setValueMainCost(backport.getIntegralFormat(cost.get(currency)))
            model.setIconMainCost(img)
    ...
    def __updateIsEnough(self):
        ...
        self.bottomContentViewModel.setNotEnoughMain(not isEnough)
