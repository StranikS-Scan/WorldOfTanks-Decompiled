...
from new_year.celebrity.celebrity_quests_helpers import checkBreedBuyingAbility, checkSacksBuyingAbility
...
from skeletons.new_year import INewYearController, IGiftMachineController, IFriendServiceController, ICelebrityController, ICelebritySceneController, INewYearTutorialController
...
class NewYearNavigation(object):
    ...
    def switchTo(cls, objectName, instantly=False, viewAlias=None, doAutoRouting=False, *args, **kwargs):
        ...
        if not isIntroShown:
            cls.__nyTutorialController.startIntro(callback=partial(cls.switchTo, objectName, instantly, viewAlias, doAutoRouting, *args, **kwargs))
            return
        elif cls._navigationState.isSceneInFade:
    ...
    def __doAutoRouting(cls, objectName):
        if objectName == NYObjects.CHALLENGE:
            ...
            elif cls.__nyController.isTokenReceived(GuestsQuestsTokens.TOKEN_CAT) and cls.__nyController.isTokenReceived(GuestsQuestsTokens.TOKEN_DOG) and (checkBreedBuyingAbility() or checkSacksBuyingAbility()):
                objectName = NYObjects.CELEBRITY_D
            else:
