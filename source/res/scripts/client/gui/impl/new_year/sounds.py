...
class NewYearSoundVars(CONST_CONTAINER):
    ...
    RTPC_DOG_TYPE = 'RTPC_ext_newyear_dog_type'
...
class NewYearSoundStates(CONST_CONTAINER):
    ...
    STATE_OVERLAY_HANGAR_GENERAL = 'STATE_overlay_hangar_general'
...
class NewYearSoundEvents(CONST_CONTAINER):
    ...
    ADD_COLOR_FIR = 'hangar_newyear_tree_color_set'
    ...
    REMOVE_COLOR_FIR = 'hangar_newyear_tree_color_reset'
    REMOVE_DOG_EFFECT = 'hangar_newyear_level_post_down'
    ...
    CELEBRITY_SCREEN_REWARD_EXIT = 'hangar_newyear_celeb_screen_reward_exit'
...
class GuestCVideoEvents(CONST_CONTAINER):
    VIDEO_START_GUEST_C = 'gui_lootbox_video_special_guest'
    VIDEO_DONE = 'gui_lootbox_video_stop'
    VIDEO_PAUSE = 'gui_lootbox_video_pause'
    VIDEO_RESUME = 'gui_lootbox_video_resume'


class GuestCVideoStates(CONST_CONTAINER):
...
class NewYearSoundsManager(object):
    ...
    @classmethod
    def playEvent(cls, eventName):
        if cls._nyController.isEnabled():
            WWISE.WW_eventGlobal(eventName)

    @classmethod
    def setRTPC(cls, name, value):
        if cls._nyController.isEnabled():
            WWISE.WW_setRTCPGlobal(name, value)

    @classmethod
    def setState(cls, state, value):
        if cls._nyController.isEnabled():
            WWISE.WW_setState(state, value)

    @classmethod
    def setSwitch(cls, group, switch):
        if cls._nyController.isEnabled():
    ...
    def setEnterViewState(self):
        ...
        if stateValue:
            self.setState(self.__getCustomStateGroup(), stateValue)

    @classmethod
    def setStylesSwitchBox(cls, toySetting):
        switchValue = _STYLES_BOX_SWITCHES_MAP.get(toySetting)
        if switchValue is not None:
            cls.setSwitch(NewYearSoundVars.SWITCH_STYLES_BOX, switchValue)
        return
    ...
    @classmethod
    def setStylesTalismanSwitchBox(cls, talismanType):
        switchValue = _STYLES_TALISMAN_SWITCHES_MAP.get(talismanType)
        if switchValue is not None:
            cls.setSwitch(NewYearSoundVars.SWITCH_STYLES_TALISMAN, switchValue)
        return

    @classmethod
    def setHangarFilteredState(cls, on):
        state = States.HANGAR_FILTERED_ON if on else States.HANGAR_FILTERED_OFF
        cls.setState(StatesGroup.HANGAR_FILTERED, state)

    @classmethod
    def setHangarPlaceGarage(cls):
        cls.setState(NewYearSoundStates.STATE_PLACE, NewYearSoundStates.STATE_PLACE_GARAGE)

    @classmethod
    def setGiftMachineState(cls, isShop):
        state = NewYearSoundStates.HANGAR_VENDOR_SHOP if isShop else NewYearSoundStates.HANGAR_VENDOR_MAIN
        cls.setState(NewYearSoundStates.HANGAR_VENDOR, state)

    @classmethod
    def setGiftMachineAnimState(cls, on):
        state = NewYearSoundStates.HANGAR_VENDOR_ANIM_ON if on else NewYearSoundStates.HANGAR_VENDOR_ANIM_OFF
        cls.setState(NewYearSoundStates.HANGAR_VENDOR_ANIM, state)

    @classmethod
    def setOverlayHangarFilteredState(cls, on):
        if on:
            state = NewYearSoundStates.OVERLAY_HANGAR_FILTERED_ON
        else:
            state = NewYearSoundStates.OVERLAY_HANGAR_FILTERED_OFF
        cls.setState(NewYearSoundStates.OVERLAY_HANGAR_FILTERED, state)

    @classmethod
    def setHangarPlaceFriends(cls):
        cls.setState(NewYearSoundVars.STATE_NEWYEAR_PLACE, NewYearSoundStates.FRIENDS)

    @classmethod
    def setFriendHangarState(cls, on):
        if on:
            cls.setState(NewYearSoundStates.FRIEND_HANGAR, NewYearSoundStates.FRIEND_HANGAR_ENTER)
            cls.playEvent(NewYearSoundEvents.FRIEND_HANGAR_ENTER)
        else:
            cls.playEvent(NewYearSoundEvents.FRIEND_HANGAR_EXIT)
            cls.setState(NewYearSoundStates.FRIEND_HANGAR, NewYearSoundStates.FRIEND_HANGAR_EXIT)

    @classmethod
    def setOverlayState(cls, on):
        if on:
            cls.setState(NewYearSoundStates.STATE_OVERLAY_HANGAR_GENERAL, NewYearSoundStates.STATE_OVERLAY_HANGAR_GENERAL_ON)
        else:
            cls.setState(NewYearSoundStates.STATE_OVERLAY_HANGAR_GENERAL, NewYearSoundStates.STATE_OVERLAY_HANGAR_GENERAL_OFF)

    @classmethod
    def setDogType(cls, dogType):
        cls.setRTPC(NewYearSoundVars.RTPC_DOG_TYPE, dogType)

    def __playEvent(self, eventKey):
        eventName = self.__getValueByKey(eventKey)
        if eventName:
            self.playEvent(eventName)
