...
from gui.impl.new_year.views.album.toy_page_presenter import NYToyPagePresenter, ToyOldPagePresenter
...
class _SubCollectionPresenter(object):
    ...
    def totalRankToys(self, rank):
        return len(self.__toys.get(rank, ()))
    ...
    def currentToys(self):
        ...
        for rank in self.__toys:
            result += self.currentRankToys(rank)

        return result

    def currentRankToys(self, rank):
        result = 0
        if rank in self.__toys:
...
class NY21SubCollectionBuilder(_SubCollectionBuilder):
    YEAR_NAME = Collections.NewYear21
    ORDER = (ToySettings.NEW_YEAR,
     ToySettings.ORIENTAL,
     ToySettings.FAIRYTALE,
     ToySettings.CHRISTMAS,
