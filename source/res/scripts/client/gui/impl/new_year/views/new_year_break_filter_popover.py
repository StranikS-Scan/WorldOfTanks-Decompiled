...
@loggerTarget(logKey=NY_LOG_KEYS.NY_BREAK_FILTER_POPOVER, loggerCls=NYLogger)
class NewYearBreakFilterPopover(PopOverViewImpl):
    ...
    @loggerEntry
    @simpleLog(action=NY_LOG_ACTIONS.NY_BREAK_POPOVER_OPEN)
    def _initialize(self, *args, **kwargs):
        ...
        with self.viewModel.transaction() as model:
            ...
            for idx in xrange(new_year.CONSTS.MAX_COLLECTION_LEVEL):
                level = ButtonModel()
                level.setRawLabel(formatRomanNumber(idx + 1))
    ...
    @simpleLog(action=NY_LOG_ACTIONS.NY_BREAK_POPOVER_LEVELS_CHANGED)
    def __onFilterLevelsChanged(self, _=None):
        self.__onFilterChanged()

    @simpleLog(action=NY_LOG_ACTIONS.NY_BREAK_POPOVER_TYPES_CHANGED)
    def __onFilterTypesChanged(self, _=None):
        self.__onFilterChanged()

    @simpleLog(action=NY_LOG_ACTIONS.NY_BREAK_POPOVER_MEGA_CHANGED)
