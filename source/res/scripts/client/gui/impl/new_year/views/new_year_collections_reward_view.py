...
class NewYearCollectionsRewardView(NewYearHistoryNavigation):
    ...
    __slots__ = ('__yearName', '__collectionName', '__additionalData', '__currentStyle', '__styleTooltipData')
    ...
    def _initialize(self, year=Collections.NewYear20, collection=None):
    ...
    def _getInfoForHistory(self):
        return dict(viewName=self.__yearName, year=self.__yearName, collection=self.__collectionName)

    def __invalidate(self, year, collection):
        self.__yearName = year
        self.__collectionName = collection or YEARS_INFO.getCollectionTypesByYear(year, useMega=False)[0]
    ...
    def __onGoToCollectionButton(self, *_):
        self._goToAlbumView(saveHistory=True, yearName=self.__yearName, stateInfo=(NewYearTabCache.PAGE_STATE, {'typeName': self.__collectionName}))
    ...
    def __onPreviewStyleButton(self, *_):
        ...
        self._resetObject()
        showStylePreview(self.__getVehcilePreviewID(self.__currentStyle), self.__currentStyle, self.__currentStyle.getDescription(), lambda *_: NewYearHistoryNavigation.showMainView(**paramDict), backBtnDescrLabel=backport.text(R.strings.ny.collectionsReward.backLabel()))

    def __getVehcilePreviewID(self, style):
    ...
    def __updateCollectionState(self, model):
        ...
        for collectionID in collectionPresenter.getCollectionSettingsIDs():
