...
from helpers import dependency, int2roman, isPlayerAccount
...
class NewYearDecorationsPopover(NewYearNavigation, PopOverViewImpl):
    ...
    __slots__ = ('__slotID', '__decorationType', '__selectedDecoration', '__newSeenToys', '__slotsForBreak', '__breakToysAnimation')
    ...
    def __updateSlotsList(self):
        ...
        curRank = 0
        if selectedToyID:
            slotModel = PopoverToyPresenter(self.__selectedDecoration).asSlotModel()
            slotModel.setCount(slotModel.getCount() + 1)
            slots.addViewModel(slotModel, isSelected=True)
            curRank = self.__selectedDecoration.getRank()
        for toyDescriptor in toys:
            if toyDescriptor.getID() != selectedToyID:
                slots.addViewModel(PopoverToyPresenter(toyDescriptor, curRank).asSlotModel())
    ...
    def __updateHeader(self, tx, slotItem=None):
        ...
        if slotItem is not None:
            ...
            rank = int2roman(self.__selectedDecoration.getRank())
        else:
