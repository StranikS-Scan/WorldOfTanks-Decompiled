...
class NewYearStyleRewardView(ViewImpl):
    ...
    def __setRewards(self, collectionStrID, rewards):
        ...
        with self.getViewModel().transaction() as model:
            model.setStyleStr('_'.join(getCollectionByIntID(collectionIntID)[::-1]))
    ...
    def __getRewards(cls, bonuses):
        formatter = BonusNameQuestsBonusComposer(_MAX_CAPACITY, getNYAwardsPacker())
...
class NewYearStyleRewardWindow(LobbyOverlay):
    __slots__ = ()

    def __init__(self, *args, **kwargs):
        super(NewYearStyleRewardWindow, self).__init__(decorator=None, content=NewYearStyleRewardView(*args, **kwargs), parent=None)
        return
