...
class NYSelectVehicleForDiscountPopover(VehicleSelectorBase, NYSelectVehiclePopoverMeta):
    ...
    def __initControls(self):
        headers = [packHeaderColumnData('nations', 49, 40, tooltip=DIALOGS.TRADEINPOPOVER_SORTING_NATION, icon=RES_ICONS.MAPS_ICONS_FILTERS_NATIONS_ALL, enabled=True), packHeaderColumnData('type', 45, 40, tooltip=DIALOGS.TRADEINPOPOVER_SORTING_VEHTYPE, icon=RES_ICONS.MAPS_ICONS_FILTERS_TANKS_ALL, enabled=True), packHeaderColumnData('shortUserName', 148, 40, label=DIALOGS.TRADEINPOPOVER_SORTING_VEHNAME_HEADER, tooltip=DIALOGS.TRADEINPOPOVER_SORTING_VEHNAME, enabled=True, verticalTextAlign='center')]
    ...
    def __onSelectVehicle(self, vehicleCD):
        ...
        yield future_async.wg_await(showCelebrityChallengeConfirmDialog(DialogViews.DISCOUNT, **confirmKwargs))
