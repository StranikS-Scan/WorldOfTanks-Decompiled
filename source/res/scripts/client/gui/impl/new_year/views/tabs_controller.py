...
from account_helpers.AccountSettings import AccountSettings, NY_DOG_PAGE_VISITED, NY_NARKET_PLACE_PAGE_VISITED, NY_CAT_PAGE_VISITED, NY_CELEBRITY_DAY_QUESTS_VISITED_MASK, NY_CELEBRITY_ADD_QUESTS_VISITED_MASK, NY_NO_FRIENDS_PAGE_RESET_TIME
...
from new_year.ny_constants import NyWidgetTopMenu, NyTabBarMainView, NyTabBarFriendGladeView, NyTabBarChallengeView, NyTabBarMarketplaceView, Collections, DAYS_BETWEEN_FRIEND_TAB_REMINDER
...
class ChallengeTabsController(NyTabsController):
    ...
    def _updateCelebrityD(self, viewModel, _=False):
        if self.__nyController.isDogTokenReceived():
            sacksCount = self.__nyController.sacksHelper.getSacksCount()
...
class MarketplaceTabsController(NyTabsController):
    ...
    @tabUpdateFunc(NyTabBarMarketplaceView.NY23_CATEGORY)
    def _updateCategory2023(self, viewModel, _=False):
        viewModel.setIsCompleted(isCollectionReceived(Collections.NewYear23))
...
class RewardKitsEntryViewTabsController(NyTabsController):
    ...
    def tabOrderKey(self, tabName):
        return CATEGORIES_GUI_ORDER_NY.index(tabName)

    def setInitState(self, initRewardKitType, initCategory):
        if initRewardKitType in CATEGORIES_GUI_ORDER_NY:
        ...
        elif initCategory in CATEGORIES_GUI_ORDER_NY:
    ...
    def getSelectedBoxCategory(self):
        return CATEGORIES_GUI_ORDER_NY[self._selectedTabIdx]
    ...
    def getInitCategory(self):
        category = None
        for categoryName in CATEGORIES_GUI_ORDER_NY:
