...
class PopOverWindow(WindowImpl):
    ...
    def __init__(self, event, content, parent):
        super(PopOverWindow, self).__init__(wndFlags=WindowFlags.POP_OVER, decorator=WindowView(layoutID=event.decoratorID, flags=ViewFlags.POP_OVER_DECORATOR, viewModelClazz=PopOverWindowModel), content=content, parent=parent, areaID=R.areas.pop_over())
