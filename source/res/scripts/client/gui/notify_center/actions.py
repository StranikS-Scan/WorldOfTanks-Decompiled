...
# Embedded file name: scripts/client/gui/notify_center/actions.py
import BigWorld
...
class OpenExternalBrowser(_OpenBrowser):
    ...
    def invoke(self, notID, actor=None):
        ...
        try:
            BigWorld.openWebBrowser(processedUrl)
        except (AttributeError, TypeError):
            LOG_CURRENT_EXCEPTION()
...
class OpenWindow(_Action):
    ...
    def invoke(self, notID, actor=None):
        g_notifyCenterEvents.onItemShowByAction(notID, self._target)
...
class ReplaceButtons(_Action):
    ...
    def invoke(self, notID, actor=None):
        ...
        g_notifyCenterEvents.onItemUpdatedByAction(notID, actor)
