...
# Embedded file name: scripts/client/gui/notify_center/image_notification_helper.py
import logging
...
def showPaymentMethodLinkNotification(method, imageUrl):
    ...
    SystemMessages.pushMessage(text=backport.text(R.strings.messenger.serviceChannelMessages.sysMsg.titles.paymentMethodLink(), method=method), type=SystemMessages.SM_TYPE.PaymentMethodLinkNotifyCenter, messageData={'imageBlock': _packImageBlock(localPath)})
...
def showPaymentMethodUnlinkNotification(method, imageUrl):
    helper = WebImageHelper()
    localPath = yield helper.getLocalPath(imageUrl)
    SystemMessages.pushMessage(text=backport.text(R.strings.messenger.serviceChannelMessages.sysMsg.titles.paymentMethodUnlink(), method=method), type=SystemMessages.SM_TYPE.PaymentMethodUnlinkNotifyCenter, messageData={'imageBlock': _packImageBlock(localPath)})
