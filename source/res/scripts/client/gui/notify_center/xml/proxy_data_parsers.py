...
# Embedded file name: scripts/client/gui/notify_center/xml/proxy_data_parsers.py
import logging
import json
...
class _ShowInBrowserParser(SectionParser):
    ...
    def parse(self, section, parentSection=None):
        ...
        if not url:
            _logger.error('NotifyCenter show_in_browser item has no URL')
            return
...
class _ClanNotificationParser(SectionParser):
    ...
    def parse(self, section, parentSection=None):
        ...
        _logger.warning('NotifyCenter update_clan_news_counter item has no alias')
...
class _ShowReferralWindowParser(SectionParser):
    ...
    def parse(self, section, parentSection=None):
        ...
        if not relativeUrl:
            _logger.warning('NotifyCenter show_referral_window item has no relative_url')
...
class _PaymentMethodChangeParser(SectionParser):
    ...
    def parse(self, section, parentSection=None):
        ...
        if not method:
            _logger.error('NotifyCenter %s item has no method', self.getTagName())
        cdnUrl = section.readString('imageUrl')
        if not cdnUrl:
            _logger.warning('NotifyCenter %s item has no imageUrl', self.getTagName())
