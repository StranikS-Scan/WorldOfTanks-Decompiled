...
# Embedded file name: scripts/client/gui/prb_control/entities/rts_battles_training/entity.py
import typing
...
from gui.prb_control.entities.rts_battles_training.limits import RtsTrainingLimits
from gui.prb_control.entities.rts_battles_training.requester import RtsTrainingListRequester
...
class RtsTrainingEntryPoint(LegacyEntryPoint):

    def __init__(self):
        super(RtsTrainingEntryPoint, self).__init__(FUNCTIONAL_FLAG.TRAINING)

    def create(self, ctx, callback=None):
        if not isinstance(ctx, RtsTrainingSettingsCtx):
...
class RtsTrainingIntroEntryPoint(LegacyIntroEntryPoint):

    def __init__(self):
        super(RtsTrainingIntroEntryPoint, self).__init__(FUNCTIONAL_FLAG.TRAINING | FUNCTIONAL_FLAG.LOAD_PAGE, PREBATTLE_TYPE.RTS_TRAINING)


class RtsTrainingIntroEntity(LegacyIntroEntity):

    def __init__(self):
        super(RtsTrainingIntroEntity, self).__init__(FUNCTIONAL_FLAG.TRAINING, PREBATTLE_TYPE.RTS_TRAINING, RtsTrainingListRequester())
        self.__watcher = None
        return

    def init(self, clientPrb=None, ctx=None):
        result = super(RtsTrainingIntroEntity, self).init(clientPrb=clientPrb, ctx=ctx)
        g_eventDispatcher.loadRtsTrainingList()
    ...
    def fini(self, clientPrb=None, ctx=None, woEvents=False):
        result = super(RtsTrainingIntroEntity, self).fini(clientPrb=clientPrb, ctx=ctx, woEvents=woEvents)
        if not woEvents:
            aliasToLoad = [PREBATTLE_ALIASES.RTS_TRAINING_LIST_VIEW_PY, PREBATTLE_ALIASES.RTS_TRAINING_ROOM_VIEW_PY]
    ...
    def doSelectAction(self, action):
        ...
        return super(RtsTrainingIntroEntity, self).doSelectAction(action)

    def _createActionsValidator(self):
        return TrainingIntroActionsValidator(self)


class RtsTrainingEntity(LegacyEntity):
    ...
    def __init__(self, settings):
        requests = {REQUEST_TYPE.ASSIGN: self.assign,
         ...
         REQUEST_TYPE.CHANGE_USER_STATUS: self.changeUserCommanderStatus,
         REQUEST_TYPE.KICK: self.kickPlayer,
         REQUEST_TYPE.SEND_INVITE: self.sendInvites}
        super(RtsTrainingEntity, self).__init__(FUNCTIONAL_FLAG.TRAINING, settings, permClass=TrainingPermissions, limits=RtsTrainingLimits(self), requestHandlers=requests)
        self.__settingRecords = []
        self.__watcher = None
        return

    def getQueueType(self):
        return QUEUE_TYPE.RTS

    @legacy_storage_getter(PREBATTLE_TYPE.RTS_TRAINING)
    def storage(self):
        return None

    def init(self, clientPrb=None, ctx=None):
        result = super(RtsTrainingEntity, self).init(clientPrb=clientPrb, ctx=ctx)
        ...
        g_eventDispatcher.addRtsTrainingToCarousel(False)
    ...
    def fini(self, clientPrb=None, ctx=None, woEvents=False):
        result = super(RtsTrainingEntity, self).fini(clientPrb=clientPrb, ctx=ctx, woEvents=woEvents)
    ...
    def resetPlayerState(self):
        super(RtsTrainingEntity, self).resetPlayerState()
        g_eventDispatcher.loadHangar()
    ...
    def doSelectAction(self, action):
        if action.actionName == PREBATTLE_ACTION_NAME.TRAININGS_LIST:
            self.__enterTrainingRoom()
            return SelectResult(True)
        return super(RtsTrainingEntity, self).doSelectAction(action)
    ...
    def prb_onPlayerStateChanged(self, pID, roster):
        if pID == account_helpers.getPlayerID():
            ...
            if playerInfo.isVehicleSpecified():
                self.storage.isCommander = playerInfo.getVehicle().isCommander
        super(RtsTrainingEntity, self).prb_onPlayerStateChanged(pID, roster)
    ...
    def changeUserCommanderStatus(self, ctx, callback=None):
        if ctx.isCommander():
        ...
        if g_currentVehicle.isObserver():
            if not self._processValidationResult(ctx, ValidationResult(False, PREBATTLE_RESTRICTION.VEHICLE_NOT_SUPPORTED)):
                if callback:
                    callback(False)
                return
        super(RtsTrainingEntity, self)._setPlayerReady(ctx, callback)

    def _createActionsValidator(self):
        return TrainingActionsValidator(self)

    def __enterTrainingRoom(self, isInitial=False):
        if self.storage.isCommander:
            self.changeUserCommanderStatus(SetPlayerCommanderStateCtx(True, True, isInitial=isInitial, waitingID='prebattle/change_user_status'), self.__onPlayerReady)
        else:
    ...
    def __onPlayerReady(self, result):
        if result:
            g_eventDispatcher.loadRtsTrainingRoom()
        else:
            g_eventDispatcher.loadHangar()
