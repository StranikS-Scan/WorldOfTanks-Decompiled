...
# Embedded file name: scripts/client/gui/prb_control/entities/rts_battles_training/requester.py
import BigWorld
...
class RtsTrainingListRequester(IPrbListRequester):
    UPDATE_LIST_TIMEOUT = 5

    def __init__(self):
        super(RtsTrainingListRequester, self).__init__()
    ...
    def __request(self):
        ...
        if hasattr(BigWorld.player(), 'requestPrebattles'):
            BigWorld.player().requestPrebattles(PREBATTLE_TYPE.RTS_TRAINING, PREBATTLE_CACHE_KEY.CREATE_TIME, False, 0, 50)
        return
    ...
    def __pe_onPrebattlesListReceived(self, prbType, count, prebattles):
        if prbType != PREBATTLE_TYPE.RTS_TRAINING:
            return
