...
def _highlightsMap(settings, vehicle=None):
    config = (((AUTO_RELOAD_PROP_NAME,), _Highlight(lambda : settings.checkAutoReloadHighlights(increase=True))),
     ...
     ((DUAL_ACCURACY_COOLING_DELAY, SHOT_DISPERSION_ANGLE), _Highlight(lambda : vehicle.descriptor.hasDualAccuracy and not vehicle.descriptor.isDualgunVehicle and settings.checkDualAccuracyHighlights(increase=True))),
     ((AUTOSHOOT_FLAME_CHANGE_SHELL_TIME,
       AUOTSHOOT_FLAME_OVERHEAT_COOLING_TIME,
       RELOAD_TIME_PER_SECOND,
       AVG_DAMAGE_PER_SECOND,
       FLAME_MAX_DISTANCE), _Highlight(lambda : vehicle.descriptor.isAutoShootFlamethrower and settings.checkFlamethrowerHighlights(increase=True))),
     ((THERMAL_VISION_REUSE_AND_DURATION,
       THERMAL_VISION_DISTANCE,
       THERMAL_VISION_RELOAD_TIME,
       THERMAL_VISION_OBSERVE_TIME), _Highlight(lambda : vehicle.descriptor.hasThermalVision and settings.checkThermalVisionHighlights(increase=True))),
     ((SHOT_DISPERSION_ANGLE,
       DUAL_ACCURACY_COOLING_DELAY,
       CLIP_FIRE_RATE,
       RELOAD_TIME_SECS_PROP_NAME,
       DUAL_GUN_CHARGE_TIME), _Highlight(lambda : vehicle.descriptor.hasDualAccuracy and vehicle.descriptor.isDualgunVehicle and settings.checkDualGunDualAccuracyHighlights(increase=True))))
