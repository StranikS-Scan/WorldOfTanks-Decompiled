...
class FortChoiceDivisionWindow(AbstractWindowView, View, FortChoiceDivisionWindowMeta, AppRef):
    ...
    def __makeData(self):
        ...
        data['description'] = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.MAIN_TEXT, i18n.makeString(I18N_FORTIFICATIONS.CHOICEDIVISION_DESCRIPTION))
    ...
    def __makePlayerCount(self, minPlayerCount, maxPlayerCount):
        separator = '-'
        result = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.MAIN_TEXT, str(minPlayerCount) + separator + str(maxPlayerCount))
        return result
    ...
    def __makeDivisionsData(self, list):
        ...
        for item in list:
            ...
            minLevel = 1
            maxLevel = item['level']
            divisionType['divisionName'] = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.HIGH_TITLE, i18n.makeString(I18N_FORTIFICATIONS.CHOICEDIVISION_DIVISIONFULLNAME, divisionType=title))
            divisionType['divisionProfit'] = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.STANDARD_TEXT, i18n.makeString(I18N_FORTIFICATIONS.CHOICEDIVISION_DIVISIONPROFIT, defResCount=profit))
            minLevelStr = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.MAIN_TEXT, fort_formatters.getTextLevel(minLevel))
            maxLevelStr = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.MAIN_TEXT, fort_formatters.getTextLevel(maxLevel))
            divisionType['vehicleLevel'] = self.app.utilsManager.textManager.getText(TEXT_MANAGER_STYLES.STANDARD_TEXT, i18n.makeString(I18N_FORTIFICATIONS.CHOICEDIVISION_VEHICLELEVEL, minLevel=minLevelStr, maxLevel=maxLevelStr))
