...
class BattleSessionWindow(BattleSessionWindowMeta):
    ...
    __SORTINGS_AND_KEYS = [(_R_SORT.byOrder(), PREBATTLE_PLAYERS_SORT_TYPES.OBSERVERS_TO_BOTTOM, PREBATTLE_PLAYERS_SORT_TYPES.REGULAR),
    ...
    def requestToKickPlayer(self, value):
        yield self.prbDispatcher.sendPrbRequest(KickPlayerCtx(value, 'prebattle/kick'))
    ...
    def __setSorting(self):
        data = [ {'name': backport.text(key),
         'id': str(index)} for index, (key, _, _) in enumerate(self.__SORTINGS_AND_KEYS) ]
