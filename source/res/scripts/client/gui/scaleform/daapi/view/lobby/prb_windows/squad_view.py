...
from gui.prb_control.entities.base.unit.ctx import KickPlayerUnitCtx
...
class EventSquadView(SquadView):
    ...
    def excludePlayer(self, index):
        entity = self.prbEntity
        unitMgrID = entity.getID()
        _, unit = entity.getUnit(unitMgrID=unitMgrID)
        member = unit.getMembers().get(index)
        if member is not None:
            self._kickPlayerFromUnit(member['accountDBID'])
        return
...
class _EventHeaderPresenter(_HeaderPresenter):
    ...
    def _getInfoIconTooltipParams(self):
        ...
        tooltipBody = backport.text(R.strings.tooltips.squadWindow.eventVehicle.body(), tankName=', '.join(vehiclesNames))
        return (makeTooltip(header=backport.text(R.strings.tooltips.squadWindow.eventVehicle.header()), body=tooltipBody), TOOLTIPS_CONSTANTS.COMPLEX)
