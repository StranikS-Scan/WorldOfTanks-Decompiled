# Python bytecode 2.7 (decompiled from Python 2.7)
# Embedded file name: scripts/client/gui/Scaleform/genConsts/BATTLE_VIEW_ALIASES.py


class BATTLE_VIEW_ALIASES(object):
    BATTLE_LOADING = 'battleLoading'
    TEAM_BASES_PANEL = 'teamBasesPanel'
    VEHICLE_MESSAGES = 'battleVehicleMessages'
    VEHICLE_ERROR_MESSAGES = 'battleVehicleErrorMessages'
    PLAYER_MESSAGES = 'battlePlayerMessages'
    DAMAGE_PANEL = 'damagePanel'
    BATTLE_STATISTIC_DATA_CONTROLLER = 'battleStatisticDataController'
    FRAG_CORRELATION_BAR = 'fragCorrelationBar'
    BATTLE_TIMER = 'battleTimer'
    PREBATTLE_TIMER = 'prebattleTimer'
    DEBUG_PANEL = 'debugPanel'
    SIXTH_SENSE = 'sixthSense'
    DAMAGE_INFO_PANEL = 'damageInfoPanel'
    BATTLE_MESSENGER = 'battleMessenger'
    CONSUMABLES_PANEL = 'consumablesPanel'
    DAMAGE_INDICATOR = 'damageIndicator'
    PREDICTION_INDICATOR = 'predictionIndicator'
    TIMERS_PANEL = 'timersPanel'
    POINT_OF_INTEREST_NOTIFICATIONS_PANEL = 'pointsOfInterestNotificationPanel'
    STATUS_NOTIFICATIONS_PANEL = 'statusNotificationsPanel'
    DUAL_GUN_PANEL = 'dualGunPanel'
    REPORT_BUG = 'reportBug'
    FULL_STATS = 'fullStats'
    RIBBONS_PANEL = 'ribbonsPanel'
    PERKS_PANEL = 'perksPanel'
    MINIMAP = 'minimap'
    RADIAL_MENU = 'radialMenu'
    PLAYERS_PANEL = 'playersPanel'
    REPAIR_POINT_TIMER = 'repairPointTimer'
    FLAG_NOTIFICATION = 'flagNotification'
    BATTLE_END_WARNING_PANEL = 'battleEndWarningPanel'
    POSTMORTEM_PANEL = 'postmortemPanel'
    BATTLE_DAMAGE_LOG_PANEL = 'battleDamageLogPanel'
    CROSSHAIR_PANEL = 'crosshairPanel'
    MARKERS_2D = 'markers2D'
    HIT_DIRECTION = 'hitDirection'
    SIEGE_MODE_INDICATOR = 'siegeModeIndicator'
    ROCKET_ACCELERATOR_INDICATOR = 'rocketAcceleratorIndicator'
    THERMAL_VISION_INDICATOR = 'thermalVisionIndicator'
    BOOTCAMP_CONSUMABLES_PANEL = 'bootcampConsumablesPanel'
    BOOTCAMP_RIBBONS_PANEL = 'bootcampRibbonsPanel'
    BOOTCAMP_SECONDARY_HINT = 'bootcampSecondaryHint'
    ANIMATED_BATTLE_HINT = 'animatedBattleHint'
    GAME_MESSAGES_PANEL = 'gameMessagesPanel'
    EPIC_RESPAWN_VIEW = 'epicRespawnView'
    EPIC_SCORE_PANEL = 'epicScorePanel'
    EPIC_TIMER = 'epicTimer'
    EPIC_DEPLOYMENT_MAP = 'epicDeploymentMap'
    EPIC_OVERVIEW_MAP_SCREEN = 'epicOverviewMapScreen'
    EPIC_MISSIONS_PANEL = 'epicMissionsPanel'
    EPIC_SPECTATOR_VIEW = 'epicSpectatorView'
    EPIC_REINFORCEMENT_PANEL = 'epicReinforcementPanel'
    EPIC_PROGRESSION_CMP = 'epicProgressionCmp'
    EPIC_BATTLE_QUESTS = 'epicBattleQuests'
    EPIC_BATTLE_QUESTS_TAB = 'epicBattleQuestsTab'
    RECOVERY_PANEL = 'recoveryPanel'
    EPIC_INGAME_RANK = 'epicInGameRank'
    SUPER_PLATOON_PANEL = 'superPlatoonPanel'
    BATTLE_TANK_CAROUSEL = 'battleTankCarousel'
    BATTLE_TANK_CAROUSEL_FILTER_POPOVER = 'battleTankCarouselFilterPopover'
    EPIC_CAROUSEL_FILTER_POPOVER = 'epicCarouselFilterPopoverView'
    QUEST_PROGRESS_TOP_VIEW = 'questProgressTopView'
    HINT_PANEL = 'battleHintPanel'
    HELP_DETAILED = 'helpDetailed'
    PLAYERS_PANEL_EVENT = 'playersPanelEvent'
    BATTLE_HINT = 'battleHint'
    EVENT_POINT_COUNTER = 'eventPointCounter'
    EVENT_TIMER = 'eventTimer'
    EVENT_STATS = 'eventStats'
    EVENT_DESTROY_TIMERS_PANEL = 'eventDestroyTimersPanel'
    EVENT_BUFFS_PANEL = 'eventBuffsPanel'
    EVENT_OBJECTIVES = 'eventObjectives'
    CALLOUT_PANEL = 'calloutPanel'
    BATTLE_NOTIFIER = 'battleNotifier'
    CORRODING_SHOT_INDICATOR = 'corrodingShotIndicator'
    PERSONAL_RESERVES_TAB = 'personalReservesTab'
    EVENT_BUFF_NOTIFICATION_SYSTEM = 'eventBuffNotificationSystem'
    EVENT_FULL_MAP = 'eventFullMap'
    HISTORICAL_BATTLES_BUFF_NOTIFICATION_SYSTEM = 'historicalBattlesBuffNotificationSystem'
    HISTORICAL_BATTLES_FULL_MAP = 'historicalBattlesFullMap'
    HISTORICAL_BATTLES_TIMER = 'historicalBattlesTimer'
    HISTORICAL_BATTLES_PLAYERS_PANEL = 'historicalBattlesPlayersPanel'
    HISTORICAL_BATTLES_PHASE_INDICATOR = 'historicalBattlesPhaseIndicator'
    HISTORICAL_BATTLES_DESTROY_TIMERS_PANEL = 'historicalBattlesDestroyTimersPanel'
    HISTORICAL_BATTLES_STATS_WIDGET = 'historicalBattlesStatsWidget'
    HISTORICAL_BATTLES_BASE_HINT = 'battleBaseHint'
    HISTORICAL_BATTLES_RESPAWN = 'historicalBattlesRespawn'
    HISTORICAL_BATTLES_SPG_PANEL = 'historicalBattlesSPGPanel'
    HISTORICAL_BATTLES_ENEMIES_PANEL = 'historicalBattlesEnemiesPanel'
    EVENT_POSTMORTEM_PANEL = 'eventPostmortemPanel'
    BATTLE_VEHICLE_CONFIGURATOR = 'battleVehicleConfigurator'
    BATTLE_LEVEL_PANEL = 'levelPanel'
    BATTLE_TEAM_PANEL = 'teamPanel'
    BATTLE_VEH_MODULES_CONFIGURATOR_CMP = 'battleVehModulesConfiguratorCmp'
    RADAR_BUTTON = 'radarButton'
    UPGRADE_PANEL = 'upgradePanel'
    BR_SELECT_RESPAWN = 'BRSelectRespawn'
    BR_RESPAWN_MESSAGE_PANEL = 'BRRespawnMessagePanel'
    FRAG_PANEL = 'fragPanel'
    BR_PLAYER_STATS_IN_BATTLE = 'brPlayerStatsInBattle'
    BR_TIMERS_PANEL = 'brTimersPanel'
    ROLE_DESCRIPTION = 'roleDescription'
    PREBATTLE_AMMUNITION_PANEL = 'prebattleAmmunitionPanel'
    EPIC_RESPAWN_AMMUNITION_PANEL = 'epicRespawnAmmunitionPanel'
    BATTLE_ROYALE_WINNER_CONGRATS = 'battleRoyaleWinnerCongrats'
    PREBATTLE_CAROUSEL_VIEW = 'prebattleCarouselView'
    MAPS_TRAINING_GOALS = 'mapsTrainingGoals'
    PIERCING_DEBUG_PANEL = 'piercingDebugPanel'
    MAP_INFO_TIP = 'mapInfoTip'
    INFO_BATTLE_CONTEXT_HINT = 'infoBattleContextHint'
    SIXTH_SENSE_CONTEXT_HINT = 'sixthSenseContextHint'
    COMP7_TANK_CAROUSEL = 'comp7BattleTankCarousel'
    COMP7_TANK_CAROUSEL_FILTER_POPOVER = 'comp7TankCarouselFilterPopover'
    COSMIC_HUD = 'cosmicHud'
