...
class TokenBonusFormatter(SimpleBonusFormatter):
    ...
    def _formatTicket(self, token, bonus):
        ...
        return PreformattedBonus(bonusName=token.id, images=images, label=self._formatBonusLabel(token.count), userName=header, labelFormatter=self._getLabelFormatter(bonus), align=LABEL_ALIGN.RIGHT, isCompensation=self._isCompensation(bonus), specialAlias=TOOLTIPS_CONSTANTS.EVENT_BATTLES_TICKET, isWulfTooltip=True)
    ...
    def _format(self, bonus):
        if bonus.getName() != 'groups':
            ...
            if bonusesCount == bossSize:
                ...
                collectionType = None
            return [] if not collectionType else [PreformattedBonus(label='', images=self._getImages(collectionType), tooltip=self._makeTooltip(collectionType))]
