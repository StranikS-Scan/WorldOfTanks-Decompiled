...
class MainPrizeDiscountTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ...
        return discountPerToken * discountTokenCount if amount > 0 else None
...
class EpicSelectTokensBonus(TokensBonus):
    ...
    def getTooltip(self):
        return createTooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EPIC_BATTLE_INSTRUCTION_TOOLTIP, specialArgs=[self.__getGiftTokenName()])
    ...
    def formattedList(self):
        ...
        for tokenID, tokenVal in self._value.iteritems():
            ...
            if lootBox is not None:
                ...
                wtLootboxesTypes = (WhiteTigerLootBoxes.WT_HUNTER, WhiteTigerLootBoxes.WT_BOSS)
...
class EntitlementBonus(SimpleBonus):
    ...
    def getUserName(cls, entitlementID):
        return backport.text(R.strings.quests.bonusName.entitlements.dyn(entitlementID)()) if cls.hasConfiguredName(entitlementID) else ''
...
def personalMissionsTokensFactory(name, value, isCompensation=False, ctx=None):
    ...
    for tID, tValue in value.iteritems():
        ...
        if tID.startswith(PM3_OFFER_TOKEN_PREFIX):
...
def tokensFactory(name, value, isCompensation=False, ctx=None):
    ...
    for tID, tValue in value.iteritems():
        ...
        if tID.startswith(LOOTBOX_CUSTOMIZATION_PROGRESS_PREFIX):
            result.append(LbC11nProgressTokenBonus({tID: tValue}, isCompensation, ctx))
        if tID == dependency.instance(IWhiteTigerController).getConfig().ticketToken:
            result.append(TicketTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        if tID == dependency.instance(IWhiteTigerController).getConfig().vipTicketToken:
            result.append(TicketTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        if tID == dependency.instance(IWhiteTigerController).getConfig().stamp:
            result.append(StampTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        if tID == dependency.instance(IWhiteTigerController).getConfig().mainPrizeDiscountToken:
...
class WtCustomizationsBonus(CustomizationsBonus):
    ...
    def _format(self, styleSubset):
        ...
        if self._name is not None and item is not None:
            ...
            text = makeHtmlString('html_templates:lobby/quests/bonuses/{}/wtEvent'.format(self._name), itemType, {'name': c11nItem.userName,
             'value': value})
            if text != self._name:
                return text
        return item
...
class CrewBooksBonus(SimpleBonus):
    ...
    def getXP(self):
        if not self._value:
            ...
            return 0 if crewBook is None else crewBook.getXP()
    ...
    def getBookType(self):
        if not self._value:
            ...
            crewBook = self.itemsCache.items.getItemByCD(crewBookParam)
            return crewBook.getBookType() if crewBook is not None else ''
...
class PreferredMapSlotsBonus(IntegralBonus):
    ...
    def getIconBySize(self, size):
        ...
        return super(PreferredMapSlotsBonus, self).getIconBySize(size) if iconName is None else iconName
