...
class WtQuest(Quest):
    ...
    def isAvailable(self):
        return ValidationResult(False, WT_QUEST_UNAVAILABLE_NOT_ENOUGH_TICKETS_REASON) if self.isBossQuest and not self.gameEventController.hasTokensByName('wtevent:boss1') and not self.gameEventController.hasSpecialBoss() else super(WtQuest, self).isAvailable()
...
def getPM3QuestTypeByQuestID(questID):
    ...
    elif divisionResult == 1:
        return PM3QuestLineTypes.KILLS
    elif divisionResult == 2:
        return PM3QuestLineTypes.ASSIST
    elif divisionResult == 3:
