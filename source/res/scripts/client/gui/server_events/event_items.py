...
class CelebrityGroup(Group):
    ...
    def isGroupCompleted(self):
        if self.__needUpdate:
            self.__update()
        return self.bonusQuest.isCompleted() if self.isValid else False

    @property
    def bonusQuest(self):
        if self.__needUpdate:
            self.__update()
        return self.__bonusQuest if self.isValid else None

    @property
    def regularQuests(self):
        if self.__needUpdate:
            self.__update()
        return self.__regularQuests if self.isValid else ()
    ...
    def getActiveQuest(self, tokens):
        ...
        if simplificationToken is None:
            _logger.error('Missing simplification token for Celebrity quests group. groupID: %s.', prefix)
            return
