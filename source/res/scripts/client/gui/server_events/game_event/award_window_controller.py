...
class GameEventAwardWindowController(CallbackDelayer, IGlobalListener):
    ...
    def __showAwardWindow(self):
        ...
        for item in difficultyCtrl.getItems():
            ...
            if level == 1:
                continue
            if item.isCompleted() and not difficultyCtrl.isDifficultyLevelShown(level) and self._gameEventController.isEventActiveAndEnable():
                if self.isBattleResultsOpen:
                    self.delayCallback(self._CALLBACK_SHOW_AWARD_WAIT_TIME, self.__showAwardWindow)
                    return
                from gui.shared.event_dispatcher import showDifficultyUnlockWindow
                showDifficultyUnlockWindow(level)
                return
    ...
    def isBattleResultsOpen(self):
        if self.__app is not None:
            if self.__app.containerManager is not None:
                lobbySubContainer = self.__app.containerManager.getContainer(WindowLayer.FULLSCREEN_WINDOW)
