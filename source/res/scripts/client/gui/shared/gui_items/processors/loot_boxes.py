...
class LootBoxOpenProcessor(Processor):
    ...
    def __init__(self, lootBoxItem, count=1, specificRollNumber=0):
        super(LootBoxOpenProcessor, self).__init__()
        self.__lootBox = lootBoxItem
        self.__count = count
        self.__specificRollNumber = specificRollNumber
    ...
    def _successHandler(self, code, ctx=None):
        mergedBonuses = getMergedBonusesFromDicts(ctx)
        fmt = QuestAchievesFormatter.formatQuestAchieves(mergedBonuses, False)
        if fmt is not None:
    ...
    def _request(self, callback):
        ...
        BigWorld.player().tokens.openLootBox(self.__lootBox.getID(), self.__count, self.__specificRollNumber, lambda code, errStr, ext: self._response(code, callback, ctx=ext, errStr=errStr))
