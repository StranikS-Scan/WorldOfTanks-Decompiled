...
class TankmansLockedValidator(SyncValidator):
    ...
    def _validate(self):
        return makeError('FORBIDDEN') if any((tankmen.ownVehicleHasTags(item.strCD, (VEHICLE_TAGS.CREW_LOCKED,)) for item in self._tankmans)) else makeSuccess()
...
class StallStateValidator(SyncValidator):
    ...
    def _validate(self):
        return makeError('stall_state_disabled') if not self.__lobbyContext.getServerSettings().stallConfig.isEnabled else makeSuccess()
...
class StallPurchaseParamsValidator(SyncValidator):
    ...
    def _validate(self):
        ...
        return makeError('stall_daily_purchase_limit_reached') if purchasedCount >= products[self.__productCode]['dayLimit'] else makeSuccess()
