...
class _ShopSalesEventProcessor(Processor):
    ...
    def _request(self, callback):
        self._getRequester()(lambda resultCode, errStr='', ctx=None: self._response(resultCode, callback, ctx=ctx))
        return
...
class ShopSalesEventReRollProcessor(_ShopSalesEventProcessor):
    ...
    __SYS_MESSAGE_TYPE = {Currency.GOLD: SM_TYPE.ShopSaleRerollWithGold,
     Currency.CREDITS: SM_TYPE.ShopSaleRerollWithCredits}
    ...
    def _makeSuccess(self, code, ctx=None):
        ...
        if rerollWasPaid:
            ...
            kwargs = {'sysMsgKey': 'shop_sales_event/success',
             'money': formatPrice(price, ignoreZeros=True),
             'type': self.__SYS_MESSAGE_TYPE.get(transactionCurrency, Currency.CREDITS)}
        else:
            kwargs = {'sysMsgKey': 'shop_sales_event/success_free'}
        return makeI18nSuccess(auxData=ctx, **kwargs)
