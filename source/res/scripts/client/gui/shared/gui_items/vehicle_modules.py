...
class VehicleGun(VehicleModule):
    ...
    def isAutoReloadableDualGun(self, vehicleDescr=None):
        typeToCheck = GUN_AUTO_RELOAD_DUAL_GUN if vehicleDescr is not None else GUN_CAN_BE_AUTO_RELOAD_DUAL_GUN
        return self.getReloadingType(vehicleDescr) == typeToCheck

    def isClipDualGun(self, vehicleDescr=None):
    ...
    def _getShortInfoKey(self, vehicleDescr=None):
        ...
        if self.isAutoReloadableDualGun(vehicleDescr):
            return '/'.join((key, 'autoReloadDualGun'))
        return '/'.join((key, 'clipDualGun')) if self.isClipDualGun(vehicleDescr) else key
...
class Shell(FittingItem):
    ...
    @property
    def kind(self):
        return self.descriptor.kind + '_DF' if self.descriptor.distanceFactor is not None else self.descriptor.kind
