...
# Embedded file name: scripts/client/gui/shared/tooltips/bob_tooltips.py
...
class BobSelectorTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args):
        ...
        if primeTime is not None:
            todayPeriods = primeTime.getPeriodsBetween(todayStart, min(todayEnd, currentCycleEnd), preferPeriodBounds=True)
            if tomorrowStart < currentCycleEnd:
                tomorrowPeriods = primeTime.getPeriodsBetween(tomorrowStart, min(tomorrowEnd, currentCycleEnd), preferPeriodBounds=True)
    ...
    def _packHeaderBlock(self):
        return formatters.packTitleDescBlock(title=text_styles.highTitle(backport.text(R.strings.bob.selectorTooltip.title())), desc=text_styles.main(backport.text(R.strings.bob.selectorTooltip.desc())))
    ...
    def _packPeriods(self, periods):
        if periods:
            ...
            for periodStart, periodEnd in periods:
                ...
                periodStr = backport.text(R.strings.bob.calendarDay.time(), start=startTime, end=endTime)
                if periodStr not in formattedPeriods:
                    formattedPeriods.append(periodStr)

            return '\n'.join(formattedPeriods)
