...
class ServersInfoTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        serverBlocks = [formatters.packTextBlockData(text_styles.middleTitle(backport.text(R.strings.tooltips.header.menu.server())), padding=formatters.packPadding(0, 0, 4))]
...
def makeRemovalPriceBlock(price, currencySetting, neededValue=None, oldPrice=None, percent=0, valueWidth=-1, leftPadding=61, forcedText='', isDeluxe=False, gap=15, canUseDemountKit=False, isWotPlusEnabled=False, isFreeToDemount=False, isFreeDeluxeEnabled=False, isFreeDemountEnabled=False):
    ...
    wotPlusLabel = text_styles.wotPlusText(backport.text(R.strings.demount_kit.equipmentDemount.wotPlus() if not isWotPlusEnabled else R.strings.demount_kit.equipmentDemount.optionFree()))
...
def makeCompoundPriceBlock(currencySetting, itemPrice, blockWidth=150, forcedText=''):
    ...
    if settings is None:
        ...
        blocks = [formatters.packTextBlockData(text_styles.main(forcedText if forcedText else settings.text)), formatters.packItemPriceBlockData(itemPrice, padding=formatters.packPadding(top=-4))]
...
class VehicleHistoricalReferenceTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks = [formatters.packImageBlockData(img='../maps/icons/flags/160x100/{}.png'.format(item.nationName), align=BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT, padding={'left': -19}), formatters.packTextBlockData(text_styles.highTitle(TOOLTIPS.VEHICLEPREVIEW_HISTORICALREFERENCE_TITLE), padding={'top': -72}), formatters.packTextBlockData(text_styles.main(item.fullDescription), padding={'top': 10})]
