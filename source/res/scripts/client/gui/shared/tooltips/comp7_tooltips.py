...
class RoleSkillLobbyTooltipData(BlocksTooltipData):
    ...
    def __packDescriptionBlock(cls, equipment, roleName, startLevel):
        ...
        if startLevel > 0:
            blocks.append(formatters.packImageTextBlockData(desc=text_styles.main(backport.text(R.strings.tooltips.roleSkill.startLevel.lobby(), startLevel=startLevel)).format(**cls._PARAMS_TEMPLATE.source), descPadding=formatters.packPadding(0, -15, -50), img=backport.image(R.images.gui.maps.icons.comp7.icons.dyn('abilityChargeLvl{}_130x124'.format(startLevel))()), imgPadding=formatters.packPadding(4, -35), padding=formatters.packPadding(-40, 0, -19)))
