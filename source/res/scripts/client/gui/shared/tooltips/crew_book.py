...
class RandomCrewBookTooltipDataBlock(BlocksTooltipData):
    ...
    def _packBlocks(self, itemIntCD, item=None, **kwargs):
        ...
        block = [formatters.packTextBlockData(text=text_styles.highTitle(backport.text(R.strings.tooltips.randomCrewbook.dyn(item.getBookType()).header()))), formatters.packTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.randomCrewbook.body(), exp=item.getXP())))]
        items.append(formatters.packBuildUpBlockData(block))
        return items
