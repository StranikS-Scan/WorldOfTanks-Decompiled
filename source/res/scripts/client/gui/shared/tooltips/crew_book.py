...
class CrewBookRestrictedTooltipDataBlock(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        _, validationMssk, _, crewLists = detachment_customization.validateCrewToLearnCrewBook(crew, vehTypeCD)
...
class BookItemToolTipData(BlocksTooltipData):
    _WIDTH = 320

    def __init__(self, context):
        super(BookItemToolTipData, self).__init__(context, None)
    ...
    def _packBlocks(self, *args, **kwargs):
        bookItem = self.context.buildItem(*args, **kwargs)
        return [formatters.packTitleDescAsComplexBlock(bookItem.userName, bookItem.fullDescription, headerStyle=text_styles.middleTitle)]
