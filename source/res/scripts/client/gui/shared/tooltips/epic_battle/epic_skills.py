...
_SETUP_INFO_SCHEME_IMG = {VEHICLE_CLASS_NAME.LIGHT_TANK: 'default',
 VEHICLE_CLASS_NAME.MEDIUM_TANK: 'mediumTank',
 VEHICLE_CLASS_NAME.HEAVY_TANK: 'heavyTank',
 VEHICLE_CLASS_NAME.AT_SPG: 'AT_SPG',
 VEHICLE_CLASS_NAME.SPG: 'default'}
...
class EpicSkillSlotSetupInfoTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, vehicle):
        ...
        if vehicle is None:
            ...
            items.append(formatters.packImageTextBlockData(title=text_styles.highTitle(backport.text(resourceID.title.dyn(vType.replace('-', '_'))())), desc=text_styles.main(backport.text(resourceID.subtitle())), img=backport.image(R.images.gui.maps.icons.vehicleTypes.large.dyn(vType.replace('-', '_'))()), imgPadding=formatters.packPadding(left=-15, top=-10), txtOffset=56, padding=formatters.packPadding(top=5, bottom=-5)))
            blocks = []
            blocks.append(formatters.packTextBlockData(text=text_styles.middleTitle(backport.text(resourceID.scheme())), padding=formatters.packPadding(left=52, bottom=20)))
            blocks.append(formatters.packImageBlockData(img=backport.image(R.images.gui.maps.icons.epicBattles.tooltips.battleAbility.dyn(_SETUP_INFO_SCHEME_IMG[vType])()), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER))
