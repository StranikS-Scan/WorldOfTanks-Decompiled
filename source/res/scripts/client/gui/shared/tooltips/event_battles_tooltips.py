...
class EventGiftTokenTooltipData(BlocksTooltipData):

    def __init__(self, context):
        super(EventGiftTokenTooltipData, self).__init__(context, TOOLTIP_TYPE.EVENT_GIFT_TOKEN)
        self._setContentMargin(top=20, left=20, bottom=20, right=20)
    ...
    def _packBlocks(self, tokenID, isReceived=False, **kwargs):
        ...
        items = [self.__packImageBlock(shortName), self.__packGiftNameBlocks(shortName)]
        return items

    @staticmethod
    def __packImageBlock(shortName):
        image = R.images.gui.maps.icons.wtevent.tooltips.dyn(shortName)
        if not image.exists():
            image = R.images.gui.maps.icons.wtevent.tooltips.wt_tankmen_gift
        return formatters.packImageBlockData(img=backport.image(image()), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)

    @classmethod
    def __packGiftNameBlocks(cls, shortName):
        rOffer = R.strings.tooltips.eventOffer
        blocks = [formatters.packTextBlockData(text=text_styles.highTitle(backport.text(rOffer.title.dyn(shortName)()))), formatters.packBuildUpBlockData([formatters.packTextBlockData(text=text_styles.standard(backport.text(R.strings.tooltips.eventOffer.footer.dyn(shortName)())))])]
