...
class MarathonEventTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, prefix, *args, **kwargs):
        self._marathonCtrl = self._marathonsCtrl.getMarathon(prefix)
        self.__tooltipData = self._marathonCtrl.getTooltipData()
        self.__iconsData = self._marathonCtrl.getIconsData()
    ...
    def _getHeader(self, _):
        icon, text = self._marathonCtrl.getTooltipHeader()
    ...
    def _getBody(self, state):
        if state == MARATHON_STATE.FINISHED:
            text = text_styles.main(_ms(self.__tooltipData.bodyExtra, day=self._marathonCtrl.getExtraDaysToBuy()))
        else:
