...
class VoteForDiscountTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        maxNumber, available, currentNumber = args
        if available:
            ...
            maximumFreeBlock = formatters.packImageTextBlockData(title=text_styles.neutral(backport.text(R.strings.tooltips.shopSales.voteForDiscount.available.maximumVotes(), max_number=maxNumber - currentNumber)), img=backport.image(R.images.gui.maps.icons.library.attentionIconFilled()), imgPadding=formatters.packPadding(top=2), txtPadding=formatters.packPadding(left=3))
            progressBlock = formatters.packVotesForDiscountBlockData(progress=currentNumber, padding=formatters.packPadding(top=15, bottom=-15))
            return [formatters.packBuildUpBlockData(blocks=[titleBlock], blockWidth=345, padding=formatters.packPadding(bottom=-10)), formatters.packBuildUpBlockData(blocks=[maximumFreeBlock, progressBlock], blockWidth=BLOCK_WIDTH - 20, padding=formatters.packPadding(bottom=-28))]
