...
# Embedded file name: scripts/client/gui/veh_post_porgression/models/progression.py
import typing
...
from gui.veh_post_porgression.models.iterators import OrderedStepsIterator, UnorederdStepsIterator
...
class PostProgressionItem(object):
    ...
    def isActive(self, veh):
        ...
        return self.isExists(settings) and not self.__postProgressionCtrl.isDisabledFor(veh, settings)

    def isAvailable(self, veh):
        result = PostProgressionAvailability.AVAILABLE
        unlocked = self.isUnlocked(veh)
        if not unlocked:
            result = unlocked.reason
        elif veh.isLocked:
    ...
    def isFeatureEnabled(self, featureName):
        settings = self.__postProgressionCtrl.getSettings()
        return settings.isEnabled and featureName in settings.enabledFeatures

    def isRoleSlotAvailable(self, veh):
        hasFeature = self.__state.hasFeature(vehicles.g_cache.postProgression().featureIDs[ROLESLOT_FEATURE])
        return self.isFeatureEnabled(ROLESLOT_FEATURE) and not self.isDisabled(veh) and hasFeature

    def isSetupSwitchAvailable(self, veh, gID):
    ...
    def isUnlocked(self, veh):
        ...
        elif veh.rentalIsOver and veh.intCD in enabledRentedVehicles:
    ...
    def hasPurchasableSteps(self, balance):
