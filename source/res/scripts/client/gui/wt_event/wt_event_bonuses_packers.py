...
from event_common import WT_PROGRESSION_TOKEN_CREW_OFFER, WT_PROGRESSION_TOKEN_CREW_GIFT_OFFER
...
def getWtEventBonusPackerMap():
    ...
    mapping.update({'items': WtItemBonusUIPacker(),
     ...
     SELECTABLE_BONUS_NAME: WTTokenBonusUIPacker()})
    return mapping
...
class WTTokenBonusUIPacker(SelectableBonusPacker):
    ...
    def _packSingleBonus(cls, bonus):
        ...
        for key in bonus.getValue():
            if cls.__hasOffers(key):
                model.setName('wtCommanderClaimable')
                model.setLabel(label if label else '')
                return model

        model.setName('wtCommanderClaimed')
        return model

    @classmethod
    def __hasOffers(cls, token):
    ...
    def _getToolTip(cls, bonus):
        ...
        for tokenID in bonus.getValue().iterkeys():
            tokenID = tokenID.replace(WT_PROGRESSION_TOKEN_CREW_OFFER, WT_PROGRESSION_TOKEN_CREW_GIFT_OFFER)
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EVENT_GIFT_TOKEN, specialArgs=[tokenID]))

        return tooltipData

    @classmethod
    def _getLocalizedBonusName(cls, name, _):
        labelStr = R.strings.event.bonusName.commander
        if labelStr.exists():
...
class WtCustomizationBonusUIPacker(CustomizationBonusUIPacker):
    _boxCtrl = dependency.descriptor(IWTLootBoxesController)
...
class WtLootboxTokenBonusPacker(WtTokenBonusPacker):
    ...
    def _packTokenTooltip(cls, token):
        lootBox = cls._itemsCache.items.tokens.getLootBoxByTokenID(token.id)
        return createTooltipData(tooltip=TOOLTIPS_CONSTANTS.EVENT_LOOTBOX, isWulfTooltip=True, isSpecial=False, specialAlias=None, specialArgs=[lootBox.getType()])
...
class WtTicketTokenBonusPacker(WtTokenBonusPacker):
    ...
    @classmethod
    def _packTokenTooltip(cls, token):
        return createTooltipData(tooltip=TOOLTIPS_CONSTANTS.EVENT_BATTLES_TICKET, isWulfTooltip=True, specialAlias=None, specialArgs=[])
...
class WtStampTokenBonusPacker(WtTokenBonusPacker):
    ...
    @classmethod
    def _packTokenTooltip(cls, token):
        return createTooltipData(tooltip=TOOLTIPS_CONSTANTS.EVENT_STAMP, isWulfTooltip=True, specialAlias=None, specialArgs=[])
