...
from gui.shared.missions.packers.bonus import BonusUIPacker, getDefaultBonusPackersMap, TokenBonusUIPacker, ItemBonusUIPacker, CustomizationBonusUIPacker, VehiclesBonusUIPacker, SimpleBonusUIPacker, BaseBonusUIPacker, GroupsBonusUIPacker
...
class WtCustomizationBonusUIPacker(CustomizationBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, label):
        ...
        model.setTypeIcon(cls._ICONS[cls.__getTypeIcon(customizationItem)])
    ...
    def __getIcon(cls, item):
        itemTypeName = cls.__getTypeIcon(item)
        return '_'.join([itemTypeName, str(item.id)])

    @classmethod
    def __getTypeIcon(cls, item):
...
class WtVehiclesBonusUIPacker(VehiclesBonusUIPacker):
    ...
    def _createUIName(cls, bonus, isRent):
        return 'wt_rental_tank' if isRent else bonus.getName()
