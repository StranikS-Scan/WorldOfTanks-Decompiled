...
def pushReRollSuccessNotification(currency, rollPrice):
    ...
    systemMessages.proto.serviceChannel.pushClientMessage(message=backport.text(R.strings.wt_event.notifications.reroll.body(), price=textPrice), msgType=SCH_CLIENT_MSG_TYPE.REROLL_LOOTBOX, auxData=[SystemMessages.SM_TYPE.WtEventReRoll.name(),
     NotificationPriorityLevel.MEDIUM,
     {'header': backport.text(R.strings.wt_event.notifications.reroll.header())},
     currency + 'Icon'])


def pushReRollFailedNotification():
    SystemMessages.pushMessage(text=backport.text(R.strings.wt_event.notifications.reroll.error()), priority=NotificationPriorityLevel.MEDIUM, type=SystemMessages.SM_TYPE.Error)
