...
# Embedded file name: scripts/client/hangar_music_stage/music_stage_camera.py
import math
import BigWorld
import Math
import math_utils
...
from hangar_music_stage.cam_sound_balancer import CameraSoundBalancerMixin
...
class MusicStageCamera(CameraSoundBalancerMixin):
    hangarSpace = dependency.descriptor(IHangarSpace)

    def __init__(self):
        super(MusicStageCamera, self).__init__()
        self.__params = None
    ...
    def activate(self, targetPos, cameraParams):
        if self.__isActive:
            ...
            if cameraParams.isCentered:
                self._startBalanceSound(initYaw, cameraParams.yawConstraints)
    ...
    def __processMouseEvent(self, dx, dy, dz):
        ...
        self._onCameraAnglesChanged(yaw)
