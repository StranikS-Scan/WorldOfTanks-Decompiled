...
@clientBuffComponent('auraVictimEffect', configs.ClientBuffComponentConfig)
...
@clientBuffComponent('storageDataModifier')
class StorageDataModifierComponent(ClientBuffComponent):

    def __init__(self, name, config, owner):
        super(StorageDataModifierComponent, self).__init__(name, config, owner)
        self._storageID = None
        return

    @enableIfAvatarHasComponent('hwGameplayDataStorage')
    def _apply(self):
        _logger.debug('Apply storage data modifier %s for vehicleID %s', self._config.storageKey, self._owner.id)
        self._storageID = BigWorld.player().hwGameplayDataStorage.addData(self._config.storageKey, self._owner.id, self._config.data)

    @enableIfAvatarHasComponent('hwGameplayDataStorage')
    def _unapply(self):
        _logger.debug('Unapply storage data modifier %s of vehicleID %s', self._config.storageKey, self._owner.id)
        BigWorld.player().hwGameplayDataStorage.delData(self._config.storageKey, self._owner.id, self._storageID)
        self._storageID = None
        return
