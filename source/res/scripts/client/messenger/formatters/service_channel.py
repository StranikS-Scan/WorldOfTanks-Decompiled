...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    __BRResultKeys = {BattleRoyaleResult.DISQUALIFIED: u'battleRoyaleDisqualifiedResult',
     BattleRoyaleResult.LOSE: u'battleRoyaleDefeatResult',
     BattleRoyaleResult.DRAW: u'battleRoyaleDefeatResult',
     BattleRoyaleResult.WIN: u'battleRoyaleVictoryResult'}
    ...
    def _prepareFormatData(self, message):
        ...
        nyBonus = battleResults.get(u'detailedRewards', {}).get(u'nyBattleBonus', {})
        xp = battleResults.get(u'xp')
        if xp:
            ctx[u'xp'] = backport.getIntegralFormat(xp + nyBonus.get(u'xp', 0))
        ...
        accCredits = battleResults.get(Currency.CREDITS) + nyBonus.get(u'credits', 0) + battleResults.get(u'teamSubsBonusCredits', 0) - battleResults.get(u'creditsToDraw', 0)
        if accCredits:
        ...
        ctx[u'goldBankGain'] = self.__makeGoldBankGainString(battleResults.get(u'goldBankGain'))
    ...
    def __makeVehicleLockString(self, vehicleNames, battleResults):
        ...
        for vehIntCD, battleResult in battleResults.get(u'playerVehicles', {}).iteritems():
            ...
            if vehicleName is None:
                continue
            locks.append(g_settings.htmlTemplates.format(u'battleResultLocks', ctx={u'vehicleName': vehicleName.userName,
    ...
    def __makeGoldBankGainString(self, gold_):
        return u'' if not gold_ else g_settings.htmlTemplates.format(u'goldBankGain', ctx={u'gold': self.__makeCurrencyString(Currency.GOLD, gold_)})
    ...
    def __isBRDisqualified(battleResults):
        fairplayViolations = battleResults.get(u'fairplayViolations', None)
...
class CurrencyUpdateFormatter(ServiceChannelFormatter):
    ...
    def _defaultFormatter(self, message, currencyCode, amountDelta, transactionTime, emitterID):
        formatted = g_settings.msgTemplates.format(u'currencyUpdate', ctx={u'title': backport.text(self._EMITTER_ID_TO_TITLE.get(emitterID, self._DEFAULT_TITLE)),
        ...
        return [MessageData(formatted, self._getGuiSettings(message, u'currencyUpdate'))]
    ...
    def _referralProgramFormatter(self, message, currencyCode, amountDelta, transactionTime, emitterID):
        if currencyCode == Currency.GOLD:
            formatted = g_settings.msgTemplates.format(u'ReferralProgramCashbackInvoiceReceived', ctx={u'amount': amountDelta})
            return [MessageData(formatted, self._getGuiSettings(message, u'ReferralProgramCashbackInvoiceReceived'))]
    ...
    def getFormattedAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True):
        ...
        if not asBattleFormatter:
            ...
            if crystal:
                ...
                result.append(cls.__makeQuestsAchieve(u'battleQuestsCrystal', crystal=formatter(crystal)))
            eventCoin = data.get(Currency.EVENT_COIN, 0)
            if eventCoin:
                formatter = getBWFormatter(Currency.EVENT_COIN)
                result.append(cls.__makeQuestsAchieve(u'battleQuestsEventCoin', eventCoin=formatter(eventCoin)))
            equipCoin = data.get(Currency.EQUIP_COIN, 0)
            if equipCoin:
                formatter = getBWFormatter(Currency.EQUIP_COIN)
                result.append(cls.__makeQuestsAchieve(u'battleQuestsEquipCoin', equipCoin=formatter(equipCoin)))
            gold = data.get(Currency.GOLD, 0)
            if gold:
                formatter = getBWFormatter(Currency.GOLD)
                result.append(cls.__makeQuestsAchieve(u'battleQuestsGold', gold=formatter(gold)))
            bpcoin = data.get(Currency.BPCOIN, 0)
            if bpcoin:
                formatter = getBWFormatter(Currency.BPCOIN)
                result.append(cls.__makeQuestsAchieve(u'battleQuestsBpcoin', bpcoin=formatter(bpcoin)))
            vehiclesBattlePassPoints = data.get(Currency.BATTLE_PASS_POINTS, {}).get(u'vehicles')
            if vehiclesBattlePassPoints:
                ...
                if questBattlePassPoints:
                    ...
                    result.append(cls.__makeQuestsAchieve(u'battleQuestsBattlePassPoints', battleQuestsBattlePassPoints=formatter(questBattlePassPoints)))
            nyMandarins = data.get(u'tokens', {}).get(u'ny26_mandarin', {}).get(u'count', 0)
            if nyMandarins:
                fomatter = getBWFormatter(u'nyMandarins')
                result.append(cls.__makeQuestsAchieve(u'battleQuestsNYMandarins', nyMandarins=fomatter(nyMandarins)))
            ...
            for tokenID, tokenData in tokens.iteritems():
                ...
                if tokenID.startswith(constants.LOOTBOX_TOKEN_PREFIX) and intCount > 0:
                    ...
                    if lootBox and lootBox.getType() == NewYearLootBoxes.SURPRISE_COIN:
                        itemsNames.append(makeHtmlString(u'html_templates:lobby/quests/bonuses', u'lootBoxMachineNotif', {u'count': intCount}))
                    elif lootBox:
                ...
                if tokenID == constants.PERSONAL_MISSION_FREE_TOKEN_NAME:
                    itemsNames.append(backport.text(R.strings.messenger.serviceChannelMessages.battleResults.quests.items.name(), name=backport.text(R.strings.seniority_awards.personalMissions.freeSheet.title(), campaignName=backport.text(R.strings.personal_missions.campaignTitle.dyn(u'c_1')())), count=count))
                if tokenID == constants.PERSONAL_MISSION_2_FREE_TOKEN_NAME:
                    itemsNames.append(backport.text(R.strings.messenger.serviceChannelMessages.battleResults.quests.items.name(), name=backport.text(R.strings.seniority_awards.personalMissions.freeSheet.title(), campaignName=backport.text(R.strings.personal_missions.campaignTitle.dyn(u'c_2')())), count=count))
                if tokenID == constants.SENIORITY_AWARDS_VEHICLE_OFFER:
...
class PersonalMissionsQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def _processTokens(cls, data):
        quest = cls.__eventsCache.getPersonalMissions().getAllQuests().get(data.get(u'pmQuestID'))
        if quest:
            result = []
            ...
            offersTokenCount = sum((tokenData.get(u'count', 1) for tokenID, tokenData in data.get(u'tokens', {}).iteritems() if tokenID.startswith(OFFER_TOKEN_PREFIX)))
            if offersTokenCount > 0:
                ...
                offerName = g_settings.htmlTemplates.format(u'offerTokenText', {u'text': text})
                fmt = g_settings.htmlTemplates.format(u'offersAccruedInvoiceReceived', {u'offer': offerName})
...
class BattlePassQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def formatQuestAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True, isBulletsNeed=True):
        cls._SEPARATOR = cls._SEPARATOR if not isBulletsNeed else cls._SEPARATOR + cls._BULLET
        ...
        return cls._BULLET + result if result and isBulletsNeed else result
...
class BattleMattersQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def _processTokens(cls, tokens):
        if cls.isWithSelectableReward(tokens):
            token = tokens.get(u'tokens').keys()[0]
    ...
    def isWithSelectableReward(cls, awardsDict):
        return False if u'tokens' not in awardsDict else any((cls.__battleMattersController.isDelayedRewardToken(v) for v in awardsDict[u'tokens'].iterkeys()))
...
class PrbVehicleMaxFlamethrowerKickFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if vehInvID:
            vehicle = self.__itemsCache.items.getVehicle(vehInvID)
            if vehicle:
                formatted = g_settings.msgTemplates.format(u'prbVehicleMaxFlamethrowerKick', ctx={u'vehName': vehicle.userName})
        return [MessageData(formatted, self._getGuiSettings(message, u'prbVehicleMaxFlamethrowerKick'))]
...
class PersonalMissionFailedFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            data = message.data
            pmQuestsIDs = data.get(u'questIDs')
            if pmQuestsIDs:
                questsDict = {}
...
class AchievementsSMFormatter(ClientSysMessageFormatter):
    ...
    def format(self, message, *args):
        ...
        if messageType is None:
            ...
            formatted = g_settings.msgTemplates.format(template, ctx, data={u'linkageData': self.__getSavedDate(messageType, rank, subRank)})
...
class EarlyAccessCompensationFormatter(ServiceChannelFormatter):
    __COMPENSATION_TEMPLATE = u'EarlyAccessCompensationSysMessage'
    ...
    def format(self, message, *args):
        ...
        if not data or self.__earlyAccessController.getCurrentSeasonID() != 0:
            ...
            compensatedTokensCount = data[u'tokenAmount']
            compensationCredits = data[u'currencies'][Currency.CREDITS]
            formatter = getBWFormatter(Currency.CREDITS)
            formatted = g_settings.msgTemplates.format(self.__COMPENSATION_TEMPLATE, ctx={u'tokens': str(compensatedTokensCount),
             u'credits': formatter(compensationCredits)})
...
class EarlyAccessVehicleObtainFormatter(ServiceChannelFormatter):
    __TEMPLATE = u'EarlyAccessCommonSysMessage'
    __earlyAccessController = dependency.descriptor(IEarlyAccessController)
    ...
    def format(self, message, *args):
        ...
        if not vehicles:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'header': backport.text(R.strings.early_access.serviceChannelMessages.obtainVehicle.title()),
             u'text': text})
...
class EarlyAccessQuestsTokensFormatter(object):
    ...
    @classmethod
    def format(cls, data):
        for questID in data.get(u'completedQuestIDs', []):
...
class PM3CampaignCompletedPerfect(ServiceChannelFormatter):
    __PM3_EARLY_PERFECT_COMPLETE_TEMPLATE = u'PM3EarlyPerfectCompletion'
...
STRING_CONSTANT_CREDITS = u'credits'
STRING_CONSTANT_EQUIPMENT = u'equipments'
STRING_CONSTANT_INSTRUCTION = u'instructions'
STRING_CONSTANT_AMMUNITION = u'ammunitions'
STRING_CONSTANT_APPEARANCE = u'appearances'
STRING_CONSTANT_KIT = u'kits'
STRING_CONSTANT_CREW = u'crews'
...
class ParagonsFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if messageType is None:
            ...
            parameters = message.get(u'parameters')
                ...
                for key, textId in self._auxDataStrings.items():
                    ...
                    if int(value) > 0:
                        if key == STRING_CONSTANT_CREDITS:
                            ...
                            count = u'x{}'.format(value)
                            amounts = text_styles.bpcoin(count)
                        if notificationText:
                            notificationText = notificationText + u'<br/>'
                        notificationText = notificationText + backport.text(textId, amounts=amounts)

                if notificationText:
                    auxData[u'texts'] = u'<br/>'.join((backport.text(R.strings.paragons.branch.reseted.body.second()), notificationText))
                else:
                    auxData[u'texts'] = u''
            else:
                auxData = parameters
    ...
    def _getShowSelectorAndRewards(self, rewards):
        ...
        for bonus in getParagonsBonuses(rewards):
            ...
            if bonus.getName() == u'styleProgress':
                ...
                continue
            if bonus.getName() == u'tmanToken':
                for tankman in bonus.getWrappedBonusList():
                    crewName = tankman.get(u'name')

                continue
            if bonus.getName() == u'vehicleSelector':
                showSelector = True
                continue
            if bonus.getName() == u'paragonsUnlocks':
                showBranch = True
                continue
...
class SeniorityAwardsCompensationExtQuestAchievesSubFormatter(IQuestAchievesSubformatter):
    ...
    def format(cls, data, asBattleFormatter, result, processCustomizations=True, processTokens=True):
        if not asBattleFormatter:
            ...
            compensationBlank1 = data.get(u'meta', {}).get(u'compensation_token:blank_1')
            if compensationBlank1:
                if not isHeaderAppended:
                    result.append(g_settings.htmlTemplates.format(u'SeniorityAwardsCompensationHeader'))
                    isHeaderAppended = True
                campaignKey = 1
                sacoinsCount = compensationBlank1.get(u'bonus', {}).get(u'amount', 1)
                result.append(cls.__makeQuestsAchieve(u'SeniorityAwardsSacoinCompensation', item=cls.__getFreeSheetTitle(campaignKey), sacoins=str(sacoinsCount)))
            compensationBlank2 = data.get(u'meta', {}).get(u'compensation_token:blank_2')
            if compensationBlank2:
                if not isHeaderAppended:
                    result.append(g_settings.htmlTemplates.format(u'SeniorityAwardsCompensationHeader'))
                    isHeaderAppended = True
                campaignKey = 2
                sacoinsCount = compensationBlank2.get(u'bonus', {}).get(u'amount', 1)
                result.append(cls.__makeQuestsAchieve(u'SeniorityAwardsSacoinCompensation', item=cls.__getFreeSheetTitle(campaignKey), sacoins=str(sacoinsCount)))
            compensationOffer = data.get(u'tokens', {}).get(u'compensation_token:offer')
            if compensationOffer:
                if not isHeaderAppended:
                    result.append(g_settings.htmlTemplates.format(u'SeniorityAwardsCompensationHeader'))
                    isHeaderAppended = True
                credits = first(compensationOffer.get(u'extItems', [{}])).get(u'bonus', {}).get(u'amount')
                count = compensationOffer.get(u'count', 1)
                if credits:
                    result.extend([ cls.__makeQuestsAchieve(u'SeniorityAwardsCreditsCompensation', item=backport.text(R.strings.seniority_awards.vehicle_offer.title()), credits=str(credits)) for _ in range(count) ])

    @classmethod
    def __getFreeSheetTitle(cls, campaignID):
        return backport.text(R.strings.seniority_awards.personalMissions.freeSheet.title(), campaignName=backport.text(R.strings.personal_missions.campaignTitle.dyn(u'c_{}'.format(campaignID))()))
