...
class ChannelsCarouselHandler(AppRef):
    ...
    def removeChannel(self, channel):
        ...
        if clientID in self.__handlers:
            ...
            if self.app is not None and self.app.containerManager is not None:
                window = self.app.containerManager.getView(ViewTypes.WINDOW, criteria)
        ...
        if key is None:
            ...
            if 'isShowByReq' in ctx and ctx['isShowByReq'] is True:
                ...
                if isShow:
                    if clientID not in self.__handlers:
                        return
                    criteria, _ = self.__handlers[clientID]
                    window = self.app.containerManager.getView(ViewTypes.WINDOW, criteria)
                    if window is None:
                        if not self.__channelsDP.setItemField(clientID, key, value):
                            self.__showByReqs.pop(clientID)
            else:
                self.__channelsDP.setItemField(clientID, key, value)
                self.__showByReqs.pop(clientID, None)
            return
    ...
    def __handleOpenButtonClick(self, event):
        ...
        if not clientID:
            ...
            window = viewContainer.getView(ViewTypes.WINDOW, criteria)
            if window is not None:
                ...
                self.__channelsDP.setItemFields(clientID, {'isNotified': False,
                 'isInProgress': False})
                openHandler()
            return
