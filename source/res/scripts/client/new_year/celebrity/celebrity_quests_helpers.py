...
from account_helpers.AccountSettings import NY_CELEBRITY_ADV_QUESTS_VISITED_MASK, NY_CELEBRITY_DAY_QUESTS_VISITED_MASK, NY_DOG_PAGE_VISITED, NY_CAT_PAGE_VISITED
...
from items.components.ny_constants import CelebrityQuestTokenParts as QuestHelper, NyCurrency, NyDogBreeds, NySackLootBox, TOKEN_VARIADIC_DISCOUNT_PREFIX
...
from ny_common.settings import GuestsQuestsConsts, NYDogConsts
...
def getRewardCelebrityQuestByFullID(token, eventsCache=None):
    return eventsCache.getQuestByID(QuestHelper.makeRewardsQuestIDFromFullQuestID(token))
...
def getAllRewardsQuests(eventsCache=None):
    return eventsCache.getAllQuests(lambda q: QuestHelper.isRewardQuestID(q.getID(), neededFirstQType=True), includeCelebrityQuests=True)


@dependency.replace_none_kwargs(eventsCache=IEventsCache)
def getAdvancedRewardsQuests(eventsCache=None):
    return eventsCache.getAllQuests(lambda q: QuestHelper.isRewardQuestID(q.getID(), neededQuestType=QuestHelper.ADVANCED, neededFirstQType=True), includeCelebrityQuests=True)
...
def getRerollsCount(itemsCache=None):
    token = itemsCache.items.tokens.getToken(QuestHelper.REROLL_TOKEN)
    if token:
        _, count = token
        return max(count, 0)
...
@dependency.replace_none_kwargs(lobbyContext=ILobbyContext)
def getCelebrityAllQuestCount(lobbyContext=None):
    celebrityConfig = lobbyContext.getServerSettings().getNewYearCelebrityConfig()
    return celebrityConfig.getAllQuestCount()
...
def isUnseenCelebrityQuestsAvailable(celebrityController=None):
    ...
    visitedAdvQuestsMask = AccountSettings.getUIFlag(NY_CELEBRITY_ADV_QUESTS_VISITED_MASK)
    visitedDayQuestsMask |= celebrityController.completedDayQuestsMask
    visitedAdvQuestsMask |= celebrityController.completedAdvQuestsMask
    for token in iterCelebrityActiveQuestsIDs():
        qType, qNum = QuestHelper.getFullQuestOrderInfo(token)
        qNumMask = 1 << qNum - 1
        anyUnseen = qType == QuestHelper.DAY and visitedDayQuestsMask & qNumMask == 0 or qType == QuestHelper.ADVANCED and visitedAdvQuestsMask & qNumMask == 0
        if anyUnseen:
            return True
...
def hasCelebrityBubble(celebrityController=None, nyController=None):
    ...
    return not celebrityController.isChallengeVisited or isUnseenCelebrityQuestsAvailable() or not isAllAvailableDiscountApplied or not isDogVisited or not isCatVisited or sacksCount > 0
...
def getBreedPrice(breedIdx, lobbyCtx=None):
    dogConfig = lobbyCtx.getServerSettings().getNewYearDogConfig()
    price = dogConfig.getBreedPrice(breedIdx)
    return Money(**price) if price else ZERO_MONEY


@dependency.replace_none_kwargs(lobbyCtx=ILobbyContext, itemsCache=IItemsCache)
def getNextBreedIDToBuy(lobbyCtx=None, itemsCache=None):
    dogConfig = lobbyCtx.getServerSettings().getNewYearDogConfig()
    breeds = dogConfig.getBreeds()
    for breed in breeds:
        breedID = breed.get(NYDogConsts.BREED_ID)
        if breedID == NyDogBreeds.BREED_1:
            continue
        toyID, slotID = breed.get(NYDogConsts.BREED_TOY)
        if toyID not in itemsCache.items.festivity.getToys().get(slotID, []):
            return breedID


@dependency.replace_none_kwargs(lobbyCtx=ILobbyContext, itemsCache=IItemsCache)
def getCurrentBreedIndex(lobbyCtx=None, itemsCache=None):
    dogConfig = lobbyCtx.getServerSettings().getNewYearDogConfig()
    breeds = dogConfig.getBreeds()
    for idx, breed in enumerate(breeds):
        toyID, slotID = breed.get(NYDogConsts.BREED_TOY)
        if toyID == itemsCache.items.festivity.getSlotsData()[slotID]:
            return idx


def checkBreedBuyingAbility():
    return bool(getNextBreedIDToBuy())
