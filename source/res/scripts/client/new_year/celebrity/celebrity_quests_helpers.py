...
from items.components.ny_constants import CelebrityQuestTokenParts, NyCurrency, YEARS_INFO
...
def getCelebrityAdditionalTokenQuests():
    return list([ getCelebrityTokenQuestByFullQuestID(t) for t in iterCelebrityActiveQuestsIDs(CelebrityQuestTokenParts.isAddQuestID) ])
...
@dependency.replace_none_kwargs(itemsCache=IItemsCache)
def getRerollTokens(itemsCache=None):
    return [ token for token in itemsCache.items.tokens.getTokens().iterkeys() if CelebrityQuestTokenParts.isRerollToken(token) ]
...
@dependency.replace_none_kwargs(lobbyContext=ILobbyContext)
def getCelebrityAdditionalQuestsConfig(lobbyContext=None):
    celebrityConfig = lobbyContext.getServerSettings().getNewYearCelebrityConfig()
    return celebrityConfig.getAdditionalQuests()
...
def getDogConfig(lobbyCtx=None):
...
@adisp_process
def strokeDog():
    from new_year.ny_processor import StrokeDogProcessor
    yield StrokeDogProcessor().request()


@dependency.replace_none_kwargs(itemsCache=IItemsCache)
def isDogStrokeAvailable(itemsCache=None):
    return getDogConfig().getDailyStrokeCountThreshold() > itemsCache.items.festivity.getDogStrokeCount()
...
def hasCelebrityBubble(celebrityController=None, nyController=None):
    isTokenDogAvailable = nyController.isTokenReceived(GuestsQuestsTokens.TOKEN_DOG)
    isDogVisited = isDogPageVisited() if isTokenDogAvailable else True
    isTokenCatAvailable = nyController.isTokenReceived(GuestsQuestsTokens.TOKEN_CAT)
    isCatVisited = isCatPageVisited() if isTokenCatAvailable else True
    return not celebrityController.isChallengeVisited or isUnseenCelebrityQuestsAvailable() or not isDogVisited or not isCatVisited


@dependency.replace_none_kwargs(itemsCache=IItemsCache)
def _guestsQuestsAvailableDependenciesChecher(dependencies, itemsCache=None):
...
def _getGuestRewardsActionTypeChecker(guestName, tokenType):
    tokenMask = '{}:{}:{}'.format(YEARS_INFO.CURRENT_YEAR_STR, guestName, tokenType)
...
class GuestsQuestsConfigHelper(object):
    ...
    def isQuestAvailable(cls, guestQuest):
        return guestQuest.isQuestAvailable(_guestsQuestsAvailableDependenciesChecher)
    ...
    def _getGuestsActionQuests(cls, guestName, tokenType=''):
        ...
        if quests:
            ...
            return sorted(actionQuests, key=lambda q: q.getQuestID())
        return []
