...
from account_helpers.AccountSettings import AccountSettings, NY_CELEBRITY_ADV_QUESTS_COMPLETED_MASK, NY_CELEBRITY_DAY_QUESTS_COMPLETED_MASK
...
from new_year.celebrity.celebrity_quests_helpers import getCelebrityMarathonQuests, getCelebrityQuestByFullID, getCelebrityAllQuestCount, getCelebrityTokens, iterCelebrityActiveQuestsIDs, getCelebrityQuestCount
...
class CelebritySceneController(ICelebritySceneController):
    ...
    def __init__(self):
        ...
        self.__completedAdvQuestsMask = 0
        self.__questsCount = 0
        self.__allQuestsCount = 0
        self.__completedQuestsCount = 0
        self.__completedAdvQuestsCount = 0
    ...
    def isChallengeCompleted(self):
        return self.completedQuestsCount == self.allQuestsCount
    ...
    @property
    def hasNewCompletedAdvQuests(self):
        completedMask = self.__getCompletedQuestsMask(NY_CELEBRITY_ADV_QUESTS_COMPLETED_MASK)
        return bool(completedMask ^ self.__completedAdvQuestsMask)

    @property
    def advQuestsCount(self):
        return self.allQuestsCount - self.questsCount
    ...
    def completedAdvQuestsMask(self):
        return self.__completedAdvQuestsMask
    ...
    def allQuestsCount(self):
        return self.__allQuestsCount

    @property
    def completedQuestsCount(self):
        if not self.__tokens:
            self.__updateQuests()
        return self.__completedQuestsCount + self.__completedAdvQuestsCount

    @property
    def completedAdvQuestsCount(self):
        return self.__completedAdvQuestsCount

    @property
    def curActiveAdvCount(self):
        return self.completedQuestsCount - self.questsCount + 1
    ...
    def __updateQuests(self):
        ...
        for token in iterCelebrityActiveQuestsIDs():
            ...
            if qType == CelebrityQuestTokenParts.ADVANCED:
                self.__completedAdvQuestsMask |= qNumBit

        self.__questsCount = getCelebrityQuestCount()
        self.__allQuestsCount = getCelebrityAllQuestCount()
        self.__completedQuestsCount = bin(self.completedDayQuestsMask).count('1')
        self.__completedAdvQuestsCount = bin(self.completedAdvQuestsMask).count('1')
