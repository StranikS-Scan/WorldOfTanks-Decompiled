...
class CustomizationCamera(object):
    ...
    def __processMouseEvent(self, dx, dy, dz):
        ...
        pitch = mathUtils.clamp(math.radians(self.__params.pitchConstraints[0]), math.radians(self.__params.pitchConstraints[1]), pitch)
        dist = mathUtils.clamp(self.__params.distConstraints[0], self.__params.distConstraints[1], dist)
        mat = Math.Matrix()
