...
class FriendServiceController(IFriendServiceController, IGlobalListener):
    ...
    def getFriendCollectingCooldownTime(self):
        ...
        cd = cooldown - time_utils.getServerUTCTime()
    ...
    def updateFriendList(self, callback=None):
        ...
        if response.isSuccess():
            ...
            self.__showFriendsUnavailableSystemMessage()
    ...
    def isFriendOnline(self, spaId):
        contact = self.usersStorage.getUser(spaId)
        return contact.isOnline() if contact is not None else False
    ...
    @staticmethod
    def __showFriendsUnavailableSystemMessage():
        SystemMessages.pushMessage(backport.text(R.strings.ny.notification.friendService.disabled()), type=SystemMessages.SM_TYPE.ErrorSimple, priority=NotificationPriorityLevel.HIGH)
