...
_CacheItem = typing.NamedTuple('CacheItem', [('gameObject', typing.Optional[CGF.GameObject]), ('slotComponent', typing.Optional[NySlotComponent]), ('nodes', typing.List[CGF.GameObject])])
...
class LobbyCustomizableObjectsManager(CGF.ComponentManager):
    ...
    def activate(self):
        ...
        self._nyController.onUpdateSlots += self.updateSlots
    ...
    @onAddedQuery(NySlotComponent, CGF.GameObject, tickGroup='postTickUpdate')
    def onCustomizableSlotAdded(self, slotComponent, go):
        ...
        if nodeGameObjects:
            ...
            nodes.append(go)
        if slotComponent.id in self.__slotsCache:
            self.__slotsCache[slotComponent.id].nodes.extend(nodes)
        else:
    ...
    def onCustomizableSlotRemoved(self, slotComponent, go):
        slotInfo = self.__slotsCache.pop(slotComponent.id, None)
        if slotInfo:
            for node in slotInfo.nodes:
                self.__installedToys.pop(node.id, None)

        return

    if not IS_EDITOR:

        @onAddedQuery(CGF.GameObject, AppearanceDelayComponent, tickGroup='postTickUpdate')
    ...
    @onAddedQuery(HangEffectsOwner, CGF.GameObject, tickGroup='postTickUpdate')
    ...
    @onAddedQuery(CGF.GameObject, GraphicsPreferenceComponent, tickGroup='postTickUpdate')
    ...
    @onAddedQuery(NyToyInfoComponent, CGF.GameObject, tickGroup='postTickUpdate')
    ...
    def updateSlots(self, slotsInfoList):
        for slotId, toyId in slotsInfoList:
    ...
    def updateSlot(self, slotId, toyId):
        ...
        if slotInfo is None:
            ...
            for node in slotInfo.nodes:
                nodeId = node.id
                if nodeId in self.__installedToys and self.__installedToys[nodeId] == toyId:
                    continue
                self.__removeToys(slotId, nodeId)
                if toy is not None:
                    self.__loadToys(slotId, nodeId, toy)
                self.__installedToys[nodeId] = toyId
    ...
    def __loadToys(self, slotId, nodeId, toyDesc):
        if toyDesc is None or not toyDesc.hasPrefab():
            _logger.error('Could not find toy "%d" or toy prefab is not specified')
            return
        else:
    ...
    def __removeToys(self, slotId, nodeId=None):
        for nodeGO in self.__slotsCache.get(slotId, _CacheItem(None, None, ())).nodes:
            if nodeId is None or nodeGO.id == nodeId:
