...
from items.components.ny_constants import NY_STATE, TOY_TYPES_BY_OBJECT, CustomizationObjects, CurrentNYConstants, YEARS_INFO, YEARS
from items.new_year import getToyMask
from new_year.ny_constants import NewYearSysMessages, SyncDataKeys, NyWidgetTopMenu, DAYS_BETWEEN_FRIEND_TAB_REMINDER, GuestsQuestsTokens
...
from skeletons.gui.game_control import IHeroTankController, IPlatoonController
...
class NewYearController(INewYearController, IGlobalListener):
    ...
    def __init__(self):
        ...
        self.onUpdateSlots = Event(self.__em)
        self.onBoughtToy = Event(self.__em)
        self.onNyViewVisibilityChange = Event(self.__em)
        self.onCustomizationObjectLevelUp = Event(self.__em)
        self.onSacksMarkerShow = Event(self.__em)
    ...
    def isFinished(self):
        return self.__state == NY_STATE.FINISHED

    def isSuspended(self):
        return self.__state == NY_STATE.SUSPENDED
    ...
    def getAllToysDescrsByType(self, toyType):
    ...
    def checkForNewToys(self):
        for objType in CustomizationObjects.ALL:
            if self.checkForNewToysByType(objType):
                return True
    ...
    def setCustomizationObjectLevelUp(self):
        self.onCustomizationObjectLevelUp()
    ...
    def __onClientUpdated(self, diff, _):
        ...
        if festivityKey in diff:
            ...
            if SyncDataKeys.SLOTS in diff.get(CurrentNYConstants.PDATA_KEY, {}):
                self.onUpdateSlots(list(enumerate(self.requester.getSlotsData())))
