...
class JukeboxController(IJukeboxController):
    ...
    def onAnimatorEvent(self, name):
        self.__setHighlighting(name in JukeboxStateMachineConstants.ENABLE_HIGHLIGHT_EVENTS)
    ...
    def __subscribe(self):
        g_eventBus.addListener(events.NyJukeboxEvent.ON_CLICK_SIDA_A, self.__onClickSideA)
        g_eventBus.addListener(events.NyJukeboxEvent.ON_CLICK_SIDA_B, self.__onClickSideB)

    def __unsubscribe(self):
        g_eventBus.removeListener(events.NyJukeboxEvent.ON_CLICK_SIDA_A, self.__onClickSideA)
        g_eventBus.removeListener(events.NyJukeboxEvent.ON_CLICK_SIDA_B, self.__onClickSideB)

    def __onClickSideA(self, _):
        self.handleJukeboxClick(JukeboxSides.A)

    def __onClickSideB(self, _):
        self.handleJukeboxClick(JukeboxSides.B)

    @staticmethod
    def __setHighlighting(enable):
        if enable:
            g_eventBus.handleEvent(events.NyJukeboxEvent(events.NyJukeboxEvent.ON_HIGHLIGHT_ON), scope=EVENT_BUS_SCOPE.DEFAULT)
        else:
            g_eventBus.handleEvent(events.NyJukeboxEvent(events.NyJukeboxEvent.ON_HIGHLIGHT_OFF), scope=EVENT_BUS_SCOPE.DEFAULT)
