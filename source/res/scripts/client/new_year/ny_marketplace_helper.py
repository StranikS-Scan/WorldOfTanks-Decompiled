...
def isCollectionReceived(yearName):
    ...
    if items:
        for item in items:
            if not isCollectionItemReceived(item, yearName):
                return False
...
def isCollectionItemReceived(item, yearName, itemsCache=None):
    collectionDistributions = itemsCache.items.festivity.getCollectionDistributions()
    prevNYLevel = itemsCache.items.festivity.getPrevNYLevel(yearName)
