...
class NewYearCommandsProcessor(BaseFestivityProcessor):
    ...
    def setVehicleBranch(self, vehicleInvID, level, callback=None):
        self._perform(AccountCommands.CMD_NEW_YEAR_SET_NY_VEHICLE, (vehicleInvID, level, 0), callback)
...
class BuyToyProcessor(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        msg = backport.text(R.strings.ny.systemMessage.infoToy()).format(toyname=toyName, ofcollection=collection, year=year)
...
class BuyCollectionProcessor(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        msg = backport.text(R.strings.ny.systemMessage.infoCollection()).format(ofcollection=collection, year=year)
        return makeSuccess(userMsg=msg, auxData=ctx)
