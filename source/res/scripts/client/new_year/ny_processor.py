...
from gui.impl.lobby.new_year.dialogs.dialogs import showFullscreenConfirmDialog, showResourcesConvertDialog, showReplacementDialog, showDiscountDialog
...
from items.components.ny_constants import CELEBRITY_LOCK_ARENA_BONUS_TYPES, NY_CURRENCY_NAME_TO_IDX, CELEBRITY_LOCK_VEHICLE_MIN_LEVEL, NyCurrency, CelebrityQuestTokenParts, NY_CURRENCY_IDX_TO_NAME, CurrentNYConstants, getObjectByType, TOY_USUAL_TYPES
...
class NewYearCommandsProcessor(BaseFestivityProcessor):
    ...
    def buyDogBreed(self, breedIdx=1, callback=None):
        self._perform(AccountCommands.CMD_NEW_YEAR_BUY_DOG_BREED, (breedIdx,), callback)
...
class ApplyVehicleDiscountProcessor(Processor):
    ...
    def __init__(self, vehicle, discountValue, goodiesID, discountID, **kwargs):
        confirmators = [plugins.AsyncDialogConfirmator(showDiscountDialog, vehicle, discountValue)]
...
class NewYearConvertResourcesProcessor(Processor):
    ...
    def __init__(self, fromResourceType, fromValue, toResourceType, toValue, confirmator=showResourcesConvertDialog):
        confirmators = [plugins.AsyncDialogConfirmator(confirmator, fromResourceType, fromValue, toResourceType, toValue)]
...
class RerollCelebrityQuestProcessor(Processor):
    ...
    _IS_ADDITIONAL_TO_GUEST_MAP = {True: GuestsQuestsTokens.GUEST_A}
    _festivityFactory = dependency.descriptor(IFestivityFactory)

    def __init__(self, questID):
        confirmators = [plugins.AsyncDialogConfirmator(showReplacementDialog, questID)]
        super(RerollCelebrityQuestProcessor, self).__init__(confirmators)
    ...
    def _errorHandler(self, code, errStr='', ctx=None):
        ...
        return makeError(errorStr, msgType=SM_TYPE.ErrorSimple, msgPriority=NotificationPriorityLevel.MEDIUM, auxData={'clientErrKey': clientErrKey})
...
class BuyMarketplaceItemProcessor(_NyBuyProcessor):
    ...
    def __init__(self, item, categoryID, itemID, resourceID, dialog, parent=None):
        ...
        confirmators = [plugins.AsyncDialogConfirmator(showFullscreenConfirmDialog, dialog, parent), CollectionBoughtValidator(item, categoryName)]
...
class _CelebrityQuestLockedByVehicleValidator(SyncValidator):
    ...
    def _validate(self):
        return plugins.makeError('locked_by_vehicle', msgType=SM_TYPE.ErrorSimple) if isCelebrityQuestsRerollLockedByVehicle() else plugins.makeSuccess()
...
class CollectionBoughtValidator(SyncValidator):
    ...
    def _validate(self):
        return plugins.makeError('Collection already bought', msgType=SM_TYPE.ErrorSimple) if isCollectionItemReceived(self._item, self._categoryName) else plugins.makeSuccess()
...
class ResourceTypeValidator(SyncValidator):
    ...
    def _validate(self):
        return plugins.makeSuccess() if self.__resourceID in NyCurrency.ALL else plugins.makeError('invalid resource type', msgType=SM_TYPE.ErrorSimple)
...
class BuyCoinsCountValidator(SyncValidator):
    ...
    def _validate(self):
        return plugins.makeSuccess() if self.__amount > 0 else plugins.makeError('invalid coins count', msgType=SM_TYPE.ErrorSimple)
...
class ResourceEnoughValidator(SyncValidator):
    ...
    def _validate(self, nyController=None):
        ...
        return plugins.makeSuccess() if balance >= self.__price else plugins.makeError('Insufficient amount', msgType=SM_TYPE.ErrorSimple)
...
class BuySackProcessor(_NyBuyProcessor):
    ...
    def __init__(self, level, parent=None):
        confirmators = [SackLevelValidator(level)]
...
class SackLevelValidator(SyncValidator):
    ...
    def _validate(self, nyController=None, itemsCache=None):
        ...
        return plugins.makeError('Invalid sack type', msgType=SM_TYPE.ErrorSimple) if self._level <= currentLevel or self._level > currentLevel + 1 else plugins.makeSuccess()


class BuyDogBreedProcessor(_NyBuyProcessor):
    _festivityFactory = dependency.descriptor(IFestivityFactory)

    def __init__(self, breedIdx, parent=None):
        confirmators = [DogBreedValidator()]
        super(BuyDogBreedProcessor, self).__init__(confirmators)
        self.__breedIdx = breedIdx

    def _request(self, callback):
        self._festivityFactory.getProcessor().buyDogBreed(self.__breedIdx, lambda code, errStr, ext: self._response(code, callback, ctx=ext, errStr=errStr))


class DogBreedValidator(SyncValidator):

    def _validate(self, nyController=None, itemsCache=None):
        from new_year.celebrity.celebrity_quests_helpers import getDogLevel
        currentLevel = getDogLevel()
        return plugins.makeError('Dog missing', msgType=SM_TYPE.ErrorSimple) if currentLevel < 0 else plugins.makeSuccess()
