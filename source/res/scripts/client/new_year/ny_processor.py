...
from gui.impl.lobby.new_year.dialogs.dialogs import showBuyDialog, showFullscreenConfirmDialog, showResourcesConvertDialog, showReplacementDialog, showDiscountDialog
...
class CollectionBoughtValidator(SyncValidator):
    ...
    def _validate(self):
        return plugins.makeError('Collection already bought', msgType=SM_TYPE.ErrorSimple) if isCollectionItemReceived(self._item) else plugins.makeSuccess()
...
class BuyItemValidator(SyncValidator):
    ...
    def _validate(self, nyController=None, itemsCache=None):
        ...
        prevNYLevel = self._itemsCache.items.festivity.getPrevNYLevel()
        priceWithDiscount = self._item.getTotalPrice(collectionDistributions, bonusChecker, prevNYLevel)
        balance = nyController.currencies.getResouceBalance(self._resourceID)
        return plugins.makeError('not_enough_resource', msgType=SM_TYPE.ErrorSimple) if priceWithDiscount > balance else plugins.makeSuccess()
