...
from items.components.ny_constants import CurrentNYConstants, INVALID_TOY_ID, YEARS
...
class NewYearRequester(AbstractSyncDataRequester):
    ...
    def getPrevNYLevel(self, yearName):
        year = YEARS.getYearNumFromYearStr(yearName)
        return self.getCacheValue(SyncDataKeys.PREV_NY_LEVEL, {}).get(year, 0)
        ...
        if SyncDataKeys.INVENTORY_TOYS in nyData:
            ...
            for slotId, toys in nyData[SyncDataKeys.INVENTORY_TOYS].iteritems():
                ...
                for toyId, (totalCount, unseenCount, slotsCount) in toys.iteritems():
                    inventoryToys[slotId][toyId] = NewYearToy(toyId, slotId, totalCount, unseenCount, slotsCount)
...
class FriendNewYearRequester(NewYearRequester):
    ...
    def getToys(self):
        return {slotId:{toyId: NewYearToy(toyId, slotId, 1, 0, 0)} for slotId, toyId in enumerate(self.getSlotsData()) if toyId != INVALID_TOY_ID}
