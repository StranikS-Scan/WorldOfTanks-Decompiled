...
class NewYearRequester(AbstractSyncDataRequester):
    ...
    def getResourceCollecting(self):
        return self.getCacheValue(SyncDataKeys.RESOURCE_COLLECTING, (False, None, None))

    def getAutoCollectingStats(self):
        return self.getCacheValue(SyncDataKeys.AUTO_COLLECTING_STATS, {})

    def getDogStrokeCount(self):
        return self.getCacheValue(SyncDataKeys.STROKE_COUNT, 0)
...
class FriendNewYearRequester(NewYearRequester):
    ...
    def getAutoCollectingStats(self):
        raise SoftException("unexpected call of getAutoCollectingStats in friend's hangar.")

    def getDogStrokeCount(self):
        raise SoftException("unexpected call of getDogStrokeCount in friend's hangar.")
