...
from frameworks.state_machine import StateMachine, State, StateFlags, StringEventTransition, StringEvent, SingleStateObserver
...
from gui.shared.event_dispatcher import showTalismanIntro
...
_VISIBLE_CONTAINERS = (WindowLayer.OVERLAY,
 WindowLayer.CURSOR,
 WindowLayer.TOOLTIP,
 WindowLayer.WAITING,
...
class InitialTalismanStateObserver(SingleStateObserver):
    __slots__ = ()
    _talismanController = dependency.descriptor(ITalismanSceneController)

    def __init__(self):
        super(InitialTalismanStateObserver, self).__init__(_StateID.INITIAL)

    def onEnterState(self, event=None):
        self._talismanController.initialStateChanged()
...
class _HangarTransformState(_TalismanState):
    ...
    def _hideHangarAndSetCamera(self, cameraDescriptor):
        ...
        if machine is None:
            ...
            machine.prepareHangar(dofParams, cameraDescriptor.fov)
...
class _PreviewMainState(_TalismanState):
    ...
    def _onExited(self):
        self.__overlays.release()
        BigWorld.callback(self._app.fadeManager.FADE_IN_ALPHA, makeCallbackWeak(self._notifyPreviewClosed))

    def _notifyPreviewClosed(self):
        if self._talismanController:
            self._talismanController.onPreviewClosed()
...
class _GiftConfirmationState(_TalismanState):
    ...
    def __showDialog(self, talismanName):
        toTree, toyTaken = yield await(showNYTalismanGiftDialog(talismanName))
        if toTree:
            self._postEvent(_EventID.SWITCH_TO_CHRISTMAS_TREE, talismanName=talismanName, toyTaken=toyTaken)
        else:
            SoundRTPCController.updateGiftAvailabilityRTPC()
            self._postEvent(_EventID.GIFT_MOVE_BACK, talismanName=talismanName, toyTaken=toyTaken)


class _BaseMovingBackState(_HangarTransformState):
    __slots__ = ('__talismanName', '__toyTaken')

    def __init__(self, stateID):
        super(_BaseMovingBackState, self).__init__(stateID=stateID)
        self.__talismanName = None
        self.__toyTaken = False
        return

    def _onEntered(self, event):
        self.__talismanName = event.getArgument('talismanName')
        self.__toyTaken = event.getArgument('toyTaken')
        super(_BaseMovingBackState, self)._onEntered(event)
    ...
    def _changeAnimation(self):
        if self._newYearController.isTalismanToyTaken():
            ...
            self._talismanController.talismanToyTaken()
        elif self.__toyTaken:
            self._setGiftAnimation(self.__talismanName, _AnimationState.Gift.SHOW_HANGAR_IDLE)
            self._talismanController.talismanToyTaken()
        else:
            self._setGiftAnimation(self.__talismanName, _AnimationState.Gift.SHOW_GIFT_IDLE)


class _GiftMovingBackState(_BaseMovingBackState):
    __slots__ = ()
...
class _SwitchingToChristmasTreeState(_BaseMovingBackState):
    __slots__ = ()
    ...
    def _onEntered(self, event):
        super(_SwitchingToChristmasTreeState, self)._onEntered(event)
        self.__restore()
...
class TalismanSceneController(ITalismanSceneController):
    ...
    def __init__(self):
        ...
        self.__isGiftIdlePlaying = False
        self.__talismanIntroShown = False
        self.__progressLevel = None
        self.__talismanHoverObservers = set()
        self.__eventsManager = EventManager()
        self.onShowGiftLevelUpEffect = Event(self.__eventsManager)
        self.onTalismanGiftProgressChanged = Event(self.__eventsManager)
        self.onPreviewClosed = Event(self.__eventsManager)
        return
    ...
    def onLobbyInited(self, event):
        ...
        self.__machine.connect(InitialTalismanStateObserver())
        self.__machine.start()
        self.__talismanIntroShown = bool(self.__newYearController.getTalismans(isInInventory=True))
        self.__isGiftTaken = self.__newYearController.isTalismanToyTaken()
        self.__progressLevel = self.__newYearController.getTalismanProgressLevel()
    ...
    def switchToPreview(self):
        ...
        if not settings.get(NewYearStorageKeys.IS_TALISMAN_INTRO_SHOWED, False):
            showTalismanIntro()
            return
        else:
    ...
    def initialStateChanged(self):
        self.__processGiftIdleAnimation()
    ...
    def previewCongratsFinished(self, talismanName):
        self.switchToGift(talismanName)
    ...
    def addTalismanHoverObserver(self, observer):
        self.__talismanHoverObservers.add(observer)

    def removeTalismanHoverObserver(self, observer):
        if observer in self.__talismanHoverObservers:
            self.__talismanHoverObservers.remove(observer)

    def talismanToyTaken(self):
        self.__showUpdatedToysAnimation()

    def getTalismanPosition(self, talismanName, preview=False):
        talismans = self.__previewTalismans if preview else self.__talismans
        entity = talismans.get(talismanName)
        if entity is None:
            _logger.error('Missing Talisman Entity [name=%s; preview=%s]. Failed to get position.', talismanName, preview)
            return
        else:
            matrix = Math.Matrix(entity.matrix)
    ...
    def __clear(self):
        ...
        self.__hangarSpace.onNotifyCursorOver3dScene -= self.__onNotifyCursorOver3dScene
        NewYearNavigation.onObjectStateChanged -= self.__onObjectStateChanged
        self.__talismanHoverObservers.clear()
        if self.__app is not None and self.__app.containerManager is not None:
            self.__app.containerManager.onViewAddedToContainer -= self.__onViewAddedToContainer
        return
    ...
    def __onDataUpdated(self, _):
        ...
        if self.__isGiftTaken != isGiftTaken:
    ...
    def __processGiftIdleAnimation(self):
        needToPlay = not self.__isGiftTaken and self.__machine.isStateEntered(_StateID.INITIAL) and NewYearNavigation.isTalismanObject()
        if self.__isGiftIdlePlaying == needToPlay:
            return
        if needToPlay:
            self.__playGiftIdleAnimations()
        else:
            self.__stopGiftIdleAnimations()
        self.__isGiftIdlePlaying = needToPlay

    def __playGiftIdleAnimations(self):
        for talisman in self.__newYearController.getTalismans(isInInventory=True):
            talismanEntity = self.__talismans.get(talisman.getSetting())
            if talismanEntity is not None:
                talismanEntity.playGiftIdleAnimation()

        return

    def __stopGiftIdleAnimations(self):
        for talisman in self.__newYearController.getTalismans(isInInventory=True):
            talismanEntity = self.__talismans.get(talisman.getSetting())
            if talismanEntity is not None:
                talismanEntity.stopGiftIdleAnimation()

        return

    def __showUpdatedToysAnimation(self):
        progressLevel = self.__newYearController.getTalismanProgressLevel()
        if progressLevel != self.__progressLevel:
            self.__progressLevel = progressLevel
            self.onShowGiftLevelUpEffect(progressLevel)
        self.onTalismanGiftProgressChanged()
    ...
    def __onMouseEnter(self, entity):
        if isinstance(entity, NewYearTalismanBaseObject):
            ...
            for observer in self.__talismanHoverObservers:
                observer.onTalismanMouseOver(entity.talismanName)
    ...
    def __onMouseExit(self, entity):
        if isinstance(entity, NewYearTalismanBaseObject):
            ...
            for observer in self.__talismanHoverObservers:
                observer.onTalismanMouseOut(entity.talismanName)
