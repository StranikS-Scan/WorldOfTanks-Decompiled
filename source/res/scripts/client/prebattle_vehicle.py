...
class PrebattleVehicle(IPrebattleVehicle):
    ...
    def selectAny(self):
        ...
        bosses = self.itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.WT_BOSS, VEHICLE_TAGS.WT_BOSS_2025}))
        hunters = self.itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.WT_HUNTER}))
        ...
        if not self.isSquad():
            for boss in bosses.itervalues():
                bossType = getBossType(boss.descriptor.type.tags)
                if boss.isSpecialBoss:
                    vehicle = boss
                if self.__wtController.hasEnoughTickets(bossType):
                    vehicle = boss

        if not vehicle or not vehicle.isHunterOrBoss:
            vehicle = random.choice(hunters.values())
        elif vehicle.isBoss:
            bossType = getBossType(vehicle.descriptor.type.tags)
            isAnyBossAvailable = self.__wtController.hasEnoughTickets(bossType) or self.__wtController.hasSpecialBoss()
    ...
    def __getSavedEventVehicle(self):
        ...
        if not vehicle or not vehicle.isHunterOrBoss:
            ...
            bossType = getBossType(vehicle.descriptor.type.tags)
            isVehicleSuitable = vehicle.isBoss and self.__wtController.hasEnoughTickets(bossType) or vehicle.isSpecialBoss and self.__wtController.hasSpecialBoss() or not vehicle.isBoss
