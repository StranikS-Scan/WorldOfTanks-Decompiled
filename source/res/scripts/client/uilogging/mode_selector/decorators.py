...
# Embedded file name: scripts/client/uilogging/mode_selector/decorators.py
import logging
from functools import wraps
from uilogging.base.loggers import isUILoggingEnabled
from uilogging.mode_selector.constants import MS_LOG_ACTIONS
...
def logChangeMode(method):
    ...
    def _impl(self, *methodArgs, **methodKwargs):
        ...
        try:
            ...
            available = _loggingEnabled(self) and isinstance(self, _BattleSelectorItems)
            if available:
        ...
        try:
            if selected and available:
                current = selected.getData()
                if previous != current:
                    vo = selected.getVO()
                    self.logStatistic(action=MS_LOG_ACTIONS.CHANGE_MODE, previousMode=previous, currentMode=current, isNewMode=selected.isShowNewIndicator(), isFeaturedMode=bool(vo.get('specialBgIcon')))
        except Exception:
            _logger.exception('Change mode')
