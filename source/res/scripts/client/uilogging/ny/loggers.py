...
from new_year.ny_constants import AnchorNames, NyWidgetTopMenu, AdditionalCameraObject, ANCHOR_TO_OBJECT
...
class NyResourcesLogger(NyLogger):
    ...
    @noexcept
    def logAutoCollectMenuClick(self):
        self.log(action=LogActions.CLICK, item=LogElements.NY23_UILOG_RES_AUTO, info=AdditionalInfo.RESOURCES)

    @noexcept
    def logAutoCollectionClick(self):
        self.log(action=LogActions.CLICK, item=LogElements.NY23_UILOG_RES_AUTO_ON, info=AdditionalInfo.RESOURCES)

    @noexcept
    def logAutoCollectionConfirmedClick(self, state):
        self.log(action=LogActions.CLICK, item=LogElements.NY23_UILOG_RES_CONF_1, itemState=state, info=AdditionalInfo.RESOURCES)
...
class NyDogCustomizationLogger(NyLogger):
    __slots__ = ()

    @noexcept
    def logClick(self, toyID, state):
        self.log(action=LogActions.CLICK, item=LogElements.NY23_UILOG_DOG_CASTOM_CONFIRM, itemState=state + ';toyID:' + str(toyID), info=AdditionalInfo.DOG_AND_CUSTOMIZATION)
...
class NyFlowLogger(FlowLogger):
    ...
    _OBJECT_TO_VIEW = {CustomizationObjects.FIR: Views.TREE,
     ...
     AdditionalCameraObject.RESOURCES: Views.RESOURCES,
     AdditionalCameraObject.CELEBRITY: Views.CELEBRITY,
     AdditionalCameraObject.UNDER_SPACE: ViewAliases.GLADE_VIEW}
