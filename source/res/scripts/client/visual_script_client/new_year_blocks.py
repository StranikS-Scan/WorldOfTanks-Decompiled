...
CGF, Physics, GenericComponents, navigation, hangar_camera_manager, dependency, newYearSkeletons, nyConstants, fade_manager, adisp, ClientUpdateManager, celebrity_quests_helpers, event_dispatcher, events, guiShared, guiSharedUtils, InputHandler, gui_decorators, settings_constants, ny_gui_constants, ny_client_constants, CurrentVehicle = dependencyImporter('CGF', 'Physics', 'GenericComponents', 'gui.impl.new_year.navigation', 'cgf_components.hangar_camera_manager', 'helpers.dependency', 'skeletons.new_year', 'items.components.ny_constants', 'gui.Scaleform.managers.fade_manager', 'adisp', 'gui.ClientUpdateManager', 'new_year.celebrity.celebrity_quests_helpers', 'gui.shared.event_dispatcher', 'gui.shared.events', 'gui.shared', 'skeletons.gui.shared.utils', 'gui.InputHandler', 'gui.app_loader.decorators', 'account_helpers.settings_core.settings_constants', 'gui.impl.gen.view_models.views.lobby.new_year.ny_constants', 'new_year.ny_constants', 'CurrentVehicle')
...
class NavigateTo(Block, NewYearMeta):
    ...
    def _execute(self):
        ...
        countOfSacks = celebrity_quests_helpers.getTotalDogSacksCount()
        isSelfHangar = self.__friendsService.friendHangarSpaId is None
        if name == ny_client_constants.NYObjects.CHALLENGE and countOfSacks > 0 and isSelfHangar:
...
class OnPreviewVehicleOrStyleSelected(Block, NewYearMeta):
...
class IsPreviewVehicleOrStyleSelected(Block, NewYearMeta):
...
class OnCustomizationObjectLevelUp(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)

    def __init__(self, *args, **kwargs):
        super(OnCustomizationObjectLevelUp, self).__init__(*args, **kwargs)
        self._out = self._makeEventOutputSlot('out')
        self._objectName = self._makeDataOutputSlot('objectName', SLOT_TYPE.STR, None)
        return

    def onStartScript(self):
        self.__nyController.onCustomizationObjectLevelUp += self._execute
...
_MEGA_DECORATION_TOKEN_BY_CAMERANAME = {ny_client_constants.MegaDecorationsObjects.FIR: nyConstants.NYFriendServiceDataTokens.HANGAR_DECORATION_FIR,
 ny_client_constants.MegaDecorationsObjects.FAIR: nyConstants.NYFriendServiceDataTokens.HANGAR_DECORATION_FAIR,
 ny_client_constants.MegaDecorationsObjects.INSTALLATION: nyConstants.NYFriendServiceDataTokens.HANGAR_DECORATION_INSTALLATION}

class OnMegaTokenReceived(Block, NewYearMeta):
    __nyController = dependency.descriptor(newYearSkeletons.INewYearController)
    __friendsService = dependency.descriptor(newYearSkeletons.IFriendServiceController)
    __hangarSpace = dependency.descriptor(guiSharedUtils.IHangarSpace)
    ...
    def onStartScript(self):
        ...
        guiShared.g_eventBus.addListener(guiShared.events.NyGladeVisibilityEvent.GLADE_FINALIZE, self.__finalizeGlade, guiShared.EVENT_BUS_SCOPE.LOBBY)
    ...
    def _init(self, *_):
        ...
        cameraManager = CGF.getManager(self.__hangarSpace.spaceID, hangar_camera_manager.HangarCameraManager)
        if cameraManager:
            cameraManager.onCameraSwitched += self._onCameraSwitched
        self._isInitialize.setValue(False)

    def _onCameraSwitched(self, cameraName):
        if cameraName in _MEGA_DECORATION_TOKEN_BY_CAMERANAME:
            thingName = _MEGA_DECORATION_TOKEN_BY_CAMERANAME[cameraName]
            self._isReceived.setValue(True)
            self._name.setValue(thingName.split(':')[-1])
            self._out.call()

    def __finalizeGlade(self, _):
        for decoration in nyConstants.NYFriendServiceDataTokens.HANGAR_DECORATIONS:
            self.__execForToken(decoration)
...
class OnHangToy(Block, NewYearMeta):
    ...
    def onStartScript(self):
        self.__nyController.onUpdateSlots += self._executeList
        guiShared.g_eventBus.addListener(guiShared.events.NyDogEvent.START_PURCHASE, self.unsubscribeUpdateSlots, guiShared.EVENT_BUS_SCOPE.LOBBY)
        guiShared.g_eventBus.addListener(guiShared.events.NyDogEvent.END_PURCHASE, self.subscribeUpdateSlots, guiShared.EVENT_BUS_SCOPE.LOBBY)

    def onFinishScript(self):
        self.__nyController.onUpdateSlots -= self._executeList
        guiShared.g_eventBus.removeListener(guiShared.events.NyDogEvent.START_PURCHASE, self.unsubscribeUpdateSlots, guiShared.EVENT_BUS_SCOPE.LOBBY)
        guiShared.g_eventBus.removeListener(guiShared.events.NyDogEvent.END_PURCHASE, self.subscribeUpdateSlots, guiShared.EVENT_BUS_SCOPE.LOBBY)

    def unsubscribeUpdateSlots(self, _):
        self.__nyController.onUpdateSlots -= self._executeList

    def subscribeUpdateSlots(self, _):
        self.__nyController.onUpdateSlots += self._executeList

    def _executeList(self, slotsInfoList):
        for slotId, toyId in slotsInfoList:
            self._execute(slotId, toyId)
