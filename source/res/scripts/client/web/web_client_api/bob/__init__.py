...
class BobWebApi(W2CSchema):
    ...
    def handleCheckPersonalReward(self, _):
        return self.__bobController.getAvailablePersonalRewardCount()

    @w2c(W2CSchema, name='get_teams')
    def getTeams(self, _):
        return {'error': True} if self.__bobController.teamsRequester.isCacheEmpty() else self.__bobController.teamsRequester.getTeamsAsJson()

    @w2c(W2CSchema, name='get_team_skills')
    def getTeamsSkills(self, _):
        return self.__bobController.teamSkillsRequester.getSkillsAsJson()
    ...
    def __buildServersList(self):
        ...
        for idx, serverData in enumerate(hostsList):
            serverPresenter = BobServerPresenter(self.__bobController, idx, *serverData)
