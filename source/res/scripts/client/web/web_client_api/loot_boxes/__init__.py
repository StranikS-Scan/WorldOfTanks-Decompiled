...
from account_helpers.AccountSettings import IS_CN_LOOT_BOXES_CATEGORY_VISITED
...
class RandomNationalBlueprintWrapper(BonusAggregateWrapper):
    ...
    def getWrappedBonus(cls, bonus):
        return {'id': 0,
         ...
         'value': bonus.get('value', 0),
         'icon': {AWARDS_SIZES.SMALL: sanitizeResPath(backport.image(cls._R_IMAGE.medium.random())),
                  AWARDS_SIZES.BIG: sanitizeResPath(backport.image(cls._R_IMAGE.big.random()))},
         'name': '',
         'description': ''}
...
class LootBoxWebApi(object):
    ...
    def getLootBoxInfo(self, cmd):
        ...
        if lootBox is not None:
            if lootBox.getType() == ChinaLootBoxes.COMMON:
                result = self.__parseBoxInfo(self.__cnLootBoxesCtrl.getCommonBoxInfo())
            elif lootBox.getType() == ChinaLootBoxes.PREMIUM:
                result = self.__parseBoxInfo(self.__cnLootBoxesCtrl.getPremiumBoxInfo())
        return result
    ...
    def __parseBoxInfo(self, boxInfo):
        ...
        if 'slots' in boxInfo:
            for idx, slotData in boxInfo['slots'].iteritems():
                result.update({idx: {'probability': round(slotData.get('probability', 1) * 100, 2),
                       'bonuses': []}})
    ...
    def setLootBoxCategoryVisited(self, _):
        AccountSettings.setSettings(IS_CN_LOOT_BOXES_CATEGORY_VISITED, True)
