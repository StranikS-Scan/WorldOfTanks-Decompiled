...
class CCompilerTestCase(support.EnvironGuard, unittest.TestCase):
    ...
    def test_set_executables(self):
        ...
        self.assertEqual(compiler.compiler, ['env', 'OMPI_MPICC=clang', 'mpicc'])
        compiler.set_executables(compiler_cxx='env OMPI_MPICXX=clang++ mpicxx')
        self.assertEqual(compiler.compiler_cxx, ['env', 'OMPI_MPICXX=clang++', 'mpicxx'])
        compiler.set_executables(linker=u'env OMPI_MPICXX=clang++ mpiCC')
        self.assertEqual(compiler.linker, [u'env', u'OMPI_MPICXX=clang++', u'mpiCC'])
