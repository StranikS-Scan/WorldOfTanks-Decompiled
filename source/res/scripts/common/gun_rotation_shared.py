...
def getLocalAimPoint(vehicleDescriptor):
    ...
    elif vehicleDescriptor.type.localAimPoint is not None:
...
def isOutOfLimits(angle, limits):
    if limits is None:
        return
    elif abs(limits[1] - angle) < _FLT_EPSILON or abs(limits[0] - angle) < _FLT_EPSILON:
        return
    ...
    if minDiff > maxDiff:
        ...
        return limits[0] if minDiff < _DPI - maxDiff else limits[1]
