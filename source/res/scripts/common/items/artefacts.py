...
class SpeedBoost(Equipment):
    __slots__ = ('maxSpeedFactor', 'enginePowerFactor', 'rotationSpeedFactor', 'impulseForce', 'activationDuration', 'impulseModification', 'impulseModificationRange', 'impulseModDistanceMin', 'activationAddRotation')

    def __init__(self):
        super(SpeedBoost, self).__init__()
        self.activationDuration = component_constants.ZERO_INT
        self.maxSpeedFactor = component_constants.ZERO_FLOAT
        self.enginePowerFactor = component_constants.ZERO_FLOAT
        self.rotationSpeedFactor = component_constants.ZERO_FLOAT
        self.impulseForce = Math.Vector3(0.0, 0.0, 0.0)
        self.impulseModification = component_constants.ZERO_FLOAT
        self.impulseModificationRange = component_constants.ZERO_FLOAT
        self.impulseModDistanceMin = component_constants.ZERO_FLOAT
        self.activationAddRotation = Math.Vector3(0.0, 0.0, 0.0)

    def _readConfig(self, xmlCtx, section):
        super(SpeedBoost, self)._readConfig(xmlCtx, section)
        self.activationDuration = _xml.readInt(xmlCtx, section, 'activationDuration', 0)
        self.maxSpeedFactor = _xml.readPositiveFloat(xmlCtx, section, 'maxSpeedFactor')
        self.enginePowerFactor = _xml.readPositiveFloat(xmlCtx, section, 'enginePowerFactor')
        self.rotationSpeedFactor = _xml.readPositiveFloat(xmlCtx, section, 'rotationSpeedFactor')
        self.impulseForce = section.readVector3('impulseForce', Math.Vector3(0.0, 0.0, 0.0))
        self.impulseModification = _xml.readPositiveFloat(xmlCtx, section, 'impulseModification')
        self.impulseModificationRange = _xml.readPositiveFloat(xmlCtx, section, 'impulseModificationRange')
        self.impulseModDistanceMin = _xml.readPositiveFloat(xmlCtx, section, 'impulseModDistanceMin')
        self.activationAddRotation = section.readVector3('activationAddRotation', Math.Vector3(0.0, 0.0, 0.0))
