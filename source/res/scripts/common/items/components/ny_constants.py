...
TOY_TYPES_BY_FRIEND_OBJECT = {FriendCustomizationObjects.FIR: (ToyTypes.TOP,
 ...
 FriendCustomizationObjects.INSTALLATION: (ToyTypes.SCULPTURE,
                                           ...
                                           ToyTypes.PYRO,
                                           ToyTypes.KIOSK)}
DOG_CUSTOMIZATION_SLOT_GROUP = (ToyTypes.DOG_BOWL,
 ToyTypes.DOG_TOY,
 ToyTypes.DOG_COLLAR,
 ToyTypes.DOG_HOUSE)
BREED_GROUP = (ToyTypes.DOG_BREED,)
...
class CelebrityQuestTokenParts(object):
    ...
    def isBattleQuestID(cls, questID):
        ...
        if parts is None:
            ...
            questInfo = parts[-1].split('_')
            return False if len(questInfo) != 2 else questInfo[0] == cls.QUEST
    ...
    def isRewardQuestID(cls, questID, neededQuestType=None, neededFirstQType=False):
        ...
        if parts is None:
            ...
            return questPrefix == cls.PREFIX and (questInfo in cls.QUEST_TYPES if neededQuestType is None else questInfo == neededQuestType) and (qType.startswith(cls.TYPE) if not neededFirstQType else qType == '_'.join((cls.TYPE, '1'))) and questType == cls.REWARD
...
NY_DOG_INDEX_TO_BREED_ID = {key:idx for idx, key in NY_DOG_BREED_ID_TO_INDEX.iteritems()}
