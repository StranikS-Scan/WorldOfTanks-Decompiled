...
from items.readers.ny_readers import readLevelRewards, readSlots, readCollectionRewards, readTalisman
...
class g_cache(object):
    ...
    def init(self, nofail=True):
        ...
        try:
            ...
            cfg['toys'] = ny20Toys
            collections = set((getToyCollectionID(toyDescr=toyDescr) for toyDescr in chain(ny20Toys.itervalues(), ny19Toys.itervalues(), ny18Toys.itervalues())))
            ...
            cfg['slots'] = readSlots(CONSTS.COLLECTION2020_SLOTS_XML_PATH)
            cfg['levels'] = readLevelRewards(CONSTS.COLLECTION2020_LEVEL_REWARDS_XML_PATH)
            cfg['collectionRewardsByCollectionID'] = dict(chain(readCollectionRewards(CONSTS.COLLECTION2020_REWARDS_XML_PATH).iteritems(), readCollectionRewards(CONSTS.COLLECTION2019_REWARDS_XML_PATH).iteritems(), readCollectionRewards(CONSTS.COLLECTION2018_REWARDS_XML_PATH).iteritems()))
            cfg['toyCountByCollectionID'] = _countToysByCollection(chain(ny20Toys.itervalues(), ny19Toys.itervalues(), ny18Toys.itervalues()), collectingFunction=getToyCollectionID)
            cfg['talismans'] = readTalisman(CONSTS.COLLECTION2020_TALISMANS_XML_PATH)
            if IS_BASEAPP:
