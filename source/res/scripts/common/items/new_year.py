...
from items.components.ny_constants import YEARS_INFO, YEARS, MIN_ATMOSPHERE_LVL, MAX_ATMOSPHERE_LVL, LEVEL_REWARD_ID_TEMPLATE, COLLECTION_REWARD_ID_TEMPLATE, ToySettings, TOY_USUAL_TYPES, COLLECTION_SLOTS_XML_PATH, VARIADIC_DISCOUNTS_XML_PATH, TOY_TYPE_IDS_BY_NAME, TOY_TYPES_BY_OBJECT, TOY_OBJECTS_IDS_BY_NAME, DOG_TOY_TYPES, MAX_DOG_TOY_RANK
from wotdecorators import singleton
from ny_common.ny_exception import NYSoftException
from itertools import chain
from items.readers.ny_readers import readSlots, buildVariadicDiscountsCache
if TYPE_CHECKING:
...
def _buildToyGroups(toysDescrs):
    ...
    toysInfos = [(currYearMaxToyRank, ToySettings.NEW, TOY_USUAL_TYPES), (MAX_DOG_TOY_RANK, ToySettings.DOG, DOG_TOY_TYPES)]
...
class g_cache(object):
    ...
    def init(self, nofail=True):
        ...
        try:
            ...
            cfg['levelRewardsByID'] = {level:LEVEL_REWARD_ID_TEMPLATE.format(level) for level in xrange(MIN_ATMOSPHERE_LVL, MAX_ATMOSPHERE_LVL + 1)}
