...
from items.components.ny_constants import TOY_TYPES, OBJECT_MIN_LEVEL, OBJECT_MAX_LEVEL, DEFAULT_OBJECT_UNLOCK_LEVEL
...
def _readSlotSection(section):
    ...
    cfg['unlockLevelID'] = levelID = section.readInt('unlockLevelID', DEFAULT_OBJECT_UNLOCK_LEVEL)
    if not OBJECT_MIN_LEVEL <= levelID <= OBJECT_MAX_LEVEL:
        raise NYSoftException('Invalid level id: {}'.format(levelID))
    return cfg
