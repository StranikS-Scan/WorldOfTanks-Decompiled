...
class Gun(InstallableItem):
    __metaclass__ = ReflectionMetaclass
    __slots__ = ('rotationSpeed', 'reloadTime', 'aimingTime', 'maxAmmo', 'invisibilityFactorAtShot', 'effects', 'burstStartEffects', 'reloadEffect', 'reloadEffectSets', 'impulse', 'recoil', 'animateEmblemSlots', 'shotOffset', 'turretYawLimits', 'pitchLimits', 'staticTurretYaw', 'staticPitch', 'shotDispersionAngle', 'shotDispersionFactors', 'burst', 'clip', 'shots', 'shootImpulses', 'autoreload', 'autoreloadHasBoost', 'drivenJoints', 'customizableVehicleAreas', 'dualGun', 'autoShoot', 'spin', 'edgeByVisualModel', 'prefabs', 'spinEffect', 'temperature', '__weakref__', 'shootImpulses', 'dualAccuracy')
...
class Shell(BasicItem):
    __slots__ = ('caliber', 'isTracer', 'isForceTracer', 'damage', 'damageRandomization', 'piercingPowerRandomization', 'icon', 'iconName', 'isGold', 'type', 'stun', 'effectsIndex', 'tags', 'secondaryAttackReason', 'useAltDamageRandomization', 'dynamicEffectsIndexes', 'hitDeviceChanceMultiplier', 'hitCrewChanceMultiplier', 'maxDistanceInsideVehicle', 'damagedDevicesLimit', 'engineFireFactor', 'distanceDmg', 'distanceFactor', 'skipSelfDamage')
    ...
    def dmgLimits(self):
        if self.distanceDmg is not None:
            ...
            if self.distanceFactor is not None:
                ...
                return (damage * minFactor, damage * maxFactor)
            damage = self.damage[0]
            minDamage = damage
            maxDamage = damage
