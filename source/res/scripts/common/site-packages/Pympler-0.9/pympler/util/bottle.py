...
# Embedded file name: scripts/common/site-packages/Pympler-0.9/pympler/util/bottle.py
...
def _re_flatten(p):
    return p if '(' not in p else re.sub('(\\\\*)(\\(\\?P<[^>]+>|\\((?!\\?))', lambda m: m.group(0) if len(m.group(1)) % 2 else m.group(1) + '(?:', p)
...
class Router(object):
    ...
    def __init__(self, strict=False):
        ...
        self.filters = {'re': lambda conf: (_re_flatten(conf or self.default_pattern), None, None),
...
class BaseRequest(object):
    ...
    def auth(self):
        ...
        if basic:
            ...
            return (ruser, None) if ruser else None
...
class BaseResponse(object):
    ...
    def charset(self, default='UTF-8'):
        return self.content_type.split('charset=')[-1].split(';')[0].strip() if 'charset=' in self.content_type else default
...
class _ImportRedirect(object):
    ...
    def find_module(self, fullname, path=None):
        ...
        return None if packname != self.name else self
...
class FormsDict(MultiDict):
    ...
    def __getattr__(self, name, default=unicode()):
        return super(FormsDict, self).__getattr__(name) if name.startswith('__') and name.endswith('__') else self.getunicode(name, default=default)
    ...
    def run(self, app):
        ...
        class FixedHandler(WSGIRequestHandler):
            ...
            def log_request(*args, **kw):
                return WSGIRequestHandler.log_request(*args, **kw) if not self.quiet else None
...
class StplParser(object):
    ...
    def process_inline(self, chunk):
        return '_str(%s)' % chunk[1:] if chunk[0] == '!' else '_escape(%s)' % chunk
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(result, (dict, DictMixin)):
                ...
                return template(tpl_name, defaults) if result is None else result
