...
# Embedded file name: scripts/common/site-packages/reload.py
import imp
import sys
import traceback
import types
...
def xreload(mod, outputDir, srcDirs):
    r = Reload(mod)
    r.apply(outputDir, srcDirs)
...
class Reload(object):
    ...
    def apply(self, outputDir, srcDirs):
        mod = self.mod
        self._onFinishCallbacks = []
        try:
            ...
            paths = [ path.replace(outputDir, srcDir) for srcDir in srcDirs ]
    ...
    def _updateClass(self, oldclass, newclass):
        ...
        if toSet(oldBases) != toSet(newBases):
            notifyError('Changing the hierarchy of a class is not supported. %s may be inconsistent., %s, %s' % (oldclass, oldBases, newBases))
