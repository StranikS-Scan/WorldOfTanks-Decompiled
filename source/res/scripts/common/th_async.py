...
# Embedded file name: scripts/common/th_async.py
import sys
import weakref
...
def th_async(func=None, executorClass=None):
    if func is None:
        return partial(th_async, executorClass=executorClass)
    else:
...
def th_await(future, timeout=None):
...
def await_callback(func, timeout=None):
    ...
    def wrapper(*args, **kwargs):
        ...
        func(*args, **kwargs)
        return th_await(promise.get_future(), timeout)
...
def delayWhileTickPending(maxTicksToDelay=1, timeout=0.105, minTimeout=0.1, logID=None):
    ...
    for n in xrange(maxTicksToDelay):
        if not BigWorld.isNextTickPending():
            ...
            break
        yield th_await(delay(timeout - decay * n))
    else:
...
def distributeLoopOverTicks(loopIterator, minPerTick=None, maxPerTick=None, logID=None, tickLength=0.1):
    ...
    for _ in loopIterator:
        ...
        if reachedMax or reachedMin and BigWorld.isNextTickPending():
            yield th_await(delay(tickLength))
            countInTick = 0
            delayedCount += 1
