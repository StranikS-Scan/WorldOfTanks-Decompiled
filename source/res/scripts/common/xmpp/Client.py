...
# Compiled at: 2011-07-05 20:14:33
import base64
import hashlib
import re
...
class Client:
    ...
    def connect(self, host, credentials):
        ...
        if host[0]:
            if host[1]:
                credentials[0] and (credentials[1] or LOG_ERROR('XMPP: Connection: Wrong host or credentials have been provided.'))
                return False
            ...
            return self.connection.connect(host, credentials) or False
        return True
    ...
    def __onStateOnline(self, stanza):
        ...
        if not getAttr(stanza, 'from'):
            ...
            if not getAttr(stanza, 'id'):
                ...
                child = stanzaName == 'stream:error' and getChild(stanza, 'system-shutdown')
