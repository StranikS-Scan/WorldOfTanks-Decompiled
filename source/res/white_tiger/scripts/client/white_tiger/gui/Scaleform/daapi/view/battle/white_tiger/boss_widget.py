...
class WhiteTigerBossWidget(WTBossWidgetMeta):
    ...
    def setupBossInfo(self, vInfo):
        ...
        bossType = WTVehicleType.BOSS.value
        if VEHICLE_TAGS.WT_SPECIAL_BOSS in vInfo.vehicleType.tags:
            bossType = WTVehicleType.BOSS_SPECIAL.value
            self.as_setPlasmaBonusS(0)
        elif VEHICLE_TAGS.WT_BOSS_2025 in vInfo.vehicleType.tags:
            bossType = WTVehicleType.BOSS_2025.value
            self.as_setPlasmaBonusS(0)
    ...
    def __onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        if eventID == FEEDBACK_EVENT_ID.VEHICLE_HEALTH:
            ...
            newHealth = value[0]
            bossType = getBossType(vInfoVO.vehicleType.tags)
