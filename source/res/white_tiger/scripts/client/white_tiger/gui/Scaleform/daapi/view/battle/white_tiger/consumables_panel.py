...
class WhiteTigerConsumablesPanel(WTConsumablesPanelMeta):
    ...
    _BOSS_EQUIPMENT_TO_INDEX = {'builtinInstantStunShoot_wt': _EQUIPMENT_START_IDX,
     'builtinStunArea_wt': _EQUIPMENT_START_IDX,
     'builtinImpulse_wt': _EQUIPMENT_START_IDX + 1,
     'builtinPlasmaExtractor_wt': _EQUIPMENT_START_IDX + 1,
     'builtinTeleport_wt': _EQUIPMENT_START_IDX + 2,
     'builtinTeleport_wt_2025': _EQUIPMENT_START_IDX + 2,
     'builtinHyperion_wt': _EQUIPMENT_START_IDX + 3,
     'builtinHyperion_wt_2025': _EQUIPMENT_START_IDX + 3}
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        if 'UnionStrength' in item.getDescriptor().name:
            body = ''
        else:
    ...
    def __getAdditionalTooltipBodyString(self, item):
        ...
        for atr in attributes:
            if 'StunArea' in item.getDescriptor().name and atr == 'onUse':
                effectDuration = item.getDescriptor().effectDuration
                effectRadius = item.getDescriptor().effectRadius
            ...
            if strText and strText != emptyStr:
                if 'UnionStrength' in item.getDescriptor().name:
                    ...
                    resultStr = backport.text(getattr(attribs, atr)(), damageIncrease=int(receiveDamageFactor * 100), percent=backport.text(R.strings.common.common.percent()), time=int(effectDuration))
                else:
    ...
    def _updateEquipmentSlot(self, idx, item):
        ...
        if self._currentStage == EQUIPMENT_STAGES.COOLDOWN:
            if item.getDescriptor().name in ('builtinHyperion_wt', 'builtinHyperion_wt_2025'):
    ...
    def _updateActivatedSlot(self, idx, item):
        if item.getDescriptor().name == 'builtinUnionStrength_wt':
            return
