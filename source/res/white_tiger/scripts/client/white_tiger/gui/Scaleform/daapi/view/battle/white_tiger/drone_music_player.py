...
# Embedded file name: white_tiger/scripts/client/white_tiger/gui/Scaleform/daapi/view/battle/white_tiger/drone_music_player.py
import BigWorld
...
class _HunterLifeCounter(_Condition):
    ...
    def updateTeamHealth(self, *_):
        ...
        if teamLivesComponent:
            lives = [ entry.lives for entry in teamLivesComponent.teamLives ]
            totalLives = sum(lives)
            if totalLives <= self.criticalValue:
...
class WhiteTigerDroneMusicPlayer(DroneMusicPlayer):
