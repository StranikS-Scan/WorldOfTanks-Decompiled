...
class WhiteTigerBattlePage(ClassicPage, ISpawnListener):
    ...
    __OPPONENT_SWITCH_VALUES = {WT_TAGS.WT_BOSS: 'SWITCH_ext_WT_vo_opponent_of_hunters_krieger',
     WT_TAGS.WT_BOSS_2025: 'SWITCH_ext_WT_vo_opponent_of_hunters_lemarten'}
    ...
    def _onBattleLoadingStart(self):
        ...
        isHunter = VEHICLE_TAGS.WT_HUNTER in info.vehicleType.tags
    ...
    def __isBoss(self):
        ...
        return getBossType(vInfo.vehicleType.tags) is not None

    def __setOpponentSwitch(self):
        vInfo = self.sessionProvider.getArenaDP().getVehiclesInfoIterator()
        for vehicle in vInfo:
            bossType = getBossType(vehicle.vehicleType.tags)
            if bossType:
                WWISE.WW_setSwitch(self.__OPPONENT_SWITCH, self.__OPPONENT_SWITCH_VALUES[bossType])
                return
