...
class WhiteTigerVehicleMarkerPlugin(RespawnableVehicleMarkerPlugin):
    ...
    def _onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        ...
        if eventID == FEEDBACK_EVENT_ID.VEHICLE_SHOW_MESSAGE:
            ...
            self._updateStatusMarkerState(vehicleID, isShown, handle, BATTLE_MARKER_STATES.WT_UNION_STRENGTH_STATE, numberOfMarks, True, False)
        elif eventID == FEEDBACK_EVENT_ID.WT_VEHICLE_STUN_AREA_DEBUFF:
            handle = self._markers[vehicleID].getMarkerID()
            isShown = value['isShown']
            duration = value.get('duration', 0) if isShown else 0
            self._updateMarkerTimer(vehicleID, handle, duration, BATTLE_MARKER_STATES.WT_STUN_AREA_STATE, True, True)
    ...
    def _setVehicleInfo(self, marker, vInfo, guiProps, nameParts):
        ...
        if WT_TAGS.WT_BOSS in vType.tags or WT_TAGS.WT_BOSS_2025 in vType.tags:
            classTag = 'boss'
        canShowPlasma = not isBossTeam(vInfo.team) or 'builtinPlasmaExtractor_wt_user' in vInfo.vehicleType.tags
