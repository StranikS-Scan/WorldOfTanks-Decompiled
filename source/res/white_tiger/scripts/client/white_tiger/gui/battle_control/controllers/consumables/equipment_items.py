...
from gui.battle_control.controllers.consumables.equipment_ctrl import _ActivationError, InCooldownError, _TriggerItem, _AfterburningItem, _RepairKitItem, _MedKitItem, _EQUIPMENT_TAG_TO_ITEM
...
class _Teleport2025Item(_TeleportItem):
    pass
...
class _Hyperion2025Item(_HyperionItem):
    pass


class _WTUnionStrength(_TriggerItem):
    ...
    def getTags(self):
        return (BATTLE_CONSUMABLES_PANEL_TAGS.EVENT_PASSIVE_ITEM,)


class _WTRepairKit(_RepairKitItem):

    def canActivate(self, entityName=None, avatar=None):
        if self._stage and self._stage == EQUIPMENT_STAGES.UNAVAILABLE:
            result = False
            error = WtHunterAbilitiesDisabled(self._descriptor.userString)
            return (result, error)
        return super(_WTRepairKit, self).canActivate(entityName, avatar)


class _WTMedKitItem(_MedKitItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        if self._stage and self._stage == EQUIPMENT_STAGES.UNAVAILABLE:
            ...
            error = WtHunterAbilitiesDisabled(self._descriptor.userString)
            return (result, error)
        return super(_WTMedKitItem, self).canActivate(entityName, avatar)
...
def _eventItemFactory(descriptor, quantity, stage, timeRemaining, totalTime, tag=None):
    ...
    if descriptor.name.lower().endswith('teleport_wt_2025'):
        return _Teleport2025Item(descriptor, quantity, stage, timeRemaining, totalTime, tag)
    ...
    if descriptor.name.lower().endswith('hyperion_wt_2025'):
        return _Hyperion2025Item(descriptor, quantity, stage, timeRemaining, totalTime, tag)
    return _WTUnionStrength(descriptor, quantity, stage, timeRemaining, totalTime, tag) if descriptor.name.lower().endswith('unionstrength_wt') else _EventItem(descriptor, quantity, stage, timeRemaining, totalTime, tag)
...
def _wtEquipmentItemFactory(descriptor, quantity, stage, timeRemaining, totalTime, tag=None):
    if descriptor.name.lower().endswith('repairkit_wt'):
        return _WTRepairKit(descriptor, quantity, stage, timeRemaining, totalTime, tag)
    return _WTMedKitItem(descriptor, quantity, stage, timeRemaining, totalTime, tag) if descriptor.name.lower().endswith('medkit_wt') else None
...
def registerTagsToEventItemMapping():
    ...
    _EQUIPMENT_TAG_TO_ITEM.update({(BATTLE_CONSUMABLES_PANEL_TAGS.WT_EQUIPMENT_ITEM,): _wtEquipmentItemFactory})
