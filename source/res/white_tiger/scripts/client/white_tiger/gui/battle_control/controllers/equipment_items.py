...
class _WTItem(_TriggerItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        def isInCooldownStage():
            return self._timeRemaining > 0 and self._stage in (EQUIPMENT_STAGES.DEPLOYING, EQUIPMENT_STAGES.COOLDOWN, EQUIPMENT_STAGES.SHARED_COOLDOWN)

        def isUnavailable():
            return self._stage in (EQUIPMENT_STAGES.STARTUP_COOLDOWN, EQUIPMENT_STAGES.UNAVAILABLE)
        ...
        if self._isLocked:
            ...
            return (result, error)
        elif isInCooldownStage() or isUnavailable() or isTeleportationInProgress():
            result = False
