...
# Embedded file name: white_tiger/scripts/client/white_tiger/gui/battle_results/presenter/event.py
import typing
...
class _WtWidgetTicketTokenBonusPacker(WtTicketTokenBonusPacker):
    ...
    def _packToken(cls, token, model):
        ...
        model = typing.cast(WidgetModel, model)
        model.setIsActionDisabled(not (cls.__gameEventCtrl.isModeActive() and cls.__gameEventCtrl.hasEnoughTickets(WT_TAGS.WT_BOSS)))
...
def _sortMap(gameEventCtrl=None):
    return {gameEventCtrl.getConfig().ticketToken: 1,
     gameEventCtrl.getConfig().ticketToken2025: 1,
