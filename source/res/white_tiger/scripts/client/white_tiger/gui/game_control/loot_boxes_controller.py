...
_CustomData = namedtuple('_CustomData', ['video_idle',
 'video_show',
 'type',
 'icon',
 'priority',
 'sound_video_start',
 'sound_video_loop_start'])
...
class _WtLootBoxReRollProcessor(LootBoxReRollProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        if 1 < ctx.get('reRollCount', 0) <= self.__reRollMaxAttempts and self.__price:
            ...
            SystemMessages.pushMessage(text=backport.text(notifications.lootBoxReRollPrice(), at=TimeFormatter.getLongDatetimeFormat(time_utils.getServerRegionalTime()), price=formatPrice(self.__price, useStyle=True)), type=SystemMessages.SM_TYPE.FinancialTransactionWithGold)
...
class LootBoxesController(ILootBoxesController):
    ...
    def onPortalOpened(self, boxType, parentWindow, callbackFailure=None):
        ...
        if result is None or not result.success:
            ...
            if lastReward:
                ...
                showEventPortalAwardsWindow(boxType, lastReward, len(lastReward))
            else:
    ...
    def requestLootBoxReRoll(self, boxType, parentWindow=None, callback=None, reRollButtonUsed=None, callbackFailure=None):
        ...
        if result.success:
            ...
            showAwardWindow(boxItem.getType(), rewards, openedBoxes, parent=parentWindow, callback=callback)
        return
        ...
        if currentCount == 0:
            ...
            if result.success:
                ...
                if openedBoxes == 1 and receivedVehicles:
                    showAwardWindow(boxType, rewards, parent=parentWindow)
    ...
    def __packLootBoxes(self):
        ...
        if not config:
            ...
            for box in config.itervalues():
                ...
                customData = {}
                for vehName, data in customBonusItems.iteritems():
                    nationID, vehID = vehicles.g_list.getIDsByName(vehName)
                    vehicleCD = vehicles.makeIntCompactDescrByID('vehicle', nationID, vehID)
                    customData[vehicleCD] = _CustomData(**data)
...
def _convertToBonuses(rewards):
    ...
    for boxBonus in boxBonuses:
        ...
        if isinstance(probability, list):
            prob = int(probability[0] * 100)
        group['byProbabilities'].setdefault(prob, [])
        ...
        if bonuses:
            ...
            group['byProbabilities'][prob].extend(bonuses)
