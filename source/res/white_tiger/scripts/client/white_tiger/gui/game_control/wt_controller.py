...
from white_tiger_common.wt_constants import WHITE_TIGER_GAME_PARAMS_KEY, ARENA_BONUS_TYPE
...
WT_VIDEOS = ('boss_portal_idle.usm', 'boss_portal_open.usm', 'hunter_portal_idle.usm', 'hunter_portal_open.usm', 'wt_intro.usm', 'wt_outro.usm', '2_Projet_57_Ampere_v001.usm', 'CVT_Spider_v001.usm', 'Czolg_P_Wz_46_3dst_Verbesserter_v001.usm', 'Projekt_SAT_v003.usm', 'Projet_57_v001.usm', 'MAIN_2_Projet_57_Ampere_v001.usm', 'MAIN_CVT_Spider_v001.usm', 'MAIN_Czolg_P_Wz_46_3dst_Verbesserter_v001.usm', 'MAIN_Projekt_SAT_v003.usm', 'MAIN_Projet_57_v001.usm', 'vehicle_common.usm', 'vehicle1_v001.usm', 'vehicle2_v001.usm', 'vehicle3_v001.usm', 'vehicle4_v001.usm', 'vehicle5_v001.usm')
...
class WhiteTigerController(IWhiteTigerController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def getBossTicketCount(self, bossType):
        config = self.getModeSettings()
        if bossType == WT_TAGS.WT_BOSS:
            return self.__itemsCache.items.tokens.getTokenCount(config.ticketToken)
        return self.__itemsCache.items.tokens.getTokenCount(config.ticketToken2025) if bossType == WT_TAGS.WT_BOSS_2025 else 0

    def getTicketCount(self):
        config = self.getModeSettings()
        return self.__itemsCache.items.tokens.getTokenCount(config.ticketToken)

    def getTicket2025Count(self):
        config = self.getModeSettings()
        return self.__itemsCache.items.tokens.getTokenCount(config.ticketToken2025)
    ...
    def getBossTokenIDList(self):
        config = self.getModeSettings()
        return [config.ticketToken, config.ticketToken2025]
    ...
    def hasEnoughTickets(self, bossType, useQuickTicket=True):
        config = self.getModeSettings()
        if bossType == WT_TAGS.WT_BOSS:
            if useQuickTicket and self.getQuickTicketCount():
                return True
            return self.getTicketCount() >= config.ticketsToDraw
        return self.getTicket2025Count() >= config.tickets2025ToDraw if bossType == WT_TAGS.WT_BOSS_2025 else False
    ...
    def setVehicleForPreview(self, vehicleCD):
        ...
        if not query.empty():
            for ev in query.values():
                if ev.eventType == WT_TAGS.WT_HUNTER:
    ...
    def __onTokensUpdate(self, diff):
        ...
        if config.ticketToken in diff or config.ticketToken2025 in diff:
