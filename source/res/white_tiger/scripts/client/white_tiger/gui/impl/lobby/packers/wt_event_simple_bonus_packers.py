...
# Embedded file name: white_tiger/scripts/client/white_tiger/gui/impl/lobby/packers/wt_event_simple_bonus_packers.py
import logging
import copy
...
from white_tiger.gui.impl.gen.view_models.views.lobby.portal_reward import PortalReward, TooltipType
...
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, SimpleBonusUIPacker, GoodiesBonusUIPacker, ItemBonusUIPacker, VehiclesBonusUIPacker
...
def packBossMainVehicleBonus(model, bonuses, tooltipItems):
    vehicleGroup = bonuses.get(BonusGroup.GUARANTEED_ITEMS, [])
    if vehicleGroup:
        for bonusEntry in vehicleGroup.bonuses:
...
def _getStyles3dStatus():
    return _getBonusStatus(TooltipType.STYLE_3D.value, BonusGroup.CUSTOMIZATION)
...
class EventDecalPacker(StylePacker):
    _TOOLTIP_TYPE = TooltipType.DECAL
...
class SimpleEventPremiumPacker(SimpleBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, label=''):
        model = PortalReward()
        model.setIcon(_ICON_PATH.quests.bonuses.small.dyn('premium_plus_universal')())
        model.setTooltipType(TooltipType.DEFAULT)
        return model
...
class SimpleEventCrewSkinPacker(WTCrewSkinBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, book, count, _=''):
        model = PortalReward()
        model.setIcon(_EXT_ICON_PATH.rewards.small.crewSkin())
        model.setTooltipType(TooltipType.DEFAULT)
        return model
