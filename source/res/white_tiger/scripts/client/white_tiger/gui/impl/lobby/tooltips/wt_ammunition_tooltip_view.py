...
_COOLDOWN_TEMPLATE = '{}: {} {}'
_UNION_STRENGTH_EQUIPMENT_ID = 913
_STUN_AREA_EQUIPMENT_ID = 916
...
class WtAmmunitionTooltipView(ViewImpl):
    ...
    def __getOnUseStr(self, item, attribs):
        if item.descriptor.id.itemID == _UNION_STRENGTH_EQUIPMENT_ID:
            effectDuration = item.descriptor.effectDuration
            receiveDamageFactor = item.descriptor.receiveDamageFactor
            return backport.text(attribs.onUse(), damageIncrease=int(receiveDamageFactor * 100), percent=backport.text(R.strings.common.common.percent()), time=int(effectDuration))
        if item.descriptor.id.itemID == _STUN_AREA_EQUIPMENT_ID:
            effectDuration = item.descriptor.effectDuration
            effectRadius = item.descriptor.effectRadius
            return backport.text(attribs.onUse(), time=int(effectDuration), radius=int(effectRadius))
