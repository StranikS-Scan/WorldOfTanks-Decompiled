...
from white_tiger.gui.impl.gen.view_models.views.lobby.wt_event_portal_awards_model import WtEventPortalAwardsModel, currencyTooltipTypes
...
from white_tiger.gui.impl.lobby.wt_event_sound import playLootBoxPortalExit, WTEventAwardsScreenVideoSound
from white_tiger.gui.impl.lobby.wt_event_vehicle_portal import ReRollButton
from gui.impl.pub.dialog_window import DialogButtons
from gui.impl.pub.lobby_window import LobbyWindow
from white_tiger.gui.game_control.loot_boxes_controller import LootBoxAwardsManager
...
from white_tiger.gui.wt_event_models_helper import setLootBoxesCount, setGuaranteedAward, fillAdditionalAwards, fillVehicleAward
from helpers import dependency, server_settings
from helpers.CallbackDelayer import CallbackDelayer
from skeletons.gui.lobby_context import ILobbyContext
from skeletons.gui.shared import IItemsCache
from frameworks.wulf.gui_constants import ShowingStatus
from white_tiger.gui.shared.event_dispatcher import showVehicleAwardWindow, showEventPortalAwardsWindow
...
class WtEventPortalAwards(WtEventBasePortalAwards, CallbackDelayer):
    ...
    def __init__(self, lootBoxType, awards, count, openedCount, *args, **kwargs):
        settings = ViewSettings(layoutID=R.views.white_tiger.lobby.PortalAwardsView(), model=WtEventPortalAwardsModel())
        settings.args = args
    ...
    def _updateModel(self):
        ...
        with self.viewModel.transaction() as model:
            ...
            if isBossLootBox:
                ...
                _fillMainAwards(WhiteTigerLootBoxes.WT_HUNTER, model.rewards, self._awards, self._tooltipItems)
    ...
    def __setRerollParameters(self, model, isBossLootBox):
        ...
        if reRollPrice is not None:
            ...
            self.__rerollCost = reRollPrice.get(self.__priceType)
            ...
            if self.__priceType == 'gold':
                model.setCurrencyTooltipType(currencyTooltipTypes.GOLD)
            else:
                model.setCurrencyTooltipType(currencyTooltipTypes.CREDITS)
    ...
    def __animationEnded(self, args):
        ...
        if isAnimationEnded:
            showVehicleAwardWindow(parent=self.getParentWindow())
...
def _fillBossAwards(model, bonuses, tooltipItems):
    ...
    _fillMainAwards(WhiteTigerLootBoxes.WT_BOSS, model.rewards, groupedBonuses.main, tooltipItems)
