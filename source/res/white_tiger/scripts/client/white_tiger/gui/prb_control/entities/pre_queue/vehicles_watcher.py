...
from white_tiger.gui.gui_constants import WT_TAGS
...
class WhiteTigerBattlesVehiclesWatcher(BaseVehiclesWatcher):
    ...
    def _update(self, *_):
        ...
        if self.__wtController.hasEnoughTickets(WT_TAGS.WT_BOSS):
            self.__clearTicketsShortageState()
        else:
            self.__setTicketsShortageState()
    ...
    def __getTicketsShortageVehicles(self):
        vehs = self.__itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.WT_BOSS}) | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.WT_BOSS_2025}) | REQ_CRITERIA.VEHICLE.HAS_NO_TAG({VEHICLE_TAGS.WT_SPECIAL_BOSS})).values()
        return vehs
