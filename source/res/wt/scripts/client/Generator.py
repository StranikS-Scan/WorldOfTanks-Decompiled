...
# Embedded file name: wt/scripts/client/Generator.py
import BigWorld
...
class Generator(DynamicScriptComponent):
        ...
        if activation.progress > 0:
            ...
            if self.__marker:
                if self.__generatorMarker:
                    self.__generatorMarker.onGeneratorCapture(generatorID, activation.progress, activation.timeLeft, activation.numInvaders)
                if self.__minimapMarker:
                    self.__minimapMarker.onGeneratorCapture(generatorID, activation.progress, activation.timeLeft, activation.numInvaders)
            ctrl = self.__guiSessionProvider.shared.feedback
            if ctrl:
                ctrl.onGeneratorCapture(generatorID, activation.progress, activation.timeLeft, activation.numInvaders, activation.isBlocked)
            ...
            self.__stopCapture(wasCaptured=self.__wasCaptured)
    ...
    def capture(self):
        self.__wasCaptured = True
        ctrl = self.__guiSessionProvider.shared.feedback
        if ctrl:
            ctrl.onGeneratorStopCapture(self.__getIndex(), self.__wasCaptured)

    def locked(self, isLocked):
        entityID = self.entity.id
        generatorID = self.__getIndex()
        if self.__marker:
            if self.__generatorMarker:
                self.__generatorMarker.onGeneratorLocked(generatorID, isLocked)
        if self.__minimapMarker:
            self.__minimapMarker.onGeneratorLocked(generatorID, isLocked)
        ctrl = self.__guiSessionProvider.shared.feedback
        if ctrl:
            ctrl.onGeneratorLocked(generatorID, isLocked, entityID)
    ...
    def __stopCapture(self, wasCaptured):
        if self.__marker:
            self.__generatorMarker = self.__fetchGeneratorMarkerComponent(self.__marker)
            if self.__generatorMarker:
        ...
        ctrl = self.__guiSessionProvider.shared.feedback
        if ctrl:
            ctrl.onGeneratorStopCapture(self.__getIndex(), wasCaptured)
