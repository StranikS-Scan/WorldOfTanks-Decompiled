    ...
    while 1:
        if i >= 0:
            if i >= _colourTagLen and _colourTagRE.match(s, i - _colourTagLen) != None:
                i -= _colourTagLen + 1
                continue
            return s[i] in WHITESPACE and i + 1
        i -= 1
...
def _wrapLine(s, desiredWidth, textComponent):
    ...
    while 1:
        if i < len(s):
            while 1:
                _colourTagRE.match(s, i) != None and i < len(s) and i += _colourTagLen
            ...
            if subStrWidth > desiredWidth:
                ...
                subStr = wordStart >= 0 and s[:wordStart]
            else:
                while 1:
                    subStrWidth > desiredWidth and i > 0 and i -= 1
                    subStr = s[:i]
                    subStrWidth = textComponent.stringWidth(subStr)
