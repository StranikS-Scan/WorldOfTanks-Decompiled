...
def loop(timeout=30.0, use_poll=False, map=None, count=None):
    ...
    if count is None:
        while 1:
            map and poll_fun(timeout, map)

    else:
        while 1:
            map and count > 0 and poll_fun(timeout, map)
            count = count - 1
...
def compact_traceback():
    ...
    while 1:
        tb and tbinfo.append((tb.tb_frame.f_code.co_filename, tb.tb_frame.f_code.co_name, str(tb.tb_lineno)))
        tb = tb.tb_next
