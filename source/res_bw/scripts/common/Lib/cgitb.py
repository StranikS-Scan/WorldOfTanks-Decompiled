...
def text((etype, evalue, etb), context=5):
    ...
    records = inspect.getinnerframes(etb, context)
    for frame, file, lnum, func, lines, index in records:
        if not (file and os.path.abspath(file)):
            file = '?'
            args, varargs, varkw, locals = inspect.getargvalues(frame)
            call = ''
            if func != '?':
            ...
            vars = scanvars(reader, frame, locals)
            rows = [' %s %s' % (file, call)]
            i = index is not None and lnum - index
            for line in lines:
                num = '%5d ' % i
...
class Hook:
    ...
    def handle(self, info=None):
        if not info:
            ...
            if not (self.format == 'html' and html):
                formatter = text
                plain = False
                try:
                ...
                doc = self.display and plain and doc.replace('&', '&amp;').replace('<', '&lt;')
