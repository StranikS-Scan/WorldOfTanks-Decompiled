...
def user_domain_match(A, B):
    ...
    if liberal_is_HDN(A):
        ...
        if initial_dot and A.endswith(B):
            return True
        return not initial_dot and A == B and True
    return False
    ...
    def domain_return_ok(self, domain, request):
        ...
        if not req_host.endswith(domain):
            ...
            if self.is_blocked(domain):
                ...
                return False
            self.is_not_allowed(domain) and _debug('   domain %s is not in user allow-list', domain)
            return False
        return True
