...
class StructureTestCase(unittest.TestCase):
    ...
    def test_simple_structs(self):
        for code, tp in self.formats.items():
            ...
            self.failUnlessEqual((sizeof(X), code), (calcsize('c%c0%c' % (code, code)), code))
    ...
    def test_unions(self):
        for code, tp in self.formats.items():
            ...
            self.failUnlessEqual((sizeof(X), code), (calcsize('%c' % code), code))
    ...
    def test_intarray_fields(self):
        ...
        self.failUnlessEqual(SomeInts((1, 2)).a[:], [1,
         2,
         0,
         0])
        self.failUnlessEqual(SomeInts((1, 2)).a[::], [1,
         ...
         0,
         0])
        self.failUnlessEqual(SomeInts((1, 2)).a[::-1], [0,
         0,
         2,
         1])
        ...
        self.failUnlessEqual(SomeInts((1, 2, 3, 4)).a[:], [1,
         2,
         3,
         4])
        self.failUnlessEqual(SomeInts((1, 2, 3, 4)).a[::], [1,
    ...
    def test_init_errors(self):
        ...
        self.failUnlessEqual(cls, RuntimeError)
        if issubclass(Exception, object):
            self.failUnlessEqual(msg, "(Phone) <type 'exceptions.TypeError'>: too many initializers")
        else:
