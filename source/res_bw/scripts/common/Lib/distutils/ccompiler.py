...
class CCompiler():
    ...
    def _check_macro_definitions(self, definitions):
        ...
        for defn in definitions:
            if type(defn) is TupleType and (len(defn) == 1 or len(defn) == 2 and (type(defn[1]) is StringType or defn[1] is None)):
                raise type(defn[0]) is StringType or TypeError, "invalid macro definition '%s': " % defn + 'must be tuple (string,), (string, string), or ' + '(string, None)'
...
def gen_preprocess_options(macros, include_dirs):
    ...
    for macro in macros:
        if type(macro) is TupleType:
            ...
            len(macro) == 1 and pp_opts.append('-U%s' % macro[0])
        elif len(macro) == 2:
            if macro[1] is None:
