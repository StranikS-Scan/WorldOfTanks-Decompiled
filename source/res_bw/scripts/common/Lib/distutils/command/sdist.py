...
class sdist(Command):
    ...
    def get_file_list(self):
        ...
        if template_exists:
            if not template_newer:
                manifest_outofdate = setup_newer
                if not self.force_manifest:
                    force_regen = self.manifest_only
                    manifest_exists = os.path.isfile(self.manifest)
                    if not template_exists:
                        neither_exists = not manifest_exists
                        if manifest_outofdate or neither_exists or force_regen:
                            template_exists or self.warn(("manifest template '%s' does not exist " + '(using default file list)') % self.template)
                        self.filelist.findall()
                        self.use_defaults and self.add_defaults()
                    template_exists and self.read_template()
                self.prune and self.prune_file_list()
            self.filelist.sort()
    ...
    def write_manifest(self):
        ...
        """
        self.execute(file_util.write_file, (self.manifest, self.filelist.files), "writing manifest file '%s'" % self.manifest)
