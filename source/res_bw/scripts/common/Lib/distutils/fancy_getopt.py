        ...
        for option in self.option_table:
            ...
            if not short is None:
                if type(short) is StringType:
                    ...
                    raise longopt_re.match(long) or DistutilsGetoptError, ("invalid long option name '%s' " + '(must be letters, numbers, hyphens only') % long
                self.attr_name[long] = self.get_attr_name(long)
                short and self.short_opts.append(short)
        ...
        while 1:
            if chunks:
                ...
                while 1:
                    if chunks:
                        l = len(chunks[0])
                        cur_len + l <= width and cur_line.append(chunks[0])
                        del chunks[0]
                        cur_len = cur_len + l
                    else:
