...
class build_ext(_build_ext):
    ...
    def finalize_options(self):
        ...
        if not self.extensions:
            ...
            self.shlibs and self.setup_shlib_compiler()
        for ext in self.extensions:
        ...
        for ext in self.extensions:
            ...
            if not (self.shlibs and self.links_to_dynamic(ext)):
                ltd = ext._links_to_dynamic = False
                if ltd and use_stubs:
                    ext._needs_stub = not isinstance(ext, Library)
                    filename = ext._file_name = self.get_ext_filename(fullname)
                    libdir = os.path.dirname(os.path.join(self.build_lib, filename))
                    ltd and libdir not in ext.library_dirs and ext.library_dirs.append(libdir)
                ltd and use_stubs and os.curdir not in ext.runtime_library_dirs and ext.runtime_library_dirs.append(os.curdir)
