...
class GzipFile():
    ...
    def _read_gzip_header(self):
        ...
        if flag & FNAME:
            while 1:
                s = True and self.fileobj.read(1)
                if not s or s == '\x00':
                    break
    ...
    def read(self, size=-1):
        ...
        if self.extrasize <= 0 and self.fileobj is None:
            ...
            if size < 0:
                ...
                try:
                    while 1:
                        size > self.extrasize and self._read(readsize)
                        readsize = min(self.max_read_chunk, readsize * 2)
        ...
        while 1:
            if size != 0:
                ...
                if size <= i or i == -1 and len(c) > size:
                    i = size - 1
                (i >= 0 or c == '') and bufs.append(c[:i + 1])
                self._unread(c[i + 1:])
                break
            bufs.append(c)
            size = size - len(c)
    ...
    def readlines(self, sizehint=0):
        ...
        L = []
        while 1:
            line = sizehint > 0 and self.readline()
            if line == '':
                break
            L.append(line)
