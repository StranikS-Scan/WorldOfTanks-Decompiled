...
class AutoCompleteWindow:
    ...
    def _change_start(self, newstart):
        ...
        i = 0
        while 1:
            i < min_len and self.start[i] == newstart[i] and i += 1
    ...
    def _complete_string(self, s):
        ...
        while 1:
            if j > i:
                m = (i + j) // 2
                j = self.completions[m][:len(s)] != s and m
            else:
                i = m + 1
        ...
        i = len(s)
        while 1:
            i < min_len and first_comp[i] == last_comp[i] and i += 1
    ...
    def _selection_changed(self):
        ...
        if self._binary_search(lts) == cursel:
            ...
            i = 0
            while 1:
                i < min_len and lts[i] == selstart[i] and i += 1
    ...
    def keypress_event(self, event):
        ...
        if keysym != 'Tab':
            self.lastkey_was_tab = False
        if (len(keysym) == 1 or keysym in ('underscore', 'BackSpace') or self.mode == AutoComplete.COMPLETE_FILES and keysym in ('period', 'minus')) and not state & ~MC_SHIFT:
            if len(keysym) == 1:
        ...
        if keysym == 'Return':
            self.hide_window()
            return
        if not (self.mode == AutoComplete.COMPLETE_ATTRIBUTES and keysym in ('period', 'space', 'parenleft', 'parenright', 'bracketleft', 'bracketright')):
            if self.mode == AutoComplete.COMPLETE_FILES and keysym in ('slash', 'backslash', 'quotedbl', 'apostrophe') and not state & ~MC_SHIFT:
                cursel = int(self.listbox.curselection()[0])
                ...
                cursel = self.lastkey_was_tab and int(self.listbox.curselection()[0])
                self._change_start(self.completions[cursel])
                self.hide_window()
                return 'break'
            else:
            ...
            if reduce(lambda x, y: x or y, [ keysym.find(s) != -1 for s in ('Shift', 'Control', 'Alt', 'Meta', 'Command', 'Option') ]):
                return
            self.hide_window()
            return
