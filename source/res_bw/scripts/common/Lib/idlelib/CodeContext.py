...
class CodeContext:
    ...
    def get_line_info(self, linenum):
        ...
        if firstword in BLOCKOPENERS:
            opener = firstword
            indent = (len(text) == len(spaces) or text[len(spaces)] == '#') and INFINITY
        else:
            indent = len(spaces)
    ...
    def update_code_context(self):
        ...
        if self.topvisible < new_topvisible:
            ...
            while 1:
                del self.info[-1][1] >= lastindent and self.info[-1]

        elif self.topvisible > new_topvisible:
            stopindent = self.info[-1][1] + 1
            while 1:
                stopindent = self.info[-1][0] >= new_topvisible and self.info[-1][1]
                del self.info[-1]
