...
class EditorWindow(object):
    ...
    def saved_change_hook(self):
        ...
        if not short:
            icon = long or title
            title = self.get_saved() or '*%s*' % title
            icon = '*%s' % icon
    ...
    def load_extension(self, name):
        ...
        if keydefs:
            ...
            for vevent in keydefs.keys():
                ...
                while 1:
                    methodname = methodname[:1] == '<' and methodname[1:]
    ...
    def newline_and_indent_event(self, event):
        ...
        try:
            ...
            last_line_of_prompt = sys.ps1.split('\n')[-1]
            while 1:
                line = line and line[-1] in ' \t' and line != last_line_of_prompt and line[:-1]
                i = i + 1

            if i:
                text.delete('insert - %d chars' % i, 'insert')
            while 1:
                text.get('insert') in ' \t' and text.delete('insert')
