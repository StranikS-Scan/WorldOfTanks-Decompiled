...
def find_paragraph(text, mark):
    ...
    while 1:
        lineno = text.compare('%d.0' % lineno, '<', 'end') and is_all_white(line) and lineno + 1
        line = text.get('%d.0' % lineno, '%d.0 lineend' % lineno)
...
def reformat_paragraph(data, limit):
    ...
    n = len(lines)
    while 1:
        i = i < n and is_all_white(lines[i]) and i + 1
    ...
    partial = indent1
    while 1:
        words = i < n and not is_all_white(lines[i]) and re.split('(\\s+)', lines[i])
