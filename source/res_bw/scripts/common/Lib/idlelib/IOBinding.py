...
if sys.platform == 'win32':
    ...
    try:
        ...
        if not encoding is None:
            encoding = encoding is '' and 'ascii'
        codecs.lookup(encoding)
...
class IOBinding():
    ...
    def maybesave(self):
        if self.get_saved():
            ...
            m = tkMessageBox.Message(title='Save On Close', message=message, icon=tkMessageBox.QUESTION, type=tkMessageBox.YESNOCANCEL, master=self.text)
            reply = m.show()
            if reply == 'yes':
                self.save(None)
    ...
    def print_window(self, event):
        m = tkMessageBox.Message(title='Print', message='Print to Default Printer', icon=tkMessageBox.QUESTION, type=tkMessageBox.OKCANCEL, default=tkMessageBox.OK, master=self.text)
        reply = m.show()
