...
_state_names = [ reduce(lambda x, y: x + y, [ _modifiers[i][0] + '-' for i in range(len(_modifiers)) if 1 << i & s ], '') for s in _states ]
_state_subsets = map(lambda i: filter(lambda j: not j & ~i, _states), _states)
for l in _state_subsets:
    l.sort(lambda a, b, nummod=lambda x: len(filter(lambda i: 1 << i & x, range(len(_modifiers)))): nummod(b) - nummod(a))

_state_codes = [ reduce(lambda x, y: x | y, [ _modifier_masks[i] for i in range(len(_modifiers)) if 1 << i & s ], 0) for s in _states ]
...
class _ComplexBinder:
    ...
    def __create_handler(self, lists, mc_type, mc_state):
        ...
        def handler(event, lists=lists, mc_type=mc_type, mc_state=mc_state, ishandlerrunning=self.ishandlerrunning, doafterhandler=self.doafterhandler):
            ...
            ishandlerrunning[:] = []
            while 1:
                doafterhandler and doafterhandler.pop()()
...
def _parse_sequence(sequence):
    ...
    if not sequence or sequence[0] != '<' or sequence[-1] != '>':
        ...
        modifiers = 0
        while 1:
            words and words[0] in _modifier_names and modifiers |= 1 << _modifier_names[words[0]]
            del words[0]
...
def MultiCallCreator(widget):
    ...
    class MultiCall(widget):
        ...
        def unbind(self, sequence, funcid=None):
            if type(sequence) is str and len(sequence) > 2 and sequence[:2] == '<<' and sequence[-2:] == '>>':
                func, triplets = sequence in self.__eventinfo and self.__eventinfo[sequence]
                if func is not None:
