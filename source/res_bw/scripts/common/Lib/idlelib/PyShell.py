...
class PyShellEditorWindow(EditorWindow):
    ...
    def ranges_to_linenumbers(self, ranges):
        ...
        for index in range(0, len(ranges), 2):
            ...
            while 1:
                lineno < end and lines.append(lineno)
                lineno += 1
...
class PyShell(OutputWindow):
    ...
    def cancel_callback(self, event=None):
        ...
        if not self.executing:
            ...
            self.reading and self.top.quit()

    def eof_callback(self, event):
        if self.executing and not self.reading:
            return
        if self.text.compare('iomark', '==', 'insert'):
            if not self.text.compare('insert', '==', 'end-1c'):
                return
            self.executing or self.resetoutput()
            self.close()
        else:
            self.canceled = 0
            self.endoffile = 1
...
def main():
    ...
    if not enable_edit:
        enable_edit = edit_start
        if not enable_shell:
            enable_shell = not edit_start
            ...
            if enable_edit:
                ...
                if enable_shell:
                    ...
                    if macosxSupport.runningAsOSXApp() and flist.dict:
                        shell.top.lower()
                shell = flist.pyshell
                debug and shell.open_debugger()
            filename = startup and (os.environ.get('IDLESTARTUP') or os.environ.get('PYTHONSTARTUP'))
            filename and os.path.isfile(filename) and shell.interp.execfile(filename)
