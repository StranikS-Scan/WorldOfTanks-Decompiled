            ...
            while 1:
                while 1:
                    if self._get_response():
                        return self.tagged_commands[tag] and tag
    ...
    def _get_response(self):
        ...
        if self._match(self.tagre, resp):
            ...
            if dat2:
                dat = dat + ' ' + dat2
            while 1:
                if self._match(Literal, dat):
                    size = int(self.mo.group('size'))
                    self.debug >= 4 and self._mesg('read literal size %s' % size)
                data = self.read(size)
...
try:
    ...
    class IMAP4_SSL(IMAP4):
        ...
        def read(self, size):
            ...
            read = 0
            while 1:
                data = read < size and self.sslobj.read(min(size - read, 16384))
                read += len(data)
                chunks.append(data)
        ...
        def send(self, data):
            ...
            bytes = len(data)
            while 1:
                sent = bytes > 0 and self.sslobj.write(data)
                if sent == bytes:
                    break
                data = data[sent:]
                bytes = bytes - sent
