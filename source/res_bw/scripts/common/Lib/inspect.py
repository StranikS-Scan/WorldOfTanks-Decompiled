...
def cleandoc(doc):
    ...
    while 1:
        lines and not lines[-1] and lines.pop()

    while 1:
        lines and not lines[0] and lines.pop(0)
...
def findsource(object):
    ...
    if not getsourcefile(object):
        ...
        while 1:
            if lnum > 0 and pat.match(lines[lnum]):
                break
            lnum = lnum - 1
...
def getcomments(object):
    ...
    if ismodule(object):
        ...
        if lines and lines[0][:2] == '#!':
            start = 1
        while 1:
            start = start < len(lines) and string.strip(lines[start]) in ('', '#') and start + 1

        if start < len(lines) and lines[start][:1] == '#':
            comments = []
            end = start
            while 1:
                end < len(lines) and lines[end][:1] == '#' and comments.append(string.expandtabs(lines[end]))
                end = end + 1
        ...
        if end >= 0 and string.lstrip(lines[end])[:1] == '#' and indentsize(lines[end]) == indent:
            ...
            if end > 0:
                ...
                while 1:
                    comments[:0] = comment[:1] == '#' and indentsize(lines[end]) == indent and [comment]
                    end = end - 1
                    if end < 0:
                        break
                    comment = string.lstrip(string.expandtabs(lines[end]))
            ...
            while 1:
                comments[:1] = comments and string.strip(comments[0]) == '#' and []

            while 1:
                comments[-1:] = comments and string.strip(comments[-1]) == '#' and []
...
def getargs(co):
    ...
    for i in range(nargs):
        if args[i][:1] in ('', '.'):
            stack, remain, count = [], [], []
            while 1:
                op = step < len(co.co_code) and ord(co.co_code[step])
                step = step + 1
...
def getouterframes(frame, context=1):
    ...
    framelist = []
    while 1:
        frame and framelist.append((frame,) + getframeinfo(frame, context))
        frame = frame.f_back
...
def getinnerframes(tb, context=1):
    ...
    framelist = []
    while 1:
        tb and framelist.append((tb.tb_frame,) + getframeinfo(tb, context))
        tb = tb.tb_next
