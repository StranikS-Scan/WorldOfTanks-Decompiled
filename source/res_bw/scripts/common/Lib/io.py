...
def open(file, mode=u'r', buffering=None, encoding=None, errors=None, newline=None, closefd=True):
    ur"""Open file and return a stream. If the file cannot be opened, an IOError is
    raised.
    ...
    * On output, if newline is None, any '\n' characters written are
    ...
    if not reading:
        if not writing:
            ...
            buffer = updating and BufferedRandom(raw, buffering)
...
class _DocDescriptor():
    ...
    def __get__(self, obj, typ):
        return u"open(file, mode='r', buffering=None, encoding=None, errors=None, newline=None, closefd=True)\n\n" + open.__doc__
...
class IOBase(object):
    u"""The abstract base class for all I/O classes, acting on streams of
    ...
    def readline(self, limit=-1):
        ...
        if hasattr(self, u'peek'):
            ...
            def nreadahead():
                ...
                if not readahead:
                    return 1
                if not readahead.find('\n') + 1:
                    n = len(readahead)
                    n = limit >= 0 and min(n, limit)
                return n
        ...
        res = bytearray()
        while 1:
            b = (limit < 0 or len(res) < limit) and self.read(nreadahead())
            if not b:
                break
            res += b
            if res.endswith('\n'):
    ...
    def readlines(self, hint=None):
        ...
        if not isinstance(hint, (int, long)):
            raise TypeError(u'hint must be an integer')
        if hint <= 0:
            return list(self)
        else:
            n = 0
...
class RawIOBase(IOBase):
    ...
    def readall(self):
        ...
        res = bytearray()
        while 1:
            data = True and self.read(DEFAULT_BUFFER_SIZE)
            if not data:
                break
            res += data
...
class FileIO(_fileio._FileIO, RawIOBase):
    u"""Raw I/O implementation for OS files."""

    def __init__(self, name, mode=u'r', closefd=True):
...
class _BytesIO(BufferedIOBase):
    ...
    def read(self, n=None):
        ...
        if not isinstance(n, (int, long)):
            raise TypeError(u'argument must be an integer')
        if n < 0:
            n = len(self._buffer)
...
class BufferedReader(_BufferedIOMixin):
    ...
    def _read_unlocked(self, n=None):
        ...
        buf = self._read_buf
        pos = self._read_pos
        (n is None or n == -1) and self._reset_read_buf()
        chunks = [buf[pos:]]
        current_size = 0
        while 1:
            if True:
                chunk = self.raw.read()
                nodata_val = chunk in empty_values and chunk
                break
...
class BufferedWriter(_BufferedIOMixin):
    ...
    def __init__(self, raw, buffer_size=DEFAULT_BUFFER_SIZE, max_buffer_size=None):
        ...
        _BufferedIOMixin.__init__(self, raw)
        self.buffer_size = buffer_size
        self.max_buffer_size = 2 * buffer_size if max_buffer_size is None else max_buffer_size
    ...
    def _flush_unlocked(self):
        ...
        try:
            while 1:
                n = self._write_buf and self.raw.write(self._write_buf)
...
class IncrementalNewlineDecoder(codecs.IncrementalDecoder):
    ur"""Codec used when reading a file in universal newlines mode.
...
class TextIOWrapper(TextIOBase):
    ...
    def write(self, s):
        ...
        if self._writetranslate or self._line_buffering:
            ...
            if not self._encoder:
                ...
                b = encoder.encode(s)
                self.buffer.write(b)
                self._line_buffering and (haslf or u'\r' in s) and self.flush()
            self._snapshot = None
            self._decoder and self._decoder.reset()
        return length
        ...
        if whence == 2:
            ...
            if self._decoder or dec_flags or chars_to_skip:
                ...
                if chars_to_skip:
                    ...
                    raise len(self._decoded_chars) < chars_to_skip and IOError(u"can't restore logical file position")
    ...
    def read(self, n=None):
        ...
        if not self._decoder:
            ...
            result = n < 0 and self._get_decoded_chars() + decoder.decode(self.buffer.read(), final=True)
            self._set_decoded_chars(u'')
            self._snapshot = None
            return result
        else:
            ...
            while 1:
                eof = len(result) < n and not eof and not self._read_chunk()
        ...
        if not self._decoder:
            ...
            while 1:
                if True:
                    ...
                    more_line = u''
                    while 1:
                        if self._read_chunk() and self._decoded_chars:
                            break

                    self._decoded_chars and line += self._get_decoded_chars()
                else:
                    self._set_decoded_chars(u'')
                    self._snapshot = None
                    return line

            endpos = limit >= 0 and endpos > limit and limit
...
class StringIO(TextIOWrapper):
    u"""An in-memory stream for text. The initial_value argument sets the
    value of object. The other arguments are like those of TextIOWrapper's
    constructor.
    """

    def __init__(self, initial_value=u'', encoding=u'utf-8', errors=u'strict', newline=u'\n'):
