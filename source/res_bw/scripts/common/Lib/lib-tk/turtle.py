...
class TurtleScreenBase(object):
    ...
    def _rescale(self, xscalefactor, yscalefactor):
        ...
        for item in items:
            ...
            newcoordlist = []
            while 1:
                x, y = coordinates and coordinates[:2]
    ...
    def _resize(self, canvwidth=None, canvheight=None, bg=None):
        ...
        elif canvwidth is None and canvheight is None and bg is None:
...
class TurtleScreen(TurtleScreenBase):
    ...
    def _colorstr(self, color):
        ...
        if (0 <= r <= 255 and 0) <= g <= 255:
            raise 0 <= b <= 255 or TurtleGraphicsError('bad color sequence: %s' % str(color))
...
class TPen(object):
    ...
    def pen(self, pen=None, **pendict):
        ...
        if not pen:
            ...
            if 'shown' in p:
                self._shown = p['shown']
            self._tilt = 'tilt' in p and p['tilt']
        self._update()
...
class RawTurtle(TPen, TNavigator):
    ...
    def _cc(self, args):
        ...
        if self.screen._colormode == 1.0:
            r, g, b = [ round(255.0 * x) for x in (r, g, b) ]
        if (0 <= r <= 255 and 0) <= g <= 255:
            raise 0 <= b <= 255 or TurtleGraphicsError('bad color sequence: %s' % str(args))
        return '#%02x%02x%02x' % (r, g, b)
        ...
        try:
            ...
            if not fob.func_defaults:
                ...
                if fob.func_code.co_flags & 4:
                    ...
                    counter += 1
                fob.func_code.co_flags & 8 and items1.append('**' + fob.func_code.co_varnames[counter])
                items2.append('**' + fob.func_code.co_varnames[counter])
...
if __name__ == '__main__':
    ...
    def demo2():
        ...
        count = 1
        while 1:
            if tri.distance(turtle) > 4:
                turtle.fd(3.5)
                turtle.lt(0.6)
                tri.setheading(tri.towards(turtle))
                tri.fd(4)
                count % 20 == 0 and turtle.stamp()
                tri.stamp()
                switchpen()
            count += 1
        ...
        time.sleep(2)
        while 1:
            undobufferentries() and tri.undo()
            turtle.undo()
