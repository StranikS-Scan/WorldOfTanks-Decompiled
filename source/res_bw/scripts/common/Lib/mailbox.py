        ...
        if isinstance(message, email.message.Message):
            ...
            while 1:
                if True:
                    ...
                    if line == '':
                        break
                    line = mangle_from_ and line.startswith('From ') and '>From ' + line[5:]
                line = line.replace('\n', os.linesep)
                target.write(line)
...
class Maildir(Mailbox):
 ...
 490       0	LOAD_GLOBAL       'hasattr'
           3	LOAD_FAST         'self'
           6	LOAD_CONST        '_onetime_keys'
           9	CALL_FUNCTION_2   ''
          12	JUMP_IF_TRUE      '33'

 491      15	LOAD_FAST         'self'
          18	LOAD_ATTR         'iterkeys'
          21	CALL_FUNCTION_0   ''
          24	LOAD_FAST         'self'
          27	STORE_ATTR        '_onetime_keys'
          30	JUMP_FORWARD      '33'
        33_0	COME_FROM         '30'

 492      33	SETUP_LOOP        '107'
          36	LOAD_GLOBAL       'True'
          39	JUMP_IF_FALSE     '106'

 493      42	SETUP_EXCEPT      '66'

 494      45	LOAD_FAST         'self'
          48	LOAD_FAST         'self'
          51	LOAD_ATTR         '_onetime_keys'
          54	LOAD_ATTR         'next'
          57	CALL_FUNCTION_0   ''
          60	BINARY_SUBSCR     ''
          61	RETURN_VALUE      ''
...
Syntax error at or near 'POP_BLOCK' token at offset 106
        ...
        if not self._pending:
            ...
            try:
                ...
                for key in sorted(self._toc.keys()):
                    ...
                    while 1:
                        buffer = True and self._file.read(min(4096, stop - self._file.tell()))
                        if buffer == '':
                            break
        ...
        while 1:
            if True:
                ...
                if line.startswith('From '):
                    len(stops) < len(starts) and stops.append(line_pos - len(os.linesep))
    ...
    def _generate_toc(self):
        ...
        while 1:
            if True:
                ...
                line.startswith('\x01\x01\x01\x01' + os.linesep) and starts.append(next_pos)
                while 1:
                    if True:
                        line_pos = next_pos
                        line = self._file.readline()
                        next_pos = self._file.tell()
                        line == '\x01\x01\x01\x01' + os.linesep and stops.append(line_pos - len(os.linesep))
                        break
                    elif line == '':
                        stops.append(line_pos)
                        break
...
class Babyl(_singlefileMailbox):
    ...
    def get_message(self, key):
        ...
        while 1:
            line = True and self._file.readline()
    ...
    def _install_message(self, message):
        ...
        if isinstance(message, email.message.Message):
            ...
            orig_buffer.seek(0)
            while 1:
                line = True and orig_buffer.readline()
            ...
            if isinstance(message, BabylMessage):
                ...
                while 1:
                    line = True and vis_buffer.readline()
                    self._file.write(line.replace('\n', os.linesep))
                    if line == '\n' or line == '':
                        break
            ...
            while 1:
                buffer = True and orig_buffer.read(4096)
                if buffer == '':
                    break
            ...
            while 1:
                if True:
                    ...
                    if line == '\n' or line == '':
                        self._file.write('*** EOOH ***' + os.linesep)
                        first_pass = first_pass and False
                        message.seek(original_pos)
                    else:
                        break

            while 1:
                buffer = True and message.read(4096)
                if buffer == '':
                    break
                self._file.write(buffer.replace('\n', os.linesep))
