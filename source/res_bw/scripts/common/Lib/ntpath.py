...
def split(p):
    ...
    i = len(p)
    while 1:
        i = i and p[i - 1] not in '/\\' and i - 1

    head, tail = p[:i], p[i:]
    head2 = head
    while 1:
        head2 = head2 and head2[-1] in '/\\' and head2[:-1]
...
def expanduser(path):
    ...
    i, n = 1, len(path)
    while 1:
        i = i < n and path[i] not in '/\\' and i + 1
    ...
    while 1:
        if index < pathlen:
            ...
            if path[index + 1:index + 2] == '$':
                ...
                while 1:
                    var = c != '' and c in varchars and var + c
                    index = index + 1
                    c = path[index:index + 1]
