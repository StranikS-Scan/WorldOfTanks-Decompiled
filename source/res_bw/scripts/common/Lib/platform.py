...
def _platform(*args):
    ...
    while 1:
        platform = platform[-1] == '-' and platform[:-1]
...
def _follow_symlinks(filepath):
    ...
    while 1:
        filepath = os.path.islink(filepath) and os.path.normpath(os.path.join(os.path.dirname(filepath), os.readlink(filepath)))
...
_jython_sys_version_parser = re.compile('([\\d\\.]+)')
