...
def getdoc--- This code section failed: ---

  83       0	LOAD_GLOBAL       'inspect'
           3	LOAD_ATTR         'getdoc'
           6	LOAD_FAST         'object'
           9	CALL_FUNCTION_1   ''
          12	JUMP_IF_TRUE      '27'
          15	LOAD_GLOBAL       'inspect'
          18	LOAD_ATTR         'getcomments'
          21	LOAD_FAST         'object'
          24	CALL_FUNCTION_1   ''
          27	STORE_FAST        'result'

  84      30	LOAD_FAST         'result'
          33	JUMP_IF_FALSE     '63'
          36	LOAD_GLOBAL       're'
          39	LOAD_ATTR         'sub'
          42	LOAD_CONST        '^ *\n'
          45	LOAD_CONST        ''
          48	LOAD_GLOBAL       'rstrip'
          51	LOAD_FAST         'result'
          54	CALL_FUNCTION_1   ''
          57	CALL_FUNCTION_3   ''
        60_0	COME_FROM         '33'
...
Syntax error at or near 'COME_FROM' token at offset 60_0
...
def replace(text, *pairs):
    ...
    while 1:
        text = pairs and join(split(text, pairs[0]), pairs[1])
        pairs = pairs[2:]
...
def source_synopsis(file):
    line = file.readline()
    while 1:
        line = (line[:1] == '#' or not strip(line)) and file.readline()
        if not line:
            break
    ...
    if line[:3] == '"""':
        ...
        if line[-1:] == '\\':
            line = line[:-1]
        while 1:
            line = strip(line) or file.readline()
            if not line:
                break
    ...
    def markup(self, text, escape=None, funcs={}, classes={}, methods={}):
        ...
        while 1:
            if True:
                ...
                url = scheme and escape(all).replace('"', '&quot;')
        ...
        if not name:
            ...
            while 1:
                if attrs:
                    ...
                    attrs = thisclass is __builtin__.object and inherited
                    continue
...
def locate(path, forceload=0):
    ...
    while 1:
        if n < len(parts):
            ...
            module, n = nextmodule and nextmodule, n + 1
        else:
            break
...
1734      16	SETUP_LOOP        '150'
          19	LOAD_GLOBAL       'True'
          22	JUMP_IF_FALSE     '149'

1735      25	SETUP_EXCEPT      '57'

1736      28	LOAD_FAST         'self'
          31	LOAD_ATTR         'getline'
          34	LOAD_CONST        'help> '
          37	CALL_FUNCTION_1   ''
          40	STORE_FAST        'request'

1737      43	LOAD_FAST         'request'
          46	JUMP_IF_TRUE      '53'
          49	BREAK_LOOP        ''
          50	JUMP_FORWARD      '53'
        53_0	COME_FROM         '50'
...
1740      81	LOAD_GLOBAL       'strip'
          84	LOAD_GLOBAL       'replace'
          87	LOAD_FAST         'request'
          90	LOAD_CONST        '"'
          93	LOAD_CONST        ''
          96	LOAD_CONST        "'"
          99	LOAD_CONST        ''
         102	CALL_FUNCTION_5   ''
         105	CALL_FUNCTION_1   ''
         108	STORE_FAST        'request'

1741     111	LOAD_GLOBAL       'lower'
         114	LOAD_FAST         'request'
         117	CALL_FUNCTION_1   ''
         120	LOAD_CONST        ('q', 'quit')
         123	COMPARE_OP        'in'
         126	JUMP_IF_FALSE     '133'
         129	BREAK_LOOP        ''
         130	JUMP_FORWARD      '133'
       133_0	COME_FROM         '130'

1742     133	LOAD_FAST         'self'
         136	LOAD_ATTR         'help'
         139	LOAD_FAST         'request'
         142	CALL_FUNCTION_1   ''
         145	POP_TOP           ''
         146	JUMP_BACK         '19'
         149	POP_BLOCK         ''
