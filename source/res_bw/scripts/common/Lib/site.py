...
def addusersitepackages(known_paths):
    ...
    if os.name == 'nt':
        ...
        ENABLE_USER_SITE and os.path.isdir(USER_SITE) and addsitedir(USER_SITE, known_paths)
    return known_paths
...
def addsitepackages(known_paths):
    ...
    for prefix in PREFIXES:
        ...
        if sys.platform in ('os2emx', 'riscos'):
            ...
            sitedirs.append(os.path.join(prefix, 'lib', 'python' + sys.version[:3], 'site-packages'))
        ...
        if sys.platform == 'darwin':
            if 'Python.framework' in prefix:
                sitedirs.append(os.path.expanduser(os.path.join('~', 'Library', 'Python', sys.version[:3], 'site-packages')))
