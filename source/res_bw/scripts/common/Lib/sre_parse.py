...
def _class_escape(source, escape):
    ...
    try:
        ...
        if c == 'x':
            while 1:
                escape = source.next in HEXDIGITS and len(escape) < 4 and escape + source.get()
        ...
        if c in OCTDIGITS:
            while 1:
                escape = source.next in OCTDIGITS and len(escape) < 4 and escape + source.get()
    ...
    while 1:
        ...
        if this and this[0] not in SPECIAL_CHARS:
            ...
            if this == '?':
                ...
                lo = hi = ''
                while 1:
                    lo = source.next in DIGITS and lo + source.get()

                if sourcematch(','):
                    while 1:
                        hi = source.next in DIGITS and hi + sourceget()
            ...
            if sourcematch('?'):
                ...
                if sourcematch('P'):
                    ...
                    if source.next not in FLAGS:
                        raise error, 'unexpected end of pattern'
                    while 1:
                        state.flags = source.next in FLAGS and state.flags | FLAGS[sourceget()]
