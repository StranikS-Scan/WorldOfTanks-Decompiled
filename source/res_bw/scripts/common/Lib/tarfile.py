...
class _Stream():
    ...
    def __write(self, s):
        ...
        self.buf += s
        while 1:
            len(self.buf) > self.bufsize and self.fileobj.write(self.buf[:self.bufsize])
    ...
    def read(self, size=None):
        ...
        if size is None:
            t = []
            while 1:
                buf = True and self._read(self.bufsize)
                if not buf:
                    break
                t.append(buf)
    ...
    def _read(self, size):
        ...
        t = [self.dbuf]
        while 1:
            buf = c < size and self.__read(self.bufsize)
            if not buf:
                break
            try:
    ...
    def __read(self, size):
        ...
        t = [self.buf]
        while 1:
            buf = c < size and self.fileobj.read(self.bufsize)
            if not buf:
                break
            t.append(buf)
            c += len(buf)
...
class _BZ2Proxy(object):
    ...
    def read(self, size):
        ...
        x = len(self.buf)
        while 1:
            raw = x < size and self.fileobj.read(self.blocksize)
            if not raw:
                break
            try:
...
class _FileInFile(object):
    ...
    def readsparse(self, size):
        ...
        data = []
        while 1:
            buf = size > 0 and self.readsparsesection(size)
            if not buf:
                break
            size -= len(buf)
            data.append(buf)
...
class TarInfo(object):
    ...
    def _posix_split_name(self, name):
        ...
        while 1:
            prefix = prefix and prefix[-1] != '/' and prefix[:-1]
    ...
    def _proc_sparse(self, tarfile):
        ...
        while 1:
            buf = isextended == 1 and tarfile.fileobj.read(BLOCKSIZE)
            pos = 0
            for i in xrange(21):
                try:
                    offset = nti(buf[pos:pos + 12])
    ...
    def _proc_pax(self, tarfile):
        ...
        pos = 0
        while 1:
            match = True and regex.match(buf, pos)
            if not match:
                break
...
class TarFile(object):
            ...
            while 1:
                if True:
                    if self.next() is None:
                        self.offset > 0 and self.fileobj.seek(-BLOCKSIZE, 1)
                    break
        ...
        if mode in ('r', 'r:*'):
            ...
            if not filemode:
                ...
                if '|' in mode:
                    ...
                    raise filemode not in 'rw' and ValueError("mode must be 'r' or 'w'")
                t = cls(name, filemode, _Stream(name, filemode, comptype, fileobj, bufsize), **kwargs)
                t._extfileobj = False
                return t
            return mode in 'aw' and cls.taropen(name, mode, fileobj, **kwargs)
    ...
    def gettarinfo(self, name=None, arcname=None, fileobj=None):
        ...
        while 1:
            arcname = arcname[0:1] == '/' and arcname[1:]
    ...
    def add(self, name, arcname=None, recursive=True, exclude=None):
        ...
        if exclude is not None and exclude(name):
            ...
            return
        elif self.name is not None and os.path.abspath(name) == self.name:
...
2301       0	LOAD_FAST         'self'
           3	LOAD_ATTR         '_check'
           6	LOAD_CONST        'ra'
           9	CALL_FUNCTION_1   ''
          12	POP_TOP           ''

2302      13	LOAD_FAST         'self'
          16	LOAD_ATTR         'firstmember'
          19	LOAD_CONST        ''
          22	COMPARE_OP        'is not'
          25	JUMP_IF_FALSE     '50'

2303      28	LOAD_FAST         'self'
          31	LOAD_ATTR         'firstmember'
          34	STORE_FAST        'm'

2304      37	LOAD_CONST        ''
          40	LOAD_FAST         'self'
          43	STORE_ATTR        'firstmember'

2305      46	LOAD_FAST         'm'
          49	RETURN_END_IF     ''

2308      50	LOAD_FAST         'self'
          53	LOAD_ATTR         'fileobj'
          56	LOAD_ATTR         'seek'
          59	LOAD_FAST         'self'
          62	LOAD_ATTR         'offset'
          65	CALL_FUNCTION_1   ''
...
2309      69	SETUP_LOOP        '258'
          72	LOAD_GLOBAL       'True'
          75	JUMP_IF_FALSE     '257'

2310      78	SETUP_EXCEPT      '135'

2311      81	LOAD_FAST         'self'
          84	LOAD_ATTR         'tarinfo'
          87	LOAD_ATTR         'fromtarfile'
          90	LOAD_FAST         'self'
          93	CALL_FUNCTION_1   ''
          96	STORE_FAST        'tarinfo'

2312      99	LOAD_FAST         'tarinfo'
         102	LOAD_CONST        ''
         105	COMPARE_OP        'is'
...
2314     115	LOAD_FAST         'self'
         118	LOAD_ATTR         'members'
         121	LOAD_ATTR         'append'
         124	LOAD_FAST         'tarinfo'
         127	CALL_FUNCTION_1   ''
         130	POP_TOP           ''
         131	POP_BLOCK         ''
         132	JUMP_FORWARD      '253'
       135_0	COME_FROM         '78'

2316     135	DUP_TOP           ''
         136	LOAD_GLOBAL       'HeaderError'
         139	COMPARE_OP        'exception match'
         142	JUMP_IF_FALSE     '252'
         145	POP_TOP           ''
         146	STORE_FAST        'e'
         149	POP_TOP           ''

2317     150	LOAD_FAST         'self'
         153	LOAD_ATTR         'ignore_zeros'
         156	JUMP_IF_FALSE     '209'

2318     159	LOAD_FAST         'self'
         162	LOAD_ATTR         '_dbg'
         165	LOAD_CONST        2
         168	LOAD_CONST        '0x%X: %s'
         171	LOAD_FAST         'self'
         174	LOAD_ATTR         'offset'
         177	LOAD_FAST         'e'
         180	BUILD_TUPLE_2     ''
         183	BINARY_MODULO     ''
         184	CALL_FUNCTION_2   ''
...
2319     188	LOAD_FAST         'self'
         191	DUP_TOP           ''
         192	LOAD_ATTR         'offset'
         195	LOAD_GLOBAL       'BLOCKSIZE'
         198	INPLACE_ADD       ''
         199	ROT_TWO           ''
         200	STORE_ATTR        'offset'

2320     203	CONTINUE          '72'
         206	JUMP_ABSOLUTE     '253'

2322     209	LOAD_FAST         'self'
         212	LOAD_ATTR         'offset'
         215	LOAD_CONST        0
         218	COMPARE_OP        '=='
         221	JUMP_IF_FALSE     '245'

2323     224	LOAD_GLOBAL       'ReadError'
         227	LOAD_GLOBAL       'str'
         230	LOAD_FAST         'e'
         233	CALL_FUNCTION_1   ''
         236	CALL_FUNCTION_1   ''
...
2327     258	LOAD_FAST         'tarinfo'
         261	RETURN_VALUE      ''

Syntax error at or near 'POP_BLOCK' token at offset 257
