...
class _RLock(_Verbose):
    ...
    def __repr__(self):
        owner = self.__owner
        return '<%s(%s, %d)>' % (self.__class__.__name__, owner and owner.name, self.__count)
    ...
    def wait(self, timeout=None):
        ...
        try:
            if timeout is None:
                ...
                delay = 0.0005
                while 1:
                    gotit = True and waiter.acquire(0)
                    if gotit:
                        break
...
class _Semaphore(_Verbose):
    ...
    def acquire(self, blocking=1):
        ...
        self.__cond.acquire()
        while 1:
            if not (self.__value == 0 and blocking):
                break
...
class Thread(_Verbose):
                ...
                try:
                    ...
                    if _sys:
                        ...
                        try:
                            ...
                            while 1:
                                print >> exc_tb and self.__stderr, '  File "%s", line %s, in %s' % (exc_tb.tb_frame.f_code.co_filename, exc_tb.tb_lineno, exc_tb.tb_frame.f_code.co_name)
    ...
    def join(self, timeout=None):
        ...
        try:
            if timeout is None:
                while 1:
                    self.__stopped or self.__block.wait()
                ...
                while 1:
                    if not self.__stopped:
                        ...
                        delay <= 0 and self._note('%s.join(): timed out', self)
                        break
...
def _test():
    ...
    class BoundedQueue(_Verbose):
        ...
        def put(self, item):
            self.mon.acquire()
            while 1:
                len(self.queue) >= self.limit and self._note('put(%s): queue full', item)
                self.wc.wait()
        ...
        def get(self):
            self.mon.acquire()
            while 1:
                self.queue or self._note('get(): queue empty')
                self.rc.wait()
    ...
    class ProducerThread(Thread):
        ...
        def run(self):
            ...
            counter = 0
            while 1:
                counter = counter < self.quota and counter + 1
    ...
    class ConsumerThread(Thread):
        ...
        def run(self):
            while 1:
                item = self.count > 0 and self.queue.get()
                print item
