...
def generate_tokens(readline):
    ...
    while 1:
        ...
        if contstr:
            ...
            while 1:
                if column < indents[-1]:
                    raise column not in indents and IndentationError('unindent does not match any outer indentation level', ('<tokenize>',
                     lnum,
                     pos,
                     line))
                indents = indents[:-1]
            ...
            continued = 0
        while 1:
            pseudomatch = pos < max and pseudoprog.match(line, pos)
            if pseudomatch:
                ...
                if initial in numchars or initial == '.' and token != '.':
                    ...
                    strstart = (initial in single_quoted or token[:2] in single_quoted or token[:3] in single_quoted) and token[-1] == '\n' and (lnum, start)
                    if not endprogs[initial]:
                        if not endprogs[token[1]]:
