        ...
        if not trace:
            if not report:
                if not listfuncs:
                    ...
                    report and no_report and _err_exit('cannot specify both --report and --no-report')
                report and not counts_file and _err_exit('--report requires a --file')
            no_report and len(prog_argv) == 0 and _err_exit('missing name of file to run')
        results = report and CoverageResults(infile=counts_file, outfile=counts_file)
