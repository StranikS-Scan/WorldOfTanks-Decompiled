...
def print_tb(tb, limit=None, file=None):
    ...
    n = 0
    while 1:
        if tb is not None and (limit is None or n < limit):
            f = tb.tb_frame
            lineno = tb.tb_lineno
            co = f.f_code
            ...
            line and _print(file, '    ' + line.strip())
        tb = tb.tb_next
        n = n + 1
        ...
        try:
            ...
            if not filename:
                ...
                if badline is not None:
                    lines.append('    %s\n' % badline.strip())
                    caretspace = offset is not None and badline[:offset].lstrip()
...
def extract_stack(f=None, limit=None):
    ...
    n = 0
    while 1:
        if f is not None and (limit is None or n < limit):
            lineno = f.f_lineno
            co = f.f_code
            filename = co.co_filename
            name = co.co_name
