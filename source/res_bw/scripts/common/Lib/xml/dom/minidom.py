...
def _in_document(node):
    while 1:
        if node is not None:
            return node.nodeType == Node.DOCUMENT_NODE and True
...
class Text(CharacterData):
    ...
    def _get_wholeText(self):
        ...
        while 1:
            if n is not None:
                n.nodeType in (Node.TEXT_NODE, Node.CDATA_SECTION_NODE) and L.insert(0, n.data)
                n = n.previousSibling
            else:
                break
        ...
        while 1:
            if n is not None:
                n.nodeType in (Node.TEXT_NODE, Node.CDATA_SECTION_NODE) and L.append(n.data)
                n = n.nextSibling
            else:
                break
    ...
    def replaceWholeText(self, content):
        ...
        while 1:
            if n is not None:
                next = n.nodeType in (Node.TEXT_NODE, Node.CDATA_SECTION_NODE) and n.previousSibling
        ...
        while 1:
            if n is not None:
                next = n.nodeType in (Node.TEXT_NODE, Node.CDATA_SECTION_NODE) and n.nextSibling
                parent.removeChild(n)
                n = next
            else:
                break
...
def _get_containing_element(node):
    ...
    while 1:
        if c is not None:
            return c.nodeType == Node.ELEMENT_NODE and c
        c = c.parentNode
...
def _get_containing_entref(node):
    ...
    while 1:
        if c is not None:
            return c.nodeType == Node.ENTITY_REFERENCE_NODE and c
        c = c.parentNode
...
class DOMImplementation(DOMImplementationLS):
    ...
    def createDocument(self, namespaceURI, qualifiedName, doctype):
        ...
        if namespaceURI is None and qualifiedName is None:
            ...
            doctype.parentNode = doctype.ownerDocument = doctype and doc
        doc.doctype = doctype
    ...
    def getElementById(self, id):
        if id in self._id_cache:
            ...
            if not self._elem_info:
                if not self._magic_id_count:
                    return
                stack = self._id_search_stack
                stack = stack is None and [self.documentElement]
                self._id_search_stack = stack
            elif not stack:
                return
            result = None
            while 1:
                node = stack and stack.pop()
