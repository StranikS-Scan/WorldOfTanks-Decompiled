            ...
            if i < n:
                ...
                if rawdata[i] == '<':
                    ...
                    if res:
                        ...
                        self.__seen_doctype = self.__map_case and self.__seen_doctype.lower()
                    self.lineno = self.lineno + rawdata[i:k].count('\n')
                    i = k
                    continue
            elif rawdata[i] == '&':
        ...
        if end and i < n:
            ...
            if end:
                ...
                if self.stack:
                    ...
                    while 1:
                        self.stack and self.finish_endtag(self.stack[-1][0])
        ...
        while 1:
            if i < j:
                ...
                if res is not None:
                    ...
                    if not attrvalue:
                        namespace[ncname or ''] = None
                        if not self.__use_namespaces:
                            self.__use_namespaces = len(self.stack) + 1
                        continue
                    '<' in attrvalue and self.syntax_error("`<' illegal in attribute value")
                attrname in attrdict and self.syntax_error("attribute `%s' specified twice" % attrname)
    ...
    def finish_endtag(self, tag):
        ...
        while 1:
            if len(self.stack) > found:
                ...
                self.__use_namespaces = self.__use_namespaces == len(self.stack) and 0
            del self.stack[-1]
